...
(define-const float__last Float16
  (fp #b0 #b11110 #b1111111111))

(declare-const numerator_0 Float16)
(declare-const denominator_0 Float16)

(assert (isFinite numerator_0))
(assert (isFinite denominator_0))

(define-const numerator_1 Float16
  (ite (fp.lt denominator_0 (_ +zero 5 11))
       (fp.neg numerator_0)
       numerator_0))

(define-const denominator_1 Float16
  (ite (fp.lt denominator_0 (_ +zero 5 11))
...
(declare-const sign_numerator_1 Float16)
(assert (=> (fp.gt numerator_1 (_ +zero 5 11))
            (fp.eq sign_numerator_1 ((_ to_fp 5 11) RNE 1.0))))
(assert (=> (fp.lt numerator_1 (_ +zero 5 11))
            (fp.eq sign_numerator_1 ((_ to_fp 5 11) RNE (- 1.0)))))
(assert (=> (fp.eq numerator_1 (_ +zero 5 11))
            (fp.isZero sign_numerator_1)))

(define-const overflow Bool
  (and (fp.lt denominator_1 ((_ to_fp 5 11) RNE 1.0))
       (fp.geq (fp.mul RNE numerator_1 sign_numerator_1)
               (fp.mul RNE denominator_1 float__last))))

(define-const result Float16 (fp.div RNE numerator_1 denominator_1))
