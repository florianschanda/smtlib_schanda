...
(assert (forall ((container us_rep1)) (forall ((item Int)) (! (=> (dynamic_invariant2 item true true true true) (let ((result (find container item))) (and (=> (< 0 result) (and (<= result (length1 container)) (= (equivalent_elements item (get container result)) true))) (dynamic_invariant result true false true true)))) :pattern ((find container item))))))
...
(assert (forall ((container us_rep2)) (! (let ((result (is_empty container))) (and (= (= result true) (forall ((item Int)) (not (and (in_range5 item) (= (contains container item) true))))) (= (= result true) (= (length2 container) 0)))) :pattern ((is_empty container)))))
...
(assert (and (= o (length right)) (and (in_range o) (<= o (to_rep (rec__p__my_sets__set__capacity (us_split_discrs1 right)))))))
