...
(assert (forall ((unused Bool)) (forall ((parent__state us_private)) (! (dynamic_invariant (proxy unused parent__state) true false true) :pattern ((proxy unused parent__state))))))
(declare-const state us_private)
(assert (not (= (proxy (of_int 0) state) 0)))
(check-sat)
(exit)
