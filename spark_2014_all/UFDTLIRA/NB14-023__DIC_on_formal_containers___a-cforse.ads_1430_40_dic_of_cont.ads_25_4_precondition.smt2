...
(assert (forall ((container us_rep)) (! (=> (not (= (is_empty container) true)) (let ((result (last_element container))) (and (and (= result (get (elements container) (length container))) (= (e_bigger_than_range (elements container) 1 (- (length container) 1) result) true)) (dynamic_invariant result true false true true)))) :pattern ((last_element container)))))
(assert (forall ((attr__tag Int)) (forall ((container us_rep)) (! (dynamic_invariant (last_element1 attr__tag container) true false true true) :pattern ((last_element1 attr__tag container))))))
