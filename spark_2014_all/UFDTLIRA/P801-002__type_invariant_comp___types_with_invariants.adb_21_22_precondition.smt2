...
(assert (forall ((x us_rep)) (! (dynamic_invariant1 (to_integer x) true false true true) :pattern ((to_integer x)))))
...
(assert true)
(assert (forall ((x Int)) (! (=> (dynamic_invariant1 x true true true true) (= (from_integer x) (mk___rep (mk___split_fields (>= x 0) (of_rep (abs x)))))) :pattern ((from_integer x)))))
(declare-const i Int)
...
(define-fun default_initial_assumption1 ((temp___expr_253 us_rep) (temp___skip_top_level_254 Bool)) Bool (and (= (rec__types_with_invariants__my_integer__sign (us_split_fields1 temp___expr_253)) (distinct 1 0)) (= (to_rep (rec__types_with_invariants__my_integer__val (us_split_fields1 temp___expr_253))) 0)))
