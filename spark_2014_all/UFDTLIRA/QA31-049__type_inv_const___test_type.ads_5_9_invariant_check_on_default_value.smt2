...
(define-fun default_initial_assumption ((temp___expr_136 us_rep) (temp___skip_top_level_137 Bool)) Bool (= (to_rep (rec__test_type__t__f (us_split_fields1 temp___expr_136))) 0))
(define-fun type_invariant ((temp___138 us_rep)) Bool (in_range1 (to_rep (rec__test_type__t__f (us_split_fields1 temp___138)))))
...
(assert (= x (mk___rep (mk___split_fields (of_rep (- 1))))))
(declare-const temp___140 integer)
(assert (= (to_rep temp___140) 0))
(assert (not (in_range1 (to_rep temp___140))))
(check-sat)
(exit)
