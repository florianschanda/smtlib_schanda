...
(define-fun default_initial_assumption ((temp___expr_390 us_rep2) (temp___skip_top_level_391 Bool)) Bool (=> (not (= temp___skip_top_level_391 true)) (= (length1 temp___expr_390) 0)))
(assert (forall ((container us_rep2)) (forall ((item Int)) (! (=> (dynamic_invariant1 item true true true true) (let ((result (find container item))) (and (=> (< 0 result) (and (<= result (length1 container)) (= (equivalent_elements item (get container result)) true))) (dynamic_invariant result true false true true)))) :pattern ((find container item))))))
...
(define-fun default_initial_assumption1 ((temp___expr_354 us_rep1) (temp___skip_top_level_355 Bool)) Bool (=> (not (= temp___skip_top_level_355 true)) (= (is_empty temp___expr_354) true)))
...
(assert (forall ((container us_rep1)) (! (let ((result (is_empty container))) (and (= (= result true) (forall ((item Int)) (not (and (in_range4 item) (= (contains container item) true))))) (= (= result true) (= (length2 container) 0)))) :pattern ((is_empty container)))))
...
(assert (and (= o (length (mk___rep target__split_discrs (mk___split_fields target__split_fields)))) (and (in_range o) (<= o (to_rep (rec__my_ordered_sets__m__set__capacity target__split_discrs))))))
(assert (= o1 (- (to_rep (rec__my_ordered_sets__m__set__capacity target__split_discrs)) o)))
