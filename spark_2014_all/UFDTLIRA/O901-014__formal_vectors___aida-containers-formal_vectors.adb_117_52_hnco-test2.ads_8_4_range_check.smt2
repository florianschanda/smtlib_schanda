...
(define-fun m_elements_reversed ((left us_rep2) (right us_rep2)) Bool (and (and (= (length left) (length right)) (ite (forall ((i Int)) (=> (and (<= 1 i) (<= i (last left))) (= (get left i) (get right (+ (- (last left) i) 1))))) true false)) (ite (forall ((i Int)) (=> (and (<= 1 i) (<= i (last right))) (= (get right i) (get left (+ (- (last left) i) 1))))) true false)))
...
(assert (= (to_rep1 (rec__hnco__test2__v_type_owner__vector_type__capacity container__split_discrs)) r265s))
(assert (in_range r265s))
(assert (not (in_range1 r265s)))
(check-sat)
(exit)
