...
(assert true)
(assert (in_range2 x))
(assert (and (and (<= 0 x) (<= x (- (length p) 1))) (forall ((j Int)) (=> (and (<= 0 j) (<= j (last (model__ p)))) (in_range1 (+ (to_rep (rec__sum_elem__interval__first (us_split_fields1 (get (model__ p) j)))) (to_rep (rec__sum_elem__interval__last (us_split_fields1 (get (model__ p) j))))))))))
(assert (not (<= 0 x)))
(check-sat)
(exit)
