...
(assert (in_range2 x))
(assert (and (and (<= 0 x) (<= x (- (length p) 1))) (forall ((j1 Int)) (=> (and (<= 0 j1) (<= j1 (last (model__ p)))) (in_range1 (+ (to_rep (rec__sum_elem__interval__first (us_split_fields1 (get (model__ p) j1)))) (to_rep (rec__sum_elem__interval__last (us_split_fields1 (get (model__ p) j1))))))))))
(assert (not (<= 0 x)))
(check-sat)
(exit)
