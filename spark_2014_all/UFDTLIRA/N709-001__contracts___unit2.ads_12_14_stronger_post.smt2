...
(assert true)
(assert true)
(assert (forall ((x us_rep1)) (! (= (to_int1 (is_zero x)) (to_int1 (is_zero1 us_tag1 x))) :pattern ((is_zero1 us_tag1 x)))))
(assert (forall ((x us_rep1)) (! (= (= (is_zero x) true) (and (= (is_zero2 (to_base x)) true) (= (to_rep (rec__unit2__t2__c2 (us_split_fields3 x))) 0))) :pattern ((is_zero x)))))
...
(assert true)
(assert true)
(assert (forall ((x us_rep)) (! (= (to_int1 (is_zero (of_base x))) (to_int1 (is_zero3 us_tag1 x))) :pattern ((is_zero3 us_tag1 x)))))
(assert (forall ((x us_rep)) (! (= (to_int1 (is_zero2 x)) (to_int1 (is_zero3 us_tag x))) :pattern ((is_zero3 us_tag x)))))
(assert (forall ((x us_rep)) (! (= (= (is_zero2 x) true) (= (to_rep (rec__unit1__t1__c1 (us_split_fields1 x))) 0)) :pattern ((is_zero2 x)))))
(declare-const x__split_fields integer)
(declare-const x__split_fields1 integer)
(declare-const x__split_fields2 us_private)
(assert (and (= (to_rep x__split_fields1) 0) (= (to_rep x__split_fields) 0)))
(assert (not (= (is_zero (mk___rep1 (mk___split_fields1 x__split_fields x__split_fields1 x__split_fields2) x__attr__tag)) true)))
(check-sat)
(exit)
