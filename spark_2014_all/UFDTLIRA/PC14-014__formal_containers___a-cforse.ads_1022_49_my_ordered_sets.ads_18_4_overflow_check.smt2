...
(define-fun e_is_find ((container us_rep2) (item us_rep1) (position Int)) Bool (and (ite (< 0 position) (e_bigger_than_range container 1 (- position 1) item) (distinct 1 0)) (ite (< position (length2 container)) (e_smaller_than_range container (+ position 1) (length2 container) item) (distinct 1 0))))
...
(assert (forall ((container us_rep2)) (forall ((item us_rep1)) (! (let ((result (find container item))) (and (=> (< 0 result) (and (<= result (length2 container)) (= (equivalent_elements item (get container result)) true))) (dynamic_invariant result true false true true))) :pattern ((find container item))))))
