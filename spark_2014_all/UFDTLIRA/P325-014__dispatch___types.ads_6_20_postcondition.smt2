...
(assert true)
(assert true)
(assert (forall ((x us_rep)) (! (= (to_int1 (check__ x)) (to_int1 (check__1 us_tag x))) :pattern ((check__1 us_tag x)))))
(assert (forall ((x us_rep)) (! (= (to_int1 (check__2 x)) (to_int1 (check__1 us_tag1 x))) :pattern ((check__1 us_tag1 x)))))
(assert (forall ((x us_rep)) (! (= (check__ x) true) :pattern ((check__ x)))))
(define-fun default_initial_assumption1 ((temp___expr_149 us_rep) (temp___skip_top_level_150 Bool)) Bool (= (attr__tag temp___expr_149) us_tag1))
(assert true)
(assert true)
(assert (forall ((x us_rep)) (! (= (to_int1 (check__2 x)) (to_int1 (check__21 us_tag1 x))) :pattern ((check__21 us_tag1 x)))))
