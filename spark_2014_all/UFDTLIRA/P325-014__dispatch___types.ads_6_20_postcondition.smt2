...
(declare-datatypes () ((us_split_fields (mk___split_fields (rec__types__t1__j integer) (rec__ext__ us_private)))))
(define-fun us_split_fields_J__projection ((a us_split_fields)) integer (rec__types__t1__j a))
...
(define-fun bool_eq ((a us_rep) (b us_rep)) Bool (ite (= (to_rep (rec__types__t1__j (us_split_fields1 a))) (to_rep (rec__types__t1__j (us_split_fields1 b)))) true false))
...
(assert true)
(assert true)
(assert (forall ((x us_rep)) (! (= (ite (check__ x) 1 0) (ite (check__1 us_tag x) 1 0)) :pattern ((check__1 us_tag x)))))
(assert (forall ((x us_rep)) (! (= (ite (check__2 x) 1 0) (ite (check__1 us_tag1 x) 1 0)) :pattern ((check__1 us_tag1 x)))))
(assert (forall ((x us_rep)) (! (= (check__ x) true) :pattern ((check__ x)))))
(define-fun default_initial_assumption1 ((temp___expr_174 us_rep) (temp___skip_top_level_175 Bool)) Bool (= (attr__tag temp___expr_174) us_tag1))
(assert true)
(assert true)
(assert (forall ((x us_rep)) (! (= (ite (check__2 x) 1 0) (ite (check__21 us_tag1 x) 1 0)) :pattern ((check__21 us_tag1 x)))))
(assert (forall ((x us_rep)) (! (not (= (check__2 x) true)) :pattern ((check__2 x)))))
(declare-const x__split_fields integer)
(declare-const x__split_fields1 us_private)
(assert (not (= (check__ (mk___rep (mk___split_fields x__split_fields x__split_fields1) x__attr__tag)) true)))
(check-sat)
(exit)
