...
(assert (forall ((x node_enum)) (! (in_range (to_rep x)) :pattern ((to_rep x)))))
...
(define-fun in_range1 ((rec__main__node_variant_type__m_variant1 Int) (a us_rep)) Bool (= rec__main__node_variant_type__m_variant1 (to_rep (rec__main__node_variant_type__m_variant (us_split_discrs1 a)))))
...
(define-fun dynamic_invariant1 ((temp___expr_171 Int) (temp___is_init_168 Bool) (temp___skip_constant_169 Bool) (temp___do_toplevel_170 Bool)) Bool (=> (or (= temp___is_init_168 true) (<= 0 0)) (in_range temp___expr_171)))
...
(declare-const main__create_empty_branch__branch__fields1 Bool)
(declare-const result node_enum)
...
(assert (and (= x__split_fields1 false) (= (to_rep x__split_fields) 0)))
(assert (= x__split_fields1 false))
(assert (not true))
(check-sat)
(exit)
