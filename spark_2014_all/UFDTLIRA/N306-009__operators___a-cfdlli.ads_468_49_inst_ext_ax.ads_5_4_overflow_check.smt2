...
(define-fun default_initial_assumption ((temp___expr_195 us_rep1) (temp___skip_top_level_196 Bool)) Bool (= (to_rep (rec__inst_ext_ax__ok_lists1__cursor__node (us_split_fields3 temp___expr_195))) 0))
(declare-const no_element us_rep1)
(declare-const attr__ATTRIBUTE_ADDRESS Int)
(declare-fun length (us_rep) Int)
(declare-fun length__function_guard (Int us_rep) Bool)
(assert (forall ((container us_rep)) (! (let ((result (length container))) (and (<= result (to_rep (rec__inst_ext_ax__ok_lists1__list__capacity (us_split_discrs1 container)))) (dynamic_invariant result true false true))) :pattern ((length container)))))
...
(define-fun default_initial_assumption2 ((temp___expr_234 us_rep1) (temp___skip_top_level_235 Bool)) Bool (= (to_rep (rec__inst_ext_ax__ok_lists1__cursor__node (us_split_fields3 temp___expr_234))) 0))
...
(declare-const temp___567 count_type)
(assert (= (to_rep o) 0))
(assert (= o o1))
(assert (= temp___567 o1))
(assert (= (mk___rep1 (mk___split_fields1 temp___567)) no_element))
(assert (in_range2 new_item))
(assert (in_range1 count))
(assert (not (in_range (- (to_rep (rec__inst_ext_ax__ok_lists1__list__capacity container__split_discrs)) count))))
(check-sat)
(exit)
