...
(declare-sort tstatscountB 0)
(declare-fun tstatscountBqtint (tstatscountB) Int)
(assert (forall ((i tstatscountB)) (and (<= (- 2147483648) (tstatscountBqtint i)) (<= (tstatscountBqtint i) 2147483647))))
(define-fun in_range ((x Int)) Bool (and (<= (- 2147483648) x) (<= x 2147483647)))
(declare-fun attr__ATTRIBUTE_IMAGE (Int) us_image)
(declare-fun attr__ATTRIBUTE_VALUE__pre_check (us_image) Bool)
(declare-fun attr__ATTRIBUTE_VALUE (us_image) Int)
(declare-fun user_eq (tstatscountB tstatscountB) Bool)
(declare-const dummy tstatscountB)
(declare-datatypes () ((tstatscountB__ref (mk_tstatscountB__ref (tstatscountB__content tstatscountB)))))
(define-fun tstatscountB__ref___projection ((a tstatscountB__ref)) tstatscountB (tstatscountB__content a))
...
(declare-fun user_eq1 (statscount statscount) Bool)
(declare-const dummy1 statscount)
...
(define-fun to_rep ((x statscount)) Int (statscountqtint x))
(declare-fun of_rep (Int) statscount)
(assert (forall ((x statscount)) (! (= (of_rep (to_rep x)) x) :pattern ((to_rep x)))))
(assert (forall ((x statscount)) (! (in_range1 (to_rep x)) :pattern ((to_rep x)))))
...
(define-fun bool_eq ((a us_rep) (b us_rep)) Bool (ite (and (and (and (= (to_rep (rec__stats__t__successentry (us_split_fields1 a))) (to_rep (rec__stats__t__successentry (us_split_fields1 b)))) (= (to_rep (rec__stats__t__failentry (us_split_fields1 a))) (to_rep (rec__stats__t__failentry (us_split_fields1 b))))) (= (to_rep (rec__stats__t__successbio (us_split_fields1 a))) (to_rep (rec__stats__t__successbio (us_split_fields1 b))))) (= (to_rep (rec__stats__t__failbio (us_split_fields1 a))) (to_rep (rec__stats__t__failbio (us_split_fields1 b))))) true false))
...
(define-fun dynamic_invariant ((temp___expr_368 Int) (temp___is_init_364 Bool) (temp___skip_constant_365 Bool) (temp___do_toplevel_366 Bool) (temp___do_typ_inv_367 Bool)) Bool (=> (or (= temp___is_init_364 true) (<= 0 2147483647)) (in_range1 temp___expr_368)))
(declare-const thestats__split_fields statscount)
(assert (not (= (to_rep thestats__split_fields) 2147483647)))
(assert (not (in_range (+ (to_rep thestats__split_fields) 1))))
(check-sat)
(exit)
