...
(assert true)
(assert (forall ((x1 us_rep)) (! (= (decr_int x1) (mk___rep (mk___split_fields (of_rep (- (to_rep (rec__test_type__t__f (us_split_fields1 x1))) 1))))) :pattern ((decr_int x1)))))
(declare-fun saturate (us_rep) us_rep)
(declare-fun saturate__function_guard (us_rep us_rep) Bool)
(assert true)
(assert (forall ((x1 us_rep)) (! (= (saturate x1) (ite (< (to_rep (rec__test_type__t__f (us_split_fields1 x1))) 0) (mk___rep (mk___split_fields (of_rep 0))) x1)) :pattern ((saturate x1)))))
...
(assert (not (< (- 2147483648) (to_rep (rec__test_type__t__f (us_split_fields1 x))))))
(check-sat)
(exit)
