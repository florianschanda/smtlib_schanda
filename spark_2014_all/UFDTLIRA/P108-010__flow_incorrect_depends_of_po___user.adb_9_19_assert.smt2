...
(declare-datatypes () ((us_split_fields (mk___split_fields (rec__p__po_type__content integer)))))
(define-fun us_split_fields_Content__projection ((a us_split_fields)) integer (rec__p__po_type__content a))
...
(declare-datatypes () ((po_type__ref (mk_po_type__ref (po_type__content us_rep)))))
(define-fun po_type__ref___projection ((a po_type__ref)) us_rep (po_type__content a))
(declare-fun to_rep (integer) Int)
(declare-fun of_rep (Int) integer)
(assert (forall ((x integer)) (! (= (of_rep (to_rep x)) x) :pattern ((to_rep x)))))
(assert (forall ((x integer)) (! (in_range (to_rep x)) :pattern ((to_rep x)))))
(assert (forall ((x Int)) (! (=> (in_range x) (= (to_rep (of_rep x)) x)) :pattern ((to_rep (of_rep x))))))
(define-fun default_initial_assumption ((temp___expr_172 us_rep) (temp___skip_top_level_173 Bool)) Bool (= (to_rep (rec__p__po_type__content (us_split_fields1 temp___expr_172))) 0))
...
(declare-const the_po__split_fields integer)
(declare-const us_self__compl1 integer)
(declare-const result integer)
(declare-const the_po__split_fields1 integer)
(declare-const the_po__split_fields2 integer)
