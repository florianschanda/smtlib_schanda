...
(assert (forall ((x Int)) (! (=> (dynamic_invariant x true true true true) (dynamic_invariant (f_while x) true false true true)) :pattern ((f_while x)))))
(declare-fun f_not_spark (Int) Int)
(declare-fun f_not_spark__function_guard (Int Int) Bool)
(assert (forall ((x Int)) (! (=> (dynamic_invariant x true true true true) (dynamic_invariant (f_not_spark x) true false true true)) :pattern ((f_not_spark x)))))
(declare-fun f_call (Int) Int)
(declare-fun f_call__function_guard (Int Int) Bool)
(assert (forall ((x Int)) (! (=> (dynamic_invariant x true true true true) (dynamic_invariant (f_call x) true false true true)) :pattern ((f_call x)))))
(declare-const x Int)
...
(assert (= result5 temp___156))
(assert (= temp___1561 y6))
(assert (not (in_range1 (- y6 1))))
(check-sat)
(exit)
