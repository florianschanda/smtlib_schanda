...
(define-fun olt ((left us_rep1) (right us_rep1)) Bool (and (< (length1 left) (length1 right)) (ite (forall ((n Int)) (=> (and (<= first2 n) (<= n (last4 left))) (= (get left n) (get right n)))) true false)))
...
(define-fun dynamic_invariant5 ((temp___expr_235 Int) (temp___is_init_231 Bool) (temp___skip_constant_232 Bool) (temp___do_toplevel_233 Bool) (temp___do_typ_inv_234 Bool)) Bool (=> (or (= temp___is_init_231 true) (<= (- 2147483648) 2147483647)) (in_range5 temp___expr_235)))
(assert true)
...
(define-fun dynamic_invariant6 ((temp___expr_228 Int) (temp___is_init_224 Bool) (temp___skip_constant_225 Bool) (temp___do_toplevel_226 Bool) (temp___do_typ_inv_227 Bool)) Bool (=> (or (= temp___is_init_224 true) (<= first last)) (dynamic_property first last temp___expr_228)))
(assert (= first fst))
(assert (= last lst))
...
(assert (= last6 last3))
(assert (forall ((container us_rep1)) (! (let ((result (length1 container))) (and (<= (+ (- first2 1) result) last3) (dynamic_invariant7 result true false true true))) :pattern ((length1 container)))))
...
(define-fun v2__split_fields1 () us_split_fields (mk___split_fields v2__split_fields))
(assert (dynamic_property2 0 last2 (to_rep1 (rec__use_vectors__my_vectors__vector__capacity (us_split_discrs1 v1)))))
(assert (dynamic_property2 0 last2 (to_rep1 (rec__use_vectors__my_vectors__vector__capacity v2__split_discrs))))
(assert (and (= o (last4 (model__ v1))) (and (dynamic_property4 first3 last5 o) (= o (last4 (model__ v1))))))
(assert (= (mk_int__ref result) (mk_int__ref lst1)))
(assert (= lst2 o))
(assert (in_range1 lst2))
(assert (and (= (length (mk___rep v2__split_discrs v2__split_fields1)) 0) (dynamic_property2 0 last2 (to_rep1 (rec__use_vectors__my_vectors__vector__capacity v2__split_discrs)))))
(assert (= lst2 r1b))
...
(assert (<= first i1))
(assert (<= i1 r1b))
(assert (not (= (length (mk___rep v2__split_discrs v2__split_fields1)) (- i1 first))))
(check-sat)
(exit)
