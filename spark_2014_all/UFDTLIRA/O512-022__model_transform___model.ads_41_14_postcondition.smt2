...
(assert (forall ((self us_rep)) (! (dynamic_invariant (to__ self) true false true true) :pattern ((to__ self)))))
(assert (forall ((self us_rep)) (! (= (to__ self) (to_rep (rec__model__uml_transition__to_data (us_split_fields1 self)))) :pattern ((to__ self)))))
...
(declare-const temp___408 uml_state_access)
(declare-const temp___4081 uml_state_access)
...
(assert (= temp___4081 o2))
(assert (= result self__split_fields))
(assert (= result1 self__split_fields1))
(assert (= temp___408 self__split_fields2))
(assert (= temp___4081 self__split_fields3))
...
(assert (not (= (to__ (mk___rep (mk___split_fields self__split_fields4 self__split_fields5))) val__)))
(check-sat)
(exit)
