...
(declare-fun get (us_private) Int)
(declare-fun get__function_guard (Int us_private) Bool)
...
(assert (forall ((state__state us_private)) (! (dynamic_invariant (get state__state) true false true true) :pattern ((get state__state)))))
(declare-const state us_private)
(declare-const state1 us_private)
(declare-const state2 us_private)
(assert (= state state1))
(assert (= state2 state))
(assert (not (<= 0 (get state1))))
(check-sat)
(exit)
