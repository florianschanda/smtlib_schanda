...
(declare-fun do_stuff__specific_post (Int us_split_fields Int us_split_fields) Bool)
(assert true)
...
(assert true)
(assert true)
(assert (forall ((x us_rep)) (! (= (ite (is_valid x) 1 0) (ite (is_valid1 us_tag x) 1 0)) :pattern ((is_valid1 us_tag x)))))
(assert (forall ((x us_rep)) (! (= (= (is_valid x) true) (= (rec__object__t__b (us_split_fields1 x)) true)) :pattern ((is_valid x)))))
(declare-const x__split_fields Bool)
(declare-const x__split_fields1 us_private)
(assert (= x__split_fields true))
(assert (not (= (is_valid1 x__attr__tag (mk___rep (mk___split_fields x__split_fields x__split_fields1) x__attr__tag)) true)))
(check-sat)
(exit)
