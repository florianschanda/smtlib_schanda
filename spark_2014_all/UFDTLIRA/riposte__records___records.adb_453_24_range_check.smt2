...
(assert (forall ((x command_t)) (! (in_range1 (to_rep1 x)) :pattern ((to_rep1 x)))))
...
(define-fun dynamic_invariant1 ((temp___expr_214 Int) (temp___is_init_210 Bool) (temp___skip_constant_211 Bool) (temp___do_toplevel_212 Bool) (temp___do_typ_inv_213 Bool)) Bool (=> (or (= temp___is_init_210 true) (<= 0 2)) (in_range1 temp___expr_214)))
...
(assert (= (to_rep1 (rec__records_prooffuncs__packet__command (us_split_fields3 p))) 0))
(assert (= result r__split_fields))
(assert (= result1 r__split_fields1))
(assert (= (mk___split_fields r__split_fields2 r__split_fields3) (us_split_fields1 (rec__records_prooffuncs__packet__data (us_split_fields3 p)))))
(assert (not (in_range (+ (to_rep r__split_fields2) 1))))
(check-sat)
(exit)
