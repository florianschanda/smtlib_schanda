...
(define-fun olt ((left us_rep2) (right us_rep2)) Bool (and (< (length1 left) (length1 right)) (ite (forall ((n Int)) (=> (and (<= 1 n) (<= n (last left))) (= (get left n) (get right n)))) true false)))
...
(assert (= (to_rep1 (rec__hnco__test2__v_type_owner__vector_type__capacity container__split_discrs)) r262s))
(assert (in_range r262s))
(assert (and (= us r262s) (in_range1 r262s)))
(assert (not (< 0 (length (mk___rep (mk___split_discrs container__split_fields) (mk___split_fields container__split_fields1))))))
(check-sat)
(exit)
