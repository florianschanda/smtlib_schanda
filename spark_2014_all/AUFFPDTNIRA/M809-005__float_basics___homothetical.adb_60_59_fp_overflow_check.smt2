...
(assert (= lambda2 o6))
(assert (and (= o7 (fp.mul RNE (to_rep (select ka i1)) (fp.abs (to_rep (select d j1))))) (fp.isFinite32 (fp.mul RNE (to_rep (select ka i1)) (fp.abs (to_rep (select d j1)))))))
(assert (not (fp.isFinite32 (fp.mul RNE (to_rep (select ka j1)) (fp.abs (to_rep (select d i1)))))))
(check-sat)
(exit)
