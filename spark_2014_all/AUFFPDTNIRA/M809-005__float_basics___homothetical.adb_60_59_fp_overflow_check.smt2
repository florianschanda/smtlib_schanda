...
(assert (= lambda5 o20))
(assert (and (= o21 (fp.mul RNE (to_rep (select ka i1)) (fp.abs (to_rep (select d j2))))) (fp.isFinite32 (fp.mul RNE (to_rep (select ka i1)) (fp.abs (to_rep (select d j2)))))))
(assert (not (fp.isFinite32 (fp.mul RNE (to_rep (select ka j2)) (fp.abs (to_rep (select d i1)))))))
(check-sat)
(exit)
