...
(assert (not (= scan_sfun21 true)))
(declare-const i Int)
(assert (<= 0 i))
(assert (<= i (to_rep1 (rec__step_function__step_function_t__number_of_delimiters (us_split_fields3 sfun2)))))
(assert (not (exists ((j Int)) (and (and (<= 0 j) (<= j (- im1 1))) (= (to_rep2 (rec__step_function__delimiter_entry__delimiter (us_split_fields1 (select (rec__step_function__step_function_t__step (us_split_fields3 sfun2)) i)))) (to_rep2 (rec__step_function__delimiter_entry__delimiter (us_split_fields1 (select (rec__step_function__step_function_t__step merge__split_fields2) j)))))))))
(check-sat)
(exit)
