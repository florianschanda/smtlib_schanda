...
(assert (forall ((i2 Int)) (=> (and (<= (to_rep1 (first (rt arr))) i2) (<= i2 (to_rep1 (last (rt arr))))) (=> (not (= i2 (to_rep1 (first (rt arr))))) (fp.lt (to_rep (select (elts arr) (- i2 1))) (to_rep (select (elts arr) i2)))))))
(assert (<= (to_rep1 (first (rt arr))) i1))
(assert (<= i1 (to_rep1 (last (rt arr)))))
(assert (<= (to_rep1 (first (rt arr))) j))
(assert (<= j (to_rep1 (last (rt arr)))))
(assert (< i1 j))
(assert (not (<= (to_rep1 (first (rt arr))) j)))
(check-sat)
(exit)
