...
(assert (and (= algorithm__buildgapvector__B_10__dist__assume o13) (fp.isFinite32 o13)))
(assert (= algorithm__buildgapvector__B_10__dist__assume dist2))
(assert (fp.isFinite32 dist2))
(assert (fp.leq mingapwidth dist2))
(assert (= o14 (create__2 (rec__algorithm__laser_scan_data__second rayr__split_fields7) (fp.neg robotradius) 1)))
(assert (not (< (length (mk___rep2 gapvec__split_discrs (us_split_fields__content2 gapvec__split_fields6))) (to_rep4 (rec__algorithm__gap_vectors__list__capacity gapvec__split_discrs)))))
(check-sat)
(exit)
