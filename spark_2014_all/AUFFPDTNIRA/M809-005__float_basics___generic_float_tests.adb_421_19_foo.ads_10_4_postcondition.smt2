...
(declare-datatypes () ((map__ref (mk_map__ref (map__content (Array Int ft))))))
(declare-fun slide ((Array Int ft) Int Int) (Array Int ft))
(assert (forall ((a (Array Int ft))) (forall ((first Int)) (! (= (slide a first first) a) :pattern ((slide a first first))))))
(assert (forall ((a (Array Int ft))) (forall ((old_first Int)) (forall ((new_first Int)) (forall ((i Int)) (! (= (select (slide a old_first new_first) i) (select a (- i (- new_first old_first)))) :pattern ((select (slide a old_first new_first) i))))))))
(declare-fun concat1 ((Array Int ft) Int Int (Array Int ft) Int Int) (Array Int ft))
...
(assert (forall ((v ft)) (forall ((i Int)) (! (= (select (singleton1 v i) i) v) :pattern ((select (singleton1 v i) i))))))
(define-fun bool_eq ((a (Array Int ft)) (a__first Int) (a__last Int) (b (Array Int ft)) (b__first Int) (b__last Int)) Bool (ite (and (ite (<= a__first a__last) (and (<= b__first b__last) (= (- a__last a__first) (- b__last b__first))) (< b__last b__first)) (forall ((temp___idx_132 Int)) (=> (and (<= a__first temp___idx_132) (<= temp___idx_132 a__last)) (= (to_rep (select a temp___idx_132)) (to_rep (select b (+ (- b__first a__first) temp___idx_132))))))) true false))
...
(assert (forall ((temp___210 Float32)) (forall ((temp___211 Int)) (= (select (foo__float_tests__test_array_1__a__aggregate_def temp___210) temp___211) (of_rep temp___210)))))
...
(assert (= result a))
(assert (= a1 (foo__float_tests__test_array_1__a__aggregate_def (fp #b0 #b00000000 #b00000000000000000000000))))
(assert (= (to_rep o) x))
