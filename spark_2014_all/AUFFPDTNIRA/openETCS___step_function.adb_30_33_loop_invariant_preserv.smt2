...
(assert (= result1 (ite (<= (to_rep2 (rec__step_function__delimiter_entry__delimiter (us_split_fields1 (select (rec__step_function__step_function_t__step (us_split_fields3 sfun)) i2)))) x) o7 false)))
(assert (not (= result1 true)))
(assert (= step_function__get_value__result1 step_function__get_value__result))
(assert (not (= i2 (- (to_rep1 (rec__step_function__step_function_t__number_of_delimiters (us_split_fields3 sfun))) 1))))
(assert (= result2 i2))
(assert (= i3 (+ i2 1)))
(declare-const j Int)
(assert (<= 1 j))
