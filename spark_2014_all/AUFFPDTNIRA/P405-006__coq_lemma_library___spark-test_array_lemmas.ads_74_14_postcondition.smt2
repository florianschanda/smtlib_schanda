...
(assert (forall ((i Int)) (=> (and (<= (+ (to_rep1 (first (rt arr))) 1) i) (<= i (to_rep1 (last (rt arr))))) (fp.lt (to_rep (select (elts arr) (- i 1))) (to_rep (select (elts arr) i))))))
...
(assert (< i j))
(assert (not (fp.lt (to_rep (select (elts arr) i)) (to_rep (select (elts arr) j)))))
(check-sat)
(exit)
