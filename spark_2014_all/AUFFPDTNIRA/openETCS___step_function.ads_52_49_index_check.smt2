...
(assert (forall ((x function_range)) (! (in_range2 (to_rep1 x)) :pattern ((to_rep1 x)))))
...
(declare-const i Int)
(assert (<= 1 i))
(assert (<= i (to_rep (rec__step_function__step_function_t__number_of_delimiters (us_split_fields3 sfun1)))))
(assert (not (<= 0 i)))
(check-sat)
(exit)
