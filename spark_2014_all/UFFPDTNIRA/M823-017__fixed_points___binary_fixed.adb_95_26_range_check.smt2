...
(assert (forall ((x Int)) (! (ite (= x 0) (= (to_int4 x) 0) (ite (< 0 x) (pos_div_relation (to_int4 x) x 256) (pos_div_relation (- (to_int4 x)) (- x) 256))) :pattern ((to_int4 x)))))
...
(assert (forall ((x Int)) (! (ite (= x 0) (= (to_int5 x) 0) (ite (< 0 x) (pos_div_relation1 (to_int5 x) x 256) (pos_div_relation1 (- (to_int5 x)) (- x) 256))) :pattern ((to_int5 x)))))
...
(assert (= result5 y3))
(assert (= y31 o4))
(assert (not (in_range5 (to_int4 (- (+ (to_fixed1 x) (to_fixed1 x)) (to_fixed1 x))))))
(check-sat)
(exit)
