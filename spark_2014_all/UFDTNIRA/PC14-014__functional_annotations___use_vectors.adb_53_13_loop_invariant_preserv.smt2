...
(assert (<= first j))
(assert (<= j (- i3 1)))
(assert (not (= (get (model__ (mk___rep v__split_discrs v__split_fields3)) (+ j (length (mk___rep v__split_discrs (mk___split_fields v__split_fields))))) (get (model__ (mk___rep v__split_discrs (mk___split_fields v__split_fields))) j))))
(check-sat)
(exit)
