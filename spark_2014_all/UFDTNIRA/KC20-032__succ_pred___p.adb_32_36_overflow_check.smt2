...
(declare-sort volt 0)
(define-fun in_range ((x Int)) Bool (and (<= 0 x) (<= x 2040)))
...
(assert (forall ((x Int)) (! (ite (= x 0) (= (to_int1 x) 0) (ite (< 0 x) (pos_div_relation (to_int1 x) (* x 1) 8) (pos_div_relation (- (to_int1 x)) (* (- x) 1) 8))) :pattern ((to_int1 x)))))
(declare-fun of_int (Int) Int)
(assert (forall ((x Int)) (! (ite (= x 0) (= (of_int x) 0) (ite (< 0 x) (pos_div_relation (of_int x) (* x 8) 1) (pos_div_relation (- (of_int x)) (* (- x) 8) 1))) :pattern ((of_int x)))))
...
(declare-const dummy1 tvoltB)
(declare-fun attr__ATTRIBUTE_IMAGE1 (Int) us_image)
(declare-fun attr__ATTRIBUTE_VALUE__pre_check1 (us_image) Bool)
(declare-fun attr__ATTRIBUTE_VALUE1 (us_image) Int)
(declare-fun to_fixed1 (tvoltB) Int)
(declare-fun of_fixed1 (Int) tvoltB)
(declare-fun user_eq1 (tvoltB tvoltB) Bool)
...
(assert (forall ((x tvoltB)) (! (= (of_fixed1 (to_fixed1 x)) x) :pattern ((to_fixed1 x)))))
(assert (forall ((x tvoltB)) (in_range1 (to_fixed1 x))))
...
(assert (not (= (to_fixed v) 2040)))
(assert (not (not (= (to_fixed v) 32767))))
(check-sat)
(exit)
