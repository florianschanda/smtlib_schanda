...
(assert (and (= o2 j2) (in_range1 j2)))
(assert (not (= (divides o2 o3) true)))
(assert (forall ((x3 Int)) (=> (and (<= (+ c2 1) x3) (<= x3 j3)) (not (= (divides x3 (ite (< a b) a b)) true)))))
(assert (and (=> (<= r4b (- (ite (< a b) a b) 1)) (dynamic_property r4b (- (ite (< a b) a b) 1) j3)) (and (<= r4b j3) (<= j3 (- (ite (< a b) a b) 1)))))
(assert (not (= j3 (- (ite (< a b) a b) 1))))
(assert (= j3 result4))
(assert (= j4 (+ j3 1)))
(assert (and (= o7 (ite (< a b) a b)) (in_range1 (ite (< a b) a b))))
(assert (and (= o6 j4) (in_range1 j4)))
(assert (not (= (divides o6 o7) true)))
(declare-const x3 Int)
(assert (<= (+ c2 1) x3))
(assert (<= x3 j4))
(assert (not (not (= (divides x3 (ite (< a b) a b)) true))))
(check-sat)
(exit)
