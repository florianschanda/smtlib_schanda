...
(assert (forall ((x Int)) (=> (and (<= (+ c2 1) x) (<= x j2)) (not (= (divides x (ite (< a b) a b)) true)))))
(assert (and (=> (<= r4b (- (ite (< a b) a b) 1)) (dynamic_property r4b (- (ite (< a b) a b) 1) j2)) (and (<= r4b j2) (<= j2 (- (ite (< a b) a b) 1)))))
(assert (not (= j2 (- (ite (< a b) a b) 1))))
(assert (= j2 result3))
(assert (= j3 (+ j2 1)))
(assert (and (= o5 (ite (< a b) a b)) (in_range1 (ite (< a b) a b))))
(assert (and (= o4 j3) (in_range1 j3)))
(assert (not (= (divides o4 o5) true)))
(declare-const x Int)
(assert (<= (+ c2 1) x))
(assert (<= x j3))
(assert (not (not (= (divides x (ite (< a b) a b)) true))))
(check-sat)
(exit)
