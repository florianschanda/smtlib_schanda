...
(declare-fun to_fixed (tsignal_typeB) Int)
(declare-fun of_fixed (Int) tsignal_typeB)
...
(assert (forall ((x tsignal_typeB)) (! (= (of_fixed (to_fixed x)) x) :pattern ((to_fixed x)))))
(assert (forall ((x tsignal_typeB)) (in_range (to_fixed x))))
(assert (forall ((x Int)) (! (=> (in_range x) (= (to_fixed (of_fixed x)) x)) :pattern ((to_fixed (of_fixed x))))))
(declare-datatypes () ((tsignal_typeB__ref (mk_tsignal_typeB__ref (tsignal_typeB__content tsignal_typeB)))))
(define-fun tsignal_typeB__ref___projection ((a tsignal_typeB__ref)) tsignal_typeB (tsignal_typeB__content a))
...
(declare-fun to_fixed1 (signal_type) Int)
(declare-fun of_fixed1 (Int) signal_type)
...
(assert (forall ((x signal_type)) (! (= (of_fixed1 (to_fixed1 x)) x) :pattern ((to_fixed1 x)))))
(assert (forall ((x signal_type)) (in_range1 (to_fixed1 x))))
(assert (forall ((x Int)) (! (=> (in_range1 x) (= (to_fixed1 (of_fixed1 x)) x)) :pattern ((to_fixed1 (of_fixed1 x))))))
(declare-datatypes () ((signal_type__ref (mk_signal_type__ref (signal_type__content signal_type)))))
(define-fun signal_type__ref___projection ((a signal_type__ref)) signal_type (signal_type__content a))
(declare-const a signal_type)
(declare-const attr__ATTRIBUTE_ADDRESS Int)
(declare-const b signal_type)
(declare-const attr__ATTRIBUTE_ADDRESS1 Int)
(assert (not (in_range (+ (to_fixed1 a) (to_fixed1 b)))))
(check-sat)
(exit)
