...
(assert (forall ((x ttB)) (in_range1 (to_fixed x))))
...
(declare-sort t 0)
(define-fun in_range2 ((x Int)) Bool (and (<= (- 2560) x) (<= x 2560)))
...
(assert (forall ((x t)) (in_range2 (to_fixed1 x))))
...
(assert (not (<= 0 (to_fixed1 x))))
(assert (< (to_fixed1 x) 0))
(assert (< (* (to_fixed1 x) 2) (to_fixed1 x)))
(assert (not (< (+ (to_fixed1 x) (to_fixed1 x)) (to_fixed1 x))))
(check-sat)
(exit)
