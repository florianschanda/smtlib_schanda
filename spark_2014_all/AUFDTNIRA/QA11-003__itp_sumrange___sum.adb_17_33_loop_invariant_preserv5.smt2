...
(assert (and (= o4 (+ s2 (to_rep (select a k2)))) (in_range1 (+ s2 (to_rep (select a k2))))))
(assert (= s2 result2))
(assert (= s3 o4))
(assert (not (= k2 (- j 1))))
(assert (= result3 k2))
(assert (= k3 (+ k2 1)))
(assert (forall ((a1 (Array Int bounded_integer)) (x Int) (j1 Int)) (=> (and (< 0 j1) (<= j1 300)) (=> (and (< 0 x) (<= x j1)) (= (sum a1 (- j1 x) j1) (+ (sum a1 (- j1 x) (- j1 1)) (to_rep (select a1 (- j1 1)))))))))
(assert (forall ((x Int) (j1 Int)) (=> (and (< 0 j1) (<= j1 300)) (=> (and (< 0 x) (<= x j1)) (= (sum a (- j1 x) j1) (+ (sum a (- j1 x) (- j1 1)) (to_rep (select a (- j1 1)))))))))
(assert (not (= s3 (sum a i k3))))
(check-sat)
(exit)
