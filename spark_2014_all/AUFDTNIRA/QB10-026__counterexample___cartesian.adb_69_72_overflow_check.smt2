...
(assert (<= (to_rep (first (rt array1))) i2))
(assert (<= i2 (to_rep (last (rt array1)))))
(assert (<= (to_rep (first (rt array2))) j6))
(assert (<= j6 (to_rep (last (rt array2)))))
(assert (and (<= (to_rep (first (rt array1))) i2) (<= i2 (to_rep (last (rt array1))))))
(assert (= o114 i2))
(assert (= o115 (select (elts array1) o114)))
(assert (= o116 (to_rep o115)))
(assert (not (in_range1 (- j6 (to_rep (first (rt array2)))))))
(check-sat)
(exit)
