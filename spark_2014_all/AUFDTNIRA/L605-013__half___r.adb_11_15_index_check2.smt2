...
(assert (and (= (to_rep a__first) 1) (<= 1 (to_rep a__last))))
(assert (=> (<= (- 2147483648) 2147483647) (in_range1 h)))
(assert (= (mk___t a (mk (to_rep a__first) (to_rep a__last))) a_old))
(assert (dynamic_invariant1 a_old true false true true))
(assert (= result h))
(assert (= h1 (div1 (to_rep a__last) 2)))
(assert (<= (* 2 h1) (to_rep a__last)))
(assert (<= (+ (to_rep a__first) h1) (to_rep a__last)))
(assert (= h1 r2b))
(assert (in_range1 r2b))
(assert (= result1 q))
(assert (= q1 (to_rep a__first)))
(assert (<= (to_rep a__first) q1))
(assert (<= q1 r2b))
(assert (and (<= (to_rep a__first) q2) (<= q2 r2b)))
(assert (and (= (to_rep a__first) (to_rep (first (rt a_old)))) (= (to_rep a__last) (to_rep (last (rt a_old))))))
(assert (and (= temp___176 (+ q2 h1)) (in_range1 (+ q2 h1))))
(assert (not (<= (to_rep a__first) temp___176)))
(check-sat)
(exit)
