...
(assert (<= i5 (to_rep (last (rt array1)))))
(declare-const j9 Int)
(assert (<= (to_rep (first (rt array2))) j9))
(assert (<= j9 (to_rep (last (rt array2)))))
(assert (not (= (to_rep (select (select (map__content1 result__23) (+ (+ (* (- i5 (to_rep (first (rt array1)))) (length array2)) (- j9 (to_rep (first (rt array2))))) 1)) 2)) (to_rep (select (elts array2) j9)))))
(check-sat)
(exit)
