...
(declare-const k Int)
(assert (<= 0 k))
(assert (<= k (- (to_rep1 (rec__partition_refinement__interval__first p_elem__split_fields3)) 1)))
(assert (not (<= (to_rep1 (rec__partition_refinement__interval__first (us_split_fields1 (get (model__ (mk___rep1 p__split_discrs p__split_fields3)) (to_rep2 (select (map__content f6) k)))))) k)))
(check-sat)
(exit)
