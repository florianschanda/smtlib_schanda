...
(declare-const v Int)
(assert (<= value v))
(assert (<= v (ite (< 100000 (- (+ value (abs (- value test_prime_and_coprime_numbers__p__nearest_prime_number__result4))) 1)) 100000 (- (+ value (abs (- value test_prime_and_coprime_numbers__p__nearest_prime_number__result4))) 1))))
(assert (not (not (= (is_prime v) true))))
(check-sat)
(exit)
