...
(assert (= o7 (getblock (rec__certtypes__rawdatat__rawdata (us_split_fields1 rawcertdata)) j2 size3)))
(assert (= result8 (mk_map__ref block)))
(assert (= block1 o7))
(assert (in_range8 retvalupd3))
(assert (not (not (= retvalupd3 0))))
(assert (not (= j2 loopmax1)))
(assert (not (in_range2 (- bytesleft2 100))))
(check-sat)
(exit)
