...
(assert (<= (+ j6 1) k18))
(assert (<= k18 (- (length (mk___rep1 p__split_discrs p__split_fields4)) 1)))
(declare-const l4 Int)
(assert (<= (to_rep1 (rec__partition_refinement__interval__first (us_split_fields1 (get (model__ (mk___rep1 p__split_discrs p__split_fields4)) k18)))) l4))
(assert (<= l4 (to_rep1 (rec__partition_refinement__interval__last (us_split_fields1 (get (model__ (mk___rep1 p__split_discrs p__split_fields4)) k18))))))
(assert (not (= (to_rep2 (select (map__content f8) l4)) k18)))
(check-sat)
(exit)
