...
(assert (= loopmax1 (+ (div1 (- (to_rep3 (rec__certtypes__rawdatat__datalength (us_split_fields1 rawcertdata))) 1) 100) 1)))
(assert (= result5 (mk_int__ref bytesleft)))
(assert (= bytesleft1 (to_rep3 (rec__certtypes__rawdatat__datalength (us_split_fields1 rawcertdata)))))
(assert (= loopmax1 r29b))
(assert (in_range2 r29b))
(assert (= result6 (mk_int__ref j)))
(assert (= j1 1))
(assert (<= 1 j1))
(assert (<= j1 r29b))
(assert (not (= loopmax1 (+ (div1 (- (to_rep3 (rec__certtypes__rawdatat__datalength (us_split_fields1 rawcertdata))) 1) 100) 1))))
(check-sat)
(exit)
