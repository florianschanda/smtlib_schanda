...
(assert (= result7 (mk_int__ref fileindex2)))
(assert (= fileindex3 (to_rep2 (select logfilestate__split_fields2 fileindexinusedlist3))))
(assert (not (< (to_rep5 (select logfilestate__split_fields6 fileindex3)) 1024)))
(assert (= result8 (mk_t__ref thefile2)))
(assert (= thefile3 (select logfiles2 fileindex3)))
(assert (= result9 (mk_bool__ref archivefault4)))
(assert (= archivefault5 (ite (not (= ok6 true)) true false)))
(assert (not (= archivefault5 true)))
(assert (= (to_rep6 o9) 1))
(assert (= o10 (store logfilesstatus2 fileindex3 o9)))
(assert (= result10 (mk_map__ref4 logfilesstatus2)))
(assert (= logfilesstatus3 o10))
(assert (=> (not (= archivedfilecount3 0)) (= thefile5 thefile6)))
(assert (=> (not (= archivedfilecount3 0)) (= description3 description4)))
(assert (=> (not (= archivedfilecount3 0)) (= auditsystemfault3 auditsystemfault4)))
(assert (= result11 (mk_map__ref3 logfiles2)))
(assert (= logfiles3 (store logfiles2 fileindex3 thefile5)))
(assert (not (in_range9 (+ archivedfilecount3 1))))
(check-sat)
(exit)
