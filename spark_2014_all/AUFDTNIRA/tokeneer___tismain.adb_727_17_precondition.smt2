...
(assert (= result3 (currentadminactivitypossible status (us_private__content state5) tokenpresence (us_private__content state1) tokentry tokenid (us_split_fields__content9 authcert__split_fields) (us_split_fields__content12 idcert__split_fields))))
(assert (= result3 true))
(assert (not (not (= (enrolmentisinprogress status (us_private__content state5)) true))))
(check-sat)
(exit)
