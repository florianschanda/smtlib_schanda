(set-info :smt-lib-version 2.5)
(set-info :license |https://www.gnu.org/licenses/gpl-3.0.html|)
(set-logic AUFDTNIRA)
(set-info :source |Benchmarks generated by Florian Schanda from the public SPARK 2014 testsuite and post-processed with a script to set more precise logics.|)
(set-info :category industrial)
(set-info :status unknown)

(declare-datatypes () ((tuple0 (Tuple0))))
(declare-sort us_private 0)
(declare-fun private__bool_eq (us_private us_private) Bool)
(declare-const us_null_ext__ us_private)
(declare-sort us_type_of_heap 0)
(declare-datatypes () ((us_type_of_heap__ref (mk___type_of_heap__ref (us_type_of_heap__content us_type_of_heap)))))
(declare-sort us_image 0)
(declare-datatypes () ((int__ref (mk_int__ref (int__content Int)))))
(declare-datatypes () ((bool__ref (mk_bool__ref (bool__content Bool)))))
(declare-datatypes () ((real__ref (mk_real__ref (real__content Real)))))
(declare-datatypes () ((us_private__ref (mk___private__ref (us_private__content us_private)))))
(define-fun int__ref___projection ((a int__ref)) Int (int__content a))
(define-fun bool__ref___projection ((a bool__ref)) Bool (bool__content a))
(define-fun real__ref___projection ((a real__ref)) Real (real__content a))
(define-fun us_private__ref___projection ((a us_private__ref)) us_private (us_private__content a))
(declare-fun div1 (Int Int) Int)
(declare-fun mod1 (Int Int) Int)
(assert (forall ((x Int) (y Int)) (=> (not (= y 0)) (= x (+ (* y (div1 x y)) (mod1 x y))))))
(assert (forall ((x Int) (y Int)) (=> (and (<= 0 x) (< 0 y)) (and (<= 0 (div1 x y)) (<= (div1 x y) x)))))
(assert (forall ((x Int) (y Int)) (=> (not (= y 0)) (and (< (- (abs y)) (mod1 x y)) (< (mod1 x y) (abs y))))))
(assert (forall ((x Int) (y Int)) (=> (and (<= 0 x) (< 0 y)) (<= 0 (div1 x y)))))
(assert (forall ((x Int) (y Int)) (=> (and (<= x 0) (< 0 y)) (<= (div1 x y) 0))))
(assert (forall ((x Int) (y Int)) (=> (and (<= 0 x) (not (= y 0))) (<= 0 (mod1 x y)))))
(assert (forall ((x Int) (y Int)) (=> (and (<= x 0) (not (= y 0))) (<= (mod1 x y) 0))))
(assert (forall ((x Int) (y Int)) (=> (not (= y 0)) (<= (abs (* (div1 x y) y)) (abs x)))))
(assert (forall ((x Int)) (= (div1 x 1) x)))
(assert (forall ((x Int)) (= (mod1 x 1) 0)))
(assert (forall ((x Int) (y Int)) (=> (and (<= 0 x) (< x y)) (= (div1 x y) 0))))
(assert (forall ((x Int) (y Int)) (=> (and (<= 0 x) (< x y)) (= (mod1 x y) x))))
(assert (forall ((x Int) (y Int) (z Int)) (! (=> (and (< 0 x) (and (<= 0 y) (<= 0 z))) (= (div1 (+ (* x y) z) x) (+ y (div1 z x)))) :pattern ((div1 (+ (* x y) z) x)))))
(assert (forall ((x Int) (y Int) (z Int)) (! (=> (and (< 0 x) (and (<= 0 y) (<= 0 z))) (= (mod1 (+ (* x y) z) x) (mod1 z x))) :pattern ((mod1 (+ (* x y) z) x)))))
(define-fun mod2 ((x Int) (y Int)) Int (ite (< 0 y) (mod x y) (+ (mod x y) y)))
(define-fun in_range ((x Int)) Bool (or (= x 0) (= x 1)))
(declare-fun attr__ATTRIBUTE_IMAGE (Bool) us_image)
(declare-fun attr__ATTRIBUTE_VALUE__pre_check (us_image) Bool)
(declare-fun attr__ATTRIBUTE_VALUE (us_image) Bool)
(declare-sort integer 0)
(define-fun in_range1 ((x Int)) Bool (and (<= (- 2147483648) x) (<= x 2147483647)))
(declare-fun attr__ATTRIBUTE_IMAGE1 (Int) us_image)
(declare-fun attr__ATTRIBUTE_VALUE__pre_check1 (us_image) Bool)
(declare-fun attr__ATTRIBUTE_VALUE1 (us_image) Int)
(declare-fun user_eq (integer integer) Bool)
(declare-const dummy integer)
(declare-datatypes () ((integer__ref (mk_integer__ref (integer__content integer)))))
(define-fun integer__ref___projection ((a integer__ref)) integer (integer__content a))
(declare-fun to_rep (integer) Int)
(declare-fun of_rep (Int) integer)
(assert (forall ((x integer)) (! (= (of_rep (to_rep x)) x) :pattern ((to_rep x)))))
(assert (forall ((x integer)) (! (in_range1 (to_rep x)) :pattern ((to_rep x)))))
(assert (forall ((x Int)) (! (=> (in_range1 x) (= (to_rep (of_rep x)) x)) :pattern ((to_rep (of_rep x))))))
(declare-sort natural 0)
(define-fun in_range2 ((x Int)) Bool (and (<= 0 x) (<= x 2147483647)))
(declare-fun attr__ATTRIBUTE_IMAGE2 (Int) us_image)
(declare-fun attr__ATTRIBUTE_VALUE__pre_check2 (us_image) Bool)
(declare-fun attr__ATTRIBUTE_VALUE2 (us_image) Int)
(declare-fun user_eq1 (natural natural) Bool)
(declare-const dummy1 natural)
(declare-datatypes () ((natural__ref (mk_natural__ref (natural__content natural)))))
(define-fun natural__ref___projection ((a natural__ref)) natural (natural__content a))
(define-fun dynamic_invariant ((temp___expr_39 Int) (temp___is_init_35 Bool) (temp___skip_constant_36 Bool) (temp___do_toplevel_37 Bool) (temp___do_typ_inv_38 Bool)) Bool (=> (or (= temp___is_init_35 true) (<= 0 2147483647)) (in_range2 temp___expr_39)))
(declare-sort positive 0)
(define-fun in_range3 ((x Int)) Bool (and (<= 1 x) (<= x 2147483647)))
(declare-fun attr__ATTRIBUTE_IMAGE3 (Int) us_image)
(declare-fun attr__ATTRIBUTE_VALUE__pre_check3 (us_image) Bool)
(declare-fun attr__ATTRIBUTE_VALUE3 (us_image) Int)
(declare-fun user_eq2 (positive positive) Bool)
(declare-const dummy2 positive)
(declare-datatypes () ((positive__ref (mk_positive__ref (positive__content positive)))))
(define-fun positive__ref___projection ((a positive__ref)) positive (positive__content a))
(define-fun dynamic_invariant1 ((temp___expr_46 Int) (temp___is_init_42 Bool) (temp___skip_constant_43 Bool) (temp___do_toplevel_44 Bool) (temp___do_typ_inv_45 Bool)) Bool (=> (or (= temp___is_init_42 true) (<= 1 2147483647)) (in_range3 temp___expr_46)))
(declare-sort index 0)
(define-fun in_range4 ((x Int)) Bool (and (<= 0 x) (<= x 7)))
(declare-fun attr__ATTRIBUTE_IMAGE4 (Int) us_image)
(declare-fun attr__ATTRIBUTE_VALUE__pre_check4 (us_image) Bool)
(declare-fun attr__ATTRIBUTE_VALUE4 (us_image) Int)
(declare-fun user_eq3 (index index) Bool)
(declare-const dummy3 index)
(declare-datatypes () ((index__ref (mk_index__ref (index__content index)))))
(define-fun index__ref___projection ((a index__ref)) index (index__content a))
(declare-datatypes () ((map__ref (mk_map__ref (map__content (Array Int integer))))))
(declare-fun slide ((Array Int integer) Int Int) (Array Int integer))
(assert (forall ((a (Array Int integer))) (forall ((first Int)) (! (= (slide a first first) a) :pattern ((slide a first first))))))
(assert (forall ((a (Array Int integer))) (forall ((old_first Int)) (forall ((new_first Int)) (forall ((i Int)) (! (= (select (slide a old_first new_first) i) (select a (- i (- new_first old_first)))) :pattern ((select (slide a old_first new_first) i))))))))
(define-fun bool_eq ((a (Array Int integer)) (a__first Int) (a__last Int) (b (Array Int integer)) (b__first Int) (b__last Int)) Bool (ite (and (ite (<= a__first a__last) (and (<= b__first b__last) (= (- a__last a__first) (- b__last b__first))) (< b__last b__first)) (forall ((temp___idx_154 Int)) (=> (and (<= a__first temp___idx_154) (<= temp___idx_154 a__last)) (= (to_rep (select a temp___idx_154)) (to_rep (select b (+ (- b__first a__first) temp___idx_154))))))) true false))
(assert (forall ((a (Array Int integer)) (b (Array Int integer))) (forall ((a__first Int) (a__last Int) (b__first Int) (b__last Int)) (=> (= (bool_eq b b__first b__last a a__first a__last) true) (and (ite (<= a__first a__last) (and (<= b__first b__last) (= (- a__last a__first) (- b__last b__first))) (< b__last b__first)) (forall ((temp___idx_154 Int)) (=> (and (<= a__first temp___idx_154) (<= temp___idx_154 a__last)) (= (to_rep (select a temp___idx_154)) (to_rep (select b (+ (- b__first a__first) temp___idx_154)))))))))))
(declare-fun all_elements_in ((Array Int integer) Int) Bool)
(declare-fun all_elements_in__function_guard (Bool (Array Int integer) Int) Bool)
(assert true)
(assert (forall ((a (Array Int integer))) (forall ((max Int)) (! (= (= (all_elements_in a max) true) (forall ((k Int)) (=> (and (<= 0 k) (<= k 7)) (and (<= (- max) (to_rep (select a k))) (<= (to_rep (select a k)) max))))) :pattern ((all_elements_in a max))))))
(declare-fun all_left_elements_in ((Array Int integer) Int Int) Bool)
(declare-fun all_left_elements_in__function_guard (Bool (Array Int integer) Int Int) Bool)
(define-fun dynamic_invariant2 ((temp___expr_18 Int) (temp___is_init_14 Bool) (temp___skip_constant_15 Bool) (temp___do_toplevel_16 Bool) (temp___do_typ_inv_17 Bool)) Bool (=> (or (= temp___is_init_14 true) (<= (- 2147483648) 2147483647)) (in_range1 temp___expr_18)))
(assert true)
(assert (forall ((a (Array Int integer))) (forall ((right Int) (max Int)) (! (= (= (all_left_elements_in a right max) true) (forall ((k Int)) (=> (and (<= 0 k) (<= k 7)) (=> (< k right) (and (<= (- max) (to_rep (select a k))) (<= (to_rep (select a k)) max)))))) :pattern ((all_left_elements_in a right max))))))
(declare-fun all_right_elements_in ((Array Int integer) Int Int) Bool)
(declare-fun all_right_elements_in__function_guard (Bool (Array Int integer) Int Int) Bool)
(assert true)
(assert (forall ((a (Array Int integer))) (forall ((left Int) (max Int)) (! (= (= (all_right_elements_in a left max) true) (forall ((k Int)) (=> (and (<= 0 k) (<= k 7)) (=> (< left k) (and (<= (- max) (to_rep (select a k))) (<= (to_rep (select a k)) max)))))) :pattern ((all_right_elements_in a left max))))))
(declare-fun intermediate_form ((Array Int integer) (Array Int integer)) Bool)
(declare-fun intermediate_form__function_guard (Bool (Array Int integer) (Array Int integer)) Bool)
(assert true)
(assert (forall ((a (Array Int integer)) (b (Array Int integer))) (! (= (= (intermediate_form a b) true) (forall ((k Int)) (=> (and (<= 0 k) (<= k 7)) (ite (= (mod2 (+ k 1) 8) 0) (= (to_rep (select a k)) (+ (+ (+ (+ (+ (+ (+ (to_rep (select b 0)) (to_rep (select b 1))) (to_rep (select b 2))) (to_rep (select b 3))) (to_rep (select b 4))) (to_rep (select b 5))) (to_rep (select b 6))) (to_rep (select b 7)))) (ite (= (mod2 (+ k 1) 4) 0) (= (to_rep (select a k)) (+ (+ (+ (to_rep (select b k)) (to_rep (select b (- k 1)))) (to_rep (select b (- k 2)))) (to_rep (select b (- k 3))))) (ite (= (mod2 (+ k 1) 2) 0) (= (to_rep (select a k)) (+ (to_rep (select b k)) (to_rep (select b (- k 1))))) (= (to_rep (select a k)) (to_rep (select b k))))))))) :pattern ((intermediate_form a b)))))
(declare-const attr__ATTRIBUTE_ADDRESS Int)
(declare-const attr__ATTRIBUTE_ADDRESS1 Int)
(declare-const attr__ATTRIBUTE_ADDRESS2 Int)
(declare-const attr__ATTRIBUTE_ADDRESS3 Int)
(declare-const attr__ATTRIBUTE_ADDRESS4 Int)
(define-fun dynamic_invariant3 ((temp___expr_159 Int) (temp___is_init_155 Bool) (temp___skip_constant_156 Bool) (temp___do_toplevel_157 Bool) (temp___do_typ_inv_158 Bool)) Bool (=> (or (= temp___is_init_155 true) (<= 0 7)) (in_range4 temp___expr_159)))
(declare-const a (Array Int integer))
(declare-const output_space Int)
(declare-const space Int)
(declare-const left Int)
(declare-const right Int)
(declare-const temp___212 (Array Int integer))
(declare-const temp___218 (Array Int integer))
(declare-const temp___211 (Array Int integer))
(declare-const temp___226 (Array Int integer))
(declare-const temp___210 (Array Int integer))
(declare-const temp___225 (Array Int integer))
(declare-const temp___217 (Array Int integer))
(declare-const temp___209 Int)
(declare-const temp___207 (Array Int integer))
(declare-const temp___216 (Array Int integer))
(declare-const temp___206 Int)
(declare-const temp___223 (Array Int integer))
(declare-const temp___215 (Array Int integer))
(declare-const temp___214 (Array Int integer))
(declare-const temp___221 (Array Int integer))
(declare-const temp___205 Int)
(declare-const temp___213 (Array Int integer))
(declare-const temp___228 (Array Int integer))
(declare-const temp___219 (Array Int integer))
(declare-const temp___229 Int)
(declare-const temp___203 (Array Int integer))
(declare-const temp___199 Int)
(declare-const temp___197 (Array Int integer))
(declare-const temp___201 (Array Int integer))
(declare-const temp___196 Int)
(declare-const temp___200 (Array Int integer))
(declare-const temp___204 Int)
(declare-const o Int)
(declare-const o1 Int)
(declare-const o2 integer)
(declare-const o3 Int)
(declare-const o4 integer)
(declare-const o5 Int)
(declare-const o6 Int)
(declare-const o7 Int)
(declare-const o8 Int)
(declare-const o9 integer)
(declare-const o10 Int)
(declare-const o11 (Array Int integer))
(declare-const o12 Int)
(declare-const o13 Int)
(declare-const o14 Int)
(declare-const o15 Int)
(declare-const o16 Int)
(declare-const o17 Int)
(declare-const a1 (Array Int integer))
(declare-const left1 Int)
(declare-const right1 Int)
(declare-const result Int)
(declare-const space1 Int)
(declare-const a2 (Array Int integer))
(declare-const space2 Int)
(declare-const left2 Int)
(declare-const right2 Int)
(declare-const temp___2291 Int)
(declare-const rho Int)
(declare-const result1 Int)
(declare-const temp___2292 Int)
(declare-const result2 Int)
(declare-const left3 Int)
(declare-const a3 (Array Int integer))
(declare-const left4 Int)
(declare-const right3 Int)
(declare-const temp___2041 Int)
(declare-const result3 Int)
(declare-const temp___2042 Int)
(declare-const result4 Int)
(declare-const right4 Int)
(declare-const result5 (Array Int integer))
(declare-const a4 (Array Int integer))
(declare-const result6 Int)
(declare-const left5 Int)
(declare-const a5 (Array Int integer))
(declare-const left6 Int)
(declare-const right5 Int)
(declare-const a6 (Array Int integer))
(declare-const left7 Int)
(declare-const right6 Int)
(declare-const rho1 Int)
(declare-const a7 map__ref)
(declare-const left8 int__ref)
(declare-const right7 int__ref)
(declare-const a8 (Array Int integer))
(declare-const left9 Int)
(declare-const right8 Int)
(declare-const rho2 Int)
(declare-const result7 int__ref)
(declare-const space3 Int)
(declare-const a9 map__ref)
(declare-const space4 int__ref)
(declare-const left10 int__ref)
(declare-const right9 int__ref)
(declare-const a10 (Array Int integer))
(declare-const space5 Int)
(declare-const left11 Int)
(declare-const right10 Int)
(declare-const a11 map__ref)
(declare-const space6 int__ref)
(declare-const left12 int__ref)
(declare-const right11 int__ref)
(declare-const a12 (Array Int integer))
(declare-const space7 Int)
(declare-const left13 Int)
(declare-const right12 Int)
(declare-const result8 int__ref)
(declare-const output_space1 Int)
(declare-const a13 map__ref)
(declare-const output_space2 int__ref)
(declare-const space8 int__ref)
(declare-const left14 int__ref)
(declare-const right13 int__ref)
(declare-const a14 (Array Int integer))
(declare-const output_space3 Int)
(declare-const space9 Int)
(declare-const left15 Int)
(declare-const right14 Int)
(assert (=> (<= 1 2147483647) (in_range3 output_space)))
(assert (= (all_elements_in a 1000000) true))
(assert (= (mk_int__ref result) (mk_int__ref space)))
(assert (= space1 1))
(assert (in_range3 space1))
(assert (=> (<= 0 2147483647) (in_range2 left)))
(assert (=> (<= 0 2147483647) (in_range2 right)))
(assert (=> (< space1 8) (= temp___212 a)))
(assert (=> (< space1 8) (= temp___218 a)))
(assert (=> (< space1 8) (= temp___211 a)))
(assert (=> (< space1 8) (= temp___226 a)))
(assert (=> (< space1 8) (= temp___210 a)))
(assert (=> (< space1 8) (= temp___225 a)))
(assert (=> (< space1 8) (= temp___217 a)))
(assert (=> (< space1 8) (= temp___209 right)))
(assert (=> (< space1 8) (= temp___207 a)))
(assert (=> (< space1 8) (= temp___216 a)))
(assert (=> (< space1 8) (= temp___206 left)))
(assert (=> (< space1 8) (= temp___223 a)))
(assert (=> (< space1 8) (= temp___215 a)))
(assert (=> (< space1 8) (= temp___214 a)))
(assert (=> (< space1 8) (= temp___221 a)))
(assert (=> (< space1 8) (= temp___205 space1)))
(assert (=> (< space1 8) (= temp___213 a)))
(assert (=> (< space1 8) (= temp___228 a)))
(assert (=> (< space1 8) (= temp___219 a)))
(assert (=> (< space1 8) (= 0 temp___229)))
(assert (=> (< space1 8) (and (and (= (all_elements_in a2 (* space2 1000000)) true) (or (or (= space2 1) (= space2 2)) (= space2 4))) (forall ((k Int)) (=> (and (<= 0 k) (<= k 7)) (ite (and (= (mod2 (+ k 1) 8) 0) (= space2 8)) (= (to_rep (select a2 k)) (+ (+ (+ (+ (+ (+ (+ (to_rep (select temp___210 0)) (to_rep (select temp___211 1))) (to_rep (select temp___212 2))) (to_rep (select temp___213 3))) (to_rep (select temp___214 4))) (to_rep (select temp___215 5))) (to_rep (select temp___216 6))) (to_rep (select temp___217 7)))) (ite (and (= (mod2 (+ k 1) 4) 0) (<= 4 space2)) (= (to_rep (select a2 k)) (+ (+ (+ (to_rep (select temp___218 k)) (to_rep (select temp___219 (- k 1)))) (to_rep (select temp___221 (- k 2)))) (to_rep (select temp___223 (- k 3))))) (ite (and (= (mod2 (+ k 1) 2) 0) (<= 2 space2)) (= (to_rep (select a2 k)) (+ (to_rep (select temp___225 k)) (to_rep (select temp___226 (- k 1))))) (= (to_rep (select a2 k)) (to_rep (select temp___228 k)))))))))))
(assert (=> (< space1 8) (and (and (and (=> (<= 1 2147483647) (in_range3 space2)) (=> (<= 0 2147483647) (in_range2 left2))) (=> (<= 0 2147483647) (in_range2 right2))) (< space2 8))))
(assert (=> (< space1 8) (= temp___2291 result1)))
(assert (=> (< space1 8) (= temp___2292 space2)))
(assert (=> (< space1 8) (= left2 result2)))
(assert (=> (< space1 8) (= left3 (- space2 1))))
(assert (=> (< space1 8) (=> (< left3 8) (= temp___203 a2))))
(assert (=> (< space1 8) (=> (< left3 8) (= temp___199 right2))))
(assert (=> (< space1 8) (=> (< left3 8) (= temp___197 a2))))
(assert (=> (< space1 8) (=> (< left3 8) (= temp___201 a2))))
(assert (=> (< space1 8) (=> (< left3 8) (= temp___196 left3))))
(assert (=> (< space1 8) (=> (< left3 8) (= temp___200 a2))))
(assert (=> (< space1 8) (=> (< left3 8) (= 0 temp___204))))
(assert (=> (< space1 8) (=> (< left3 8) (and (and (and (and (and (= (mod2 (+ left4 1) space2) 0) (= (all_left_elements_in a3 left4 (* (* space2 2) 1000000)) true)) (= (all_right_elements_in a3 (- left4 1) (* space2 1000000)) true)) (= (mod2 (+ left4 1) (* space2 2)) space2)) (=> (<= 8 left4) (or (= left4 8) (= left4 9)))) (forall ((k Int)) (=> (and (<= 0 k) (<= k 7)) (ite (and (and (<= 0 k) (<= k (- left4 space2))) (= (mod2 (+ k 1) (* 2 space2)) 0)) (= (to_rep (select a3 k)) (+ (to_rep (select temp___200 k)) (to_rep (select temp___201 (- k space2))))) (= (to_rep (select a3 k)) (to_rep (select temp___203 k))))))))))
(assert (=> (< space1 8) (=> (< left3 8) (and (and (=> (<= 0 2147483647) (in_range2 left4)) (=> (<= 0 2147483647) (in_range2 right3))) (< left4 8)))))
(assert (=> (< space1 8) (=> (< left3 8) (= temp___2041 result3))))
(assert (=> (< space1 8) (=> (< left3 8) (= temp___2042 left4))))
(assert (=> (< space1 8) (=> (< left3 8) (and (= o (+ left4 space2)) (in_range1 (+ left4 space2))))))
(assert (=> (< space1 8) (=> (< left3 8) (= right3 result4))))
(assert (=> (< space1 8) (=> (< left3 8) (= right4 o))))
(assert (=> (< space1 8) (=> (< left3 8) (and (<= 0 right4) (<= right4 7)))))
(assert (=> (< space1 8) (=> (< left3 8) (= o3 right4))))
(assert (=> (< space1 8) (=> (< left3 8) (= o4 (select a3 o3)))))
(assert (=> (< space1 8) (=> (< left3 8) (= o5 (to_rep o4)))))
(assert (=> (< space1 8) (=> (< left3 8) (and (<= 0 left4) (<= left4 7)))))
(assert (=> (< space1 8) (=> (< left3 8) (= o1 left4))))
(assert (=> (< space1 8) (=> (< left3 8) (= o2 (select a3 o1)))))
(assert (=> (< space1 8) (=> (< left3 8) (= o6 (to_rep o2)))))
(assert (=> (< space1 8) (=> (< left3 8) (= o7 (+ o6 o5)))))
(assert (=> (< space1 8) (=> (< left3 8) (and (= o8 o7) (in_range1 o7)))))
(assert (=> (< space1 8) (=> (< left3 8) (= (to_rep o9) o8))))
(assert (=> (< space1 8) (=> (< left3 8) (and (<= 0 right4) (<= right4 7)))))
(assert (=> (< space1 8) (=> (< left3 8) (= o10 right4))))
(assert (=> (< space1 8) (=> (< left3 8) (= o11 (store a3 o10 o9)))))
(assert (=> (< space1 8) (=> (< left3 8) (= a3 result5))))
(assert (=> (< space1 8) (=> (< left3 8) (= a4 o11))))
(assert (=> (< space1 8) (=> (< left3 8) (and (= o12 (* space2 2)) (in_range1 (* space2 2))))))
(assert (=> (< space1 8) (=> (< left3 8) (= o13 (+ left4 o12)))))
(assert (=> (< space1 8) (=> (< left3 8) (and (= o14 o13) (in_range1 o13)))))
(assert (=> (< space1 8) (=> (< left3 8) (and (= o15 o14) (in_range2 o14)))))
(assert (=> (< space1 8) (=> (< left3 8) (= left4 result6))))
(assert (=> (< space1 8) (=> (< left3 8) (= left5 o15))))
(assert (=> (< space1 8) (=> (< left3 8) (not (< left5 8)))))
(assert (=> (< space1 8) (=> (< left3 8) (= right4 right5))))
(assert (=> (< space1 8) (=> (< left3 8) (= left5 left6))))
(assert (=> (< space1 8) (=> (< left3 8) (= a4 a5))))
(assert (=> (< space1 8) (=> (< left3 8) (= rho1 temp___2042))))
(assert (=> (< space1 8) (=> (< left3 8) (= right6 right4))))
(assert (=> (< space1 8) (=> (< left3 8) (= left7 left5))))
(assert (=> (< space1 8) (=> (< left3 8) (= a6 a4))))
(assert (=> (< space1 8) (=> (< left3 8) (= right7 (mk_int__ref right5)))))
(assert (=> (< space1 8) (=> (< left3 8) (= left8 (mk_int__ref left6)))))
(assert (=> (< space1 8) (=> (< left3 8) (= a7 (mk_map__ref a5)))))
(assert (=> (< space1 8) (=> (< left3 8) (= rho2 rho1))))
(assert (=> (< space1 8) (=> (< left3 8) (= right8 right6))))
(assert (=> (< space1 8) (=> (< left3 8) (= left9 left7))))
(assert (=> (< space1 8) (=> (< left3 8) (= a8 a6))))
(assert (=> (< space1 8) (=> (not (< left3 8)) (= right7 (mk_int__ref right2)))))
(assert (=> (< space1 8) (=> (not (< left3 8)) (= left8 (mk_int__ref left3)))))
(assert (=> (< space1 8) (=> (not (< left3 8)) (= a7 (mk_map__ref a2)))))
(assert (=> (< space1 8) (=> (not (< left3 8)) (= rho2 rho))))
(assert (=> (< space1 8) (=> (not (< left3 8)) (= right8 right2))))
(assert (=> (< space1 8) (=> (not (< left3 8)) (= left9 left3))))
(assert (=> (< space1 8) (=> (not (< left3 8)) (= a8 a2))))
(assert (=> (< space1 8) (and (= o16 (* space2 2)) (in_range1 (* space2 2)))))
(assert (=> (< space1 8) (and (= o17 o16) (in_range3 o16))))
(assert (=> (< space1 8) (= result7 (mk_int__ref space2))))
(assert (=> (< space1 8) (= space3 o17)))
(assert (=> (< space1 8) (not (< space3 8))))
(assert (=> (< space1 8) (= right9 right7)))
(assert (=> (< space1 8) (= left10 left8)))
(assert (=> (< space1 8) (= space4 (mk_int__ref space3))))
(assert (=> (< space1 8) (= a9 a7)))
(assert (=> (< space1 8) (= right10 right8)))
(assert (=> (< space1 8) (= left11 left9)))
(assert (=> (< space1 8) (= space5 space3)))
(assert (=> (< space1 8) (= a10 a8)))
(assert (=> (< space1 8) (= right11 right9)))
(assert (=> (< space1 8) (= left12 left10)))
(assert (=> (< space1 8) (= space6 space4)))
(assert (=> (< space1 8) (= a11 a9)))
(assert (=> (< space1 8) (= right12 right10)))
(assert (=> (< space1 8) (= left13 left11)))
(assert (=> (< space1 8) (= space7 space5)))
(assert (=> (< space1 8) (= a12 a10)))
(assert (=> (not (< space1 8)) (= right11 (mk_int__ref right))))
(assert (=> (not (< space1 8)) (= left12 (mk_int__ref left))))
(assert (=> (not (< space1 8)) (= space6 (mk_int__ref space1))))
(assert (=> (not (< space1 8)) (= a11 (mk_map__ref a))))
(assert (=> (not (< space1 8)) (= right12 right1)))
(assert (=> (not (< space1 8)) (= left13 left1)))
(assert (=> (not (< space1 8)) (= space7 space1)))
(assert (=> (not (< space1 8)) (= a12 a1)))
(assert (= result8 (mk_int__ref output_space)))
(assert (= output_space1 (int__content space6)))
(assert (= right13 right11))
(assert (= left14 left12))
(assert (= space8 space6))
(assert (= output_space2 (mk_int__ref output_space1)))
(assert (= a13 a11))
(assert (= right14 right12))
(assert (= left15 left13))
(assert (= space9 space7))
(assert (= output_space3 output_space1))
(assert (= a14 a12))
(assert (not (= (all_elements_in (map__content a13) 8000000) true)))
(check-sat)
(exit)
