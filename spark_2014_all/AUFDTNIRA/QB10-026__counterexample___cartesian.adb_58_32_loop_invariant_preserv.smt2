...
(assert (<= i5 (int__content index14)))
(declare-const j9 Int)
(assert (<= (to_rep (first (rt array2))) j9))
(assert (<= j9 (to_rep (last (rt array2)))))
(assert (not (= (to_rep (select (select (map__content1 result__19) (+ (+ (* (- i5 (to_rep (first (rt array1)))) (length array2)) (- j9 (to_rep (first (rt array2))))) 1)) 1)) (to_rep (select (elts array1) i5)))))
(check-sat)
(exit)
