...
(assert (forall ((k1 Int)) (=> (and (<= (to_rep1 (first (rt bk))) k1) (<= k1 (- it3 1))) (<= (price_of_item (select (elts bk) k1)) total2))))
(assert (and (and (=> (<= 0 10000) (in_range3 total2)) (=> (<= (to_rep1 (first (rt bk))) (to_rep1 (last (rt bk)))) (dynamic_property1 (to_rep1 (first (rt bk))) (to_rep1 (last (rt bk))) it3))) (and (<= (to_rep1 (first (rt bk))) it3) (<= it3 (to_rep1 (last (rt bk)))))))
(assert (and (= o (- it3 1)) (in_range1 (- it3 1))))
(assert (= o1 (ite (<= k o) true false)))
(assert (= result2 (ite (<= (to_rep1 (first (rt bk))) k) o1 false)))
(assert (= result2 true))
(assert (not (<= (to_rep1 (first (rt bk))) k)))
(check-sat)
(exit)
