...
(declare-const lo_index2 Int)
(assert (<= (to_rep3 (first (rt x))) lo_index2))
(assert (<= lo_index2 (- lo3 1)))
(declare-const hi_index3 Int)
(assert (<= (to_rep3 (first (rt x))) hi_index3))
(assert (<= hi_index3 (to_rep3 (last (rt x)))))
(assert (not (<= (sum__logic x (mk_slice_bounds (mk___split_fields (of_rep lo_index2) (of_rep1 hi_index3)))) max_sum2)))
(check-sat)
(exit)
