...
(declare-const j Int)
(assert (<= 0 j))
(assert (<= j (- (length1 (mk___rep4 p__split_discrs p__split_fields3)) 1)))
(declare-const k Int)
(assert (<= (to_rep4 (rec__partition_refinement__interval__first (us_split_fields11 (get5 (model__2 (mk___rep4 p__split_discrs p__split_fields3)) j)))) k))
(assert (<= k (to_rep4 (rec__partition_refinement__interval__last (us_split_fields11 (get5 (model__2 (mk___rep4 p__split_discrs p__split_fields3)) j))))))
(assert (not (= (to_rep5 (select f k)) j)))
(check-sat)
(exit)
