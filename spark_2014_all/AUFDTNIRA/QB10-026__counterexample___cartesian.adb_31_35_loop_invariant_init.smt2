...
(assert (<= j9 index22))
(assert (not (= (to_rep (select (select result__2 (+ (+ (* (- index11 (to_rep (first (rt array1)))) (length array2)) (- j9 (to_rep (first (rt array2))))) 1)) 1)) (to_rep (select (elts array1) index11)))))
(check-sat)
(exit)
