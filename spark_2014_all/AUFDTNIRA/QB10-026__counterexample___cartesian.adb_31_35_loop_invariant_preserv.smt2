...
(assert (not (= index23 (to_rep (last (rt array2))))))
(assert (= index23 result6))
(assert (= index24 (+ index23 1)))
(assert (and (= o38 (+ result_index3 1)) (in_range1 (+ result_index3 1))))
(assert (= result_index3 result7))
(assert (= result_index4 o38))
(assert (and (<= (to_rep result____first) result_index4) (<= result_index4 (to_rep result____last))))
(assert (= o39 result_index4))
(assert (= o40 (store result__3 o39 (temp___237 (to_rep (select (elts array1) index11)) (to_rep (select (elts array2) index24))))))
(assert (= result8 result__3))
(assert (= result__4 o40))
(declare-const j9 Int)
(assert (<= (to_rep (first (rt array2))) j9))
(assert (<= j9 index24))
(assert (not (= (to_rep (select (select result__4 (+ (+ (* (- index11 (to_rep (first (rt array1)))) (length array2)) (- j9 (to_rep (first (rt array2))))) 1)) 1)) (to_rep (select (elts array1) index11)))))
(check-sat)
(exit)
