...
(assert (<= i i))
(assert (<= i (- j 1)))
(assert (<= (abs (sum a i k2)) (* k2 2000)))
(assert (and (and (=> (<= (- 2147483648) 2147483647) (in_range1 (sum a i k2))) (=> (<= i (- j 1)) (dynamic_property i (- j 1) k2))) (and (<= i k2) (<= k2 (- j 1)))))
(assert (in_range1 (+ (sum a i k2) (to_rep (select a k2)))))
(assert (not (= k2 (- j 1))))
...
(assert (not (= (+ (sum a i k2) (to_rep (select a k2))) (sum a i (+ k2 1)))))
(check-sat)
(exit)
