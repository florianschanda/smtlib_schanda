...
(declare-const l Int)
(assert (<= (to_rep2 (rec__partition_refinement__interval__first (us_split_fields3 (get (model__ (mk___rep p__split_discrs p__split_fields3)) j2)))) l))
(assert (<= l (to_rep2 (rec__partition_refinement__interval__last (us_split_fields3 (get (model__ (mk___rep p__split_discrs p__split_fields3)) j2))))))
(assert (not (= (to_rep3 (select (map__content f6) l)) j2)))
(check-sat)
(exit)
