...
(assert (= result6 pricing__price_of_basket__result4))
(declare-const it13 Int)
(assert (<= (to_rep1 (first (rt bk))) it13))
(assert (<= it13 (to_rep1 (last (rt bk)))))
(assert (not (<= (price_of_item (select (elts bk) it13)) pricing__price_of_basket__result4)))
(check-sat)
(exit)
