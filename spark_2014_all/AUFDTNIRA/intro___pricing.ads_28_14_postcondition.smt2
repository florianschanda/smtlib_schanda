...
(assert (= result4 pricing__price_of_basket__result4))
(declare-const it10 Int)
(assert (<= (to_rep1 (first (rt bk))) it10))
(assert (<= it10 (to_rep1 (last (rt bk)))))
(assert (not (<= (price_of_item (select (elts bk) it10)) pricing__price_of_basket__result4)))
(check-sat)
(exit)
