...
(assert (forall ((addr (_ BitVec 64))) (forall ((x86__memory (Array (_ BitVec 64) unsigned8))) (! (=> (dynamic_invariant addr true true true true) (let ((result (readmem8 addr x86__memory))) (and (and (= result (to_rep (select x86__memory addr))) (bvuge result #x00)) (dynamic_invariant1 result true false true true)))) :pattern ((readmem8 addr x86__memory))))))
...
(assert (not (not (= (ecx rcx1) #x00000000))))
(check-sat)
(exit)
