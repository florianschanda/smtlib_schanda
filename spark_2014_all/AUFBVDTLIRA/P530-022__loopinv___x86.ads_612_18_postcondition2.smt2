...
(assert (and (= (to_rep (select memory1 addr)) ((_ extract 7 0) (bvand val__ #x00FF))) (forall ((i (_ BitVec 64))) (=> (and (bvule #x0000000000000000 i) (bvule i #xFFFFFFFFFFFFFFFF)) (=> (not (= i addr)) (= (to_rep (select memory1 i)) (to_rep (select memory i))))))))
(assert (=> (< 8 16) (= o (bvlshr val__ ((_ int2bv 16) 8)))))
(assert (=> (not (< 8 16)) (= o #x0000)))
(assert (= o1 (bvand o #x00FF)))
(assert (= o2 o1))
(assert (= o3 ((_ extract 7 0) o2)))
(assert (and (= (to_rep (select memory2 (bvadd addr #x0000000000000001))) o3) (forall ((i (_ BitVec 64))) (=> (and (bvule #x0000000000000000 i) (bvule i #xFFFFFFFFFFFFFFFF)) (=> (not (= i (bvadd addr #x0000000000000001))) (= (to_rep (select memory2 i)) (to_rep (select memory1 i))))))))
(assert (= memory2 memory3))
(assert (= memory4 memory2))
(assert (not (= (bvand val__ #x00FF) ((_ zero_extend 8) (to_rep (select memory3 addr))))))
(check-sat)
(exit)
