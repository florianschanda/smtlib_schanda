...
(assert (= o1 (+ b_first o)))
(assert (and (= temp___628 o1) (in_range1 o1)))
(assert (and (<= (to_rep b__first) temp___628) (<= temp___628 (to_rep b__last))))
(assert (= o6 temp___628))
...
(assert (=> (<= b_first k) (not (<= k (- (- (+ b_first (+ i2 1)) a_first) 1)))))
(assert (not (= (to_rep1 (select b2 k)) (to_rep1 (select b k)))))
(check-sat)
(exit)
