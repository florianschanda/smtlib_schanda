...
(assert (<= (to_rep b__first) k3))
(assert (<= k3 (to_rep b__last)))
(assert (<= b_first k3))
(assert (<= k3 (- (- (+ b_first (+ i2 1)) a_first) 1)))
(assert (not (= (to_rep1 (select b3 k3)) (to_rep1 (select (elts a) (- (+ a_first k3) b_first))))))
(check-sat)
(exit)
