...
(assert (forall ((addr1 (_ BitVec 64))) (forall ((x86__memory (Array (_ BitVec 64) unsigned8))) (! (=> (dynamic_invariant addr1 true true true) (let ((result (readmem8 addr1 x86__memory))) (and (= result (to_rep (select x86__memory addr1))) (dynamic_invariant2 result true false true)))) :pattern ((readmem8 addr1 x86__memory))))))
