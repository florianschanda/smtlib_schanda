...
(assert (and (<= 1 (+ (+ start_byte 5) i2)) (<= (+ (+ start_byte 5) i2) 8180)))
(assert (= o4 (+ (+ start_byte 5) i2)))
(assert (= o5 (store bytes o4 o3)))
(assert (= result1 bytes))
(assert (= bytes1 o5))
(assert (not (= i2 current_name_length)))
(assert (= result2 i2))
(assert (= i3 (+ i2 1)))
(assert (not (<= start_byte (- 8174 current_name_length))))
(check-sat)
(exit)
