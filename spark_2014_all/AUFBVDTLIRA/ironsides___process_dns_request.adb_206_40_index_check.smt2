...
(assert (<= 1 i1))
(assert (<= i1 r15b))
(assert (and (and (and (and (and (<= start_byte (- 8174 current_name_length1)) (<= 1 i2)) (<= i2 129)) (<= 1 current_name_length1)) (<= current_name_length1 129)) (forall ((j Int)) (=> (and (<= 1 j) (<= j 129)) (and (<= 0 (to_rep (select (rec__rr_type__nsec_record_type__nsecrecordtype__nextdomainname (us_split_fields3 nsec_record)) j))) (<= (to_rep (select (rec__rr_type__nsec_record_type__nsecrecordtype__nextdomainname (us_split_fields3 nsec_record)) j)) 255))))))
(assert (and (=> (<= 1 r15b) (dynamic_property 1 r15b i2)) (and (<= 1 i2) (<= i2 r15b))))
(assert (and (= o2 (to_rep (select (rec__rr_type__nsec_record_type__nsecrecordtype__nextdomainname (us_split_fields3 nsec_record)) i2))) (uint_in_range (to_rep (select (rec__rr_type__nsec_record_type__nsecrecordtype__nextdomainname (us_split_fields3 nsec_record)) i2)))))
(assert (= o3 ((_ int2bv 8) o2)))
(assert (= (to_rep1 o4) o3))
(assert (not (<= 1 (+ (- (+ start_byte 6) 1) i2))))
(check-sat)
(exit)
