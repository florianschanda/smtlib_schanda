...
(assert (and (= (readmem8 (bvadd addr ((_ int2bv 64) 7)) memory8) ((_ extract 7 0) (bvand (bvlshr val__ ((_ int2bv 64) 56)) ((_ int2bv 64) 255)))) (forall ((i (_ BitVec 64))) (=> (and (bvule ((_ int2bv 64) 0) i) (bvule i ((_ int2bv 64) 18446744073709551615))) (=> (not (= i (bvadd addr ((_ int2bv 64) 7)))) (= (to_rep (select memory8 i)) (to_rep (select memory7 i))))))))
(assert (= (readmem64 addr memory8) val__))
(assert (= memory8 memory9))
(assert (= memory10 memory8))
(assert (not (= (readmem64 addr memory9) val__)))
(check-sat)
(exit)
