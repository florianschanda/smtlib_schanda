...
(assert (forall ((k Int)) (=> (and (<= a_first k) (<= k a_last)) (= (to_rep1 (select o2 k)) #x00000000))))
...
(assert (<= a_first i1))
(assert (<= i1 a_last))
(assert (and (= (oeq (omod (oadd (num_of_big_int (mk___t o3 (mk (to_rep a__first) (to_rep a__last))) a_first (+ (- a_last a_first) 1)) (omultiply (oexpon (base Tuple0) (+ (- a_last a_first) 1)) (from_word32 a_msw2))) (num_of_big_int m m_first (+ (- a_last a_first) 1))) (omod (omultiply (omultiply (num_of_big_int b b_first (- i2 a_first)) (num_of_big_int c c_first (+ (- a_last a_first) 1))) (oexpon (inverse (num_of_big_int m m_first (+ (- a_last a_first) 1)) (base Tuple0)) (- i2 a_first))) (num_of_big_int m m_first (+ (- a_last a_first) 1)))) true) (= (olt (oadd (num_of_big_int (mk___t o3 (mk (to_rep a__first) (to_rep a__last))) a_first (+ (- a_last a_first) 1)) (omultiply (oexpon (base Tuple0) (+ (- a_last a_first) 1)) (from_word32 a_msw2))) (osubtract (omultiply (from_word32 #x00000002) (num_of_big_int m m_first (+ (- a_last a_first) 1))) (from_word32 #x00000001))) true)))
...
(assert (not (<= temp___675 (to_rep (last (rt b))))))
(check-sat)
(exit)
