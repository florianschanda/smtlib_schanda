...
(assert (= i1 1))
(assert (<= i1 name_length1))
(assert (< i1 129))
(assert (< (+ current_byte2 12) 8192))
(assert (and (and (and (and (< i2 129) (<= i2 name_length1)) (= (to_rep2 (rec__dns_types__header_type__ancount (us_split_fields1 (rec__dns_types__dns_packet__header output_packet__split_fields24)))) 0)) (<= start_byte current_byte3)) (< (+ current_byte3 12) 8192)))
(assert (and (and (and (=> (<= 1 8180) (in_range4 current_byte3)) (=> (<= 1 129) (in_range8 i2))) (= (rec__dns_types__dns_packet__header output_packet__split_fields24) (rec__dns_types__dns_packet__header output_packet__split_fields23))) (and (and (<= i2 name_length1) (< i2 129)) (< (+ current_byte3 12) 8192))))
(assert (and (= o52 (+ current_byte3 1)) (in_range4 (+ current_byte3 1))))
(assert (= result13 (mk_int__ref current_byte3)))
(assert (= current_byte4 o52))
(assert (not (uint_in_range (to_rep (select (rec__rr_type__cname_record_type__cnamerecordtype__canonicaldomainname (us_split_fields7 (select cnames 1))) i2)))))
(check-sat)
(exit)
