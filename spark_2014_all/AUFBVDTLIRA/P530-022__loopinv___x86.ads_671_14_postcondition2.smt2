...
(assert (and (= (to_rep (select memory8 (bvadd addr #x0000000000000007))) ((_ extract 7 0) (bvand (bvlshr val__ ((_ int2bv 64) 56)) #x00000000000000FF))) (forall ((i (_ BitVec 64))) (=> (and (bvule #x0000000000000000 i) (bvule i #xFFFFFFFFFFFFFFFF)) (=> (not (= i (bvadd addr #x0000000000000007))) (= (to_rep (select memory8 i)) (to_rep (select memory7 i))))))))
(assert (= memory8 memory9))
(assert (= memory10 memory8))
(assert (not (= (hasmem64 memory9 addr val__) true)))
(check-sat)
(exit)
