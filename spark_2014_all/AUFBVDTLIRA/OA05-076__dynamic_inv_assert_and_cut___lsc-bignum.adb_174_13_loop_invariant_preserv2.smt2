...
(assert (and (<= (to_rep (first (rt b))) j2) (<= j2 (to_rep (last (rt b))))))
(assert (= o12 j2))
(assert (= o13 (select (elts b) o12)))
(assert (= o14 (to_rep1 o13)))
(assert (and (<= (to_rep (first (rt a))) i2) (<= i2 (to_rep (last (rt a))))))
(assert (= o10 i2))
(assert (= o11 (select (elts a) o10)))
(assert (= o15 (to_rep1 o11)))
(assert (= result4 (ite (bvugt o15 o14) true false)))
(assert (not (= result4 true)))
(assert (not (= i2 a_first)))
(assert (= result5 i2))
(assert (= i3 (- i2 1)))
(assert (not (= (oeq (num_of_big_int a (+ i3 1) (- a_last i3)) (num_of_big_int b (+ (+ b_first (- i3 a_first)) 1) (- a_last i3))) true)))
(check-sat)
(exit)
