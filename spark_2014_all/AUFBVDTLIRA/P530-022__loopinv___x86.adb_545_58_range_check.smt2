...
(assert (and (= (to_rep (select memory1 addr)) ((_ extract 7 0) (bvand val__ #x000000FF))) (forall ((i (_ BitVec 64))) (=> (and (bvule #x0000000000000000 i) (bvule i #xFFFFFFFFFFFFFFFF)) (=> (not (= i addr)) (= (to_rep (select memory1 i)) (to_rep (select memory i))))))))
(assert (and (= (to_rep (select memory2 (bvadd addr #x0000000000000001))) ((_ extract 7 0) (bvand (bvlshr val__ ((_ int2bv 32) 8)) #x000000FF))) (forall ((i (_ BitVec 64))) (=> (and (bvule #x0000000000000000 i) (bvule i #xFFFFFFFFFFFFFFFF)) (=> (not (= i (bvadd addr #x0000000000000001))) (= (to_rep (select memory2 i)) (to_rep (select memory1 i))))))))
(assert (and (= (to_rep (select memory3 (bvadd addr #x0000000000000002))) ((_ extract 7 0) (bvand (bvlshr val__ ((_ int2bv 32) 16)) #x000000FF))) (forall ((i (_ BitVec 64))) (=> (and (bvule #x0000000000000000 i) (bvule i #xFFFFFFFFFFFFFFFF)) (=> (not (= i (bvadd addr #x0000000000000002))) (= (to_rep (select memory3 i)) (to_rep (select memory2 i))))))))
(assert (not (bvule (bvand (bvlshr val__ ((_ int2bv 32) 24)) #x000000FF) ((_ zero_extend 24) #xFF))))
(check-sat)
(exit)
