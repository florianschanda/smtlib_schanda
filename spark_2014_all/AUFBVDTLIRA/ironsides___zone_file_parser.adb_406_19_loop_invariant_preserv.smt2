...
(assert (not (< (to_rep (select (map__content timestring8) i16)) 48)))
(assert (not (< 57 (to_rep (select (map__content timestring8) i16)))))
(assert (= success5 (mk_bool__ref success3)))
(assert (= success6 success3))
(assert (not (= i16 14)))
(assert (= result9 (mk_int__ref i16)))
(assert (= i17 (+ i16 1)))
(assert (= i18 (mk_int__ref i17)))
(assert (= success9 success5))
(assert (= i19 i17))
(assert (= success10 success6))
(declare-const j1 Int)
(assert (<= 1 j1))
(assert (<= j1 (- (int__content i18) 1)))
(assert (not (<= 48 (to_rep (select (map__content timestring8) j1)))))
(check-sat)
(exit)
