...
(assert (and (= (to_rep (select memory4 (bvadd addr ((_ int2bv 64) 3)))) ((_ extract 7 0) (bvand (bvlshr val__ ((_ int2bv 32) 24)) ((_ int2bv 32) 255)))) (forall ((i (_ BitVec 64))) (=> (and (bvule ((_ int2bv 64) 0) i) (bvule i ((_ int2bv 64) 18446744073709551615))) (=> (not (= i (bvadd addr ((_ int2bv 64) 3)))) (= (to_rep (select memory4 i)) (to_rep (select memory3 i))))))))
(assert (= memory4 memory5))
(assert (= memory6 memory4))
(assert (not (= (bvand val__ ((_ int2bv 32) 255)) ((_ zero_extend 24) (to_rep (select memory5 addr))))))
(check-sat)
(exit)
