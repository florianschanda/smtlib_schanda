...
(assert (and (= (to_rep (select memory2 (bvadd addr ((_ int2bv 64) 1)))) o3) (forall ((i (_ BitVec 64))) (=> (and (bvule ((_ int2bv 64) 0) i) (bvule i ((_ int2bv 64) 18446744073709551615))) (=> (not (= i (bvadd addr ((_ int2bv 64) 1)))) (= (to_rep (select memory2 i)) (to_rep (select memory1 i))))))))
(assert (= memory2 memory3))
(assert (= memory4 memory2))
(assert (not (= (bvand val__ ((_ int2bv 16) 255)) ((_ zero_extend 8) (to_rep (select memory3 addr))))))
(check-sat)
(exit)
