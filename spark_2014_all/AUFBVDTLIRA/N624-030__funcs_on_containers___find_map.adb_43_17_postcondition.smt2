...
(assert (and (< 0 (length a)) (and (=> (= (has_key (model__ m) e) true) (and (<= (to_rep1 (first (rt a))) (get (model__ m) e)) (<= (get (model__ m) e) (to_rep1 (last (rt a)))))) (and (forall ((i Int)) (=> (and (<= (to_rep1 (first (rt a))) i) (<= i (- (get (model__ m) e) 1))) (not (= (to_rep (select (elts a) i)) e)))) (= (has_key (model__ m) e) true)))))
...
(assert (= result2 find_map__find_upto__find_map_bad__result4))
(assert (= find_map__find_upto__find_map_bad__result4 0))
(declare-const i Int)
(assert (<= (to_rep1 (first (rt a))) i))
(assert (<= i last2))
(assert (not (not (= (to_rep (select (elts a) i)) e))))
(check-sat)
(exit)
