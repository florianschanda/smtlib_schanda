...
(assert (and (= (readmem8 (bvadd addr #x0000000000000001) memory2) o3) (forall ((x (_ BitVec 64))) (=> (and (bvule #x0000000000000000 x) (bvule x #xFFFFFFFFFFFFFFFF)) (or (= x (bvadd addr #x0000000000000001)) (= (to_rep (select memory2 x)) (to_rep (select memory1 x))))))))
(assert (= memory2 memory3))
(assert (= memory4 memory2))
(assert (not (= (bvor ((_ zero_extend 8) (readmem8 addr memory3)) (ite (< 8 16) (bvshl ((_ zero_extend 8) (readmem8 (bvadd addr #x0000000000000001) memory3)) ((_ int2bv 16) 8)) #x0000)) val__)))
(check-sat)
(exit)
