...
(assert (< digit_index2 i1))
(declare-const j Int)
(assert (<= (+ digit_index2 1) j))
(assert (<= j i1))
(assert (not (= (to_rep (select (elts (rec__aco__math__very_longs__very_long__long_digits (us_split_fields1 number))) j)) #x00)))
(check-sat)
(exit)
