...
(assert (forall ((x unsigned8)) (! (uint_in_range1 (to_int3 x)) :pattern ((to_int3 x)))))
...
(assert (and (= (readmem64 rdi memory) rax) (= rdi (bvadd rdi_start (bvmul (bvsub rcx_start rcx) #x0000000000000008)))))
(assert (not (= (readmem64 (bvadd rdi_start (bvmul (bvsub rcx_start rcx) #x0000000000000008)) memory) rax)))
(check-sat)
(exit)
