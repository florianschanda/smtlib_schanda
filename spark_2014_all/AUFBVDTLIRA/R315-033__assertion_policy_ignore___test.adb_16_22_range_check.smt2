...
(assert (= dest1 o5))
(assert (forall ((j Int)) (=> (and (<= (to_rep dest__first) j) (<= j (to_rep dest__last))) (ite (and (<= index j) (<= j (+ index i2))) (= (to_rep1 (select dest2 j)) (to_rep1 (select (elts src) (- (+ src_first j) index)))) (= (to_rep1 (select dest2 j)) (to_rep1 (select dest j)))))))
(assert (and (=> (<= 0 (- src_length 1)) (dynamic_property1 0 (- src_length 1) i2)) (and (<= 0 i2) (<= i2 (- src_length 1)))))
(assert (<= (to_rep dest__first) (to_rep dest__last)))
(assert (not (<= (to_rep dest__last) 2147483647)))
(check-sat)
(exit)
