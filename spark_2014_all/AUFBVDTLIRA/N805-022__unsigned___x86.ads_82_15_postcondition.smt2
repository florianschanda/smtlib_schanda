...
(assert (forall ((addr (_ BitVec 64))) (forall ((x86__memory (Array (_ BitVec 64) unsigned_8))) (! (=> (dynamic_invariant addr true true true true) (let ((result (readmem8 addr x86__memory))) (and (and (= result (to_rep (select x86__memory addr))) (bvuge result #x00)) (dynamic_invariant1 result true false true true)))) :pattern ((readmem8 addr x86__memory))))))
...
(assert (and (= o1 (readmem8 (bvadd addr #x0000000000000001) memory)) (and (= o1 (to_rep (select memory (bvadd addr #x0000000000000001)))) (bvuge o1 #x00))))
(assert (= o2 ((_ zero_extend 8) o1)))
(assert (= o3 (bvmul o2 #x0100)))
(assert (and (= o (readmem8 addr memory)) (and (= o (to_rep (select memory addr))) (bvuge o #x00))))
...
(assert (not (= x86__readmem16__result4 (bvadd ((_ zero_extend 8) (to_rep (select memory addr))) (bvmul ((_ zero_extend 8) (to_rep (select memory (bvadd addr #x0000000000000001)))) #x0100)))))
(check-sat)
(exit)
