...
(assert (<= 1 j))
(assert (<= j (- i1 1)))
(assert (not (= (to_rep1 (select c__split_fields j)) (+ (to_rep1 (select (elts (rec__simple_unc_arrays__table__v (us_split_fields1 a))) j)) (to_rep1 (select (elts (rec__simple_unc_arrays__table__v (us_split_fields1 b))) j))))))
(check-sat)
(exit)
