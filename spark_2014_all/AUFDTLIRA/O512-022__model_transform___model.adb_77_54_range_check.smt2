...
(assert (<= 0 s11))
(assert (<= s11 100))
(assert (forall ((t2 Int)) (=> (and (<= 0 t2) (<= t2 (- s12 1))) (and (= (name (select (owned_state sm) (from (select (owned_transition sm) t2)))) (name__2 (select (map__content2 av8) (from__2 (select cfv2 t2))))) (= (name (select (owned_state sm) (to__ (select (owned_transition sm) t2)))) (name__2 (select (map__content2 av8) (to__2 (select cfv2 t2)))))))))
(assert (and (=> (<= 0 100) (in_range7 s12)) (and (<= 0 s12) (<= s12 100))))
(assert (<= 0 t1))
(assert (<= t1 (- s12 1)))
(assert (not (in_range5 t1)))
(check-sat)
(exit)
