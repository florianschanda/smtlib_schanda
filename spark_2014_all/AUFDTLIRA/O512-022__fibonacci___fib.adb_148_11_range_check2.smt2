...
(assert (in_range2 k))
(assert (and (<= 0 k) (and (<= k 100) (and (forall ((i Int)) (=> (and (<= 0 i) (<= i k)) (= (to_rep (select b1 i)) (to_rep (select b2 i))))) (forall ((i Int)) (=> (and (<= (+ k 1) i) (<= i 100)) (= (to_rep (select c1 (- i k))) (to_rep (select c2 (- i k))))))))))
(assert (<= 0 k))
(assert (not (<= 0 0)))
(check-sat)
(exit)
