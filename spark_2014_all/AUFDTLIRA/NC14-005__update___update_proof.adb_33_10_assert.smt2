...
(assert (= result2 b))
(assert (= b1 (update_proof__B_2__b__aggregate_def (temp___245 11 12) (temp___250 21 22))))
(assert (= b1 o_b))
(assert (= result3 b1))
(assert (= b2 (store b1 2 (temp___203 0))))
(assert (= (bool_eq (select b2 1) 1 2 (select o_b 1) 1 2) true))
(assert (= (bool_eq (select b2 2) 1 2 (temp___211 0) 1 2) true))
(assert (= (bool_eq2 b2 1 2 (temp___219 o_b 2 (temp___225 0)) 1 2) true))
(declare-const i Int)
(assert (<= 1 i))
(assert (<= i 2))
(assert (not (= (bool_eq (select b2 i) 1 2 (ite (= i 2) (temp___231 0) (select o_b i)) (to_rep (first (mk 1 2))) (to_rep (last (mk 1 2)))) true)))
(check-sat)
(exit)
