...
(assert (= result8 (mk_int__ref ia2)))
(assert (= ia3 (+ ia2 1)))
(assert (= (= result9 true) (exists ((ib2 Int)) (and (and (<= (to_rep2 (first (rt b))) ib2) (<= ib2 (to_rep2 (last (rt b))))) (and (= (to_rep (select (elts a) ia3)) (to_rep (select (elts b) ib2))) (= (is_perm (remove a ia3) (remove b ib2)) true))))))
(assert (not (= result9 true)))
(assert (= perm__get_witnesses__result6 perm__get_witnesses__result5))
(declare-const ka1 Int)
(assert (<= 1 ka1))
(assert (<= ka1 ia3))
(declare-const kb1 Int)
(assert (<= (to_rep2 (first (rt b))) kb1))
(assert (<= kb1 (to_rep2 (last (rt b)))))
(assert (not (not (and (= (to_rep (select (elts a) ka1)) (to_rep (select (elts b) kb1))) (= (is_perm (remove a ka1) (remove b kb1)) true)))))
(check-sat)
(exit)
