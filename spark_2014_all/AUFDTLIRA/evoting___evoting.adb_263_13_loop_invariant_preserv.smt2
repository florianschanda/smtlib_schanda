...
(assert (= result18 (mk_map__ref1 winners25)))
(assert (= winners26 (store winners25 i16 (of_int 0))))
(assert (not (= i16 20)))
(assert (= result19 (mk_int__ref i16)))
(assert (= i17 (+ i16 1)))
(assert (= i18 (mk_int__ref i17)))
(assert (= winners27 (mk_map__ref1 winners26)))
(assert (= i19 i17))
(assert (= winners28 winners26))
(declare-const j28 Int)
(assert (<= (+ last_candidate 1) j28))
(assert (<= j28 (- (int__content i18) 1)))
(assert (not (not (= (select (map__content1 winners27) j28) true))))
(check-sat)
(exit)
