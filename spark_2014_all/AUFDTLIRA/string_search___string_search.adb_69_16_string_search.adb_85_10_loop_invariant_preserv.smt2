...
(assert (<= r58b2 k3))
(assert (<= k3 r59b2))
(assert (not (= (to_rep1 (select (elts haystack) (+ i1 (length needle)))) (to_rep1 (select (elts needle) (+ (- (+ i1 (length needle)) k3) 1))))))
(assert (forall ((l Int)) (=> (and (<= 1 l) (<= l k4)) (not (= (match_at needle haystack l) true)))))
(assert (and (=> (<= r58b2 r59b2) (dynamic_property3 r58b2 r59b2 k4)) (and (<= r58b2 k4) (<= k4 r59b2))))
(assert (not (= k4 r59b2)))
(assert (= k4 result12))
(assert (= k5 (+ k4 1)))
(assert (not (= (to_rep1 (select (elts haystack) (+ i1 (length needle)))) (to_rep1 (select (elts needle) (+ (- (+ i1 (length needle)) k5) 1))))))
(declare-const l Int)
(assert (<= 1 l))
(assert (<= l k5))
