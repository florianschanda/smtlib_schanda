...
(assert (and (= o12 (+ (to_rep1 (first (rt source))) 9)) (in_range1 (+ (to_rep1 (first (rt source))) 9))))
(assert (= o15 (ite (<= o12 index) true false)))
(assert (= result (ite (= o15 true) o14 false)))
(assert (= (= o16 true) (forall ((index1 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 9) index1) (<= index1 (+ (to_rep1 (first (rt source))) 10))) (= (is_digit (to_rep (select (elts source) index1))) true)))))
...
(assert (not (<= (to_rep1 (first (rt source))) temp___1959)))
(check-sat)
(exit)
