...
(declare-datatypes () ((unconstr_ar__ref (mk_unconstr_ar__ref (unconstr_ar__content us_t)))))
(define-fun unconstr_ar__ref___projection ((a unconstr_ar__ref)) us_t (unconstr_ar__content a))
(define-fun dynamic_invariant ((temp___expr_169 us_t) (temp___is_init_165 Bool) (temp___skip_constant_166 Bool) (temp___do_toplevel_167 Bool) (temp___do_typ_inv_168 Bool)) Bool (=> (not (= temp___skip_constant_166 true)) (dynamic_property (- 2147483648) 2147483647 (first1 temp___expr_169) (last1 temp___expr_169))))
...
(assert (= result b))
(assert (= b1 (ite (= (length x) 20) true false)))
(assert (= b1 b2))
(assert (= b3 b1))
(assert (= b2 true))
(assert (not (= (length x) 20)))
(check-sat)
(exit)
