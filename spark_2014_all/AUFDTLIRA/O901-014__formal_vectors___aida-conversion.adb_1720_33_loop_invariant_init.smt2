...
(assert (and (= (length source) 11) (and (and (and (and (= (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 0))) 45) (= (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 1))) 50)) (= (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 2))) 49)) (< (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 3))) 52)) (forall ((index2 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 3) index2) (<= index2 (+ (to_rep1 (first (rt source))) 10))) (= (is_digit (to_rep (select (elts source) index2))) true))))))
...
(assert (<= temp___2180 index1))
(assert (<= index1 temp___2181))
...
(assert (= n2 o8))
(declare-const j Int)
(assert (<= (+ (to_rep1 (first (rt source))) 3) j))
(assert (<= j index1))
(assert (not (<= (to_rep1 (select n2 j)) (to_rep1 (select int_array1 j)))))
(check-sat)
(exit)
