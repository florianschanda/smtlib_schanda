...
(assert (and (< 0 (length list)) (and (< (to_rep (last (rt list))) 2147483647) (forall ((j Int)) (=> (and (<= (to_rep (first (rt list))) j) (<= j (to_rep (last (rt list))))) (forall ((k Int)) (=> (and (<= (to_rep (first (rt list))) k) (<= k (to_rep (last (rt list))))) (=> (= (to_rep (select (elts list) j)) (to_rep (select (elts list) k))) (= j k)))))))))
...
(assert (not (in_range1 (+ position1 1))))
(check-sat)
(exit)
