...
(assert (= result27 (bool__content names__is_domain_name__result42)))
(assert (= (bool__content names__is_domain_name__result42) true))
(declare-const i22 Int)
(assert (<= (to_rep1 (first (rt name))) i22))
(assert (<= i22 (to_rep1 (last (rt name)))))
(assert (not (or (or (= (is_letter (to_rep (select (elts name) i22))) true) (= (is_digit (to_rep (select (elts name) i22))) true)) (= (to_rep (select (elts name) i22)) 45))))
(check-sat)
(exit)
