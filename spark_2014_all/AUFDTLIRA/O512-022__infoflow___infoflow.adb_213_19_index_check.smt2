...
(assert (=> (= result14 true) (= (= result13 true) (forall ((m12 Int)) (=> (and (<= (+ k_1 1) m12) (<= m12 (to_rep a_v1__last))) (= (to_rep (select (map__content a_v118) m12)) (to_rep (select (elts c_v1) (- m12 k_1)))))))))
(assert (=> (not (= result14 true)) (= result13 false)))
(assert (= result13 true))
(assert (<= (to_rep a_v2__first) m7))
(assert (<= m7 k_2))
(assert (and (<= (to_rep (first (rt b_v2))) m7) (<= m7 (to_rep (last (rt b_v2))))))
(assert (= o45 m7))
(assert (= o46 (select (elts b_v2) o45)))
(assert (= o47 (to_rep o46)))
(assert (not (<= (to_rep a_v2__first) m7)))
(check-sat)
(exit)
