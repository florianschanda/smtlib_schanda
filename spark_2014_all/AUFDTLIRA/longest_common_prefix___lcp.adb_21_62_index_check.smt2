...
(assert (= result1 true))
(assert (forall ((k1 Int)) (=> (and (<= 0 k1) (<= k1 (- l2 1))) (= (to_rep (select (elts a) (+ x k1))) (to_rep (select (elts a) (+ y k1)))))))
...
(assert (<= 0 k))
(assert (<= k (- l2 1)))
(assert (and (<= (to_rep (first (rt a))) (+ y k)) (<= (+ y k) (to_rep (last (rt a))))))
(assert (= o8 (+ y k)))
(assert (= o9 (select (elts a) o8)))
(assert (= o10 (to_rep o9)))
(assert (not (<= (to_rep (first (rt a))) (+ x k))))
(check-sat)
(exit)
