...
(assert (<= 1 i2))
(assert (<= i2 10))
(assert (= (to_rep o) (to_rep (select inkeys_v1 i2))))
(assert (= o1 (store outkeys_v1 i2 o)))
(assert (= (mk_map__ref result1) (mk_map__ref outkeys_v1)))
(assert (= outkeys_v12 o1))
(assert (forall ((k Int)) (=> (and (<= 1 k) (<= k 10)) (=> (<= k i3) (= (to_rep (select outkeys_v13 k)) (to_rep (select inkeys_v1 k)))))))
(assert (and (and (forall ((temp___363 Int)) (=> (and (<= 1 temp___363) (<= temp___363 10)) (=> (< i3 temp___363) (= (select outkeys_v13 temp___363) (select outkeys_v1 temp___363))))) (=> (<= 1 10) (in_range2 i3))) (and (<= 1 i3) (<= i3 10))))
(assert (not (= i3 10)))
(assert (= result2 i3))
(assert (= i4 (+ i3 1)))
(assert (= (to_rep o2) (to_rep (select inkeys_v1 i4))))
(assert (= o3 (store outkeys_v13 i4 o2)))
(assert (= result3 outkeys_v13))
(assert (= outkeys_v14 o3))
(declare-const k Int)
(assert (<= 1 k))
(assert (<= k 10))
(assert (<= k i4))
(assert (not (= (to_rep (select outkeys_v14 k)) (to_rep (select inkeys_v1 k)))))
(check-sat)
(exit)
