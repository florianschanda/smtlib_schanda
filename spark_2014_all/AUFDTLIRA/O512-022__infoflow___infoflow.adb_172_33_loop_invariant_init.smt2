...
(assert (= (mk_int__ref result2) (mk_int__ref i_v1)))
(assert (= i_v12 (to_rep a_v1__first)))
(assert (<= (to_rep a_v1__first) i_v12))
(assert (<= i_v12 k_1))
(declare-const m12 Int)
(assert (<= (to_rep a_v1__first) m12))
(assert (<= m12 (- i_v12 1)))
(assert (not (= (to_rep (select a_v1 m12)) (to_rep (select (elts b_v1) m12)))))
(check-sat)
(exit)
