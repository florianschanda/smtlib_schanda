...
(assert (<= 0 s2))
(assert (<= s2 100))
(assert (forall ((t2 Int)) (=> (and (<= 0 t2) (<= t2 (- s3 1))) (= (name (select (owned_state sm) t2)) (name__2 (select av2 t2))))))
(assert (and (=> (<= 0 100) (in_range6 s3)) (and (<= 0 s3) (<= s3 100))))
(assert (and (= temp___434 (owned_state sm)) (= temp___434 (rec__model__uml_state_machine__owned_state_data (us_split_fields5 sm)))))
(assert (= (mk___rep (mk___split_fields o3)) (select temp___434 s3)))
(assert (and (= o4 (name (mk___rep (mk___split_fields o3)))) (and (in_range1 o4) (= o4 (to_rep o3)))))
(assert (= (to_rep o5) o4))
(assert (= o5 o6))
(assert (= o7 o6))
(assert (= o8 (store av2 s3 (mk___rep3 (mk___split_fields3 o7)))))
(assert (= result4 (mk_map__ref2 av2)))
(assert (= av3 o8))
(assert (not (= s3 100)))
(assert (= result5 (mk_int__ref s3)))
(assert (= s4 (+ s3 1)))
(declare-const t2 Int)
(assert (<= 0 t2))
(assert (<= t2 (- s4 1)))
(assert (not (= (name (select (owned_state sm) t2)) (name__2 (select av3 t2)))))
(check-sat)
(exit)
