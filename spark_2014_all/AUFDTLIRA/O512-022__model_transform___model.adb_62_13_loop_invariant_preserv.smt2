...
(assert (<= 0 s1))
(assert (<= s1 100))
(assert (forall ((t Int)) (=> (and (<= 0 t) (<= t (- s2 1))) (= (name (select (owned_state sm) t)) (name__2 (select av2 t))))))
(assert (and (=> (<= 0 100) (in_range6 s2)) (and (<= 0 s2) (<= s2 100))))
(assert (and (= temp___475 (owned_state sm)) (= temp___475 (rec__model__uml_state_machine__owned_state_data (us_split_fields5 sm)))))
(assert (= o6 (select temp___475 s2)))
...
(assert (<= 0 t))
(assert (<= t (- s3 1)))
(assert (not (= (name (select (owned_state sm) t)) (name__2 (select av3 t)))))
(check-sat)
(exit)
