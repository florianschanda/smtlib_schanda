...
(assert (and (= o8 (+ (to_rep1 (first (rt source))) 8)) (in_range1 (+ (to_rep1 (first (rt source))) 8))))
(assert (= o11 (ite (<= o8 index) true false)))
(assert (= result (ite (= o11 true) o10 false)))
(assert (= (= o12 true) (forall ((index1 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 8) index1) (<= index1 (+ (to_rep1 (first (rt source))) 10))) (= (is_digit (to_rep (select (elts source) index1))) true)))))
...
(assert (not (<= (to_rep1 (first (rt source))) temp___1996)))
(check-sat)
(exit)
