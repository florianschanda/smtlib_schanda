...
(declare-const unseen10 us_rep1)
...
(declare-const avail5 us_rep)
(declare-const unseen12 us_rep1)
...
(assert (forall ((j Int)) (=> (and (<= 1 j) (<= j (length avail2))) (in_range8 (get avail2 j)))))
...
(declare-const j Int)
(assert (<= 1 j))
(assert (<= j (- (length (sequence__content avail4)) 1)))
(assert (not (= (to_rep (rec__list_mod_allocator__cell__next (us_split_fields1 (select data (get (sequence__content avail4) j))))) (get (sequence__content avail4) (+ j 1)))))
(check-sat)
(exit)
