...
(assert (<= 1 j4))
(assert (<= j4 100))
(assert (= (rec__tree_model__path_type__k (us_split_fields1 (select r2 j4))) true))
(assert (not (= j4 root)))
(assert (not (<= 1 (to_rep (rec__binary_trees__cell__parent (us_split_fields3 (select (rec__binary_trees__forest__c (us_split_fields5 f)) j4)))))))
(check-sat)
(exit)
