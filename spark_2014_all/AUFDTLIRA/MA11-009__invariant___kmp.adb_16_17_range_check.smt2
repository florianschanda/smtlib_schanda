...
(assert (=> (<= 0 r12b3) (and (and (<= (to_rep (first (rt p))) 0) (<= 0 (to_rep (last (rt p))))) (and (<= (to_rep (first (rt p))) r12b3) (<= r12b3 (to_rep (last (rt p))))))))
(assert (= o5 (elts p)))
(assert (= temp___173 o5))
(assert (= temp___1731 (mk4 0 r12b3)))
(assert (<= r8b3 r9b3))
(assert (not (<= (to_rep (first (rt p))) r8b3)))
(check-sat)
(exit)
