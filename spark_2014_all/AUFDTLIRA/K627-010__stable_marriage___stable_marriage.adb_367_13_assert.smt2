...
(assert (<= 1 g1_id))
(assert (<= g1_id 64))
(assert (forall ((idx Int)) (=> (and (<= 1 idx) (<= idx (to_rep3 (rec__stable_marriage__matching__group1_set__cardinality unmatched_g1_set__split_fields4)))) (not (= (to_rep (select (rec__stable_marriage__matching__group1_set__elements unmatched_g1_set__split_fields4) idx)) g1_id)))))
(assert (not (< 0 (to_rep3 (select proposals_made1 g1_id)))))
(check-sat)
(exit)
