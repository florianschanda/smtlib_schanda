...
(assert (and (= o (extended_perm aa bb)) (and (in_range3 o) (=> (= (distinct o 0) true) (forall ((ia Int)) (=> (and (<= (to_rep2 (first (rt aa))) ia) (<= ia (to_rep2 (last (rt aa))))) (exists ((ib Int)) (and (and (<= (to_rep2 (first (rt bb))) ib) (<= ib (to_rep2 (last (rt bb))))) (and (= (to_rep (select (elts aa) ia)) (to_rep (select (elts bb) ib))) (= (is_perm (remove aa ia) (remove bb ib)) true))))))))))
(assert (= result (mk_int__ref ih)))
(assert (= ih1 o))
(assert (in_range3 ih1))
(assert (not (= (distinct ih1 0) true)))
(check-sat)
(exit)
