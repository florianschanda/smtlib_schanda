...
(assert (and (= temp___2392 (remove (mk___t interm1 (mk 1 100)) o9)) (and (dynamic_invariant3 temp___2392 true false true true) (and (= (invariant__ temp___2392) true) (and (= (to_rep1 (last temp___2391)) (- (to_rep1 (last (mk 1 100))) 1)) (and (forall ((k Int)) (=> (and (<= 1 k) (<= k (- o9 1))) (= (to_rep (select interm1 k)) (to_rep (select temp___239 k))))) (and (forall ((k Int)) (=> (and (<= (+ o9 1) k) (<= k (to_rep1 (last (mk 1 100))))) (= (to_rep (select interm1 k)) (to_rep (select temp___239 (- k 1)))))) (forall ((k Int)) (=> (and (<= o9 k) (<= k (- (to_rep1 (last (mk 1 100))) 1))) (= (to_rep (select interm1 (+ k 1))) (to_rep (select temp___239 k))))))))))))
(assert (and (= o10 x) (in_range4 x)))
(assert (not (<= (to_rep1 (first (mk 1 100))) o10)))
(check-sat)
(exit)
