...
(declare-const temp___204 (Array Int value))
(declare-const o1 (Array Int value))
(declare-const o2 value)
(declare-const temp___2041 (Array Int value))
...
(assert (= temp___204 (store v j1 o)))
(assert (dynamic_predicate temp___204))
(assert (= o1 temp___204))
(assert (= result4 v))
(assert (= v1 o1))
(assert (= prev_value3 (to_rep (select v j2))))
(assert (forall ((k Int)) (=> (and (<= 1 k) (<= k j2)) (= (to_rep (select v2 k)) (to_rep (select v (ite (< 1 (- k 1)) (- k 1) 1)))))))
(assert (forall ((k Int)) (=> (and (<= (+ j2 1) k) (<= k 100)) (= (to_rep (select v2 k)) (to_rep (select v k))))))
(assert (and (and (and (and (and (=> (<= 0 10000) (in_range1 prev_value3)) (= (sorted v2) true)) (forall ((temp___207 Int)) (=> (and (<= 1 temp___207) (<= temp___207 100)) (=> (< j2 temp___207) (= (select v2 temp___207) (select v temp___207)))))) (=> (<= 0 10000) (in_range1 tmp_value2))) (=> (<= 1 100) (in_range2 j2))) (and (<= 1 j2) (<= j2 100))))
(assert (not (= j2 100)))
(assert (= j2 result5))
(assert (= j3 (+ j2 1)))
(assert (= tmp_value2 result6))
(assert (= tmp_value3 prev_value3))
(assert (= prev_value3 result7))
(assert (= prev_value4 (to_rep (select v2 j3))))
(assert (= (to_rep o2) tmp_value3))
(assert (= temp___2041 (store v2 j3 o2)))
(assert (dynamic_predicate temp___2041))
(assert (= o3 temp___2041))
(assert (= result8 v2))
(assert (= v3 o3))
(declare-const k Int)
(assert (<= (+ j3 1) k))
(assert (<= k 100))
(assert (not (= (to_rep (select v3 k)) (to_rep (select v k)))))
(check-sat)
(exit)
