...
(assert (and (= o2 (remove b o1)) (and (dynamic_invariant2 o2 true false true) (and (= (invariant__ o2) true) (and (= (to_rep2 (last (rt o2))) (- (to_rep2 (last (rt b))) 1)) (and (forall ((k Int)) (=> (and (<= 1 k) (<= k (- o1 1))) (= (to_rep (select (elts b) k)) (to_rep (select (elts o2) k))))) (and (forall ((k Int)) (=> (and (<= (+ o1 1) k) (<= k (to_rep2 (last (rt b))))) (= (to_rep (select (elts b) k)) (to_rep (select (elts o2) (- k 1)))))) (forall ((k Int)) (=> (and (<= o1 k) (<= k (- (to_rep2 (last (rt b))) 1))) (= (to_rep (select (elts b) (+ k 1))) (to_rep (select (elts o2) k))))))))))))
