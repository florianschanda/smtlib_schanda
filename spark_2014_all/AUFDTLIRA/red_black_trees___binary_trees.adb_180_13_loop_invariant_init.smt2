...
(assert (<= j 100))
(assert (not (= j root)))
(assert (not (and (not (= (to_rep (rec__binary_trees__cell__parent (us_split_fields1 (select (rec__binary_trees__forest__c (us_split_fields3 f)) j)))) 0)) (= (rec__tree_model__path_type__k (us_split_fields5 (select r1 (to_rep (rec__binary_trees__cell__parent (us_split_fields1 (select (rec__binary_trees__forest__c (us_split_fields3 f)) j))))))) true))))
(assert (not (not (= (rec__tree_model__path_type__k (us_split_fields5 (select r1 j))) true))))
(check-sat)
(exit)
