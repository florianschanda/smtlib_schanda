...
(assert (< j i))
(assert (and (<= (to_rep1 (first (rt a))) j) (<= j (to_rep1 (last (rt a))))))
(assert (= o5 j))
(assert (= o6 (select (elts a) o5)))
(assert (= o7 (to_rep o6)))
(assert (and (<= (to_rep1 (first (rt aa))) j) (<= j (to_rep1 (last (rt aa))))))
(assert (= o3 j))
(assert (= o4 (select (elts aa) o3)))
(assert (= o8 (to_rep o4)))
(assert (= result (ite (= o8 o7) true false)))
(assert (= result true))
(assert (and (<= (to_rep1 (first (rt b))) j) (<= j (to_rep1 (last (rt b))))))
(assert (= o j))
(assert (= o1 (select (elts b) o)))
(assert (= o2 (to_rep o1)))
(assert (not (<= (to_rep1 (first (rt aa))) j)))
(check-sat)
(exit)
