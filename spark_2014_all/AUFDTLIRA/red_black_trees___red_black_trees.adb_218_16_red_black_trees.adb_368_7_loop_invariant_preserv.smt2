...
(assert (not (= n32 100)))
(assert (= result14 (mk_int__ref n32)))
(assert (= n33 (+ n32 1)))
(assert (= n34 (mk_int__ref n33)))
(assert (= n35 n33))
(declare-const i14 Int)
(assert (<= 1 i14))
(assert (<= i14 100))
(assert (= (rec__tree_model__path_type__k (us_split_fields1 (select (model__ (rec__red_black_trees__rbt__struct (us_split_fields9 c67b))) i14))) true))
(assert (< (to_rep (rec__red_black_trees__black_count__depth (us_split_fields11 (select (nb_blacks c67b) i14)))) (int__content n34)))
(assert (not (= i14 (root (rec__red_black_trees__rbt__struct (us_split_fields9 c68b))))))
(assert (not (= (eq (select (nb_blacks c68b) i14) (select (nb_blacks c67b) i14)) true)))
(check-sat)
(exit)
