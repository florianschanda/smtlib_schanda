...
(assert (<= 1 i14))
(assert (<= i14 100))
(assert (= (rec__tree_model__path_type__k (us_split_fields1 (select (model__ (rec__red_black_trees__rbt__struct (us_split_fields9 c81b))) i14))) true))
(assert (< (to_rep (rec__red_black_trees__black_count__depth (us_split_fields11 (select (nb_blacks c81b) i14)))) n61))
(assert (not (= i14 y)))
(assert (not (= (eq (select (nb_blacks c82b) i14) (select (nb_blacks c81b) i14)) true)))
(check-sat)
(exit)
