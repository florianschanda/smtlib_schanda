...
(assert (<= 1 j))
(assert (<= j (- i1 1)))
(assert (not (= (to_rep1 (rec__dynamic_array__rec__f1 (us_split_fields1 (select (elts1 (rec__dynamic_array__holder__content1 h__split_fields2)) j)))) (increment (select (elts (rec__dynamic_array__holder__content (us_split_fields3 (new_holder c)))) j)))))
(check-sat)
(exit)
