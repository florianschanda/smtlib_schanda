...
(assert (forall ((j1 Int)) (=> (and (<= (to_rep (first (rt a))) j1) (<= j1 i3)) (not (= (to_rep (select (elts a) j1)) 0)))))
(assert (and (and (=> (<= (to_rep (first (rt a))) last4) (dynamic_property4 (to_rep (first (rt a))) last4 i3)) (=> (<= (- 2147483648) 2147483647) (in_range1 e3))) (and (<= (to_rep (first (rt a))) i3) (<= i3 last4))))
(assert (not (= i3 last4)))
(assert (= i3 result3))
(assert (= i4 (+ i3 1)))
(assert (and (<= (to_rep (first (rt a))) i4) (<= i4 (to_rep (last (rt a))))))
(assert (= o5 i4))
...
(assert (<= j1 i4))
(assert (not (not (= (to_rep (select (elts a) j1)) 0))))
(check-sat)
(exit)
