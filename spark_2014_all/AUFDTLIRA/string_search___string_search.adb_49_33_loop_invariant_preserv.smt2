...
(assert (= o35 (store bad_shift3 (to_rep1 (select (elts needle) j3)) o34)))
(assert (= result6 bad_shift3))
(assert (= bad_shift4 o35))
(declare-const c4 Int)
(assert (<= 0 c4))
(assert (<= c4 255))
(assert (= (to_rep (select bad_shift4 c4)) (+ (length needle) 1)))
(declare-const k4 Int)
(assert (<= 1 k4))
(assert (<= k4 j3))
(assert (not (not (= c4 (to_rep1 (select (elts needle) k4))))))
(check-sat)
(exit)
