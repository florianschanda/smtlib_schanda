...
(assert (and (<= (to_rep (first (rt c_v2))) temp___458) (<= temp___458 (to_rep (last (rt c_v2))))))
(assert (= o60 temp___458))
(assert (= o61 (select (elts c_v2) o60)))
(assert (= o62 (to_rep o61)))
(assert (= (to_rep o63) o62))
(assert (and (<= (to_rep a_v2__first) i_v214) (<= i_v214 (to_rep a_v2__last))))
(assert (= o64 i_v214))
(assert (= o65 (store a_v212 o64 o63)))
(assert (= result17 (mk_map__ref a_v212)))
(assert (= a_v213 o65))
(assert (not (= i_v214 (to_rep a_v2__last))))
(assert (= result18 (mk_int__ref i_v214)))
(assert (= i_v215 (+ i_v214 1)))
(assert (= i_v216 (mk_int__ref i_v215)))
(assert (= a_v214 (mk_map__ref a_v213)))
(assert (= i_v217 i_v215))
(assert (= a_v215 a_v213))
(declare-const m12 Int)
(assert (<= (to_rep a_v2__first) m12))
(assert (<= m12 k_2))
(assert (not (= (to_rep (select (map__content a_v214) m12)) (to_rep (select (elts b_v2) m12)))))
(check-sat)
(exit)
