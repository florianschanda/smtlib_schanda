...
(assert (not (= (get (rec__tree_model__path_type__a (us_split_fields5 (select (model__ f v) j13))) (+ (length (rec__tree_model__path_type__a (us_split_fields5 (select (model__ f v) k)))) 1)) 0)))
(assert (not (< (to_rep (select values k)) (to_rep (select values j13)))))
(check-sat)
(exit)
