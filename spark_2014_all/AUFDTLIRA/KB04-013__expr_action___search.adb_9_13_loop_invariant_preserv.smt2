...
(assert (=> (= (to_rep (select t i2)) v) (= pos2 result5)))
(assert (=> (= (to_rep (select t i2)) v) (= pos3 i2)))
(assert (=> (not (= (to_rep (select t i2)) v)) (= pos3 pos2)))
(assert (not (= i2 10)))
(assert (= result6 i2))
(assert (= i3 (+ i2 1)))
(assert (not (or (and (= pos3 0) (forall ((j2 Int)) (=> (and (<= 1 j2) (<= j2 (- i3 1))) (not (= (to_rep (select t j2)) v))))) (and (= (to_rep (select t pos3)) v) (forall ((j2 Int)) (=> (and (<= 1 j2) (<= j2 (- pos3 1))) (not (= (to_rep (select t j2)) v))))))))
(check-sat)
(exit)
