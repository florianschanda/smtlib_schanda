...
(assert (forall ((rank11 Int)) (=> (and (<= 1 rank11) (<= rank11 64)) (forall ((rank2 Int)) (=> (and (<= 1 rank2) (<= rank2 64)) (=> (not (= rank11 rank2)) (not (= (to_rep (select r1 rank11)) (to_rep (select r1 rank2))))))))))
...
(assert (<= 1 prev))
(assert (<= prev (- rank1 1)))
(assert (not (= (to_rep1 (select result__1 (to_rep (select r1 prev)))) prev)))
(check-sat)
(exit)
