...
(assert (= (= o4 true) (forall ((index1 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 10) index1) (<= index1 (+ (to_rep1 (first (rt source))) 10))) (= (is_digit (to_rep (select (elts source) index1))) true)))))
(assert (and (= temp___1927 (+ (to_rep1 (first (rt source))) 10)) (in_range1 (+ (to_rep1 (first (rt source))) 10))))
(assert (not (<= (to_rep1 (first (rt source))) temp___1927)))
(check-sat)
(exit)
