...
(assert (<= 90 m))
(assert (<= m 99))
(assert (or (or (or (not (= m i4)) (not (= n i3))) (not (= o5 i2))) (not (= p i1))))
(assert (not (= (to_rep (select (select (select (select a2 m) n) o5) p)) (to_rep (select (select (select (select a m) n) o5) p)))))
(check-sat)
(exit)
