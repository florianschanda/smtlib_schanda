...
(assert (<= (to_rep (first (rt x))) j2))
(assert (<= j2 (to_rep (last (rt x)))))
(assert (and (=> (<= (- 2147483648) 2147483647) (in_range1 k2)) (and (<= (to_rep (first (rt x))) j3) (<= j3 (to_rep (last (rt x)))))))
(assert (= (to_rep o) (to_rep (select (elts x) j3))))
(assert (and (<= (to_rep z__first) k2) (<= k2 (to_rep z__last))))
(assert (= o1 k2))
(assert (= o2 (store z2 o1 o)))
(assert (= z2 result2))
(assert (= z3 o2))
