...
(assert (= (int__content indexbis__result32) 0))
(assert (<= (to_rep1 (first (rt s))) i2))
(assert (<= i2 (to_rep1 (last (rt s)))))
(assert (<= (to_rep1 (first (rt c))) j3))
(assert (<= j3 (to_rep1 (last (rt c)))))
(assert (and (<= (to_rep1 (first (rt c))) j3) (<= j3 (to_rep1 (last (rt c))))))
(assert (= o24 j3))
(assert (= o25 (select (elts c) o24)))
