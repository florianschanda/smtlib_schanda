...
(assert (= (= o4 true) (forall ((index1 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 1) index1) (<= index1 (+ (to_rep1 (first (rt source))) 2))) (= (is_digit (to_rep (select (elts source) index1))) true)))))
(assert (and (= temp___2540 (+ (to_rep1 (first (rt source))) 0)) (in_range1 (+ (to_rep1 (first (rt source))) 0))))
(assert (not (<= (to_rep1 (first (rt source))) temp___2540)))
(check-sat)
(exit)
