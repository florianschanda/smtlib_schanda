...
(assert (<= 1 i))
(assert (<= i 100))
(assert (= (rec__tree_model__path_type__k (us_split_fields1 (select (model__ f2 root) i))) true))
(assert (< (length (rec__tree_model__path_type__a (us_split_fields1 (select (model__ f2 root) i)))) n1))
(assert (not (= (rec__tree_model__path_type__k (us_split_fields1 (select (model__ f1 root) i))) true)))
(check-sat)
(exit)
