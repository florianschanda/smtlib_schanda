...
(assert (and (= (length source) 11) (and (and (and (= (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 0))) 45) (= (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 1))) 50)) (= (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 2))) 48)) (forall ((index Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 2) index) (<= index (+ (to_rep1 (first (rt source))) 10))) (= (is_digit (to_rep (select (elts source) index))) true))))))
...
(assert (<= (+ (to_rep1 (first (rt source))) 2) (+ (to_rep1 (first (rt source))) 10)))
(assert (not (<= (- 2147483648) (+ (to_rep1 (first (rt source))) 2))))
(check-sat)
(exit)
