...
(assert (dynamic_invariant queue true false true true))
(assert (not (= (empty queue) true)))
(assert (not (<= (to_rep2 (first (rt (rec__bounded_queue_v2__queue_type__items (us_split_fields1 queue))))) (to_rep (rec__bounded_queue_v2__queue_type__rear (us_split_fields1 queue))))))
(check-sat)
(exit)
