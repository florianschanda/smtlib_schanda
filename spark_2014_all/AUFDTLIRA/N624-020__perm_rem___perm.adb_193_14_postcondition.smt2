...
(assert (= ia17 ia15))
(assert (= ih39 ih37))
(assert (= result45 (int__content perm__extended_perm__result8)))
(assert (= (of_int (int__content perm__extended_perm__result8)) true))
(declare-const ia18 Int)
(assert (<= (to_rep2 (first (rt a))) ia18))
(assert (<= ia18 (to_rep2 (last (rt a)))))
(assert (not (exists ((ib29 Int)) (and (and (<= (to_rep2 (first (rt b))) ib29) (<= ib29 (to_rep2 (last (rt b))))) (and (= (to_rep (select (elts a) ia18)) (to_rep (select (elts b) ib29))) (= (is_perm (remove a ia18) (remove b ib29)) true))))))
(check-sat)
(exit)
