...
(assert (=> (= result2 true) (=> (and (<= 1 n2) (<= n2 100)) (= temp___638 n2))))
(assert (=> (= result2 true) (=> (and (<= 1 n2) (<= n2 100)) (forall ((i Int)) (=> (and (<= 1 i) (<= i 100)) (=> (and (and (= (rec__tree_model__path_type__k (us_split_fields7 (select (model__ (rec__red_black_trees__rbt__struct (us_split_fields9 c61b))) i))) true) (< (to_rep (rec__red_black_trees__black_count__depth (us_split_fields11 (select (nb_blacks c61b) i)))) n3)) (not (= i (root (rec__red_black_trees__rbt__struct (us_split_fields9 c62b)))))) (= (eq (select (nb_blacks c62b) i) (select (nb_blacks c61b) i)) true)))))))
(assert (=> (= result2 true) (=> (and (<= 1 n2) (<= n2 100)) (and (=> (<= 1 100) (in_range4 n3)) (and (<= 1 n3) (<= n3 100))))))
