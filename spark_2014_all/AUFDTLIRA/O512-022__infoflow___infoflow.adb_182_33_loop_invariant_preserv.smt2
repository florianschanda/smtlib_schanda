...
(assert (and (= temp___437 (- i_v112 k_1)) (in_range1 (- i_v112 k_1))))
(assert (and (<= (to_rep (first (rt c_v1))) temp___437) (<= temp___437 (to_rep (last (rt c_v1))))))
(assert (= o24 temp___437))
(assert (= o25 (select (elts c_v1) o24)))
(assert (= o26 (to_rep o25)))
(assert (= (to_rep o27) o26))
(assert (and (<= (to_rep a_v1__first) i_v112) (<= i_v112 (to_rep a_v1__last))))
(assert (= o28 i_v112))
(assert (= o29 (store a_v110 o28 o27)))
(assert (= a_v110 result7))
(assert (= a_v111 o29))
(assert (not (= i_v112 (to_rep a_v1__last))))
(assert (= result8 i_v112))
(assert (= i_v113 (+ i_v112 1)))
(assert (= i_v113 i_v114))
(assert (= a_v111 a_v112))
(assert (= i_v115 i_v113))
(assert (= a_v113 a_v111))
(declare-const m12 Int)
(assert (<= (to_rep a_v1__first) m12))
(assert (<= m12 k_1))
(assert (not (= (to_rep (select a_v112 m12)) (to_rep (select (elts b_v1) m12)))))
(check-sat)
(exit)
