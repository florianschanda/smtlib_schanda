...
(assert (<= 1 j))
(assert (<= j 100))
(assert (not (and (= (rec__tree_model__path_type__k (us_split_fields5 (select r1 j))) true) (not (= j root)))))
(assert (not (= (length (rec__tree_model__path_type__a (us_split_fields5 (select r1 j)))) 0)))
(check-sat)
(exit)
