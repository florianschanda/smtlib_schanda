...
(assert (<= (+ l 1) i1))
(assert (<= i1 u))
(assert (=> (< (to_rep (select arr i1)) (to_rep (select arr k1))) (= k1 result1)))
(assert (=> (< (to_rep (select arr i1)) (to_rep (select arr k1))) (= k2 i1)))
(assert (=> (not (< (to_rep (select arr i1)) (to_rep (select arr k1)))) (= k2 k1)))
(assert (and (and (and (and (<= (+ l 1) i2) (<= i2 u)) (< l u)) (forall ((x1 Int)) (=> (and (<= l x1) (<= x1 i2)) (<= (to_rep (select arr k3)) (to_rep (select arr x1)))))) (and (<= l k3) (<= k3 u))))
(assert (and (and (=> (<= (+ l 1) u) (dynamic_property (+ l 1) u i2)) (=> (<= 1 100) (in_range1 k3))) (and (<= (+ l 1) i2) (<= i2 u))))
(assert (<= l x))
(assert (<= x i2))
(assert (not (<= 1 x)))
(check-sat)
