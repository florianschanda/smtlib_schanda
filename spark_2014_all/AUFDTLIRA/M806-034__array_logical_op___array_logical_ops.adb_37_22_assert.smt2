...
(assert (dynamic_invariant r3 true false true true))
(assert (= (to_rep1 (first (rt r3))) 6))
(declare-const i Int)
(assert (<= (to_rep1 (first (rt r3))) i))
(assert (<= i 25))
(assert (not (= (distinct (to_rep (select (elts r3) i)) 0) true)))
(check-sat)
(exit)
