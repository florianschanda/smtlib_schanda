...
(assert (and (= (to_rep (select a1 o)) 0) (forall ((j Int)) (=> (and (<= (to_rep a__first) j) (<= j (to_rep a__last))) (=> (not (= o j)) (= (to_rep (select a1 j)) (to_rep (select a j))))))))
(assert (forall ((j Int)) (=> (and (<= (to_rep a__first) j) (<= j i2)) (= (to_rep (select a2 j)) 0))))
...
(assert (and (= (to_rep (select a3 o1)) 0) (forall ((j Int)) (=> (and (<= (to_rep a__first) j) (<= j (to_rep a__last))) (=> (not (= o1 j)) (= (to_rep (select a3 j)) (to_rep (select a2 j))))))))
(declare-const j Int)
(assert (<= (to_rep a__first) j))
(assert (<= j i3))
(assert (not (= (to_rep (select a3 j)) 0)))
(check-sat)
(exit)
