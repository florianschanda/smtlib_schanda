...
(declare-const x11 (Array Int integer))
(assert (= result k))
(assert (= k1 1))
(assert (in_range k1))
(assert (= result1 j))
(assert (= j1 1))
(assert (< 0 (to_rep (select x j1))))
(assert (= (to_rep o) (to_rep (select x j1))))
(assert (and (<= 1 k1) (<= k1 10)))
(assert (= o1 k1))
(assert (= o2 (store x o1 o)))
(assert (= (mk_map__ref result2) (mk_map__ref x)))
(assert (= x2 o2))
(assert (not (in_range (+ k1 1))))
(check-sat)
(exit)
