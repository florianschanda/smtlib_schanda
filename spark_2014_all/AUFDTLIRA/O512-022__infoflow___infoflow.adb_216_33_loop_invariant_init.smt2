...
(assert (and (and (forall ((m12 Int)) (=> (and (<= (to_rep a_v1__first) m12) (<= m12 k_1)) (= (to_rep (select (map__content a_v118) m12)) (to_rep (select (elts b_v1) m12))))) (forall ((m12 Int)) (=> (and (<= (+ k_1 1) m12) (<= m12 (to_rep a_v1__last))) (= (to_rep (select (map__content a_v118) m12)) (to_rep (select (elts c_v1) (- m12 k_1))))))) (forall ((m12 Int)) (=> (and (<= (to_rep a_v2__first) m12) (<= m12 k_2)) (= (to_rep (select (map__content a_v210) m12)) (to_rep (select (elts b_v2) m12)))))))
(assert (and (= temp___464 (+ k_2 1)) (in_range1 (+ k_2 1))))
(assert (= result15 (mk_int__ref i_v21)))
(assert (= i_v213 temp___464))
(assert (<= temp___464 i_v213))
(assert (<= i_v213 (to_rep a_v2__last)))
(declare-const m12 Int)
(assert (<= (to_rep a_v2__first) m12))
(assert (<= m12 k_2))
(assert (not (= (to_rep (select (map__content a_v210) m12)) (to_rep (select (elts b_v2) m12)))))
(check-sat)
(exit)
