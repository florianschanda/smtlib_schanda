...
(assert (and (= o20 (+ (to_rep1 (first (rt source))) 8)) (in_range1 (+ (to_rep1 (first (rt source))) 8))))
(assert (= o23 (ite (<= o20 index) true false)))
(assert (= result (ite (= o23 true) o22 false)))
(assert (= (= o24 true) (forall ((index1 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 8) index1) (<= index1 (+ (to_rep1 (first (rt source))) 10))) (= (is_digit (to_rep (select (elts source) index1))) true)))))
...
(assert (not (<= (to_rep1 (first (rt source))) temp___1993)))
(check-sat)
(exit)
