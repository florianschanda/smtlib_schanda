...
(assert (<= 0 i1))
(assert (<= i1 (- length1 1)))
(assert (forall ((j Int)) (=> (and (<= (to_rep b__first) j) (<= j (to_rep b__last))) (and (=> (and (<= b_first j) (<= j (- (+ b_first i2) 1))) (= (to_rep (select b1 j)) (to_rep (select (elts a) (- (+ a_first j) b_first))))) (=> (=> (<= b_first j) (not (<= j (- (+ b_first i2) 1)))) (= (to_rep (select b1 j)) (to_rep (select b j))))))))
(assert (and (=> (<= 0 (- length1 1)) (dynamic_property1 0 (- length1 1) i2)) (and (<= 0 i2) (<= i2 (- length1 1)))))
(assert (forall ((j Int)) (=> (and (<= (to_rep b__first) j) (<= j (to_rep b__last))) (and (=> (and (<= b_first j) (<= j (- (+ b_first (+ i2 1)) 1))) (= (to_rep (select b2 j)) (to_rep (select (elts a) (- (+ a_first j) b_first))))) (=> (=> (<= b_first j) (not (<= j (- (+ b_first (+ i2 1)) 1)))) (= (to_rep (select b2 j)) (to_rep (select b j))))))))
(assert (=> (<= 0 (- length1 1)) (dynamic_property1 0 (- length1 1) i2)))
...
(assert (<= b_first j))
(assert (<= j (- (+ b_first i3) 1)))
(assert (not (= (to_rep (select b2 j)) (to_rep (select (elts a) (- (+ a_first j) b_first))))))
(check-sat)
(exit)
