...
(assert (= i1 1))
(assert (in_range2 i1))
(assert (not (= (to_rep (select a i1)) x)))
(assert (< i1 10))
(assert (and (< i2 10) (forall ((m Int)) (=> (and (<= 1 m) (<= m i2)) (not (= (to_rep (select a m)) x))))))
(assert (and (=> (<= 1 10) (in_range2 i2)) (and (not (= (to_rep (select a i2)) x)) (< i2 10))))
(assert (and (= o (+ i2 1)) (in_range2 (+ i2 1))))
(assert (= i2 result1))
(assert (= i3 o))
(assert (not (= (to_rep (select a i3)) x)))
(assert (< i3 10))
(assert (not (< i3 10)))
(check-sat)
(exit)
