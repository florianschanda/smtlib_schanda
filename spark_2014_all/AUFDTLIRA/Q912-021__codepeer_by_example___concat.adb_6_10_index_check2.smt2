...
(assert (= j1 (to_rep (first (rt x)))))
(assert (<= (to_rep (first (rt x))) j1))
(assert (<= j1 (to_rep (last (rt x)))))
(assert (and (=> (<= (- 2147483648) 2147483647) (in_range1 k2)) (and (<= (to_rep (first (rt x))) j2) (<= j2 (to_rep (last (rt x)))))))
(assert (= (to_rep o) (to_rep (select (elts x) j2))))
(assert (not (<= (to_rep z__first) k2)))
(check-sat)
(exit)
