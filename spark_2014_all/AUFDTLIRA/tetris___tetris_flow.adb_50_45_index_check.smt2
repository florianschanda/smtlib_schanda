...
(assert (= (to_rep cur_piece__split_fields) 1))
(assert (= result y))
(assert (= y1 0))
(assert (<= 0 y1))
(assert (<= y1 3))
...
(assert (<= 0 x1))
(assert (<= x1 3))
(assert (and (forall ((temp___767 Int)) (=> (and (<= 1 temp___767) (<= temp___767 50)) (=> (not (= (+ (to_rep2 cur_piece__split_fields3) y2) temp___767)) (= (select cur_board1 temp___767) (select cur_board temp___767))))) (and (<= 0 x2) (<= x2 3))))
(assert (= (get (select possible_i_shapes (to_rep3 cur_piece__split_fields1)) y2 x2) true))
(assert (= (to_rep4 o) (to_rep cur_piece__split_fields)))
(assert (and (<= 1 (+ (to_rep1 cur_piece__split_fields2) x2)) (<= (+ (to_rep1 cur_piece__split_fields2) x2) 10)))
(assert (= o1 (+ (to_rep1 cur_piece__split_fields2) x2)))
(assert (= o2 (store (select cur_board1 (+ (to_rep2 cur_piece__split_fields3) y2)) o1 o)))
(assert (not (<= (+ (to_rep2 cur_piece__split_fields3) y2) 50)))
(check-sat)
(exit)
