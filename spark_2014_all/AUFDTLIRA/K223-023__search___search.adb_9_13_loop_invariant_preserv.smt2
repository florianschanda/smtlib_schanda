...
(assert (= result3 i2))
(assert (= i3 (+ i2 1)))
(assert (not (or (and (= pos3 0) (forall ((j Int)) (=> (and (<= 1 j) (<= j (- i3 1))) (not (= (to_rep (select t j)) v))))) (and (= (to_rep (select t pos3)) v) (forall ((j Int)) (=> (and (<= 1 j) (<= j (- pos3 1))) (not (= (to_rep (select t j)) v))))))))
(check-sat)
(exit)
