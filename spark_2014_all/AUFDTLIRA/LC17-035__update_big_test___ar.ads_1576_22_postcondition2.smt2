...
(assert (<= 1 o7))
(assert (<= o7 10))
(assert (or (or (not (= m i)) (not (= n j))) (not (= o7 k))))
(assert (not (= (to_rep (select (select (select r__split_fields7 m) n) o7)) (to_rep (select (select (select r__split_fields1 m) n) o7)))))
(check-sat)
(exit)
