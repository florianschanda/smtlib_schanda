...
(assert (<= 1 g1_id))
(assert (<= g1_id 64))
(assert (= (= result1 true) (forall ((idx Int)) (=> (and (<= 1 idx) (<= idx (to_rep2 unmatched_g1_set__split_fields1))) (not (= (to_rep (select unmatched_g1_set__split_fields idx)) g1_id))))))
(assert (= result1 true))
(assert (< 0 (to_rep2 (select proposals_made g1_id))))
(assert (not (in_range2 (to_rep2 (select proposals_made g1_id)))))
(check-sat)
(exit)
