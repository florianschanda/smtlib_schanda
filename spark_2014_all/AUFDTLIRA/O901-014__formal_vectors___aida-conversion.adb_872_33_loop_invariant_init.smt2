...
(assert (and (= (length source) 7) (forall ((index2 Int)) (=> (and (<= (to_rep1 (first (rt source))) index2) (<= index2 (to_rep1 (last (rt source))))) (= (is_digit (to_rep (select (elts source) index2))) true)))))
...
(assert (<= j index1))
(assert (not (= (to_rep1 (select n2 j)) (convert_character_digit_to_integer (to_rep (select (elts source) j))))))
(check-sat)
(exit)
