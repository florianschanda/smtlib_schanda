...
(assert (= (get (rec__tree_model__path_type__a (us_split_fields1 (select (model__ f root) j1))) (+ (length (rec__tree_model__path_type__a (us_split_fields1 (select (model__ f root) i2)))) 1)) 0))
(assert (not (< (to_rep (select values j1)) (to_rep (select values i2)))))
(check-sat)
(exit)
