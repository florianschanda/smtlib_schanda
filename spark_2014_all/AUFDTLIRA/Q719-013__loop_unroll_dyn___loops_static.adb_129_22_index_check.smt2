...
(assert (= (mk_int__ref result4) (mk_int__ref j)))
(assert (= j4 (to_rep1 a__first)))
(assert (<= (to_rep1 a__first) j4))
(assert (<= j4 i1))
(assert (and (<= (to_rep1 a__first) j5) (<= j5 i1)))
(assert (not (<= (to_rep1 a__first) j5)))
(check-sat)
(exit)
