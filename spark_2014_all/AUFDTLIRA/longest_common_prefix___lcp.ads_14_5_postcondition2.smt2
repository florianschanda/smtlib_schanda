...
(assert (=> (= (= x y) true) (= lcp__result4 (+ (- (to_rep (last (rt a))) x) 1))))
(assert (=> (not (= (ite (= (not (= (to_rep (select (elts a) x)) (to_rep (select (elts a) y)))) false) (= x y) true) true)) (< 0 lcp__result4)))
(assert (= result10 lcp__result4))
(declare-const k Int)
(assert (<= 0 k))
