...
(assert (not (= xr1 0)))
(assert (= (get (rec__tree_model__path_type__a (us_split_fields1 (select (model__ (rec__search_trees__search_tree__struct t__split_fields8) y1) xr1))) 1) 0))
(assert (forall ((j3 Int)) (=> (and (<= 1 j3) (<= j3 100)) (=> (= (rec__tree_model__path_type__k (us_split_fields1 (select (model__ (mk___rep2 f_3__split_fields3) x1) j3))) true) (< (to_rep (select (rec__search_trees__search_tree__values t__split_fields8) j3)) (to_rep (select (rec__search_trees__search_tree__values t__split_fields8) y1)))))))
(assert (= (ordered_leafs (mk___rep2 f_3__split_fields3) y1 (rec__search_trees__search_tree__values t__split_fields8)) true))
(assert (not (in_range5 xr1)))
(check-sat)
(exit)
