...
(assert true)
(assert (forall ((s us_t)) (forall ((ch Int) (n Int)) (! (= (= (has_char_in_string s ch n) true) (exists ((idx Int)) (and (and (<= (first1 s) idx) (<= idx (+ (- (first1 s) 1) n))) (= (to_rep (select (to_array s) idx)) ch)))) :pattern ((has_char_in_string s ch n))))))
...
(assert (not (<= (- (+ (to_rep1 (first (rt s))) n) 1) (to_rep1 (last (rt s))))))
(check-sat)
(exit)
