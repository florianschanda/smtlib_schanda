...
(assert (= o22 (ite (<= index o21) true false)))
(assert (and (= o20 (+ (to_rep1 (first (rt source))) 7)) (in_range1 (+ (to_rep1 (first (rt source))) 7))))
(assert (= o23 (ite (<= o20 index) true false)))
(assert (= result (ite (= o23 true) o22 false)))
(assert (= (= o24 true) (forall ((index1 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 7) index1) (<= index1 (+ (to_rep1 (first (rt source))) 10))) (= (is_digit (to_rep (select (elts source) index1))) true)))))
...
(assert (not (<= (to_rep1 (first (rt source))) temp___2030)))
(check-sat)
(exit)
