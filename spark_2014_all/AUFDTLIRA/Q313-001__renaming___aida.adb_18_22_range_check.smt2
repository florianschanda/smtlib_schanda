...
(assert (and (dynamic_property2 1 (length value) (to_rep1 result____first) (to_rep1 result____last)) (and (= (to_rep1 result____first) 1) (= (to_rep1 result____last) (length value)))))
(assert (= result from))
(assert (= from1 (to_rep1 (first (rt value)))))
(assert (in_range from1))
(assert (=> (<= (- 2147483648) 2147483647) (in_range code)))
(assert (<= from1 (to_rep1 (last (rt value)))))
(assert (<= from1 (to_rep1 (last (rt value)))))
(assert (not (in_range1 from1)))
(check-sat)
(exit)
