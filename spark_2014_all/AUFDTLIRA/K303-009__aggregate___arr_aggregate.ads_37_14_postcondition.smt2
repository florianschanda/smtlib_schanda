...
(assert (in_range b))
(assert (= one 1))
(assert (and (ite (= b 1) (and (= (to_rep (select a one)) one) (= (to_rep (select a 2)) 2)) (ite (= b 2) (forall ((k Int)) (=> (and (<= 1 k) (<= k 2)) (= (to_rep (select a k)) one))) (ite (= b 3) (forall ((k Int)) (=> (and (<= 1 k) (<= k 2)) (= (to_rep (select a k)) one))) (ite (= b 4) (and (= (to_rep (select a one)) 2) (= (to_rep (select a 2)) one)) (and (= (to_rep (select a one)) one) (= (to_rep (select a 2)) 2)))))) (and (= a a1) (= a2 a))))
(assert (= b 1))
(assert (not (= (bool_eq a1 1 2 (temp___192 one 2) 1 2) true)))
(check-sat)
(exit)
