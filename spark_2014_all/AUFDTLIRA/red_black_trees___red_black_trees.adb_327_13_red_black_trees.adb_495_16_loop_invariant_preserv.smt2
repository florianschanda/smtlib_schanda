...
(assert (not (= (let ((subject n1)) n12) 100)))
(assert (= result17 (let ((subject n1)) (mk_int__ref n12))))
(assert (= (let ((subject n1)) n13) (+ (let ((subject n1)) n12) 1)))
(assert (= n14 (let ((subject n1)) (mk_int__ref n13))))
(assert (= n15 n13))
(declare-const i Int)
(assert (<= 1 i))
(assert (<= i 100))
(assert (= (rec__tree_model__path_type__k (us_split_fields1 (select (model__ (rec__red_black_trees__rbt__struct (us_split_fields9 c106b))) i))) true))
(assert (< (to_rep (rec__red_black_trees__black_count__depth (us_split_fields11 (select (nb_blacks c106b) i)))) (int__content n14)))
(assert (not (= i y3)))
(assert (not (= i c108b)))
(assert (not (= (eq (select (nb_blacks c107b) i) (select (nb_blacks c106b) i)) true)))
(check-sat)
(exit)
