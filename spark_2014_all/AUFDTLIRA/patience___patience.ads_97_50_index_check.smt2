...
(assert (=> (= result1 true) (= (= result true) (forall ((i1 Int)) (=> (and (<= 0 i1) (<= i1 (- (to_rep (rec__patience__state__numstacks (us_split_fields1 s))) 1))) (forall ((j1 Int)) (=> (and (<= 0 j1) (<= j1 (- (to_rep (select (rec__patience__state__stacksizes (us_split_fields1 s)) i1)) 2))) (forall ((k Int)) (=> (and (<= (+ j1 1) k) (<= k (- (to_rep (select (rec__patience__state__stacksizes (us_split_fields1 s)) i1)) 1))) (< (to_rep (select (select (rec__patience__state__stacks (us_split_fields1 s)) i1) j1)) (to_rep (select (select (rec__patience__state__stacks (us_split_fields1 s)) i1) k))))))))))))
