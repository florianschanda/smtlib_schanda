...
(assert (= i2 result2))
(assert (= i3 (+ i2 1)))
(assert (and (= o4 (add_one (to_rep (select (elts2 a) i3)))) (and (in_range3 o4) (= o4 (+ (to_rep (select (elts2 a) i3)) 1)))))
(assert (and (= o5 o4) (in_range1 o4)))
(assert (= (to_rep o6) o5))
(assert (= o7 (store r2 i3 o6)))
(assert (= result3 r2))
(assert (= r3 o7))
(declare-const k Int)
(assert (<= (to_rep1 (first4 (rt2 a))) k))
(assert (<= k i3))
(assert (not (= (to_rep (select r3 k)) (add_one (to_rep (select (elts2 a) k))))))
(check-sat)
(exit)
