...
(assert (<= 1 j))
(assert (<= j 100))
(assert (= (select todo2 j) true))
(declare-const k Int)
(assert (<= 1 k))
(assert (<= k 100))
(assert (= (rec__tree_model__path_type__k (us_split_fields1 (select r1 k))) true))
(assert (not (not (= (olt (rec__tree_model__path_type__a (us_split_fields1 (select r1 j))) (rec__tree_model__path_type__a (us_split_fields1 (select r1 k)))) true))))
(check-sat)
(exit)
