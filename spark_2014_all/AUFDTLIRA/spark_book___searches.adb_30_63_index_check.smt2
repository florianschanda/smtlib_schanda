...
(assert (and (= o8 (+ (to_rep (last (rt list))) 1)) (in_range1 (+ (to_rep (last (rt list))) 1))))
(assert (= o9 (ite (<= position3 o8) true false)))
(assert (= result3 (ite (<= (to_rep (first (rt list))) position3) o9 false)))
(assert (= result3 true))
(assert (<= (to_rep (first (rt list))) j1))
(assert (<= j1 (- position3 1)))
(assert (not (<= (to_rep (first (rt list))) j1)))
(check-sat)
(exit)
