...
(assert (= (= o4 true) (forall ((index11 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 1) index11) (<= index11 (+ (to_rep1 (first (rt source))) 7))) (= (is_digit (to_rep (select (elts source) index11))) true)))))
(assert (and (= temp___2288 (+ (to_rep1 (first (rt source))) 0)) (in_range1 (+ (to_rep1 (first (rt source))) 0))))
(assert (not (<= (to_rep1 (first (rt source))) temp___2288)))
(check-sat)
(exit)
