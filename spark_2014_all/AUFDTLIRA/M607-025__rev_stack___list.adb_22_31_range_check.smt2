...
(assert (and (= (to_rep (last (rt (to__ (mk___rep s__split_fields4))))) (+ (- i2 first4) 1)) (forall ((j Int)) (=> (and (<= first4 j) (<= j i2)) (= (to_rep (select (elts (to__ (mk___rep s__split_fields4))) (+ (- j first4) 1))) (get (model__ l) j))))))
(assert (and (=> (<= first4 last5) (dynamic_property3 first4 last5 i2)) (and (<= first4 i2) (<= i2 last5))))
(assert (not (= i2 last5)))
(assert (= result3 (mk_int__ref i2)))
(assert (= i3 (+ i2 1)))
(assert (not (in_range7 i3)))
(check-sat)
(exit)
