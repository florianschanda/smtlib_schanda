...
(assert (= src1 o3))
(assert (and (forall ((j Int)) (=> (and (<= 1 j) (<= j index2)) (= (to_rep (select dest2 j)) (to_rep (select src j))))) (forall ((j Int)) (=> (and (<= (+ index2 1) j) (<= j 10)) (= (to_rep (select src2 j)) (to_rep (select src j)))))))
...
(assert (<= 1 j))
(assert (<= j index3))
(assert (not (= (to_rep (select dest3 j)) (to_rep (select src j)))))
(check-sat)
(exit)
