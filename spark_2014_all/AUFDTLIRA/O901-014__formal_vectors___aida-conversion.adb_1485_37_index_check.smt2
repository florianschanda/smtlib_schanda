...
(assert (and (= o (+ (to_rep1 (first (rt source))) 7)) (in_range1 (+ (to_rep1 (first (rt source))) 7))))
(assert (= o3 (ite (<= o index) true false)))
(assert (= result (ite (= o3 true) o2 false)))
(assert (= (= o4 true) (forall ((index1 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 7) index1) (<= index1 (+ (to_rep1 (first (rt source))) 10))) (= (is_digit (to_rep (select (elts source) index1))) true)))))
(assert (and (= temp___1933 (+ (to_rep1 (first (rt source))) 7)) (in_range1 (+ (to_rep1 (first (rt source))) 7))))
(assert (not (<= (to_rep1 (first (rt source))) temp___1933)))
(check-sat)
(exit)
