...
(declare-const temp___1962 int__ref)
...
(assert (= tab28 o26))
(assert (and (= o27 (+ j33 1)) (in_range1 (+ j33 1))))
(assert (= result29 (mk_int__ref j33)))
(assert (= j34 o27))
(assert (= temp___1962 (mk_int__ref temp___1961)))
(assert (= j35 (mk_int__ref j34)))
(assert (= tab29 (mk_map__ref tab28)))
(assert (= temp___1963 temp___1961))
(assert (= j36 j34))
(assert (= tab30 tab28))
(declare-const l Int)
(assert (<= a l))
(assert (<= l (- (int__content j35) 1)))
(assert (not (= (to_rep (select (map__content tab29) l)) l)))
(check-sat)
(exit)
