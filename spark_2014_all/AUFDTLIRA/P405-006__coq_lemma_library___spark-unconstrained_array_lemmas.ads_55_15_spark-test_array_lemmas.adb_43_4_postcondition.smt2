...
(assert (<= (to_rep (first (rt arr))) i2))
(assert (<= i2 (to_rep (last (rt arr)))))
(declare-const j1 Int)
(assert (<= (to_rep (first (rt arr))) j1))
(assert (<= j1 (to_rep (last (rt arr)))))
(assert (< i2 j1))
(assert (not (< (to_rep (select (elts arr) i2)) (to_rep (select (elts arr) j1)))))
(check-sat)
(exit)
