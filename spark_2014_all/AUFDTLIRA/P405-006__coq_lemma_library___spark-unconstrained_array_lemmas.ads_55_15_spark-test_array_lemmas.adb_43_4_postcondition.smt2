...
(assert (forall ((i Int)) (=> (and (<= (to_rep (first (rt arr))) i) (<= i (to_rep (last (rt arr))))) (=> (not (= i (to_rep (first (rt arr))))) (< (to_rep (select (elts arr) (- i 1))) (to_rep (select (elts arr) i)))))))
(declare-const i Int)
...
(assert (< i j))
(assert (not (< (to_rep (select (elts arr) i)) (to_rep (select (elts arr) j)))))
(check-sat)
(exit)
