...
(assert (<= 2 i1))
(assert (<= i1 r35b))
(assert (forall ((j Int)) (=> (and (<= 1 j) (<= j (- i2 1))) (<= (to_rep2 (select (elts (rec__simple_unc_arrays__table__v (us_split_fields1 a))) j)) res2))))
(assert (and (and (=> (<= (- 2147483648) 2147483647) (in_range3 res2)) (=> (<= 2 r35b) (dynamic_property2 2 r35b i2))) (and (<= 2 i2) (<= i2 r35b))))
(assert (and (<= (to_rep1 (first (rt (rec__simple_unc_arrays__table__v (us_split_fields1 a))))) i2) (<= i2 (to_rep1 (last (rt (rec__simple_unc_arrays__table__v (us_split_fields1 a))))))))
(assert (= o3 i2))
...
(assert (= result2 true))
(assert (not (<= i2 (to_rep1 (last (rt (rec__simple_unc_arrays__table__v (us_split_fields1 a))))))))
(check-sat)
(exit)
