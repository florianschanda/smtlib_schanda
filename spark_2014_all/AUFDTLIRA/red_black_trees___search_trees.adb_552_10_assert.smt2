...
(assert (= (get (rec__tree_model__path_type__a (us_split_fields1 (select (model__ f1 root) j2))) (+ (length (rec__tree_model__path_type__a (us_split_fields1 (select (model__ f1 root) i2)))) 1)) 0))
(assert (not (< (to_rep (select values j2)) (to_rep (select values i2)))))
(check-sat)
(exit)
