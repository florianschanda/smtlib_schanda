...
(assert (not (= n19 100)))
(assert (= result7 (mk_int__ref n19)))
(assert (= n20 (+ n19 1)))
(declare-const i14 Int)
(assert (<= 1 i14))
(assert (<= i14 100))
(assert (= (rec__tree_model__path_type__k (us_split_fields1 (select (model__ (rec__red_black_trees__rbt__struct (us_split_fields9 c61b))) i14))) true))
(assert (< (to_rep (rec__red_black_trees__black_count__depth (us_split_fields11 (select (nb_blacks c61b) i14)))) n20))
(assert (not (= i14 (root (rec__red_black_trees__rbt__struct (us_split_fields9 c62b))))))
(assert (not (= (eq (select (nb_blacks c62b) i14) (select (nb_blacks c61b) i14)) true)))
(check-sat)
(exit)
