...
(assert (= (= o true) (forall ((index13 Int)) (=> (and (<= (to_rep1 (first (rt source))) index13) (<= index13 (to_rep1 (last (rt source))))) (= (is_digit (to_rep (select (elts source) index13))) true)))))
(assert (= (length source) 10))
(assert (and (= temp___1557 (+ (to_rep1 (first (rt source))) 0)) (in_range1 (+ (to_rep1 (first (rt source))) 0))))
(assert (not (<= (to_rep1 (first (rt source))) temp___1557)))
(check-sat)
(exit)
