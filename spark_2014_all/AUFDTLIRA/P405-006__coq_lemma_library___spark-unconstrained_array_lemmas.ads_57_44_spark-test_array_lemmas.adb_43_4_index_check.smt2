...
(assert (forall ((i2 Int)) (=> (and (<= (to_rep (first (rt arr))) i2) (<= i2 (to_rep (last (rt arr))))) (=> (not (= i2 (to_rep (first (rt arr))))) (< (to_rep (select (elts arr) (- i2 1))) (to_rep (select (elts arr) i2)))))))
(assert (<= (to_rep (first (rt arr))) i1))
(assert (<= i1 (to_rep (last (rt arr)))))
(assert (<= (to_rep (first (rt arr))) j))
(assert (<= j (to_rep (last (rt arr)))))
(assert (< i1 j))
(assert (and (<= (to_rep (first (rt arr))) j) (<= j (to_rep (last (rt arr))))))
(assert (= o4 j))
(assert (= o5 (select (elts arr) o4)))
(assert (= o6 (to_rep o5)))
(assert (not (<= (to_rep (first (rt arr))) i1)))
(check-sat)
(exit)
