...
(assert (=> (and (<= 0 j2) (<= j2 i)) (and (forall ((k Int)) (=> (and (<= 0 k) (<= k j3)) (=> (<= k j3) (= (to_rep (select tmp3 k)) (- 23))))) (= i temp___275))))
...
(assert (= j10 (+ j9 1)))
(assert (= result6 tmp9))
(assert (= tmp10 (store tmp9 j10 rliteral2)))
(assert (= j10 j11))
(assert (= tmp10 tmp11))
(assert (= j12 j10))
(assert (= tmp12 tmp10))
...
(assert (<= k j11))
(assert (< k i))
(assert (not (= (to_rep (select tmp11 k)) (- 23))))
(check-sat)
(exit)
