...
(assert (and (= (length source) 4) (and (= (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 0))) 45) (forall ((index Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 1) index) (<= index (+ (to_rep1 (first (rt source))) 3))) (= (is_digit (to_rep (select (elts source) index))) true))))))
