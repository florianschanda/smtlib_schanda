...
(assert (= (= result1 true) (forall ((j2 Int)) (=> (and (<= 1 j2) (<= j2 (- (to_rep (rec__simple_unc_arrays__table__last a__split_discrs)) 1))) (<= (to_rep1 (select a__split_fields4 j2)) (to_rep1 (select a__split_fields4 (+ j2 1))))))))
(assert (= result1 true))
(assert (<= 1 j))
...
(assert (and (<= (to_rep2 (first a__split_fields5)) k) (<= k (to_rep2 (last a__split_fields5)))))
(assert (= o k))
(assert (= o1 (select a__split_fields4 o)))
(assert (= o2 (to_rep1 o1)))
(assert (not (<= (to_rep2 (first a__split_fields1)) j)))
(check-sat)
(exit)
