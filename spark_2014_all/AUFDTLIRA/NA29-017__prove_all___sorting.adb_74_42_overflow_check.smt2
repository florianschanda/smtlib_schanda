...
(assert (= b9 b7))
(assert (= a3 a1))
(assert (= (= o2 true) (forall ((k Int)) (=> (and (<= (to_rep a__first) k) (<= k (to_rep a__last))) (=> (=> (<= a_first k) (not (<= k (- (+ (+ a_first length1) length2) 1)))) (= (to_rep (select (map__content a2) k)) (to_rep (select a k))))))))
