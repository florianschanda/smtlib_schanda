...
(assert (<= 1 low1))
(assert (<= low1 99))
(assert (and (= o (find_smallest table low1 100)) (and (in_range2 o) (and (and (<= low1 o) (<= o 100)) (forall ((x Int)) (=> (and (<= low1 x) (<= x 100)) (<= (to_rep (select table o)) (to_rep (select table x)))))))))
(assert (= (mk_int__ref result1) (mk_int__ref key)))
(assert (= key1 o))
(assert (=> (not (= key1 low1)) (and (= (to_rep (select table2 low1)) (to_rep (select table key1))) (and (= (to_rep (select table2 key1)) (to_rep (select table low1))) (and (forall ((x Int)) (=> (and (<= 1 x) (<= x 100)) (=> (and (not (= x low1)) (not (= x key1))) (= (to_rep (select table2 x)) (to_rep (select table x)))))) (= (perm table2 table) true))))))
(assert (=> (not (not (= key1 low1))) (= table2 table)))
(assert (=> (not (not (= key1 low1))) (= table2 table1)))
(assert (and (and (forall ((x Int)) (=> (and (<= 1 x) (<= x low2)) (<= (to_rep (select table3 x)) (to_rep (select table3 (+ x 1)))))) (forall ((x Int)) (=> (and (<= low2 x) (<= x 100)) (<= (to_rep (select table3 low2)) (to_rep (select table3 x)))))) (= (perm table3 table) true)))
(assert (and (and (=> (<= 1 100) (in_range2 key2)) (=> (<= 1 99) (in_range4 low2))) (and (<= 1 low2) (<= low2 99))))
(assert (not (= low2 99)))
(assert (= low2 result2))
(assert (= low3 (+ low2 1)))
(assert (and (= o1 (find_smallest table3 low3 100)) (and (in_range2 o1) (and (and (<= low3 o1) (<= o1 100)) (forall ((x Int)) (=> (and (<= low3 x) (<= x 100)) (<= (to_rep (select table3 o1)) (to_rep (select table3 x)))))))))
(assert (= result3 key2))
(assert (= key3 o1))
(assert (=> (not (= key3 low3)) (and (= (to_rep (select table4 low3)) (to_rep (select table3 key3))) (and (= (to_rep (select table4 key3)) (to_rep (select table3 low3))) (and (forall ((x Int)) (=> (and (<= 1 x) (<= x 100)) (=> (and (not (= x low3)) (not (= x key3))) (= (to_rep (select table4 x)) (to_rep (select table3 x)))))) (= (perm table4 table3) true))))))
(assert (=> (not (not (= key3 low3))) (= table4 table3)))
(declare-const x Int)
(assert (<= 1 x))
(assert (<= x low3))
(assert (not (<= (to_rep (select table4 x)) (to_rep (select table4 (+ x 1))))))
(check-sat)
(exit)
