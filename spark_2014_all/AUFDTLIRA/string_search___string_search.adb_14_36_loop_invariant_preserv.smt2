...
(assert (not (= j3 (to_rep1 (last (rt needle))))))
(assert (= j3 result4))
(assert (= j4 (+ j3 1)))
(assert (and (= temp___2641 (+ j4 (- i1 1))) (in_range1 (+ j4 (- i1 1)))))
(assert (and (<= (to_rep1 (first (rt haystack))) temp___2641) (<= temp___2641 (to_rep1 (last (rt haystack))))))
(assert (= o14 temp___2641))
(assert (= o15 (select (elts haystack) o14)))
(assert (= o16 (to_rep o15)))
(assert (= o17 (ite (= (to_rep (select (elts needle) j4)) o16) true false)))
(assert (= o18 (ite (not (= o17 true)) true false)))
(assert (= diff4 result5))
(assert (= diff5 o18))
(assert (not (= diff5 true)))
(assert (= diff5 true))
(assert (not (not (= (to_rep (select (elts needle) j4)) (to_rep (select (elts haystack) (+ j4 (- i1 1))))))))
(check-sat)
(exit)
