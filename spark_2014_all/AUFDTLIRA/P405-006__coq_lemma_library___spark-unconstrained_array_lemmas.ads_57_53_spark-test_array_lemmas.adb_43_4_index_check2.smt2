...
(assert (forall ((i1 Int)) (=> (and (<= (to_rep (first (rt arr))) i1) (<= i1 (to_rep (last (rt arr))))) (=> (not (= i1 (to_rep (first (rt arr))))) (< (to_rep (select (elts arr) (- i1 1))) (to_rep (select (elts arr) i1)))))))
(assert (<= (to_rep (first (rt arr))) i))
(assert (<= i (to_rep (last (rt arr)))))
(assert (<= (to_rep (first (rt arr))) j))
(assert (<= j (to_rep (last (rt arr)))))
(assert (< i j))
(assert (not (<= (to_rep (first (rt arr))) j)))
(check-sat)
(exit)
