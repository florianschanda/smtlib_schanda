...
(assert (<= i_v11 k_1))
(assert (forall ((m Int)) (=> (and (<= (to_rep a_v1__first) m) (<= m (- i_v12 1))) (= (to_rep (select a_v11 m)) (to_rep (select (elts b_v1) m))))))
(assert (and (and (forall ((temp___432 Int)) (=> (and (<= (to_rep a_v1__first) temp___432) (<= temp___432 (to_rep a_v1__last))) (=> (<= i_v12 temp___432) (= (select a_v11 temp___432) (select a_v1 temp___432))))) (=> (<= (to_rep a_v1__first) k_1) (dynamic_property1 (to_rep a_v1__first) k_1 i_v12))) (and (<= (to_rep a_v1__first) i_v12) (<= i_v12 k_1))))
(assert (and (<= (to_rep (first (rt b_v1))) i_v12) (<= i_v12 (to_rep (last (rt b_v1))))))
(assert (= o i_v12))
(assert (= o1 (select (elts b_v1) o)))
(assert (= o2 (to_rep o1)))
(assert (= (to_rep o3) o2))
(assert (not (<= (to_rep a_v1__first) i_v12)))
(check-sat)
(exit)
