...
(assert (=> (<= j1 b) (= (mk_map__ref result1) tab25)))
(assert (=> (<= j1 b) (= tab2 o2)))
...
(assert (= j27 o24))
(assert (= temp___1682 (mk_int__ref temp___1681)))
(assert (= j28 (mk_int__ref j27)))
(assert (= tab23 (mk_map__ref tab22)))
(assert (= temp___1683 temp___1681))
(assert (= j29 j27))
(assert (= tab24 tab22))
(declare-const l Int)
(assert (<= a l))
(assert (<= l (- (int__content j28) 1)))
(assert (not (= (to_rep (select (map__content tab23) l)) l)))
(check-sat)
(exit)
