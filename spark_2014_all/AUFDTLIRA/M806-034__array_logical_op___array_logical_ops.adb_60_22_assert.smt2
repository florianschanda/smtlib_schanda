...
(assert (forall ((i Int)) (=> (and (<= 1 i) (<= i 10)) (= (of_int (to_rep (select (elts r5) i))) true))))
(assert (forall ((i Int)) (=> (and (<= 31 i) (<= i 40)) (= (of_int (to_rep (select (elts r5) i))) true))))
(assert (forall ((i Int)) (=> (and (<= (to_rep1 (first (rt r5))) i) (<= i (to_rep1 (last (rt r5))))) (=> (not (and (<= 11 i) (<= i 30))) (= (of_int (to_rep (select (elts r5) i))) true)))))
(declare-const i Int)
(assert (<= 11 i))
(assert (<= i 30))
(assert (not (not (= (of_int (to_rep (select (elts r5) i))) true))))
(check-sat)
(exit)
