...
(assert (forall ((i Int)) (=> (and (<= (to_rep1 (first (rt r5))) i) (<= i (to_rep1 (last (rt r5))))) (=> (not (and (<= 11 i) (<= i 30))) (= (distinct (to_rep (select (elts r5) i)) 0) true)))))
(declare-const i Int)
(assert (<= 11 i))
(assert (<= i 30))
(assert (not (not (= (distinct (to_rep (select (elts r5) i)) 0) true))))
(check-sat)
(exit)
