...
(assert (<= i1 (- courant3 1)))
(assert (<= (to_rep1 (first (rt c))) j2))
(assert (<= j2 (to_rep1 (last (rt c)))))
(assert (and (<= (to_rep1 (first (rt c))) j2) (<= j2 (to_rep1 (last (rt c))))))
(assert (= o8 j2))
(assert (= o9 (select (elts c) o8)))
(assert (= o10 (to_rep o9)))
(assert (not (<= (to_rep1 (first (rt s))) i1)))
(check-sat)
(exit)
