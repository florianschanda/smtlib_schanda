...
(assert (and (= (length source) 2) (and (= (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 0))) 45) (forall ((index Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 1) index) (<= index (+ (to_rep1 (first (rt source))) 1))) (= (is_digit (to_rep (select (elts source) index))) true))))))
(assert (and (= temp___2439 (+ (to_rep1 (first (rt source))) 1)) (in_range1 (+ (to_rep1 (first (rt source))) 1))))
(assert (and (<= (to_rep1 (first (rt source))) temp___2439) (<= temp___2439 (to_rep1 (last (rt source))))))
(assert (= o temp___2439))
...
(assert (and (= o3 (- target1)) (in_range1 (- target1))))
(assert (= target1 result))
(assert (= target2 o3))
(assert (<= (- 9) target2))
(assert (= result1 has_failed))
(assert (= has_failed1 (of_int 0)))
(assert (= has_failed1 has_failed2))
(assert (= target2 target3))
(assert (= has_failed3 has_failed1))
(assert (= target4 target2))
(assert (not (not (= has_failed2 true))))
(check-sat)
(exit)
