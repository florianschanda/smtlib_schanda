...
(assert (and (= o24 (+ (to_rep1 (first (rt source))) 9)) (in_range1 (+ (to_rep1 (first (rt source))) 9))))
(assert (= o27 (ite (<= o24 index) true false)))
(assert (= result (ite (= o27 true) o26 false)))
(assert (= (= o28 true) (forall ((index1 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 9) index1) (<= index1 (+ (to_rep1 (first (rt source))) 10))) (= (is_digit (to_rep (select (elts source) index1))) true)))))
...
(assert (not (<= (to_rep1 (first (rt source))) temp___1956)))
(check-sat)
(exit)
