...
(assert (<= 1 j3))
(assert (<= j3 100))
(assert (= (rec__tree_model__path_type__k (us_split_fields1 (select r6 j3))) true))
(assert (not (= j3 root)))
(assert (not (not (= (to_rep (rec__binary_trees__cell__parent (us_split_fields3 (select (rec__binary_trees__forest__c (us_split_fields5 f)) j3)))) 0))))
(check-sat)
(exit)
