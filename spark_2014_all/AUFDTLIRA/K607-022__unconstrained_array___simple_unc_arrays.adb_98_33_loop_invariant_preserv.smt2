...
(assert true)
(assert (forall ((a1 us_rep)) (forall ((v1 Int) (low Int) (up Int)) (! (= (= (not_in a1 v1 low up) true) (or (< (to_rep (rec__simple_unc_arrays__table__last (us_split_discrs1 a1))) up) (forall ((j Int)) (=> (and (<= low j) (<= j up)) (not (= (to_rep1 (let ((temp___673 (rec__simple_unc_arrays__table__v (us_split_fields1 a1)))) (select (to_array temp___673) j))) v1)))))) :pattern ((not_in a1 v1 low up))))))
...
(assert (forall ((j Int)) (=> (and (<= 1 j) (<= j (- i2 1))) (not (= (to_rep1 (select (elts (rec__simple_unc_arrays__table__v (us_split_fields1 a))) j)) v)))))
...
(assert (<= 1 j))
(assert (<= j (- i3 1)))
(assert (not (not (= (to_rep1 (select (elts (rec__simple_unc_arrays__table__v (us_split_fields1 a))) j)) v))))
(check-sat)
(exit)
