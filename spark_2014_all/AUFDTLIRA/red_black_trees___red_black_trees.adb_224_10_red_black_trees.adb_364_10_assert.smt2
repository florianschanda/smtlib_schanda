...
(assert (= n1 1))
(assert (=> (and (<= 1 n1) (<= n1 100)) (= temp___638 n1)))
(assert (=> (and (<= 1 n1) (<= n1 100)) (forall ((i Int)) (=> (and (<= 1 i) (<= i 100)) (=> (and (and (= (rec__tree_model__path_type__k (us_split_fields7 (select (model__ (rec__red_black_trees__rbt__struct (us_split_fields9 c61b))) i))) true) (< (to_rep (rec__red_black_trees__black_count__depth (us_split_fields11 (select (nb_blacks c61b) i)))) n2)) (not (= i (root (rec__red_black_trees__rbt__struct (us_split_fields9 c62b)))))) (= (eq (select (nb_blacks c62b) i) (select (nb_blacks c61b) i)) true))))))
