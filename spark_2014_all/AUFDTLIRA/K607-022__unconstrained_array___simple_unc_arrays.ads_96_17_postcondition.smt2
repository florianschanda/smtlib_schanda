...
(assert (<= 1 j2))
(assert (<= j2 (- (to_rep (rec__simple_unc_arrays__table__last a__split_discrs)) 1)))
(assert (not (<= (to_rep1 (select a__split_fields4 j2)) (to_rep1 (select a__split_fields4 (+ j2 1))))))
(check-sat)
(exit)
