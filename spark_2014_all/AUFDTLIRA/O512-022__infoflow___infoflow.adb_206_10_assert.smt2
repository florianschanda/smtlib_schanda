...
(assert (=> (and (<= (to_rep a_v2__first) i_v21) (<= i_v21 k_2)) (forall ((m Int)) (=> (and (<= (to_rep a_v2__first) m) (<= m (- i_v22 1))) (= (to_rep (select a_v22 m)) (to_rep (select (elts b_v2) m)))))))
...
(assert (not (= (to_rep (select (map__content a_v1) m)) (to_rep (select (elts c_v1) (- m k_1))))))
(check-sat)
(exit)
