...
(declare-const winner1 Int)
(assert (<= 0 winner1))
(assert (<= winner1 20))
(declare-const i30 Int)
(assert (<= 1 i30))
(assert (<= i30 last_candidate))
(assert (= (select (map__content1 winners35) winner1) true))
(assert (not (= (select (map__content1 winners35) i30) true)))
(assert (not (< (to_rep (select counters i30)) (to_rep (select counters winner1)))))
(check-sat)
(exit)
