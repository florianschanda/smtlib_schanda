...
(assert (= k31 0))
(assert (<= j63 b))
(assert (= (to_rep o68) j63))
(assert (and (<= (to_rep tab__first) j63) (<= j63 (to_rep tab__last))))
(assert (= o69 j63))
(assert (= o70 (store (map__content tab52) o69 o68)))
(assert (= result56 tab52))
(assert (= tab54 o70))
(assert (and (= o71 (+ j63 1)) (in_range1 (+ j63 1))))
(assert (= result57 (mk_int__ref j63)))
(assert (= j64 o71))
(assert (and (= o72 (+ k31 1)) (in_range1 (+ k31 1))))
(assert (= result58 (mk_int__ref k31)))
(assert (= k32 o72))
(declare-const l8 Int)
(assert (<= a l8))
(assert (<= l8 (- j64 1)))
(assert (not (= (to_rep (select tab54 l8)) l8)))
(check-sat)
(exit)
