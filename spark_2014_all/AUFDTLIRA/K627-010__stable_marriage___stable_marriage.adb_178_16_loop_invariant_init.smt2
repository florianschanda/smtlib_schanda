...
(assert (forall ((g22 Int)) (=> (and (<= 1 g22) (<= g22 64)) (= (is_permutation_1 (select r2 g22)) true))))
...
(assert (<= 1 prev))
(assert (<= prev (- g21 1)))
(assert (not (= (bool_eq1 (select result__1 prev) 1 64 (invert_1 (select r2 prev)) 1 64) true)))
(check-sat)
(exit)
