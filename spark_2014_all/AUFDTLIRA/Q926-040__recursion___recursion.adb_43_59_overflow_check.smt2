...
(declare-const recursion__count_true_loop__result Int)
...
(declare-const recursion__count_true_loop__result1 Int)
(declare-const recursion__count_true_loop__result2 Int)
(declare-const recursion__count_true_loop__result3 Int)
(declare-const count10 Int)
(declare-const index10 Int)
(declare-const recursion__count_true_loop__result4 Int)
(declare-const count11 Int)
(declare-const index11 Int)
(declare-const recursion__count_true_loop__result5 Int)
(assert (dynamic_invariant1 arr true false true))
(assert (< (to_rep (first (rt arr))) (to_rep (last (rt arr)))))
(assert (= (mk_int__ref result) (mk_int__ref count)))
(assert (= count1 0))
(assert (in_range2 count1))
(assert (= (mk_int__ref result1) (mk_int__ref index)))
(assert (= index1 (to_rep (first (rt arr)))))
(assert (<= (to_rep (first (rt arr))) index1))
(assert (<= index1 (to_rep (last (rt arr)))))
(assert (< count2 (+ (- index2 (to_rep (first (rt arr)))) 1)))
(assert (and (and (=> (<= (to_rep (first (rt arr))) (to_rep (last (rt arr)))) (dynamic_property1 (to_rep (first (rt arr))) (to_rep (last (rt arr))) index2)) (=> (<= 0 2147483647) (in_range2 count2))) (and (<= (to_rep (first (rt arr))) index2) (<= index2 (to_rep (last (rt arr)))))))
(assert (and (= o (- index2 (to_rep (first (rt arr))))) (in_range1 (- index2 (to_rep (first (rt arr)))))))
(assert (= o1 (+ o 1)))
