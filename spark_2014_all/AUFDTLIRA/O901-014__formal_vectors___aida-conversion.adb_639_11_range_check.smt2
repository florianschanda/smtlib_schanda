...
(assert (and (and (= (length source) 10) (and (= (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 0))) 50) (= (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 1))) 48))) (forall ((index Int)) (=> (and (<= (to_rep1 (first (rt source))) index) (<= index (to_rep1 (last (rt source))))) (= (is_digit (to_rep (select (elts source) index))) true)))))
