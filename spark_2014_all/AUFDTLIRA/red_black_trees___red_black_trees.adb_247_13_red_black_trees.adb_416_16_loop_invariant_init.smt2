...
(assert (<= 1 i))
(assert (<= i 100))
(assert (= (rec__tree_model__path_type__k (us_split_fields7 (select (model__ (rec__red_black_trees__rbt__struct (us_split_fields9 c75b))) i))) true))
...
(assert (not (= (to_rep (rec__red_black_trees__black_count__count (us_split_fields11 (select (nb_blacks c76b) i)))) (+ (to_rep (rec__red_black_trees__black_count__count (us_split_fields11 (select (nb_blacks c75b) i)))) 1))))
(check-sat)
(exit)
