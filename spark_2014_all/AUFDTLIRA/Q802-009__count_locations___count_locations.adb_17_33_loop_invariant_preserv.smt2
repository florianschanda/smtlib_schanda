...
(assert (= result7 st2))
(assert (= st3 (+ st2 1)))
(assert (not (or (and (forall ((s3 Int)) (=> (and (<= 1 s3) (<= s3 (- st3 1))) (not (= (select sel s3) true)))) (= lowest_max3 2147483647)) (exists ((s3 Int)) (and (and (<= 1 s3) (<= s3 (- st3 1))) (and (= (select sel s3) true) (= (to_rep (select max s3)) lowest_max3)))))))
(check-sat)
(exit)
