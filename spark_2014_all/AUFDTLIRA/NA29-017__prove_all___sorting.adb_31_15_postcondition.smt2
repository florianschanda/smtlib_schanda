...
(assert (=> (and (<= 0 i1) (<= i1 (- length1 1))) (forall ((j Int)) (=> (and (<= (to_rep b__first) j) (<= j (to_rep b__last))) (and (=> (and (<= b_first j) (<= j (- (+ b_first i2) 1))) (= (to_rep (select b2 j)) (to_rep (select (elts a) (- (+ a_first j) b_first))))) (=> (=> (<= b_first j) (not (<= j (- (+ b_first i2) 1)))) (= (to_rep (select b2 j)) (to_rep (select temp___279 j)))))))))
(assert (=> (and (<= 0 i1) (<= i1 (- length1 1))) (and (=> (<= 0 (- length1 1)) (dynamic_property1 0 (- length1 1) i2)) (and (<= 0 i2) (<= i2 (- length1 1))))))
(assert (=> (and (<= 0 i1) (<= i1 (- length1 1))) (forall ((j Int)) (=> (and (<= (to_rep b__first) j) (<= j (to_rep b__last))) (and (=> (and (<= b_first j) (<= j (- (+ b_first (+ i2 1)) 1))) (= (to_rep (select b3 j)) (to_rep (select (elts a) (- (+ a_first j) b_first))))) (=> (=> (<= b_first j) (not (<= j (- (+ b_first (+ i2 1)) 1)))) (= (to_rep (select b3 j)) (to_rep (select temp___271 j)))))))))
...
(assert (=> (<= b_first j) (not (<= j (- (+ b_first length1) 1)))))
(assert (not (= (to_rep (select b8 j)) (to_rep (select b j)))))
(check-sat)
(exit)
