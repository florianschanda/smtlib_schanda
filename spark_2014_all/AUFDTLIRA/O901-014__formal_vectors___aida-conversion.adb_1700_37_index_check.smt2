...
(assert (and (= o4 (+ (to_rep1 (first (rt source))) 3)) (in_range1 (+ (to_rep1 (first (rt source))) 3))))
(assert (= o7 (ite (<= o4 index) true false)))
(assert (= result (ite (= o7 true) o6 false)))
(assert (= (= o8 true) (forall ((index1 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 3) index1) (<= index1 (+ (to_rep1 (first (rt source))) 10))) (= (is_digit (to_rep (select (elts source) index1))) true)))))
...
(assert (not (<= (to_rep1 (first (rt source))) temp___2192)))
(check-sat)
(exit)
