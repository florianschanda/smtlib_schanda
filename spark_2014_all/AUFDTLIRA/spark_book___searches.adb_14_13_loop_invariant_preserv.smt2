...
(assert (exists ((index2 Int)) (and (and (<= (to_rep (first (rt list))) index2) (<= index2 (to_rep (last (rt list))))) (= (to_rep (select (elts list) index2)) value))))
(assert (and (= o (to_rep (first (rt list)))) (in_range2 (to_rep (first (rt list))))))
(assert (= (mk_int__ref result) (mk_int__ref position)))
(assert (= position1 o))
(assert (and (<= (to_rep (first (rt list))) position1) (<= position1 (to_rep (last (rt list))))))
(assert (= o1 position1))
(assert (= o2 (select (elts list) o1)))
(assert (= o3 (to_rep o2)))
(assert (= result1 (ite (= o3 value) true false)))
(assert (not (= result1 true)))
(assert (and (= o4 (+ position1 1)) (in_range1 (+ position1 1))))
(assert (= position1 result2))
(assert (= position2 o4))
(assert (and (and (<= (to_rep (first (rt list))) position3) (<= position3 (to_rep (last (rt list))))) (forall ((j1 Int)) (=> (and (<= (to_rep (first (rt list))) j1) (<= j1 (- position3 1))) (not (= (to_rep (select (elts list) j1)) value))))))
(assert (=> (<= 1 2147483647) (in_range2 position3)))
(assert (and (<= (to_rep (first (rt list))) position3) (<= position3 (to_rep (last (rt list))))))
(assert (= o5 position3))
(assert (= o6 (select (elts list) o5)))
(assert (= o7 (to_rep o6)))
(assert (= result3 (ite (= o7 value) true false)))
(assert (not (= result3 true)))
(assert (and (= o8 (+ position3 1)) (in_range1 (+ position3 1))))
(assert (= result4 position3))
(assert (= position4 o8))
(assert (not (<= (to_rep (first (rt list))) position4)))
(check-sat)
(exit)
