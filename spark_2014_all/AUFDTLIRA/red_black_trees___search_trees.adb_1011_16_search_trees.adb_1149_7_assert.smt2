...
(assert (<= 1 j3))
(assert (<= j3 100))
(assert (= (rec__tree_model__path_type__k (us_split_fields1 (select (model__ (mk___rep2 f_3__split_fields3) x1) j3))) true))
(assert (not (< (to_rep (select (rec__search_trees__search_tree__values t__split_fields8) j3)) (to_rep (select (rec__search_trees__search_tree__values t__split_fields8) y1)))))
(check-sat)
(exit)
