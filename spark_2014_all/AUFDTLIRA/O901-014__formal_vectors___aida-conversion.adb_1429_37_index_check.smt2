...
(assert (and (= o28 (+ (to_rep1 (first (rt source))) 8)) (in_range1 (+ (to_rep1 (first (rt source))) 8))))
(assert (= o31 (ite (<= o28 index) true false)))
(assert (= result (ite (= o31 true) o30 false)))
(assert (= (= o32 true) (forall ((index1 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 8) index1) (<= index1 (+ (to_rep1 (first (rt source))) 10))) (= (is_digit (to_rep (select (elts source) index1))) true)))))
...
(assert (not (<= (to_rep1 (first (rt source))) temp___1991)))
(check-sat)
(exit)
