...
(assert (= diff2 (of_int 0)))
(assert (= (mk_int__ref result2) (mk_int__ref j)))
(assert (= j2 (to_rep1 (first (rt needle)))))
(assert (<= (to_rep1 (first (rt needle))) j2))
(assert (<= j2 (to_rep1 (last (rt needle)))))
(assert (and (= temp___264 (+ j2 (- i1 1))) (in_range1 (+ j2 (- i1 1)))))
(assert (and (<= (to_rep1 (first (rt haystack))) temp___264) (<= temp___264 (to_rep1 (last (rt haystack))))))
(assert (= o5 temp___264))
(assert (= o6 (select (elts haystack) o5)))
(assert (= o7 (to_rep o6)))
(assert (= o8 (ite (= (to_rep (select (elts needle) j2)) o7) true false)))
(assert (= o9 (ite (not (= o8 true)) true false)))
(assert (= diff2 result3))
(assert (= diff3 o9))
(assert (not (= diff3 true)))
(assert (= (partial_match_at needle haystack i1 j3) true))
(assert (= (= diff4 true) (not (= (to_rep (select (elts needle) j3)) (to_rep (select (elts haystack) (+ j3 (- i1 1))))))))
(assert (and (=> (<= (to_rep1 (first (rt needle))) (to_rep1 (last (rt needle)))) (dynamic_property2 (to_rep1 (first (rt needle))) (to_rep1 (last (rt needle))) j3)) (and (<= (to_rep1 (first (rt needle))) j3) (<= j3 (to_rep1 (last (rt needle)))))))
(assert (not (in_range1 (+ j3 (- i1 1)))))
(check-sat)
(exit)
