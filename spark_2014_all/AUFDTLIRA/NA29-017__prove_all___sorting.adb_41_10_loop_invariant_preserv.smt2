...
(assert (forall ((j3 Int)) (=> (and (<= (to_rep b__first) j3) (<= j3 (to_rep b__last))) (and (=> (and (<= b_first j3) (<= j3 (- (+ b_first (+ i2 1)) 1))) (= (to_rep (select b3 j3)) (to_rep (select (elts a) (- (+ a_first j3) b_first))))) (=> (=> (<= b_first j3) (not (<= j3 (- (+ b_first (+ i2 1)) 1)))) (= (to_rep (select b3 j3)) (to_rep (select b j3))))))))
