...
(assert (= (get (rec__tree_model__path_type__a (us_split_fields1 (select (model__ f root) j7))) (+ (length (rec__tree_model__path_type__a (us_split_fields1 (select (model__ f root) i9)))) 1)) 0))
(assert (not (< (to_rep (select values j7)) (to_rep (select values i9)))))
(check-sat)
(exit)
