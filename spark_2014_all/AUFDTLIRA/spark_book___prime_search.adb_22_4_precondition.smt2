...
(assert (forall ((i1 Int)) (=> (and (<= 1 i1) (<= i1 10)) (forall ((j1 Int)) (=> (and (<= (+ i1 1) j1) (<= j1 10)) (< (to_rep (select primes i1)) (to_rep (select primes j1))))))))
...
(assert (<= (+ j1 1) k))
(assert (<= k 10))
(assert (not (< (to_rep (select primes j1)) (to_rep (select primes k)))))
(check-sat)
(exit)
