...
(assert (forall ((temp___177 Int)) (=> (dynamic_invariant temp___177 true true true true) (forall ((temp___178 Int)) (= (select (actions__a__aggregate_def temp___177) temp___178) rliteral)))))
...
(assert (and (<= 1 x) (<= x 10)))
(assert (= o x))
(assert (= o1 (store a1 o rliteral1)))
(assert (= result1 a1))
(assert (= a2 o1))
(assert (not (= b1 true)))
(assert (not (= b2 true)))
...
(assert (= result3 (ite (<= 6 j) o3 false)))
(assert (= (= result2 true) (exists ((j1 Int)) (and (and (<= 6 j1) (<= j1 (+ y 1))) (= (to_rep (select a2 j1)) 1)))))
