...
(assert (and (= (length source) 11) (and (and (= (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 0))) 45) (< (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 1))) 50)) (forall ((index2 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 1) index2) (<= index2 (+ (to_rep1 (first (rt source))) 10))) (= (is_digit (to_rep (select (elts source) index2))) true))))))
...
(assert (<= temp___2264 index1))
(assert (<= index1 temp___2265))
...
(assert (not (<= (to_rep1 n__first) index1)))
(check-sat)
(exit)
