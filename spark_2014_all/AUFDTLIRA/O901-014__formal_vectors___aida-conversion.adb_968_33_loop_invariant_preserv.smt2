...
(assert (and (= (length source) 5) (forall ((index4 Int)) (=> (and (<= (to_rep1 (first (rt source))) index4) (<= index4 (to_rep1 (last (rt source))))) (= (is_digit (to_rep (select (elts source) index4))) true)))))
...
(assert (= n1 result2))
(assert (= n2 o8))
(assert (forall ((j Int)) (=> (and (<= (to_rep1 (first (rt source))) j) (<= j index2)) (= (to_rep1 (select n3 j)) (convert_character_digit_to_integer (to_rep (select (elts source) j)))))))
(assert (and (and (=> (<= (to_rep1 (first (rt source))) (+ (to_rep1 (first (rt source))) 4)) (dynamic_property5 (to_rep1 (first (rt source))) (+ (to_rep1 (first (rt source))) 4) index2)) (forall ((temp___1787 Int)) (=> (and (<= (to_rep1 n__first) temp___1787) (<= temp___1787 (to_rep1 n__last))) (=> (< index2 temp___1787) (= (select n3 temp___1787) (select n1 temp___1787)))))) (and (<= (to_rep1 (first (rt source))) index2) (<= index2 (+ (to_rep1 (first (rt source))) 4)))))
(assert (not (= index2 temp___1789)))
(assert (= index2 result3))
...
(assert (<= j index3))
(assert (not (= (to_rep1 (select n4 j)) (convert_character_digit_to_integer (to_rep (select (elts source) j))))))
(check-sat)
(exit)
