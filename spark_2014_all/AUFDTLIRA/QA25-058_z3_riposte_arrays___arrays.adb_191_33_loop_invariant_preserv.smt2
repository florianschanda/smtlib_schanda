...
(declare-const temp___309 (Array Int Bool))
...
(assert (forall ((i Int)) (=> (and (<= 0 i) (<= i x2)) (= (= (select r3 i) true) (= i c)))))
(assert (and (and (forall ((temp___310 Int)) (=> (and (<= 0 temp___310) (<= temp___310 255)) (=> (< x2 temp___310) (= (select r3 temp___310) (select r temp___310))))) (=> (<= 0 255) (in_range1 x2))) (and (<= 0 x2) (<= x2 255))))
(assert (not (= x2 255)))
(assert (= result2 x2))
(assert (= x3 (+ x2 1)))
(assert (= result3 r3))
(assert (= r4 (store r3 x3 (ite (= x3 c) true false))))
(declare-const i Int)
(assert (<= 0 i))
(assert (<= i x3))
(assert (= (select r4 i) true))
(assert (not (= i c)))
(check-sat)
(exit)
