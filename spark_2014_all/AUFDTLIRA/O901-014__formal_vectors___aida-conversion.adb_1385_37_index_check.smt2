...
(assert (and (= o16 (+ (to_rep1 (first (rt source))) 9)) (in_range1 (+ (to_rep1 (first (rt source))) 9))))
(assert (= o19 (ite (<= o16 index) true false)))
(assert (= result (ite (= o19 true) o18 false)))
(assert (= (= o20 true) (forall ((index1 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 9) index1) (<= index1 (+ (to_rep1 (first (rt source))) 10))) (= (is_digit (to_rep (select (elts source) index1))) true)))))
...
(assert (not (<= temp___1958 (to_rep1 (last (rt source))))))
(check-sat)
(exit)
