...
(assert (<= (int__content k21) i11))
(assert (<= i11 (to_rep1 a__last)))
(declare-const j5 Int)
(assert (<= i11 j5))
(assert (<= j5 (to_rep1 a__last)))
(assert (not (<= (to_rep (rec__frame_condition__cell__value (us_split_fields1 (select (map__content a18) j5)))) (to_rep (rec__frame_condition__cell__max_right (us_split_fields1 (select (map__content a18) i11)))))))
(check-sat)
(exit)
