...
(assert (and (forall ((i4 Int)) (=> (and (<= 1 i4) (<= i4 100)) (forall ((j Int)) (=> (and (<= 1 j) (<= j 100)) (=> (= (to_rep (select a i4)) (to_rep (select a j))) (= i4 j)))))) (forall ((i4 Int)) (=> (and (<= 1 i4) (<= i4 100)) (not (forall ((j Int)) (=> (and (<= 1 j) (<= j 100)) (not (= (to_rep (select a j)) i4)))))))))
(assert (= (mk_int__ref result) (mk_int__ref i)))
(assert (= i1 1))
(assert (<= 1 i1))
(assert (<= i1 100))
(assert (forall ((j Int)) (=> (and (<= 1 j) (<= j (- i2 1))) (= (to_rep (select b (to_rep (select a j)))) j))))
(assert (and (=> (<= 1 100) (in_range1 i2)) (and (<= 1 i2) (<= i2 100))))
(assert (= (to_rep o) i2))
(assert (= o1 (store b (to_rep (select a i2)) o)))
(assert (= b result1))
(assert (= b1 o1))
(assert (not (= i2 100)))
(assert (= result2 i2))
...
(assert (<= 1 j))
(assert (<= j (- i3 1)))
(assert (not (= (to_rep (select b1 (to_rep (select a j)))) j)))
(check-sat)
(exit)
