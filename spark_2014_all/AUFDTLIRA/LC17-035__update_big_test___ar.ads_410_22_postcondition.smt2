...
(assert (<= 1 m))
(assert (<= m 16))
(assert (or (or (not (= m k1)) (not (= n j1))) (not (= o8 i1))))
(assert (or (or (not (= m k2)) (not (= n j2))) (not (= o8 i2))))
(assert (not (= (to_rep (select (select (select a3 m) n) o8)) (to_rep (select (select (select a m) n) o8)))))
(check-sat)
(exit)
