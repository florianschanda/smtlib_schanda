...
(declare-const g2_id1 Int)
(assert (<= 1 g2_id1))
(assert (<= g2_id1 64))
(assert (not (= (select unmatched_g2_set2 g2_id1) true)))
(declare-const idx3 Int)
(assert (<= 1 idx3))
(assert (<= idx3 (to_rep3 (rec__stable_marriage__matching__group1_set__cardinality unmatched_g1_set__split_fields4))))
(assert (not (not (= (to_rep (select (rec__stable_marriage__matching__group1_set__elements unmatched_g1_set__split_fields4) idx3)) (to_rep (select result__2 g2_id1))))))
(check-sat)
(exit)
