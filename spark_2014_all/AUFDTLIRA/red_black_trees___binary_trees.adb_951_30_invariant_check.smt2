...
(assert (= result3 true))
(assert (= (= result4 true) (forall ((j2 Int)) (=> (and (<= 1 j2) (<= j2 100)) (=> (not (= (rec__tree_model__path_type__k (us_split_fields1 (select (model__ (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1)) root) j2))) true)) (= (to_rep (rec__binary_trees__cell__left (us_split_fields3 (select f__split_fields1 j2)))) (to_rep (rec__binary_trees__cell__left (us_split_fields3 (select f_old__split_fields1 j2))))))))))
(assert (= result4 true))
(assert (<= 1 j))
(assert (<= j 100))
