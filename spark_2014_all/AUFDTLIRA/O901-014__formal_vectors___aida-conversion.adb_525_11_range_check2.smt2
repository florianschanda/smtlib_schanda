...
(assert (and (and (= (length source) 10) (and (and (and (= (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 0))) 50) (= (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 1))) 49)) (= (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 2))) 52)) (< (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 3))) 55))) (forall ((index Int)) (=> (and (<= (to_rep1 (first (rt source))) index) (<= index (to_rep1 (last (rt source))))) (= (is_digit (to_rep (select (elts source) index))) true)))))
...
(assert (<= (to_rep1 (first (rt source))) (+ (to_rep1 (first (rt source))) 9)))
(assert (not (<= (- 2147483648) (to_rep1 (first (rt source))))))
(check-sat)
(exit)
