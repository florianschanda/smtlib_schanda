...
(assert (= min1 o))
(assert (and (= temp___174 (+ (to_rep (first (rt unsorted))) 1)) (in_range1 (+ (to_rep (first (rt unsorted))) 1))))
(assert (= (mk_int__ref result1) (mk_int__ref index)))
(assert (= index1 temp___174))
(assert (<= temp___174 index1))
(assert (<= index1 (to_rep (last (rt unsorted)))))
(assert (and (and (<= (to_rep (first (rt unsorted))) min2) (<= min2 (to_rep (last (rt unsorted))))) (forall ((j2 Int)) (=> (and (<= (to_rep (first (rt unsorted))) j2) (<= j2 (- index2 1))) (<= (to_rep (select (elts unsorted) min2)) (to_rep (select (elts unsorted) j2)))))))
(assert (and (and (=> (<= 1 2147483647) (in_range2 min2)) (=> (<= (+ (to_rep (first (rt unsorted))) 1) (to_rep (last (rt unsorted)))) (dynamic_property1 (+ (to_rep (first (rt unsorted))) 1) (to_rep (last (rt unsorted))) index2))) (and (<= (+ (to_rep (first (rt unsorted))) 1) index2) (<= index2 (to_rep (last (rt unsorted)))))))
(assert (and (<= (to_rep (first (rt unsorted))) min2) (<= min2 (to_rep (last (rt unsorted))))))
(assert (= o8 min2))
(assert (= o9 (select (elts unsorted) o8)))
(assert (= o10 (to_rep o9)))
(assert (and (<= (to_rep (first (rt unsorted))) index2) (<= index2 (to_rep (last (rt unsorted))))))
(assert (= o6 index2))
(assert (= o7 (select (elts unsorted) o6)))
(assert (= o11 (to_rep o7)))
(assert (= result3 (ite (< o11 o10) true false)))
(assert (=> (= result3 true) (and (= o12 index2) (in_range2 index2))))
(assert (=> (= result3 true) (= min2 result4)))
(assert (=> (= result3 true) (= min3 o12)))
(assert (=> (not (= result3 true)) (= min3 min2)))
(assert (not (= index2 (to_rep (last (rt unsorted))))))
(assert (= result5 index2))
(assert (= index3 (+ index2 1)))
(assert (not (<= (to_rep (first (rt unsorted))) min3)))
(check-sat)
(exit)
