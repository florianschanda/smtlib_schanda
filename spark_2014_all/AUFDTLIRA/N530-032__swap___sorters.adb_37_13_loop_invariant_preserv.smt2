...
(assert (= min1 o))
(assert (and (= temp___199 (+ (to_rep (first (rt unsorted))) 1)) (in_range1 (+ (to_rep (first (rt unsorted))) 1))))
(assert (= (mk_int__ref result1) (mk_int__ref index)))
(assert (= index1 temp___199))
(assert (<= temp___199 index1))
(assert (<= index1 (to_rep (last (rt unsorted)))))
(assert (and (and (<= (to_rep (first (rt unsorted))) min2) (<= min2 (to_rep (last (rt unsorted))))) (forall ((j Int)) (=> (and (<= (to_rep (first (rt unsorted))) j) (<= j (- index2 1))) (<= (to_rep (select (elts unsorted) min2)) (to_rep (select (elts unsorted) j)))))))
...
(assert (=> (= result2 true) (and (= o7 index2) (in_range2 index2))))
(assert (=> (= result2 true) (= min2 result3)))
(assert (=> (= result2 true) (= min3 o7)))
