...
(assert (= (= o32 true) (forall ((index1 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 7) index1) (<= index1 (+ (to_rep1 (first (rt source))) 10))) (= (is_digit (to_rep (select (elts source) index1))) true)))))
...
(assert (and (<= (to_rep1 (first (rt source))) temp___2029) (<= temp___2029 (to_rep1 (last (rt source))))))
(assert (= o temp___2029))
(assert (= o1 (select (elts source) o)))
(assert (= o2 (to_rep o1)))
(assert (= o3 (ite (= o2 50) true false)))
(assert (and (= temp___2028 (+ (to_rep1 (first (rt source))) 0)) (in_range1 (+ (to_rep1 (first (rt source))) 0))))
(assert (not (<= (to_rep1 (first (rt source))) temp___2028)))
(check-sat)
(exit)
