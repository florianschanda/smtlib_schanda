...
(assert (and (= (length source) 8) (and (= (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 0))) 45) (forall ((index2 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 1) index2) (<= index2 (+ (to_rep1 (first (rt source))) 7))) (= (is_digit (to_rep (select (elts source) index2))) true))))))
...
(assert (<= temp___2381 index1))
(assert (<= index1 temp___2382))
