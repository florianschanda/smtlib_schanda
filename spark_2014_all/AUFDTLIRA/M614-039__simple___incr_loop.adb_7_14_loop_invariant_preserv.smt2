...
(assert (forall ((i4 Int)) (=> (and (<= (to_rep a__first) i4) (<= i4 (to_rep a__last))) (< (to_rep (select a i4)) 2147483647))))
(assert (= (mk_int__ref result) (mk_int__ref i)))
(assert (= i1 (to_rep a__first)))
(assert (<= (to_rep a__first) i1))
(assert (<= i1 (to_rep a__last)))
(assert (and (forall ((k Int)) (=> (and (<= (to_rep a__first) k) (<= k (- i2 1))) (= (to_rep (select a1 k)) (+ (to_rep (select a k)) 1)))) (forall ((k Int)) (=> (and (<= i2 k) (<= k (to_rep a__last))) (= (to_rep (select a1 k)) (to_rep (select a k)))))))
...
(assert (and (= o (+ (to_rep (select a1 i2)) 1)) (in_range1 (+ (to_rep (select a1 i2)) 1))))
(assert (= (to_rep o1) o))
...
(assert (<= k (- i3 1)))
(assert (not (= (to_rep (select a2 k)) (+ (to_rep (select a k)) 1))))
(check-sat)
(exit)
