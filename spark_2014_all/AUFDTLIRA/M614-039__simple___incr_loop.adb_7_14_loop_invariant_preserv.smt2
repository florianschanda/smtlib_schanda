...
(assert (and (= o11 (+ (to_rep (select a2 i4)) 1)) (in_range1 (+ (to_rep (select a2 i4)) 1))))
(assert (= (to_rep o12) o11))
(assert (= o13 (store a2 i4 o12)))
(assert (= a2 result2))
(assert (= a3 o13))
(assert (not (= i4 (to_rep a__last))))
(assert (= result3 i4))
(assert (= i5 (+ i4 1)))
(declare-const k2 Int)
(assert (<= (to_rep a__first) k2))
(assert (<= k2 (- i5 1)))
(assert (not (= (to_rep (select a3 k2)) (+ (to_rep (select a k2)) 1))))
(check-sat)
(exit)
