...
(assert (and (and (= (is_perm (mk___t a (mk (to_rep a__first) (to_rep a__last))) (mk___t a1 (mk (to_rep a__first) (to_rep a__last)))) true) (and (= (to_rep (select a1 o14)) (to_rep (select a o15))) (and (= (to_rep (select a1 o15)) (to_rep (select a o14))) (forall ((z3 Int)) (=> (and (<= (to_rep a__first) z3) (<= z3 (to_rep a__last))) (=> (and (not (= z3 o14)) (not (= z3 o15))) (= (to_rep (select a1 z3)) (to_rep (select a z3))))))))) (= (to_rep a__first) 0)))
(assert (and (<= 0 j2) (<= j2 (- (ite (<= (to_rep a__first) (to_rep a__last)) (+ (- (to_rep a__last) (to_rep a__first)) 1) 0) 3))))
(assert (= (sorted (mk___t a2 (mk (to_rep a__first) (to_rep a__last))) 0 (- j2 1)) true))
(assert (= (sorted (mk___t a2 (mk (to_rep a__first) (to_rep a__last))) (+ j2 2) (+ i1 1)) true))
(assert (=> (< 0 j2) (<= (to_rep (select a2 (- j2 1))) (to_rep (select a2 (+ j2 2))))))
(assert (< x2 (to_rep (select a2 (+ j2 2)))))
(assert (= (is_perm (mk___t (elts1 b) (mk (to_rep (first2 (rt1 b))) (to_rep (last2 (rt1 b))))) (mk___t a2 (mk (to_rep a__first) (to_rep a__last)))) true))
(assert (or (and (= (to_rep (select a2 j2)) x2) (= (to_rep (select a2 (+ j2 1))) y2)) (and (= (to_rep (select a2 j2)) y2) (= (to_rep (select a2 (+ j2 1))) x2))))
