...
(assert (= j41 a))
(assert (<= j41 b))
(assert (= (to_rep o41) j41))
(assert (and (<= (to_rep tab__first) j41) (<= j41 (to_rep tab__last))))
(assert (= o42 j41))
(assert (= o43 (store (map__content tab32) o42 o41)))
(assert (= result36 tab32))
(assert (= tab34 o43))
(assert (and (= o44 (+ j41 1)) (in_range1 (+ j41 1))))
(assert (= result37 (mk_int__ref j41)))
(assert (= j42 o44))
(assert (forall ((l8 Int)) (=> (and (<= a l8) (<= l8 (- j43 1))) (= (to_rep (select tab35 l8)) l8))))
(assert (=> (<= (- 2147483648) 2147483647) (in_range1 j43)))
(assert (not (in_range1 (- j43 1))))
(check-sat)
(exit)
