...
(assert (<= 1 j))
(assert (<= j 100))
(assert (= (rec__tree_model__path_type__k (us_split_fields1 (select r1 j))) true))
(declare-const k Int)
(assert (<= 1 k))
(assert (<= k 100))
(assert (= (rec__tree_model__path_type__k (us_split_fields1 (select r1 k))) true))
(assert (= (oeq (rec__tree_model__path_type__a (us_split_fields1 (select r1 k))) (rec__tree_model__path_type__a (us_split_fields1 (select r1 j)))) true))
(assert (not (= j k)))
(check-sat)
(exit)
