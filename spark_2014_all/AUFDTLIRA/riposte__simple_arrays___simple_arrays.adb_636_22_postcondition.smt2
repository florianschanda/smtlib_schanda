...
(define-fun dynamic_invariant ((temp___expr_160 Int) (temp___is_init_156 Bool) (temp___skip_constant_157 Bool) (temp___do_toplevel_158 Bool) (temp___do_typ_inv_159 Bool)) Bool (=> (or (= temp___is_init_156 true) (<= 0 2)) (in_range2 temp___expr_160)))
(declare-fun to_rep (enum_t) Int)
(declare-fun of_rep (Int) enum_t)
(assert (forall ((x enum_t)) (! (= (of_rep (to_rep x)) x) :pattern ((to_rep x)))))
(assert (forall ((x enum_t)) (! (in_range2 (to_rep x)) :pattern ((to_rep x)))))
...
(declare-const result2 (Array Int enum_t))
...
(declare-const b3 (Array Int enum_t))
(declare-const tmp3 Int)
(assert (=> (<= 0 2) (in_range2 tmp)))
(assert (= result tmp))
(assert (= tmp1 (to_rep (select a 42))))
(assert (= (to_rep o) (to_rep (select b 23))))
(assert (= o1 (store a 42 o)))
(assert (= result1 a))
(assert (= a1 o1))
(assert (= (to_rep o2) tmp1))
(assert (= o3 (store b 23 o2)))
(assert (= result2 b))
(assert (= b1 o3))
(assert (= tmp2 tmp1))
(assert (= b1 b2))
...
(assert (= b3 b1))
(assert (= a3 a1))
(assert (not (not (= (bool_eq a2 (- 2147483648) 2147483647 a (- 2147483648) 2147483647) true))))
(check-sat)
(exit)
