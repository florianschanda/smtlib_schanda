...
(assert (<= 1 j16))
(assert (<= j16 100))
(assert (= (select todo6 j16) true))
(declare-const k Int)
(assert (<= 1 k))
(assert (<= k 100))
(assert (= (rec__tree_model__path_type__k (us_split_fields1 (select r6 k))) true))
(assert (not (not (= (olt (rec__tree_model__path_type__a (us_split_fields1 (select r6 j16))) (rec__tree_model__path_type__a (us_split_fields1 (select r6 k)))) true))))
(check-sat)
(exit)
