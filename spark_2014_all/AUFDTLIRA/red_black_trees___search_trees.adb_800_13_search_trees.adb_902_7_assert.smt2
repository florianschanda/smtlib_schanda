...
(assert (<= 1 j15))
(assert (<= j15 100))
(assert (= (rec__tree_model__path_type__k (us_split_fields1 (select (model__ (mk___rep2 (let ((subject f_4__split_fields)) f_4__split_fields2)) x1) j15))) true))
(assert (not (< (to_rep (select (rec__search_trees__search_tree__values t__split_fields9) j15)) (to_rep (select (rec__search_trees__search_tree__values t__split_fields9) y1)))))
(check-sat)
(exit)
