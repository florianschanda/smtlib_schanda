...
(assert (= (length source) 7))
(assert (and (= o1 (+ (to_rep1 (first (rt source))) 6)) (in_range1 (+ (to_rep1 (first (rt source))) 6))))
(assert (= o2 (ite (<= index1 o1) true false)))
(assert (and (= o (+ (to_rep1 (first (rt source))) 1)) (in_range1 (+ (to_rep1 (first (rt source))) 1))))
(assert (= o3 (ite (<= o index1) true false)))
(assert (= result (ite (= o3 true) o2 false)))
(assert (= (= o4 true) (forall ((index11 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 1) index11) (<= index11 (+ (to_rep1 (first (rt source))) 6))) (= (is_digit (to_rep (select (elts source) index11))) true)))))
