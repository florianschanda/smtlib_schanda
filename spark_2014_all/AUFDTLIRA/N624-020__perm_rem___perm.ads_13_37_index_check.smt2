...
(assert (= result3 true))
(assert (and (= o11 (- (to_rep1 (last (rt a))) 1)) (in_range1 (- (to_rep1 (last (rt a))) 1))))
(assert (= result4 (ite (= (to_rep1 (last perm__remove__result9)) o11) true false)))
(assert (= result4 true))
(assert (<= 1 k))
(assert (<= k (- i 1)))
...
(assert (not (<= (to_rep1 (first (rt a))) k)))
(check-sat)
(exit)
