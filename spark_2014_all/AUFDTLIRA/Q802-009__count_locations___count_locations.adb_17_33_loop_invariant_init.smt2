...
(assert (<= 1 st1))
(assert (<= st1 100))
(assert (not (or (and (forall ((s3 Int)) (=> (and (<= 1 s3) (<= s3 (- st1 1))) (not (= (select sel s3) true)))) (= lowest_max1 2147483647)) (exists ((s3 Int)) (and (and (<= 1 s3) (<= s3 (- st1 1))) (and (= (select sel s3) true) (= (to_rep (select max s3)) lowest_max1)))))))
(check-sat)
(exit)
