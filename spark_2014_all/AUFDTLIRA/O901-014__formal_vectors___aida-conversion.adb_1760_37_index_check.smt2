...
(assert (and (= o9 (+ (to_rep1 (first (rt source))) 10)) (in_range1 (+ (to_rep1 (first (rt source))) 10))))
(assert (= o10 (ite (<= index1 o9) true false)))
(assert (and (= o8 (+ (to_rep1 (first (rt source))) 2)) (in_range1 (+ (to_rep1 (first (rt source))) 2))))
(assert (= o11 (ite (<= o8 index1) true false)))
(assert (= result (ite (= o11 true) o10 false)))
(assert (= (= o12 true) (forall ((index11 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 2) index11) (<= index11 (+ (to_rep1 (first (rt source))) 10))) (= (is_digit (to_rep (select (elts source) index11))) true)))))
(assert (and (= temp___2132 (+ (to_rep1 (first (rt source))) 2)) (in_range1 (+ (to_rep1 (first (rt source))) 2))))
(assert (not (<= (to_rep1 (first (rt source))) temp___2132)))
(check-sat)
(exit)
