...
(assert (and (= o1 (+ (to_rep1 (first (rt source))) 10)) (in_range1 (+ (to_rep1 (first (rt source))) 10))))
(assert (= o2 (ite (<= index o1) true false)))
(assert (and (= o (+ (to_rep1 (first (rt source))) 2)) (in_range1 (+ (to_rep1 (first (rt source))) 2))))
(assert (= o3 (ite (<= o index) true false)))
(assert (= result (ite (= o3 true) o2 false)))
(assert (= (= o4 true) (forall ((index1 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 2) index1) (<= index1 (+ (to_rep1 (first (rt source))) 10))) (= (is_digit (to_rep (select (elts source) index1))) true)))))
(assert (and (= temp___2234 (+ (to_rep1 (first (rt source))) 2)) (in_range1 (+ (to_rep1 (first (rt source))) 2))))
(assert (not (<= (to_rep1 (first (rt source))) temp___2234)))
(check-sat)
(exit)
