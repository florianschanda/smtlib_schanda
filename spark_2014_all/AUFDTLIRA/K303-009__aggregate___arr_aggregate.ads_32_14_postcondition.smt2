...
(assert (in_range b))
(assert (= one 1))
(assert (and (ite (= b 1) (= (to_rep (select a one)) one) (forall ((k Int)) (=> (and (<= 1 k) (<= k 1)) (= (to_rep (select a k)) one)))) (and (= a a1) (= a2 a))))
(assert (= b 1))
(assert (not (= (bool_eq a1 1 1 (temp___180 one) 1 1) true)))
(check-sat)
(exit)
