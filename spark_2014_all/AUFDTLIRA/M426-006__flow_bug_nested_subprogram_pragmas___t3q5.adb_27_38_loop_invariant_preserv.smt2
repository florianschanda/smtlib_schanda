...
(assert (and (and (and (and (<= (+ l 1) i2) (<= i2 u)) (< l u)) (forall ((x Int)) (=> (and (<= l x) (<= x i2)) (<= (to_rep (select arr k3)) (to_rep (select arr x)))))) (and (<= l k3) (<= k3 u))))
(assert (and (and (=> (<= (+ l 1) u) (dynamic_property (+ l 1) u i2)) (=> (<= 1 100) (in_range1 k3))) (and (<= (+ l 1) i2) (<= i2 u))))
(assert (not (= i2 u)))
(assert (= result2 i2))
(assert (= i3 (+ i2 1)))
(assert (=> (< (to_rep (select arr i3)) (to_rep (select arr k3))) (= result3 k3)))
(assert (=> (< (to_rep (select arr i3)) (to_rep (select arr k3))) (= k4 i3)))
(assert (=> (not (< (to_rep (select arr i3)) (to_rep (select arr k3)))) (= k4 k3)))
(assert (not (<= (+ l 1) i3)))
(check-sat)
(exit)
