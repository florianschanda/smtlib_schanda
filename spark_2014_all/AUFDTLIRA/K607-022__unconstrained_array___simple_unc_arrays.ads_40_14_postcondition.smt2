...
(assert (=> (and (<= 2 i1) (<= i1 r25b)) (forall ((j Int)) (=> (and (<= 1 j) (<= j (- i2 1))) (<= res2 (to_rep1 (select (elts (rec__simple_unc_arrays__table__v (us_split_fields1 a))) j)))))))
...
(assert (not (<= simple_unc_arrays__min__result4 (to_rep1 (select (elts (rec__simple_unc_arrays__table__v (us_split_fields1 a))) j)))))
(check-sat)
(exit)
