...
(assert (<= 1 i))
(assert (<= i 100))
(assert (= (rec__tree_model__path_type__k (us_split_fields1 (select (model__ (rec__red_black_trees__rbt__struct (us_split_fields9 c87b))) i))) true))
(assert (< (to_rep (rec__red_black_trees__black_count__depth (us_split_fields11 (select (nb_blacks c87b) i)))) n10))
(assert (not (= i y1)))
(assert (not (= i c89b)))
(assert (not (= (eq (select (nb_blacks c88b) i) (select (nb_blacks c87b) i)) true)))
(check-sat)
(exit)
