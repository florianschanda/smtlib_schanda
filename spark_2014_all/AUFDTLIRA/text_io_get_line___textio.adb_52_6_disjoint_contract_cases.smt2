...
(assert (in_range2 from2))
(assert (in_range4 n))
(assert (=> (not (= n 0)) (and (and (and (and (<= (to_rep1 s1__first) from1) (<= from1 (to_rep1 s1__last))) (and (<= (to_rep1 s1__first) (- (+ from1 n) 1)) (<= (- (+ from1 n) 1) (to_rep1 s1__last)))) (and (<= (to_rep1 (first (rt s2))) from2) (<= from2 (to_rep1 (last (rt s2)))))) (and (<= (to_rep1 (first (rt s2))) (- (+ from2 n) 1)) (<= (- (+ from2 n) 1) (to_rep1 (last (rt s2))))))))
(assert (not (<= (+ (+ 0 (ite (= (= n 0) true) 1 0)) (ite (= (not (= n 0)) true) 1 0)) 1)))
(check-sat)
(exit)
