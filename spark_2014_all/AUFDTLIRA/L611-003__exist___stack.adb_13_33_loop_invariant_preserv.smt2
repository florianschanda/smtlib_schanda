...
(assert (<= 1 index1))
(assert (<= index1 100))
(assert (forall ((i Int)) (=> (and (<= 1 i) (<= i (- index2 1))) (not (= (to_rep (select t i)) (- 1))))))
(assert (and (=> (<= 1 100) (in_range4 index2)) (and (<= 1 index2) (<= index2 100))))
(assert (not (= (to_rep (select t index2)) (- 1))))
(assert (= stack__size__result1 stack__size__result))
(assert (not (= index2 100)))
(assert (= result1 index2))
(assert (= index3 (+ index2 1)))
(declare-const i Int)
(assert (<= 1 i))
(assert (<= i (- index3 1)))
(assert (not (not (= (to_rep (select t i)) (- 1)))))
(check-sat)
(exit)
