...
(assert (and (and (forall ((temp___243 Int)) (=> (and (<= 0 temp___243) (<= temp___243 1001)) (=> (< j2 temp___243) (= (select tmp3 temp___243) (select tmp1 temp___243))))) (=> (<= 0 i) (dynamic_property 0 i j2))) (and (<= 0 j2) (<= j2 i))))
(assert (not (= j2 i)))
(assert (= result3 j2))
(assert (= j3 (+ j2 1)))
(assert (= (to_rep o2) (- 23)))
(assert (= o3 (store tmp3 j3 o2)))
(assert (= result4 tmp3))
(assert (= tmp4 o3))
(declare-const k Int)
(assert (<= 0 k))
(assert (<= k j3))
(assert (<= k j3))
(assert (not (= (to_rep (select tmp4 k)) (- 23))))
(check-sat)
(exit)
