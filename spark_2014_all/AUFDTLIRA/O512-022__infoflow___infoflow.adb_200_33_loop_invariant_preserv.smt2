...
(assert (<= (to_rep a_v2__first) i_v22))
(assert (<= i_v22 k_2))
(assert (forall ((m12 Int)) (=> (and (<= (to_rep a_v2__first) m12) (<= m12 (- i_v23 1))) (= (to_rep (select a_v22 m12)) (to_rep (select (elts b_v2) m12))))))
(assert (and (and (forall ((temp___450 Int)) (=> (and (<= (to_rep a_v2__first) temp___450) (<= temp___450 (to_rep a_v2__last))) (=> (<= i_v23 temp___450) (= (select a_v22 temp___450) (select a_v2 temp___450))))) (=> (<= (to_rep a_v2__first) k_2) (dynamic_property3 (to_rep a_v2__first) k_2 i_v23))) (and (<= (to_rep a_v2__first) i_v23) (<= i_v23 k_2))))
(assert (and (<= (to_rep (first (rt b_v2))) i_v23) (<= i_v23 (to_rep (last (rt b_v2))))))
(assert (= o39 i_v23))
(assert (= o40 (select (elts b_v2) o39)))
(assert (= o41 (to_rep o40)))
(assert (= (to_rep o42) o41))
(assert (and (<= (to_rep a_v2__first) i_v23) (<= i_v23 (to_rep a_v2__last))))
(assert (= o43 i_v23))
(assert (= o44 (store a_v22 o43 o42)))
(assert (= result11 (mk_map__ref a_v22)))
(assert (= a_v23 o44))
(assert (not (= i_v23 k_2)))
(assert (= result12 (mk_int__ref i_v23)))
(assert (= i_v24 (+ i_v23 1)))
(assert (= i_v25 (mk_int__ref i_v24)))
(assert (= a_v24 (mk_map__ref a_v23)))
(assert (= i_v26 i_v24))
(assert (= a_v25 a_v23))
(declare-const m12 Int)
(assert (<= (to_rep a_v2__first) m12))
(assert (<= m12 (- (int__content i_v25) 1)))
(assert (not (= (to_rep (select (map__content a_v24) m12)) (to_rep (select (elts b_v2) m12)))))
(check-sat)
(exit)
