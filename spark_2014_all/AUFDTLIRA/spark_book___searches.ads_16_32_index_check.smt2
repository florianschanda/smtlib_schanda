...
(assert (exists ((index1 Int)) (and (and (<= (to_rep (first (rt list))) index1) (<= index1 (to_rep (last (rt list))))) (= (to_rep (select (elts list) index1)) value))))
...
(assert (and (<= (to_rep (first (rt list))) position10) (<= position10 (to_rep (last (rt list))))))
...
(assert (not (<= index (to_rep (last (rt list))))))
(check-sat)
(exit)
