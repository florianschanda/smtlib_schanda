...
(assert (and (<= (to_rep a_v1__first) i_v13) (<= i_v13 (to_rep a_v1__last))))
(assert (= o13 i_v13))
(assert (= o14 (store a_v12 o13 o12)))
(assert (= a_v12 result3))
(assert (= a_v13 o14))
(assert (not (= i_v13 k_1)))
(assert (= result4 i_v13))
(assert (= i_v14 (+ i_v13 1)))
(declare-const m12 Int)
(assert (<= (to_rep a_v1__first) m12))
(assert (<= m12 (- i_v14 1)))
(assert (not (= (to_rep (select a_v13 m12)) (to_rep (select (elts b_v1) m12)))))
(check-sat)
(exit)
