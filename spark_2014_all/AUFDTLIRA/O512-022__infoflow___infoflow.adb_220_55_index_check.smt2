...
(assert (= (= result16 true) (forall ((m12 Int)) (=> (and (<= (to_rep a_v2__first) m12) (<= m12 k_2)) (= (to_rep (select a_v212 m12)) (to_rep (select (elts b_v2) m12)))))))
(assert (= result16 true))
(assert (<= (+ k_2 1) m10))
(assert (<= m10 (- i_v214 1)))
(assert (not (<= (to_rep (first (rt c_v2))) (- m10 k_2))))
(check-sat)
(exit)
