...
(assert (=> (= (select (elts arr) index2) true) (and (= o3 (+ count2 1)) (in_range1 (+ count2 1)))))
(assert (=> (= (select (elts arr) index2) true) (= count2 result2)))
(assert (=> (= (select (elts arr) index2) true) (= count3 o3)))
(assert (=> (not (= (select (elts arr) index2) true)) (= count3 count2)))
(assert (not (= index2 (to_rep (last (rt arr))))))
(assert (= result3 index2))
(assert (= index3 (+ index2 1)))
(assert (not (< count3 (+ (- index3 (to_rep (first (rt arr)))) 1))))
(check-sat)
(exit)
