...
;; range_axiom
  (assert
  (forall ((x item_t)) (! (in_range2 (to_rep1 x)) :pattern ((to_rep1 x)) )))
...
(declare-fun has_apple () Bool)

(declare-fun mixed_basket__split_discrs () natural)

(declare-fun result () Bool)

(declare-fun has_apple1 () Bool)

;; H
  (assert
  (and
  (= 0 (to_rep
       (rec__test__fruit_rec__record_t__d apple_basket__split_discrs)))
...
;; H
  (assert (= result has_apple))

;; H
  (assert (= has_apple1 (of_int 0)))

;; H
  (assert (not (= (to_rep mixed_basket__split_discrs) 1)))

;; H
  (assert (= (to_rep mixed_basket__split_discrs) 2))

(assert
;; WP_parameter_def
 ;; File "test.adb", line 71, characters 0-0
  (not (= (to_rep mixed_basket__split_discrs) 2)))
(check-sat)
(exit)
