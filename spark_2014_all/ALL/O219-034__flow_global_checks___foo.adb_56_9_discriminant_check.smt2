...
(((mk___split_discrs (rec__foo__disc_t__valid Bool)))))
...
(((mk___split_fields (rec__foo__disc_t__f natural)))))
...
(((mk_disc_t__ref (disc_t__content us_rep)))))
...
(declare-const z__split_discrs Bool)
...
(declare-const temp___162 Bool)

(declare-const temp___1621 natural)

(declare-const temp___1622 Bool)
...
;; H
  (assert (and (= o1 (+ y 1)) (in_range1 (+ y 1))))
...
;; H
  (assert (= (of_int 0) temp___162))

;; H
  (assert (= temp___1621 o3))

;; H
  (assert (= true temp___1622))

;; H
  (assert (= z__attr__constrained true))

(assert
;; WP_parameter_def
 ;; File "foo.adb", line 36, characters 0-0
  (not (= temp___162 z__split_discrs)))
(check-sat)
(exit)
