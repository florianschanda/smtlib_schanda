...
;; H
  ...
  (= (to_rep (select content i)) (get m (+ (- i first) 1)))
  (=> (and (<= 1 i) (<= i (- (- (+ first length1) 1) 100)))
  (= (to_rep (select content i)) (get m (+ (- (+ i 100) first) 1))))))))))
...
;; H
  (assert
  (<= i (ite (< length1 (+ (- 100 first) 1)) length1 (+ (- 100 first) 1))))
