...
;; H
  (assert (= z__split_fields2 y__split_fields))

;; H
  (assert (= z__split_fields3 y__split_fields1))

;; H
  (assert (= z__split_discrs y__split_discrs))

(assert
;; WP_parameter_def
 ;; File "variants.ads", line 14, characters 0-0
  (not (= 0 (to_rep (rec__variants__t__c (us_split_discrs1 x))))))
(check-sat)
(exit)
