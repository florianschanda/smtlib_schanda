...
;; H
  (assert
  (= (let ((temp___235 (remove (remove (mk___t values2 (mk 1 100)) x)
                       (- y 1))))
     (let ((temp___234 (remove (remove (mk___t init (mk 1 100)) x) (- y 1))))
     (bool_eq (elts temp___234) (to_rep1 (first (rt temp___234)))
     (to_rep1 (last (rt temp___234))) (elts temp___235)
     (to_rep1 (first (rt temp___235))) (to_rep1 (last (rt temp___235)))))) true))
...
;; H
  (assert
  (and (= o46 (perm_transitive o37 o41 o45))
...
;; H
  (assert
  (= (to_rep (select (elts (remove (mk___t init (mk 1 100)) x)) (- y 1)))
  (to_rep (select (elts (remove (mk___t values2 (mk 1 100)) y)) x))))
...
;; H
  (assert
  (and (= o48 (remove (mk___t values2 (mk 1 100)) o47))
  (and (dynamic_invariant4 o48 true false true)
  (and (= (invariant__ o48) true)
  (and (= (to_rep1 (last (rt o48))) (- (to_rep1 (last (mk 1 100))) 1))
  (and
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- o47 1)))
  (= (to_rep (select values2 k)) (to_rep (select (elts o48) k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ o47 1) k) (<= k (to_rep1 (last (mk 1 100)))))
  (= (to_rep (select values2 k)) (to_rep (select (elts o48) (- k 1))))))
  (forall ((k Int))
  (=> (and (<= o47 k) (<= k (- (to_rep1 (last (mk 1 100))) 1)))
  (= (to_rep (select values2 (+ k 1))) (to_rep (select (elts o48) k))))))))))))
