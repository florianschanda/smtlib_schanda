...
;; H
  (assert
  (and (<= 3 (to_rep1 cur_piece__split_fields))
...
;; H
  (assert
  (and (<= 1 (+ (to_rep2 cur_piece__split_fields2) x2))
  (<= (+ (to_rep2 cur_piece__split_fields2) x2) 10)))

;; H
  (assert (= o2 (+ (to_rep2 cur_piece__split_fields2) x2)))

;; H
  (assert
  (= o3 (store (select cur_board2 (+ (to_rep3 cur_piece__split_fields3)
  y2)) o2 o1)))

(assert
;; WP_parameter_def
 ;; File "tetris_functional.ads", line 170, characters 0-0
  (not (<= 1 (+ (to_rep3 cur_piece__split_fields3) y2))))
(check-sat)
(exit)
