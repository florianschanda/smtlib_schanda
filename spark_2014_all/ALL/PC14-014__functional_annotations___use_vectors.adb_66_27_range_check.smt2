...
;; H
  (assert (<= first current1))

;; H
  (assert (<= current1 r27b))

;; H
  (assert
  (forall ((i1 Int))
  (=> (and (<= first i1) (<= i1 (- current2 1)))
  (not (= (get (model__ v) i1) e)))))
...
;; H
  (assert (and (= o1 (- current2 1)) (in_range1 (- current2 1))))

;; H
  (assert (= o2 (ite (<= i o1) true false)))

;; H
  (assert (= result2 (ite (<= first i) o2 false)))
