...
;; H
  (assert
  (and (= o14 (remove (mk___t values2 (mk 1 100)) o13))
  ...
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- o13 1)))
  (= (to_rep (select values2 k)) (to_rep (select (elts o14) k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ o13 1) k) (<= k (to_rep1 (last (mk 1 100)))))
  (= (to_rep (select values2 k)) (to_rep (select (elts o14) (- k 1))))))
  (forall ((k Int))
  (=> (and (<= o13 k) (<= k (- (to_rep1 (last (mk 1 100))) 1)))
  (= (to_rep (select values2 (+ k 1))) (to_rep (select (elts o14) k))))))))))))

(assert
;; WP_parameter_def
 ;; File "sort.ads", line 4, characters 0-0
  (not (= (invariant__ o14) true)))
(check-sat)
(exit)
