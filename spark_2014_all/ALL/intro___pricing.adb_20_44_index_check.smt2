...
(declare-datatypes ((basket__ref 0))
(((mk_basket__ref (basket__content us_t)))))
...
;; H
  (assert
  (forall ((k1 Int))
  (=> (and (<= (to_rep1 (first (rt bk))) k1) (<= k1 (- it2 1)))
  (<= (price_of_item (select (elts bk) k1)) total2))))
...
;; H
  (assert (and (= o (- it2 1)) (in_range1 (- it2 1))))

;; H
  (assert (= o1 (ite (<= k o) true false)))

;; H
  (assert (= result2 (ite (<= (to_rep1 (first (rt bk))) k) o1 false)))
