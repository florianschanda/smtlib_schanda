...
;; H
  (assert (= o (select temp___1293 i3)))
...
;; H
  (assert (= o4 (select temp___1377 i9)))

;; H
  (assert (= o5 (us_split_fields1 o4)))

;; H
  (assert (= result9 (rec__tree_model__path_type__k o5)))
...
;; H
  (assert (= o2 (select temp___1378 j12)))

;; H
  (assert (= o3 (us_split_fields1 o2)))

;; H
  (assert (= result8 (rec__tree_model__path_type__k o3)))
...
;; H
  ...
  (= (olt
     ...
     (rec__tree_model__path_type__a
     (us_split_fields1 (select (model__ f_old root) j12)))) true)))
...
;; H
  ...
  (ite (= (get
          (rec__tree_model__path_type__a
          (us_split_fields1 (select (model__ f v) j12)))
          (+ (length
             (rec__tree_model__path_type__a
             (us_split_fields1 (select (model__ f v) i9)))) 1)) 0)
  (< (to_rep (select values j12)) (to_rep (select values i9)))
  (< (to_rep (select values i9)) (to_rep (select values j12))))))
...
;; H
  (assert
  (and (=> (<= 1 100) (in_range3 j13)) (and (<= 1 j13) (<= j13 100))))
...
;; WP_parameter_def
 ;; File "tree_model.ads", line 12, characters 0-0
  (not (< (to_rep (select values j26)) (to_rep (select values i18)))))
(check-sat)
(exit)
