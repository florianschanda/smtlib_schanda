...
;; H
  (assert
  (= a1 (store a i1 (store (select a i1) j1 (store (select (select a 
  i1) j1) k1 b)))))
...
;; H
  (assert
  (= a2 (store a1 i2 (store (select a1 i2) j2 (store (select (select 
  a1 i2) j2) k2 c)))))
...
(assert
;; WP_parameter_def
 ;; File "ar.ads", line 676, characters 0-0
  (not
  (= (bool_eq5 (select (select (select a3 m) n) o) 1 10
     (select (select (select a m) n) o) 1 10) true)))
(check-sat)
