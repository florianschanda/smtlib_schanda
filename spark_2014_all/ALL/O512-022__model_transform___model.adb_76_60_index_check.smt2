...
(declare-fun temp___450 () (Array Int us_rep))

(declare-fun temp___451 () (Array Int us_rep1))
...
(define-fun o11 () us_rep (mk___rep (mk___split_fields o)))
...
;; H
  ...
  (forall ((t1 Int))
  (=> (and (<= 0 t1) (<= t1 (- s3 1)))
  (= (name (select (owned_state sm) t1)) (name__2 (select av2 t1)))))))
...
;; H
  (assert (=> (and (<= 0 s2) (<= s2 100)) (= o11 (select temp___434 s3))))

;; H
  (assert
  (=> (and (<= 0 s2) (<= s2 100))
  (and (= o1 (name o11)) (and (in_range1 o1) (= o1 (to_rep o))))))
...
;; H
  ...
  (and
  (= (name
     (select (owned_state sm) (from (select (owned_transition sm) t1))))
  (name__2 (select (map__content2 av6) (from__2 (select cfv2 t1)))))
  (= (name
     (select (owned_state sm) (to__ (select (owned_transition sm) t1))))
  (name__2 (select (map__content2 av6) (to__2 (select cfv2 t1)))))))))
...
;; H
  ...
  (and (in_range4 o8)
  (= o8 (to_rep2
        (rec__model__uml_control_flow__to_data (us_split_fields9 o7)))))))

;; H
  (assert (= o9 (select (map__content2 av6) o8)))

;; H
  (assert
  (and (= o10 (name__2 o9))
  (and (in_range1 o10)
  (= o10 (to_rep (rec__model__uml_action__name_data (us_split_fields7 o9)))))))

;; H
  (assert
  (and (= temp___450 (owned_state sm))
  (= temp___450 (rec__model__uml_state_machine__owned_state_data
                (us_split_fields5 sm)))))

;; H
  (assert
  (and (= temp___451 (owned_transition sm))
  (= temp___451 (rec__model__uml_state_machine__owned_transition_data
                (us_split_fields5 sm)))))
