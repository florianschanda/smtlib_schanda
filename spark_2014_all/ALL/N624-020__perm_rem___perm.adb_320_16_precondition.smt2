...
;; H
  (assert
  (and (= o4 (remove c ic))
  (and (dynamic_invariant2 o4 true false true)
  (and (= (invariant__ o4) true)
  (and (= (to_rep1 (last o3)) (- (to_rep1 (last (rt c))) 1))
  (and
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- ic 1)))
  (= (to_rep2 (select (elts c) k)) (to_rep2 (select o2 k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ ic 1) k) (<= k (to_rep1 (last (rt c)))))
  (= (to_rep2 (select (elts c) k)) (to_rep2 (select o2 (- k 1))))))
  (forall ((k Int))
  (=> (and (<= ic k) (<= k (- (to_rep1 (last (rt c))) 1)))
  (= (to_rep2 (select (elts c) (+ k 1))) (to_rep2 (select o2 k))))))))))))

;; H
  (assert
  (and
  (= o5 (remove b (to_rep (rec__perm__witnesses__ib (us_split_fields1 w)))))
  (and (dynamic_invariant2 o5 true false true)
  (and (= (invariant__ o5) true)
  (and (= (to_rep1 (last o1)) (- (to_rep1 (last (rt b))) 1))
  (and
  (forall ((k Int))
  (=>
  (and (<= 1 k)
  (<= k (- (to_rep (rec__perm__witnesses__ib (us_split_fields1 w))) 1)))
  (= (to_rep2 (select (elts b) k)) (to_rep2 (select o k)))))
  (and
  (forall ((k Int))
  (=>
  (and (<= (+ (to_rep (rec__perm__witnesses__ib (us_split_fields1 w))) 1) k)
  (<= k (to_rep1 (last (rt b)))))
  (= (to_rep2 (select (elts b) k)) (to_rep2 (select o (- k 1))))))
  (forall ((k Int))
  (=>
  (and (<= (to_rep (rec__perm__witnesses__ib (us_split_fields1 w))) k)
  (<= k (- (to_rep1 (last (rt b))) 1)))
  (= (to_rep2 (select (elts b) (+ k 1))) (to_rep2 (select o k))))))))))))
