...
(define-fun dynamic_invariant2 ((temp___expr_516 us_t)
...
(declare-fun x4 () (Array Int integer))
...
;; H
  (assert
  (and (and (<= (to_rep x__first) a) (<= a (to_rep x__last)))
  (and (<= (to_rep x__first) b) (<= b (to_rep x__last)))))

;; H
  (assert (and (<= (to_rep x__first) a) (<= a (to_rep x__last))))
...
;; H
  (assert (and (<= (to_rep x__first) b) (<= b (to_rep x__last))))

;; H
  (assert (= o3 b))

;; H
  (assert (= o4 (select x o3)))
...
(assert
;; WP_parameter_def
 ;; File "part.ads", line 43, characters 0-0
  (not (<= (to_rep x__first) a)))
(check-sat)
(exit)
