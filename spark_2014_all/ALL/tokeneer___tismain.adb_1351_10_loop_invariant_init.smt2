...
;; H
  ...
  (and
  (and
  (= (not (= (enrolmentisinprogress (let ((subject state)) state4)) true))
  (= (privatekeypresent state1) true))
  (or (= (enrolmentisinprogress (let ((subject state)) state4)) true)
  (= (statusisenclavequiescent (let ((subject state)) state4)) true)))
  (not
  (= (ispresent
     (mk___rep
  ...
  (not (= (statusiswaitingstartadminop (let ((subject state)) state4)) true)))
  (not
  (= (statusiswaitingfinishadminop (let ((subject state)) state4)) true)))
  (not (= (statusisshutdown (let ((subject state)) state4)) true)))
  (not (= (isgood state2) true))) (not (= (authcertvalid state2) true)))
  (not (= (theauthcertrole state2) 1))))

;; H
  (assert (= (islocked state3) true))
