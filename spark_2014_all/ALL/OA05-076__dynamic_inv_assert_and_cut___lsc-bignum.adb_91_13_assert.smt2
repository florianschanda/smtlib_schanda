...
;; H
  ...
  (and
  (=> (and (<= b_first k) (<= k (- (- (+ b_first i2) a_first) 1)))
  (= (to_rep1 (select b1 k)) (to_rep1
                             (select (elts a) (- (+ a_first k) b_first)))))
  (=> (=> (<= b_first k) (not (<= k (- (- (+ b_first i2) a_first) 1))))
  (= (to_rep1 (select b1 k)) (to_rep1 (select b k))))))))
...
;; H
  (assert
  (and (<= (to_rep b__first) temp___556) (<= temp___556 (to_rep b__last))))
