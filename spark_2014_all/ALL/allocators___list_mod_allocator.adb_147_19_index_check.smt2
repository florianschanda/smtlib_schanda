...
;; model_is_well_formed__def_axiom
  ...
  (! (=
     ...
     (= j k)))))))
     (=>
     (and (not (= list_mod_allocator__first_available 0)) (in_range2
     (to_rep
     (rec__list_mod_allocator__cell__next
     (us_split_fields1
     (select list_mod_allocator__data (get
...
;; contains__post_axiom
  ...
  (! (=> (dynamic_invariant6 item true true true)
     ...
     (=>
     (exists ((e Int))
     (and (and (in_range7 e) (= (contains1 container e) true)) (= e item)))
...
;; ole__post_axiom
  ...
  (! (= (= (ole left right) true)
     (forall ((item Int))
     (=> (and (in_range7 item) (= (contains1 left item) true))
...
;; is_empty__post_axiom
  ...
  (! (let ((result (is_empty container)))
     ...
     (= (= result true)
     (forall ((item Int))
     (not (and (in_range7 item) (= (contains1 container item) true)))))
...
;; H
  ...
  (= (to_rep1
     (rec__list_mod_allocator__cell__stat
     (us_split_fields1 (select data first_available)))) 0))
  (forall ((r Int))
  (=> (and (<= 1 r) (<= r 10000))
  (=>
  (and
  ...
  (= (to_rep1
     ...
     (select data (to_rep
                  (rec__list_mod_allocator__cell__next
                  (us_split_fields1 (select data r)))))))) 0))))))))
...
;; H
  (assert (and (= temp___301 (get o4 o10)) (in_range5 temp___301)))

(assert
;; WP_parameter_def
 ;; File "list_mod_allocator.ads", line 77, characters 0-0
  (not (<= 1 temp___301)))
(check-sat)
(exit)
