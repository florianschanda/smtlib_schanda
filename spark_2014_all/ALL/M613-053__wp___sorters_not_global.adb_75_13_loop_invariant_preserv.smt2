...
;; H
  (assert
  (and (= o1 (index_of_minimum current3 limit values3))
  (and (in_range2 o1)
  (and (and (<= current3 o1) (<= o1 limit))
  (forall ((j Int))
  (=> (and (<= current3 j) (<= j limit))
  (<= (to_rep (select values3 o1)) (to_rep (select values3 j)))))))))
...
;; H
  ...
  (and
  (and
  (= (to_rep (select values4 current3)) (to_rep (select values3 smallest3)))
  (= (to_rep (select values4 smallest3)) (to_rep (select values3 current3))))
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 64))
  (=> (and (not (= j current3)) (not (= j smallest3)))
