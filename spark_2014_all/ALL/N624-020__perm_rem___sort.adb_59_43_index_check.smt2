...
;; H
  (assert
  (and (= temp___266 (remove (mk___t values2 (mk 1 100)) o48))
...
;; H
  (assert (= o50 (select (elts temp___266) o49)))
...
;; H
  (assert
  (and (= temp___265 (remove (mk___t init (mk 1 100)) o47))
  (and (dynamic_invariant4 temp___265 true false true)
  (and (= (invariant__ temp___265) true)
  (and (= (to_rep1 (last (rt temp___265))) (- (to_rep1 (last (mk 1 100))) 1))
  (and
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- o47 1)))
  (= (to_rep (select init k)) (to_rep (select (elts temp___265) k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ o47 1) k) (<= k (to_rep1 (last (mk 1 100)))))
  (= (to_rep (select init k)) (to_rep (select (elts temp___265) (- k 1))))))
  (forall ((k Int))
  (=> (and (<= o47 k) (<= k (- (to_rep1 (last (mk 1 100))) 1)))
  (= (to_rep (select init (+ k 1))) (to_rep (select (elts temp___265) k))))))))))))

(assert
;; WP_parameter_def
 ;; File "sort.ads", line 4, characters 0-0
  (not (<= (to_rep1 (first (rt temp___265))) y)))
(check-sat)
(exit)
