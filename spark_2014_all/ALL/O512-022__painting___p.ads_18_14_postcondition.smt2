...
;; H
  ...
  (exists ((c9 Int))
  (and (and (<= 0 c9) (<= c9 2))
  (= (bool_eq (select p__split_fields c9) d) true))))
...
;; H
  (assert
  (=> (and (<= 0 c1) (<= c1 2))
  (and (= (bool_eq1 p__split_fields4 0 2 temp___210 0 2) true)
  (forall ((j Int))
  (=> (and (<= 0 j) (<= j (- c2 1)))
  (=> (= (bool_eq (select p__split_fields4 j) d) true)
  (= (bool_eq (select p__split_fields5 j) d) true)))))))
...
;; H
  (assert
  (=> (and (<= 0 c1) (<= c1 2))
  (=> (= (bool_eq (select p__split_fields4 c2) d) true)
...
;; H
  (assert
  (=> (and (<= 0 c1) (<= c1 2))
  (=> (not (= (bool_eq (select p__split_fields4 c2) d) true))
