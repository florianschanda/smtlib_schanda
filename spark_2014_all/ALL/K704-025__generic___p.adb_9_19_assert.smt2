...
;; echo__post_axiom
  (assert
  (forall ((x Int))
  (! (=> (dynamic_invariant1 x true true true) (dynamic_invariant1 (echo1 x)
     true false true)) :pattern ((echo1 x)) )))

;; echo__def_axiom
  (assert
  (forall ((x Int))
  (! (=> (dynamic_invariant1 x true true true) (= (echo1 x) x)) :pattern (
  (echo1 x)) )))
...
(assert
;; WP_parameter_def
 ;; File "gen.ads", line 5, characters 0-0
  (not (= x1 3)))
(check-sat)
(exit)
