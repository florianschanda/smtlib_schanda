...
;; H
  ...
  (ite (< (to_rep
          (select (elts b) (+ (- k (to_rep1 a__first)) (to_rep1
                                                       (first (rt b))))))
  (to_rep (select a k))) (= (to_rep (select a1 k)) (to_rep (select a k)))
...
;; H
  (assert (= result2 (ite (< (to_rep (select a1 i2)) o2) true false)))
...
;; H
  (assert (=> (= result2 true) (= o3 j2)))

;; H
  (assert (=> (= result2 true) (= o4 (select (elts b) o3))))

;; H
  (assert (=> (= result2 true) (= o5 (to_rep o4))))

;; H
  (assert (=> (= result2 true) (= (to_rep o6) o5)))

;; H
  (assert (=> (= result2 true) (= o7 (store a1 i2 o6))))

;; H
  (assert (=> (= result2 true) (= a1 result3)))

;; H
  (assert (=> (= result2 true) (= a2 o7)))

;; H
  (assert (=> (not (= result2 true)) (= a2 a1)))

;; H
  (assert (and (= o8 (+ j2 1)) (in_range1 (+ j2 1))))
...
;; WP_parameter_def
 ;; File "p_max_array.ads", line 3, characters 0-0
  (not (= (to_rep (select a2 k)) (to_rep (select a k)))))
(check-sat)
(exit)
