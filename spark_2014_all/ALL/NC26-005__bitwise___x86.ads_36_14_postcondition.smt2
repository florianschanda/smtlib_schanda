...
;; H
  (assert
  (and
  (= (readmem8 addr memory1) ((_ extract 7 0) (bvand val__ ((_ int2bv 16) 255))))
  (forall ((x (_ BitVec 64)))
  (=>
  (and (bvule ((_ int2bv 64) 0) x)
  (bvule x ((_ int2bv 64) 18446744073709551615)))
  (or (= x addr) (= (to_rep (select memory1 x)) (to_rep (select memory x))))))))
...
;; H
  (assert
  (and (= (readmem8 (bvadd addr ((_ int2bv 64) 1)) memory2) o3)
  (forall ((x (_ BitVec 64)))
  (=>
  (and (bvule ((_ int2bv 64) 0) x)
  (bvule x ((_ int2bv 64) 18446744073709551615)))
  (or (= x (bvadd addr ((_ int2bv 64) 1)))
  (= (to_rep (select memory2 x)) (to_rep (select memory1 x))))))))

;; H
  (assert (= memory2 memory3))
...
;; WP_parameter_def
 ;; File "x86.ads", line 9, characters 0-0
  (not (= (readmem16 addr memory3) val__)))
(check-sat)
(exit)
