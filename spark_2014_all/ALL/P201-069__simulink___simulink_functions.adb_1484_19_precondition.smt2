...
(declare-fun oexpon (t t) t)

(declare-fun oexpon__function_guard (t t t) Bool)
...
;; oexpon__post_axiom
  ...
  (! (=>
     ...
     true true true))
     (and (=> (eq left fliteral2) (lt fliteral2 right)) (le fliteral2 left)))
     (let ((result (oexpon left right)))
     (and
     (and (le fliteral2 result)
     (and (=> (eq right fliteral2) (eq result fliteral))
     (and (=> (eq right fliteral) (eq result left))
     (and (=> (eq left fliteral) (eq result fliteral))
     (=> (eq left fliteral2) (eq result fliteral2)))))) (dynamic_invariant
     result true false true)))) :pattern ((oexpon left right)) )))
...
;; H
  (assert (eq left fliteral2))

(assert
;; WP_parameter_def
 ;; File "system.ads", line 1, characters 0-0
  (not (lt fliteral2 right)))
(check-sat)
