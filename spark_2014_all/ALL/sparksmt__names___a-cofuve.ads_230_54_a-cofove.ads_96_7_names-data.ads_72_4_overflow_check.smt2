...
;; range_axiom
  (assert
  (forall ((x positive)) (! (in_range4 (to_rep x)) :pattern ((to_rep x)) )))
...
;; range_axiom
  (assert
  (forall ((x name_id)) (! (in_range5 (to_rep1 x)) :pattern ((to_rep1 x)) )))
...
(declare-fun dummy6 () char_table_index)
...
(define-fun us_split_fields_Table_Index__projection ((a us_split_fields)) char_table_index
...
(define-fun us_split_fields_Next_Hash__projection ((a us_split_fields)) name_id
...
(declare-datatypes ()
((name_entry__ref (mk_name_entry__ref (name_entry__content us_rep1)))))
...
(define-fun dynamic_invariant5 ((temp___expr_148 Int)
...
;; H
  (assert
  (and (= o (last left)) (and (in_range3 o) (= o (+ 0 (length left))))))
...
;; WP_parameter_def
 ;; File "a-cofuve.ads", line 201, characters 0-0
  (not (in_range1 (- (- 2147483648) offset))))
(check-sat)
(exit)
