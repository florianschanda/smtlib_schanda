...
(define-fun in_range ((x Int)) Bool (and (<= 0 x) (<= x 16)))
...
;; to_int_def
  ...
  (forall ((x Int))
  (! (ite (= x 0) (= (to_int1 x) 0)
     (ite (< 0 x) (pos_div_relation (to_int1 x) x 16) (pos_div_relation
     (- (to_int1 x)) (- x) 16))) :pattern ((to_int1 x)) )))

(declare-fun of_real (Real) Int)

;; inversion_axiom
  (assert
  (forall ((x t)) (! (= (of_fixed (to_fixed x)) x) :pattern ((to_fixed x)) )))

;; range_axiom
  (assert (forall ((x t)) (in_range (to_fixed x))))
...
;; WP_parameter_def
 ;; File "fixed.adb", line 1, characters 0-0
  (not (in_range
  (ite (< (to_fixed x1) (ite (< (to_fixed x1) (to_fixed x1)) (to_fixed x1)
  (to_fixed x1))) (to_fixed x1) (ite (< (to_fixed x1) (to_fixed x1))
  (to_fixed x1) (to_fixed x1))))))
(check-sat)
(exit)
