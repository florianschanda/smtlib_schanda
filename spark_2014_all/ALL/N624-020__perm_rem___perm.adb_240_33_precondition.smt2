...
;; H
  ...
  (and (= (to_rep2 (last (rt o24))) (- (to_rep2 (last (rt o23))) 1))
  (and
  (forall ((k Int))
  (=>
  (and (<= 1 k)
  (<= k (- (to_rep1 (rec__perm__witnesses__ib (us_split_fields1 w))) 1)))
  (= (to_rep (select (elts o23) k)) (to_rep (select (elts o24) k)))))
  (and
  (forall ((k Int))
  (=>
  (and (<= (+ (to_rep1 (rec__perm__witnesses__ib (us_split_fields1 w))) 1) k)
  (<= k (to_rep2 (last (rt o23)))))
  (= (to_rep (select (elts o23) k)) (to_rep (select (elts o24) (- k 1))))))
  (forall ((k Int))
  (=>
  (and (<= (to_rep1 (rec__perm__witnesses__ib (us_split_fields1 w))) k)
  (<= k (- (to_rep2 (last (rt o23))) 1)))
  (= (to_rep (select (elts o23) (+ k 1))) (to_rep (select (elts o24) k))))))))))))

;; H
  (assert (and (= o21 (- ib2 1)) (in_range4 (- ib2 1))))

;; H
  (assert
  (and (= o22 (remove bb o21))
  (and (dynamic_invariant2 o22 true false true)
  (and (= (invariant__ o22) true)
  (and (= (to_rep2 (last (rt o22))) (- (to_rep2 (last (rt bb))) 1))
  (and
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- o21 1)))
  (= (to_rep (select (elts bb) k)) (to_rep (select (elts o22) k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ o21 1) k) (<= k (to_rep2 (last (rt bb)))))
  (= (to_rep (select (elts bb) k)) (to_rep (select (elts o22) (- k 1))))))
  (forall ((k Int))
  (=> (and (<= o21 k) (<= k (- (to_rep2 (last (rt bb))) 1)))
  (= (to_rep (select (elts bb) (+ k 1))) (to_rep (select (elts o22) k))))))))))))

;; H
  (assert
  (and
  (= o20 (- (to_rep1 (rec__perm__witnesses__ia (us_split_fields1 w))) 1))
  (in_range4
  (- (to_rep1 (rec__perm__witnesses__ia (us_split_fields1 w))) 1))))
