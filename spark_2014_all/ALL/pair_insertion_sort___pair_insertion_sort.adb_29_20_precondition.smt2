...
;; H
  ...
  (= (to_rep (last (mk (to_rep interm__first) (to_rep interm__last))))
  (to_rep
  (last (mk (to_rep (first2 (rt1 init))) (to_rep (last2 (rt1 init))))))))
  (= (to_rep (select interm x)) o5))
  (forall ((j Int))
  (=>
  (and
  (<= (to_rep
      (first (mk (to_rep (first2 (rt1 init))) (to_rep (last2 (rt1 init)))))) j)
  (<= j (to_rep
        (last (mk (to_rep (first2 (rt1 init))) (to_rep (last2 (rt1 init))))))))
  (=> (not (= x j))
