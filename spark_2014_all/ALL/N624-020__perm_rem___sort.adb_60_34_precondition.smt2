...
;; H
  (assert
  (and (= o49 (remove (mk___t values2 (mk 1 100)) o48))
  (and (dynamic_invariant4 o49 true false true)
  (and (= (invariant__ o49) true)
  (and (= (to_rep1 (last (rt o49))) (- (to_rep1 (last (mk 1 100))) 1))
  (and
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- o48 1)))
  (= (to_rep (select values2 k)) (to_rep (select (elts o49) k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ o48 1) k) (<= k (to_rep1 (last (mk 1 100)))))
  (= (to_rep (select values2 k)) (to_rep (select (elts o49) (- k 1))))))
  (forall ((k Int))
  (=> (and (<= o48 k) (<= k (- (to_rep1 (last (mk 1 100))) 1)))
  (= (to_rep (select values2 (+ k 1))) (to_rep (select (elts o49) k))))))))))))
...
;; WP_parameter_def
 ;; File "sort.ads", line 4, characters 0-0
  (not (= (invariant__ (mk___t init (mk 1 100))) true)))
(check-sat)
(exit)
