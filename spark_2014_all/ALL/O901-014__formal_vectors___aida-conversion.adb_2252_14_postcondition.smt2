...
;; H
  ...
  (<= index (+ (to_rep1 (first (rt source))) 1)))
  (= (is_digit (to_rep (select (elts source) index))) true))))))

;; H
  (assert
  (and (= temp___2439 (+ (to_rep1 (first (rt source))) 1)) (in_range1
  (+ (to_rep1 (first (rt source))) 1))))

;; H
  (assert
  (and (<= (to_rep1 (first (rt source))) temp___2439)
  (<= temp___2439 (to_rep1 (last (rt source))))))
