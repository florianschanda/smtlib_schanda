...
;; H
  (assert
  (and (<= 1 (to_rep2 usedlogfiles__split_fields))
  (= numberlogentries (+ (* (- (to_rep2 usedlogfiles__split_fields) 1) 1024)
  (to_rep1 (select logfileentries currentlogfile))))))

;; H
  (assert (= (to_rep1 (select logfileentries currentlogfile)) 1024))
...
;; WP_parameter_def
 ;; File "auditlog.adb", line 70, characters 0-0
  (not (<= 1 (to_rep2 usedlogfiles__split_fields))))
(check-sat)
(exit)
