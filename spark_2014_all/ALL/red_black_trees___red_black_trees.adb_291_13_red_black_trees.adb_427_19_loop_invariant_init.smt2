...
;; H
  (assert (= (mk___rep4 t_old__split_fields7) c80b))

;; H
  (assert (dynamic_invariant6 c80b true false true))

;; H
  (assert (= (mk___rep4 t__split_fields8) c81b))
...
;; H
  (assert (= red_black_trees__insert__L_8__C82b__assume c82b))

;; H
  (assert (in_range2 c82b))
...
;; H
  (assert (= red_black_trees__insert__L_8__B83b__y__assume y))
...
;; H
  ...
  (< (to_rep
     (rec__red_black_trees__black_count__depth
     (us_split_fields11 (select (nb_blacks c80b) c82b)))) n5))

(assert
;; WP_parameter_def
 ;; File "red_black_trees.ads", line 59, characters 0-0
  (not
  (= (eq (select (nb_blacks c81b) y) (select (nb_blacks c80b) y)) true)))
(check-sat)
