...
;; H
  ...
  (mul RNE1 (mul RNE1 fliteral4 (of_int RNE1 (+ nb_of_fp nb_of_pp)))
  delta_time) time)) (lt time
  (sub RNE1 fliteral1
  (mul RNE1 (of_int RNE1 (+ nb_of_fp nb_of_pp)) delta_time)))))

;; H
  (assert (=> (le fliteral2 fliteral3) (in_range3 d)))

;; H
  (assert (=> (le (neg fliteral1) fliteral1) (tqtisFinite t_fp)))
...
;; H
  (assert
  (and (= o (sub RNE1 time (div1 RNE1 d1 fliteral5))) (tqtisFinite
  (sub RNE1 time (div1 RNE1 d1 fliteral5)))))
...
;; H
  (assert
  (and (= o1 (add RNE1 t_fp1 (mul RNE1 (of_int RNE1 nb_of_fp) delta_time)))
  (tqtisFinite
  (add RNE1 t_fp1 (mul RNE1 (of_int RNE1 nb_of_fp) delta_time)))))
...
;; WP_parameter_def
 ;; File "original_sample.ads", line 15, characters 0-0
  (not (tqtisFinite
  (add RNE1 t_pp1
  (mul RNE1 (mul RNE1 fliteral4 (of_int RNE1 nb_of_fp)) delta_time)))))
(check-sat)
