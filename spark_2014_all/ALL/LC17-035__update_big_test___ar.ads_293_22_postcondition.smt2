...
(declare-fun o5 () (Array Int (Array Int et1)))
...
;; H
  (assert (and (not (= i1 i2)) (not (= j1 j2))))
...
;; H
  (assert (or (not (= m j1)) (not (= n i1))))

;; H
  (assert (or (not (= m j2)) (not (= n i2))))

(assert
;; WP_parameter_def
 ;; File "ar.ads", line 287, characters 0-0
  (not
  (= (to_rep (select (select a3 m) n)) (to_rep (select (select a m) n)))))
(check-sat)
