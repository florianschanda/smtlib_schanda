...
;; H
  ...
  (forall ((k1 Int))
  (=> (and (<= 1 k1) (<= k1 j2))
  (not (= c1 (to_rep1 (select (elts needle) k1))))))
  (and
  (= (to_rep1
  ...
  (forall ((k1 Int))
  (=>
  (and (<= (+ (- (length needle) (to_rep (select bad_shift3 c1))) 2) k1)
  (<= k1 j2)) (not (= (to_rep1 (select (elts needle) k1)) c1)))))))))
...
;; H
  (assert
  (= result3 (ite (= (to_rep (select bad_shift3 c)) o14) true false)))
...
;; H
  ...
  (forall ((k1 Int))
  (=>
  (and (<= (+ (- (length needle) (to_rep (select bad_shift3 c))) 2) k1)
  (<= k1 j2)) (not (= (to_rep1 (select (elts needle) k1)) c))))))
...
;; WP_parameter_def
 ;; File "system.ads", line 1, characters 0-0
  (not (<= (to_rep2 (first (rt needle))) temp___290)))
(check-sat)
(exit)
