...
;; H
  (assert
  (=> (= (to_rep (select buffer index2)) ch)
  (and (= o (+ count2 1)) (in_range2 (+ count2 1)))))

;; H
  (assert (=> (= (to_rep (select buffer index2)) ch) (= result2 count2)))

;; H
  (assert (=> (= (to_rep (select buffer index2)) ch) (= count3 o)))

;; H
  (assert
  (=> (not (= (to_rep (select buffer index2)) ch)) (= count3 count2)))
