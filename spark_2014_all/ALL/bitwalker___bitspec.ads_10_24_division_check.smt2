...
;; H
  (assert (dynamic_invariant1 stream true false true))
...
;; H
  (assert
  (and (= (to_rep1 (first (rt stream))) 0)
  (<= (div1 pos 8) (to_rep1 (last (rt stream))))))

(assert
;; WP_parameter_def
 ;; File "bitspec.ads", line 9, characters 0-0
  (not (not (= 8 0))))
(check-sat)
(exit)
