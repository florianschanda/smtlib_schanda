...
;; H
  ...
  (=> (and (<= (to_rep1 a__first) i1) (<= i1 (to_rep1 a__last)))
  (forall ((k Int))
  (=> (and (<= (to_rep1 a__first) k) (<= k (- i2 1)))
  (ite (< (to_rep
          (select (elts b) (+ (- k (to_rep1 a__first)) (to_rep1
                                                       (first (rt b))))))
  (to_rep (select temp___281 k)))
  (= (to_rep (select a2 k)) (to_rep (select temp___279 k)))
  (= (to_rep (select a2 k)) (to_rep
                            (select (elts b) (+ (- k (to_rep1 a__first))
                            (to_rep1 (first (rt b))))))))))))
...
;; H
  ...
  (=> (and (<= (to_rep1 a__first) i1) (<= i1 (to_rep1 a__last)))
  (forall ((k Int))
  (=> (and (<= i2 k) (<= k (to_rep1 a__last)))
  (= (to_rep (select a2 k)) (to_rep (select temp___283 k)))))))
...
;; H
  (assert
  (< (to_rep
     (select (elts b) (+ (- k (to_rep1 a__first)) (to_rep1 (first (rt b))))))
  (to_rep (select a k))))

(assert
;; WP_parameter_def
 ;; File "p_max_array.ads", line 3, characters 0-0
  (not (= (to_rep (select a8 k)) (to_rep (select a k)))))
(check-sat)
(exit)
