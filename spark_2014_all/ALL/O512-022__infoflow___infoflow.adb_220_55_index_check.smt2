...
;; H
  ...
  (= (to_rep (select (map__content a_v1) m1)) (to_rep
                                              ...
                                              k_1)))))))
  (forall ((m1 Int))
  (=> (and (<= (to_rep a_v2__first) m1) (<= m1 k_2))
  (= (to_rep (select (map__content a_v2) m1)) (to_rep
                                              (select (elts b_v2) m1)))))))
...
;; H
  ...
  (forall ((m1 Int))
  (=> (and (<= (to_rep a_v2__first) m1) (<= m1 k_2))
  (= (to_rep (select a_v21 m1)) (to_rep (select (elts b_v2) m1)))))
  (forall ((m1 Int))
  (=> (and (<= (+ k_2 1) m1) (<= m1 (- i_v22 1)))
  (= (to_rep (select a_v21 m1)) (to_rep (select (elts c_v2) (- m1 k_2))))))))
...
;; WP_parameter_def
 ;; File "infoflow.ads", line 89, characters 0-0
  (not (<= (to_rep (first (rt c_v2))) (- m k_2))))
(check-sat)
(exit)
