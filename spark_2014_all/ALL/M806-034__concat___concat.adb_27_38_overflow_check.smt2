...
(declare-fun d__first () integer)

(declare-fun d__last () integer)
...
(declare-fun concat__three__d__assume () (Array Int integer))

(declare-fun concat__three__d__assume1 () t)
...
;; H
  ...
  (and (<= (to_rep (first (rt b))) (to_rep (last (rt b))))
  (< (+ (to_rep (last (rt a))) (- (to_rep (last (rt b))) (to_rep
                                                         (first (rt b))))) 2147483647))))
...
;; H
  (assert
  (=> (= (length a) 0)
  (= (mk (to_rep (first (rt b))) (to_rep (last (rt b)))) concat__three__d__assume1)))
...
;; H
  (assert
  (=> (not (= (length a) 0))
  (= (concat1 (elts a) (to_rep (first (rt a))) (to_rep (last (rt a)))
     (elts b) (to_rep (first (rt b))) (to_rep (last (rt b)))) concat__three__d__assume)))

;; H
  (assert
  (=> (not (= (length a) 0))
  (= (mk (to_rep (first (rt a))) o5) concat__three__d__assume1)))
...
;; H
  (assert (dynamic_property (- 2147483648) 2147483647 (to_rep d__first)
  (to_rep d__last)))
...
;; H
  (assert
  (= (ite (<= (to_rep d__first) (to_rep d__last))
     (+ (- (to_rep d__last) (to_rep d__first)) 1) 0) (+ (length a) (length b))))

;; H
  (assert
  (and (= o6 (- (to_rep (last (rt b))) (to_rep (first (rt b))))) (in_range
  (- (to_rep (last (rt b))) (to_rep (first (rt b)))))))

(assert
;; WP_parameter_def
 ;; File "concat.adb", line 33, characters 0-0
  (not (in_range (+ (to_rep (last (rt a))) 1))))
(check-sat)
