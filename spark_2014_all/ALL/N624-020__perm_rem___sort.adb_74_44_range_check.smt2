...
;; H
  (assert
  (and (= o26 (remove (mk___t interm1 (mk 1 100)) o25))
  ...
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- o25 1)))
  (= (to_rep (select interm1 k)) (to_rep (select (elts o26) k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ o25 1) k) (<= k (to_rep1 (last (mk 1 100)))))
  (= (to_rep (select interm1 k)) (to_rep (select (elts o26) (- k 1))))))
  (forall ((k Int))
  (=> (and (<= o25 k) (<= k (- (to_rep1 (last (mk 1 100))) 1)))
  (= (to_rep (select interm1 (+ k 1))) (to_rep (select (elts o26) k))))))))))))
