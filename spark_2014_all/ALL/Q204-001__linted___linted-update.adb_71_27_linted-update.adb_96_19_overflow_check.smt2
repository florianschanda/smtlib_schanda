...
(define-fun y6 () t__ref1 (mk_t__ref1 y1))
...
;; H
  (assert (=> (not (< c206b 0)) (= y3 o5)))

;; H
  (assert (=> (not (< c206b 0)) (= y2 y3)))
...
;; H
  (assert (=> (< c216b 0) (= result3 y6)))

;; H
  (assert (=> (< c216b 0) (= y4 o9)))
...
;; H
  (assert (=> (not (< c216b 0)) (= result4 y6)))

;; H
  (assert (=> (not (< c216b 0)) (= y5 o11)))

;; H
  (assert (=> (not (< c216b 0)) (= y4 y5)))
...
;; WP_parameter_def
 ;; File "linted-update.ads", line 41, characters 0-0
  (not (in_range2 (- c226b (- 2147483648)))))
(check-sat)
(exit)
