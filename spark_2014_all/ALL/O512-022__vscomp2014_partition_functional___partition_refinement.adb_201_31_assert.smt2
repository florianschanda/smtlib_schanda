...
;; H
  ...
  (<= (to_rep1
      (rec__partition_refinement__interval__first
      (us_split_fields1
      (get (model__ (mk___rep1 p__split_discrs p__split_fields3)) k)))) l)
  (<= l (to_rep1
        (rec__partition_refinement__interval__last
        (us_split_fields1
        (get (model__ (mk___rep1 p__split_discrs p__split_fields3)) k))))))
  (= (to_rep2 (select (map__content f6) l)) k))))))
...
;; H
  (assert (<= k (- (length (mk___rep1 p__split_discrs p__split_fields3)) 1)))
...
;; WP_parameter_def
 ;; File "partition_refinement.adb", line 18, characters 0-0
  (not (= (to_rep2 (select (map__content f6) l)) k)))
(check-sat)
