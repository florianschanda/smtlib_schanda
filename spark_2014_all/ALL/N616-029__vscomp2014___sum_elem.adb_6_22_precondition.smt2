...
;; H
  ...
  (forall ((j Int))
  (=> (and (<= 0 j) (<= j (last (model__ p)))) (in_range1
  (+ (to_rep
     (rec__sum_elem__interval__first (us_split_fields1 (get (model__ p) j))))
  (to_rep
  (rec__sum_elem__interval__last (us_split_fields1 (get (model__ p) j))))))))))

;; H
  (assert
  (and (= o6 (get (model__ p) x)) (= (bool_eq o6 (get (model__ p) x)) true)))
