...
;; H
  ...
  (and
  (<= 1 (to_rep1
        (select (rec__patience__state__stacksizes (us_split_fields1 s)) i)))
  (<= (to_rep1
      (select (rec__patience__state__stacksizes (us_split_fields1 s)) i))
  (to_rep1 (rec__patience__state__numelts (us_split_fields1 s)))))
  (forall ((j Int))
  ...
  (and (<= 0 j)
  (<= j (- (to_rep1
           (select (rec__patience__state__stacksizes (us_split_fields1 s)) i)) 1)))
  (and
  (<= 0 (to_rep1
        (select (select (rec__patience__state__stacks (us_split_fields1 s)) i) j)))
  (< (to_rep1
     (select (select (rec__patience__state__stacks (us_split_fields1 s)) i) j))
  (to_rep1 (rec__patience__state__numelts (us_split_fields1 s)))))))))))))
...
;; H
  ...
  (and
  (<= 0 (to_rep1
        (select (rec__patience__state__posheight (us_split_fields1 s)) i)))
  (<= (to_rep1
      (select (rec__patience__state__posheight (us_split_fields1 s)) i)) (-
  (to_rep1
...
;; H
  (assert (= o4 (ite (<= ip o3) true false)))

;; H
  (assert (= result2 (ite (<= 0 ip) o4 false)))
...
;; H
  (assert (and (<= 0 temp___431) (<= temp___431 99)))

;; H
  (assert (= o8 temp___431))
...
;; H
  (assert (= o11 (ite (= ip o10) true false)))
