...
;; elements_equal_except__2__post_axiom
  ...
  (! (= (= (elements_equal_except__2 left right x y) true)
     ...
     (=> (= (has_key left key) true)
     (=>
     (and (not (= (bool_eq2 key x) true)) (not (= (bool_eq2 key y) true)))
...
;; H
  ...
  (= (p_positions_swapped
     (positions
     (mk___rep l__split_discrs (mk___split_fields l__split_fields)))
     (positions (mk___rep l__split_discrs l__split_fields2)) cu1 cu2) true)))

(assert
;; WP_parameter_def
 ;; File "p.ads", line 23, characters 0-0
  (not
  (= (has_key (positions (mk___rep l__split_discrs l__split_fields2)) cu1) true)))
(check-sat)
