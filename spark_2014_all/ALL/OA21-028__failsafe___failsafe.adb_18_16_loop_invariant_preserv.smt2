...
(define-fun dynamic_invariant2 ((temp___expr_202 t) (temp___is_init_199 Bool)
...
;; H
  (assert (= result2 (ite (lt o2 fliteral2) true false)))
...
;; H
  ...
  (bvule s4 current_time)) (lt (to_rep (select battery_level_at s4))
  fliteral2)))
  (and
  (forall ((s4 (_ BitVec 8)))
  (=> (and (bvule ((_ int2bv 8) 0) s4) (bvule s4 current_time)) (lt
  (to_rep (select battery_level_at s4)) fliteral2)))
  (forall ((s4 (_ BitVec 8)))
  (=>
  (and
  ...
  (bvule s4 ((_ int2bv 8) 49))) (lt (to_rep (select battery_level_at s4))
  fliteral2))))))
...
;; H
  (assert (= result5 (ite (lt o6 fliteral2) true false)))
...
;; H
  (assert
  (and (= o7 (bvadd res3 ((_ int2bv 8) 1))) (in_range3
...
;; H
  (assert (bvuge current_time (bvsub res4 ((_ int2bv 8) 1))))
...
;; WP_parameter_def
 ;; File "failsafe.ads", line 16, characters 0-0
  (not (lt (to_rep (select battery_level_at s4)) fliteral2)))
(check-sat)
