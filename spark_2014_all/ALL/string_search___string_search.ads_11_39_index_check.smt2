...
;; H
  (assert
  (and (<= len (length needle)) (<= (- loc 1) (- (length haystack) len))))
...
;; H
  (assert (and (= temp___259 (+ loc (- i 1))) (in_range1 (+ loc (- i 1)))))

;; H
  (assert
  (and (<= (to_rep1 (first (rt haystack))) temp___259)
  (<= temp___259 (to_rep1 (last (rt haystack))))))

;; H
  (assert (= o temp___259))

;; H
  (assert (= o1 (select (elts haystack) o)))
