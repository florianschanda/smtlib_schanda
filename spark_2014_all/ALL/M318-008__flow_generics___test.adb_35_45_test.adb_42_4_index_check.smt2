...
(declare-datatypes ()
((vector_t__ref (mk_vector_t__ref (vector_t__content us_t)))))
(define-fun vector_t__ref___projection ((a vector_t__ref)) us_t (vector_t__content
                                                                a))

(define-fun dynamic_invariant ((temp___expr_162 us_t)
...
;; H
  (assert
  (and (= o (+ i2 (to_rep (first (rt b))))) (in_range1
  (+ i2 (to_rep (first (rt b)))))))

;; H
  (assert (= o1 (- o (to_rep (first (rt a))))))

;; H
  (assert (and (= temp___414 o1) (in_range1 o1)))

(assert
;; WP_parameter_def
 ;; File "test.adb", line 18, characters 0-0
  (not (<= (to_rep (first (rt b))) temp___414)))
(check-sat)
(exit)
