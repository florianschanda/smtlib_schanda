...
(declare-fun user_eq4 (tbyte_array_4D1 tbyte_array_4D1) Bool)

(declare-fun dummy4 () tbyte_array_4D1)
...
;; H
  (assert (dynamic_invariant bytes true false true))
...
;; WP_parameter_def
 ;; File "interfac.ads", line 52, characters 0-0
  (not
  (= (ite (<= (to_rep1 (first (rt bytes))) (to_rep1 (last (rt bytes))))
     (+ (- (to_rep1 (last (rt bytes))) (to_rep1 (first (rt bytes)))) 1) 0)
  (ite (<= 1 4) (+ (- 4 1) 1) 0))))
(check-sat)
(exit)
