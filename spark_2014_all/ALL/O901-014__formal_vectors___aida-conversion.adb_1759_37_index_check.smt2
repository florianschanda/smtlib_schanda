...
;; H
  (assert
  (and (= o5 (+ (to_rep1 (first (rt source))) 10)) (in_range1
...
;; H
  (assert
  (and (= o4 (+ (to_rep1 (first (rt source))) 2)) (in_range1
...
;; H
  ...
  (forall ((index1 Int))
  (=>
  (and (<= (+ (to_rep1 (first (rt source))) 2) index1)
  (<= index1 (+ (to_rep1 (first (rt source))) 10)))
  (= (is_digit (to_rep (select (elts source) index1))) true)))))

;; H
  (assert
  (and (= temp___2132 (+ (to_rep1 (first (rt source))) 2)) (in_range1
  (+ (to_rep1 (first (rt source))) 2))))

;; H
  (assert
  (and (<= (to_rep1 (first (rt source))) temp___2132)
  (<= temp___2132 (to_rep1 (last (rt source))))))

;; H
  (assert (= o temp___2132))
...
;; H
  (assert
  (and (= temp___2131 (+ (to_rep1 (first (rt source))) 1)) (in_range1
  (+ (to_rep1 (first (rt source))) 1))))

(assert
;; WP_parameter_def
 ;; File "aida-conversion.adb", line 1703, characters 0-0
  (not (<= (to_rep1 (first (rt source))) temp___2131)))
(check-sat)
(exit)
