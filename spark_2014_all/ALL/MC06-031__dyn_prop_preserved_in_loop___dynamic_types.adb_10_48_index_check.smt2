...
;; H
  (assert (dynamic_property1 first2 last2 from))
...
;; H
  (assert
  (and (= first2 (to_rep1 (first (rt a)))) (= last2 (to_rep1 (last (rt a))))))

;; H
  (assert
  (=> (= (to_rep (select (elts a) from)) c)
  (and (= o from) (in_range1 from))))

;; H
  (assert
  (=> (= (to_rep (select (elts a) from)) c)
  (= (mk_int__ref result) result__5)))

;; H
  (assert (=> (= (to_rep (select (elts a) from)) c) (= result__1 o)))

;; H
  (assert
  (=> (not (= (to_rep (select (elts a) from)) c))
  (= (mk_int__ref result1) result__5)))

;; H
  (assert (=> (not (= (to_rep (select (elts a) from)) c)) (= result__2 0)))

;; H
  (assert
  (=> (not (= (to_rep (select (elts a) from)) c)) (= result__1 result__2)))
...
;; H
  ...
  (= (= result2 true)
  (forall ((i Int))
  (=> (and (<= from i) (<= i from)) (not (= (to_rep (select (elts a) i)) c)))))))

;; H
  (assert (=> (not (= result__3 0)) (= result2 false)))

;; H
  (assert (not (= result2 true)))

;; H
  (assert (<= (to_rep1 (first (rt a))) result__3))

;; H
  (assert (<= result__3 (to_rep1 (last (rt a)))))

(assert
;; WP_parameter_def
 ;; File "system.ads", line 1, characters 0-0
  (not (<= (to_rep1 (first (rt a))) result__3)))
(check-sat)
(exit)
