...
(define-fun dynamic_invariant ((temp___expr_320 us_t)
...
(assert
;; WP_parameter_def
 ;; File "do_checks.adb", line 235, characters 0-0
  (not
  (= (ite (<= (to_rep (first (rt x))) (to_rep (last (rt x))))
     (+ (- (to_rep (last (rt x))) (to_rep (first (rt x)))) 1) 0) (ite (<= 0 10)
                                                                 (+ (- 10 0) 1)
                                                                 0))))
(check-sat)
(exit)
