...
(define-fun o9 () us_t (mk___t o1 o2))
...
;; H
  (assert
  (and (= result1 (invariant__ b))
  (= (= result1 true)
  (and (= (to_rep1 (first (rt b))) 1) (<= 0 (to_rep1 (last (rt b))))))))
...
;; H
  (assert (= result2 (ite (= o8 o7) true false)))
...
;; H
  ...
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- o 1)))
  (= (to_rep (select (elts b) k)) (to_rep (select o1 k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ o 1) k) (<= k (to_rep1 (last (rt b)))))
  (= (to_rep (select (elts b) k)) (to_rep (select o1 (- k 1))))))
  (forall ((k Int))
  (=> (and (<= o k) (<= k (- (to_rep1 (last (rt b))) 1)))
  (= (to_rep (select (elts b) (+ k 1))) (to_rep (select o1 k))))))))))))

(assert
;; WP_parameter_def
 ;; File "perm.ads", line 3, characters 0-0
  (not (= (invariant__ a) true)))
(check-sat)
(exit)
