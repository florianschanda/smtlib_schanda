...
(declare-datatypes ((arr1__ref 0)) (((mk_arr1__ref (arr1__content us_t)))))
(define-fun arr1__ref___projection ((a arr1__ref)) us_t (arr1__content a))

(declare-sort tTaSP1 0)

(define-fun in_range2 ((x Int)) Bool (and (<= 7 x) (<= x 10)))
...
(declare-const pack__init__a (Array Int integer))
...
;; H
  (assert
  (forall ((j Int))
  (=> (and (<= (to_rep o) j) (<= j (to_rep o1)))
  (= (to_rep (select pack__init__a j)) 1))))

;; H
  (assert (= result a))

;; H
  (assert (= a1 (slide pack__init__a (to_rep (first (mk 7 10))) 7)))

;; H
  (assert (= (to_rep (select a1 8)) 1))

(assert
;; WP_parameter_def
 ;; File "pack.adb", line 79, characters 0-0
  (not (= (to_rep (select a1 9)) 2)))
(check-sat)
(exit)
