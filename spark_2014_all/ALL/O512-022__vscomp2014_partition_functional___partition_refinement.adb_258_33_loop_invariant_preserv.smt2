...
;; H
  (assert
  (ite (= (ite (= (bool_eq1 (mk___rep c__split_fields3) no_element) false) (=
          (get1 (positions x) (mk___rep c__split_fields3)) (length x)) true) true)
  (= (bool_eq1 (mk___rep c__split_fields4) no_element) true)
...
;; H
  (assert (= (has_key (positions x) c) true))

(assert
;; WP_parameter_def
 ;; File "a-cofove.ads", line 807, characters 0-0
  (not
  (= (has_key1 (positions1 (mk___rep3 d__split_discrs d__split_fields3))
     (find d_old (element x c))) true)))
(check-sat)
(exit)
