...
;; H
  (assert
  (and (= o35 (remove (mk___t values2 (mk 1 100)) o34))
  ...
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- o34 1)))
  (= (to_rep (select values2 k)) (to_rep (select (elts o35) k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ o34 1) k) (<= k (to_rep1 (last (mk 1 100)))))
  (= (to_rep (select values2 k)) (to_rep (select (elts o35) (- k 1))))))
  (forall ((k Int))
  (=> (and (<= o34 k) (<= k (- (to_rep1 (last (mk 1 100))) 1)))
  (= (to_rep (select values2 (+ k 1))) (to_rep (select (elts o35) k))))))))))))
