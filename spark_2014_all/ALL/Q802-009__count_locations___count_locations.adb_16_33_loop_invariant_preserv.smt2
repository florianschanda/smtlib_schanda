...
(((mk_sum_type__ref (sum_type__content sum_type)))))
(define-fun sum_type__ref___projection ((a sum_type__ref)) sum_type (sum_type__content
                                                                    a))
...
;; partial_count_selected_locations__def_axiom
  ...
  (forall ((from Int))
  (! (=> (dynamic_invariant2 from true true true)
...
;; count_selected_locations__post_axiom
  (assert
  (forall ((sel (Array Int Bool))) (! (dynamic_invariant1
  (count_selected_locations sel) true false
  true) :pattern ((count_selected_locations sel)) )))
...
;; H
  ...
  (and
  (and
  (and (=> (<= 1 100) (in_range4 st2)) (=> (<= 0 100) (in_range3 st_count2)))
  (=> (<= 0 2147483647) (in_range1 lowest_max2)))
  (and (<= 1 st2) (<= st2 100))))

;; H
  (assert
  (=> (= (select sel st2) true)
  (and (= o (+ st_count2 1)) (in_range3 (+ st_count2 1)))))

;; H
  (assert (=> (= (select sel st2) true) (= st_count2 result3)))

;; H
  (assert (=> (= (select sel st2) true) (= st_count3 o)))

;; H
  (assert
  (=> (= (select sel st2) true)
  (=> (< (to_rep (select max st2)) lowest_max2) (= lowest_max2 result4))))

;; H
  (assert
  (=> (= (select sel st2) true)
  (=> (< (to_rep (select max st2)) lowest_max2)
  (= lowest_max3 (to_rep (select max st2))))))

;; H
  (assert
  (=> (= (select sel st2) true)
  (=> (not (< (to_rep (select max st2)) lowest_max2))
  (= lowest_max3 lowest_max2))))

;; H
  (assert (=> (not (= (select sel st2) true)) (= lowest_max3 lowest_max2)))

;; H
  (assert (=> (not (= (select sel st2) true)) (= st_count3 st_count2)))
...
;; WP_parameter_def
 ;; File "count_locations.ads", line 22, characters 0-0
  (not (<= lowest_max3 (to_rep (select max s)))))
(check-sat)
(exit)
