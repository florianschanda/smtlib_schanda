...
;; H
  (assert
  (and (= temp___876 (- (to_rep1 (last (rt tmp))) 1)) (in_range1
  (- (to_rep1 (last (rt tmp))) 1))))

;; H
  (assert
  (and (= temp___875 (+ (to_rep1 (first (rt tmp))) 1)) (in_range1
  (+ (to_rep1 (first (rt tmp))) 1))))

;; H
  (assert (<= temp___875 temp___876))

(assert
;; WP_parameter_def
 ;; File "lexer.ads", line 65, characters 0-0
  (not (<= (to_rep1 (first (rt tmp))) temp___875)))
(check-sat)
(exit)
