...
;; H
  (assert
  (and (= temp___788 (- (to_rep1 (last (rt tmp))) 1)) (in_range1
  (- (to_rep1 (last (rt tmp))) 1))))

;; H
  (assert
  (and (= temp___787 (+ (to_rep1 (first (rt tmp))) 1)) (in_range1
  (+ (to_rep1 (first (rt tmp))) 1))))

;; H
  (assert (<= temp___787 temp___788))

(assert
;; WP_parameter_def
 ;; File "lexer.ads", line 65, characters 0-0
  (not (<= (to_rep1 (first (rt tmp))) temp___787)))
(check-sat)
(exit)
