...
;; H
  (assert
  (exists ((i4 Int))
  (and (and (<= (to_rep1 (first (rt a))) i4) (<= i4 (to_rep1 (last (rt a)))))
  (= (to_rep (select (elts a) i4)) e))))
...
;; H
  (assert (not (= (to_rep (select (elts a) i1)) e)))
...
;; H
  (assert (not (= (to_rep (select (elts a) i3)) e)))
...
;; WP_parameter_def
 ;; File "p.adb", line 59, characters 0-0
  (not (not (= (to_rep (select (elts a) k)) e))))
(check-sat)
