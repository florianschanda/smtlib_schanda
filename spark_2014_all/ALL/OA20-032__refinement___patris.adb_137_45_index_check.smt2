...
;; H
  (assert
  (and (<= 1 (+ (to_rep2 cur_piece__split_fields2) x1))
  (<= (+ (to_rep2 cur_piece__split_fields2) x1) 10)))
...
;; H
  (assert
  (= o3 (store (select cur_board (+ (to_rep3 cur_piece__split_fields3)
  y1)) o2 o1)))

(assert
;; WP_parameter_def
 ;; File "patris.ads", line 196, characters 0-0
  (not (<= 1 (+ (to_rep3 cur_piece__split_fields3) y1))))
(check-sat)
(exit)
