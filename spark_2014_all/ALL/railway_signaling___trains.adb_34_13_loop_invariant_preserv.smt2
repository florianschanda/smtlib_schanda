...
;; H
  ...
  (forall ((t Int))
  (=> (and (<= 1 t) (<= t train2))
  (not (= (is_previous_track (select trains t) track) true)))))
...
;; H
  (assert (not (= train2 r7b)))

;; H
  (assert (= train2 result2))

;; H
  (assert (= train3 (+ train2 1)))

;; H
  (assert
  (and (= result3 (is_previous_track (select trains train3) track))
  (= (= result3 true)
  (exists ((id Int))
  (and (and (<= 1 id) (<= id 3))
  (or (= track (get_previous_track (select trains train3) id))
  (= track (get_other_previous_track (select trains train3) id))))))))
