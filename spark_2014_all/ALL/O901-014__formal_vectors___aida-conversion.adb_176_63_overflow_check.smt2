...
;; H
  (assert (= i1 temp___883))

;; H
  (assert (<= temp___883 i1))

;; H
  (assert (<= i1 (to_rep1 (last (rt source)))))
...
;; H
  (assert (= result2 (ite (not (= o6 true)) true false)))
...
;; H
  (assert
  (forall ((j Int))
  (=> (and (<= (+ (to_rep1 (first (rt source))) 1) j) (<= j i2))
...
;; H
  ...
  (and (<= (+ (to_rep1 (first (rt source))) 1) i2)
