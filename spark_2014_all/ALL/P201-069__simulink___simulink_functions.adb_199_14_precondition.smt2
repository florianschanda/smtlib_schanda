...
(declare-fun log__function_guard (t t) Bool)
...
;; log__post_axiom
  (assert
  (forall ((x t))
  (! (=> (and (dynamic_invariant x true true true) (lt fliteral2 x))
     (let ((result (log x)))
     (and (=> (eq x fliteral) (eq result fliteral2)) (dynamic_invariant
     result true false true)))) :pattern ((log x)) )))
