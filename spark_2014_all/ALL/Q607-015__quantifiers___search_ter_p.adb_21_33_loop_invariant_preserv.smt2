...
;; H
  (assert
  (and (= result4 (equal_subrange a i1 b))
  (= (= result4 true)
  (= (bool_eq (elts a) (to_rep (first2 (mk1 i1 (+ (- i1 1) (length b)))))
     (to_rep (last2 (mk1 i1 (+ (- i1 1) (length b))))) (elts b)
...
;; H
  (assert (= search_ter_p__search__result5 search_ter_p__search__result4))

;; H
  (assert (= result____split_fields3 result____split_fields2))

;; H
  (assert (not (= (has_sub_range_in_prefix a i2 b) true)))

;; H
  (assert
  (not (= (rec__types__option__exists result____split_fields4) true)))
...
;; H
  ...
  (- (+ (to_rep (last (rt a))) 1) (length b)) i2))
  (and (<= (to_rep (first (rt a))) i2)
  (<= i2 (- (+ (to_rep (last (rt a))) 1) (length b))))))

;; H
  (assert (= result5 (mk_int__ref temp___202)))

;; H
  (assert (= temp___2021 i2))

;; H
  (assert (not (= i2 temp___204)))
...
;; H
  (assert
  (and (= result7 (equal_subrange a i3 b))
  (= (= result7 true)
  (= (bool_eq (elts a) (to_rep (first2 (mk1 i3 (+ (- i3 1) (length b)))))
     (to_rep (last2 (mk1 i3 (+ (- i3 1) (length b))))) (elts b)
     (to_rep (first (rt b))) (to_rep (last (rt b)))) true))))
...
;; H
  (assert (= search_ter_p__search__result7 search_ter_p__search__result6))

;; H
  (assert (= result____split_fields5 result____split_fields4))

(assert
;; WP_parameter_def
 ;; File "types.ads", line 11, characters 0-0
  (not (not (= (has_sub_range_in_prefix a i3 b) true))))
(check-sat)
(exit)
