...
;; H
  ...
  (=> (and (<= 1 j1) (<= j1 8))
  (and
  (=> (= (to_rep5 (select track_signals1 j1)) 2)
  (= (to_rep5 (select track_signals3 j1)) 2))
  (=>
  (and (= (to_rep5 (select track_signals1 j1)) 1)
  (exists ((t Int))
  (and (and (<= 1 t) (<= t cur_num_trains))
  (= (is_previous_track (select trains1 t) j1) true))))
  (= (to_rep5 (select track_signals3 j1)) 1))))))
...
;; H
  ...
  (= result8 (ite (= (to_rep5 (select track_signals3 j)) 2) true false))))

;; H
  (assert
  (=> (not (= (to_rep5 (select track_signals1 j)) 2)) (= result8 (of_int 1))))
