...
;; H
  (assert
  (and (= o28 (remove b ib2))
  (and (dynamic_invariant2 o28 true false true)
  (and (= (invariant__ o28) true)
  (and (= (to_rep2 (last (rt o28))) (- (to_rep2 (last (rt b))) 1))
  (and
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- ib2 1)))
  (= (to_rep (select (elts b) k)) (to_rep (select (elts o28) k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ ib2 1) k) (<= k (to_rep2 (last (rt b)))))
  (= (to_rep (select (elts b) k)) (to_rep (select (elts o28) (- k 1))))))
  (forall ((k Int))
  (=> (and (<= ib2 k) (<= k (- (to_rep2 (last (rt b))) 1)))
  (= (to_rep (select (elts b) (+ k 1))) (to_rep (select (elts o28) k))))))))))))

(assert
;; WP_parameter_def
 ;; File "perm.ads", line 2, characters 0-0
  (not (= (invariant__ o28) true)))
(check-sat)
(exit)
