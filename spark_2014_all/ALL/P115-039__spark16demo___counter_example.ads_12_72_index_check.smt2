...
;; range_axiom
  (assert
  (forall ((x category)) (! (in_range1 (to_rep1 x)) :pattern ((to_rep1 x)) )))
...
;; H
  (assert (dynamic_invariant num_item_per_category true false true))

;; H
  (assert
  (= (mk___t
     (counter_example__num_item_per_category__aggregate_def 3 8 10000 42 98
     76 0 1) (mk 1 8)) num_item_per_category))

(assert
;; WP_parameter_def
 ;; File "counter_example.ads", line 9, characters 0-0
  (not
  (<= (to_rep2 (first (rt num_item_per_category))) (to_rep1
                                                   (rec__counter_example__item__cat
                                                   (us_split_fields1 i5s))))))
(check-sat)
(exit)
