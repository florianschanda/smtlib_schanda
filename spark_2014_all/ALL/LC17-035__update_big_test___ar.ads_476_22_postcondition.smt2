...
;; H
  (assert (=> (= e 0) (= result2 f)))

;; H
  (assert (=> (= e 0) (= f1 99)))

;; H
  (assert (=> (not (= e 0)) (and (= o1 (- e 1)) (in_range4 (- e 1)))))

;; H
  (assert (=> (not (= e 0)) (= result3 f)))
...
;; H
  (assert (= o3 (store (select (select a k_next1) j) i o2)))

;; H
  (assert (= o4 (store (select a k_next1) j o3)))

;; H
  (assert (= o5 (store a k_next1 o4)))
...
;; WP_parameter_def
 ;; File "ar.ads", line 471, characters 0-0
  (not (in_range4 (to_rep (select (select (select a2 m) n) o6)))))
(check-sat)
(exit)
