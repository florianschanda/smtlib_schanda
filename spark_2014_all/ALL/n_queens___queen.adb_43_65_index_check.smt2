...
;; H
  ...
  (=> (and (<= 1 j1) (<= j1 (- i1 1)))
  (and
  (and (not (= (to_rep (select c i1)) (to_rep (select c j1))))
  (not (= (- i1 j1) (- (to_rep (select c i1)) (to_rep (select c j1))))))
  (not (= (- i1 j1) (- (to_rep (select c j1)) (to_rep (select c i1))))))))))))))

;; H
  (assert
  (=> (< (to_rep (select c i)) r2) (= o1 (ite (not (= o true)) true false))))

;; H
  (assert (=> (not (< (to_rep (select c i)) r2)) (= o1 (of_int 1))))
