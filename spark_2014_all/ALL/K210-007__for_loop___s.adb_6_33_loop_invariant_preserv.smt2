...
(declare-fun s__contains__result () Bool)

(declare-fun s__contains__result1 () Bool)
...
;; H
  ...
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j (- index2 1)))
  (not (= (to_rep (select table j)) value)))))

;; H
  (assert
  (and (=> (<= 1 10) (in_range3 index2)) (and (<= 1 index2) (<= index2 10))))

;; H
  (assert (not (= (to_rep (select table index2)) value)))

;; H
  (assert (= s__contains__result1 s__contains__result))
...
;; WP_parameter_def
 ;; File "s.ads", line 7, characters 0-0
  (not (not (= (to_rep (select table j)) value))))
(check-sat)
(exit)
