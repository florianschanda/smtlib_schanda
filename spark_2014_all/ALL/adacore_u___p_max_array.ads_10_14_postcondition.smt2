...
;; H
  ...
  (=> (and (<= (to_rep1 (first (rt a))) i1) (<= i1 (to_rep1 (last (rt a)))))
  (forall ((k Int))
  (=> (and (<= (to_rep1 (first (rt a))) k) (<= k (- i2 1)))
  (ite (< (to_rep
          (select (elts b) (+ (- k (to_rep1 (first (rt a)))) (to_rep1
                                                             (first (rt b))))))
  (to_rep (select (elts a) k)))
  (= (to_rep (select r2 k)) (to_rep (select (elts a) k)))
  (= (to_rep (select r2 k)) (to_rep
                            (select (elts b) (+ (- k (to_rep1 (first (rt a))))
                            (to_rep1 (first (rt b))))))))))))
...
;; H
  (assert
  (= (mk___t result8 result9) (nat_array__content
                              p_max_array__max_array_2__result5)))
...
;; WP_parameter_def
 ;; File "p_max_array.ads", line 3, characters 0-0
  (not
  (= (to_rep
     (select (elts (nat_array__content p_max_array__max_array_2__result5))
     k)) (to_rep (select (elts a) k)))))
(check-sat)
(exit)
