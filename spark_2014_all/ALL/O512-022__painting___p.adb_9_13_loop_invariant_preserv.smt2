...
;; H
  ...
  (exists ((c4 Int))
  (and (and (<= 0 c4) (<= c4 2))
  (= (bool_eq (select p__split_fields c4) d) true))))
...
;; H
  (assert
  (=> (= (bool_eq (select p__split_fields2 c2) d) true)
...
;; H
  (assert
  (=> (not (= (bool_eq (select p__split_fields2 c2) d) true))
...
;; WP_parameter_def
 ;; File "p.ads", line 31, characters 0-0
  (not (= (bool_eq1 p__split_fields4 0 2 p__split_fields 0 2) true)))
(check-sat)
(exit)
