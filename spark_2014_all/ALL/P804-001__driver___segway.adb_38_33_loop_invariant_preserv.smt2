...
;; H
  (assert (= (speed_is_valid speed1 state1) true))

;; H
  (assert
  (and (and (in_range2 speed1) (in_range1 state1)) (not (= state1 0))))

;; H
  (assert
  (=> (< 0 speed1)
  (and (= (speed_is_valid speed2 state2) true)
  (and (in_range2 speed2) (in_range1 state2)))))
...
;; H
  (assert (=> (not (< 0 speed1)) (= state2 state3)))

;; H
  (assert (=> (not (< 0 speed1)) (= speed2 speed3)))
...
;; WP_parameter_def
 ;; File "segway.ads", line 16, characters 0-0
  (not (= (speed_is_valid speed2 state2) true)))
(check-sat)
(exit)
