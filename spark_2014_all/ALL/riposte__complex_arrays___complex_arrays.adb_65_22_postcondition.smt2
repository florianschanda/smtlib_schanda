...
(((mk_map__ref1 (map__content1 (Array Int (Array Int value)))))))
...
(define-fun bool_eq1 ((a (Array Int (Array Int value))) (a__first Int)
  ...
  (b__last Int)) Bool (ite (and
                           ...
                           (= (bool_eq (select a temp___idx_133) 0 1001
...
;; H
  (assert (= (to_rep o3) (to_rep (select (select a1 0) 1))))

;; H
  (assert (= o4 (store (select a1 i) j o3)))

;; H
  (assert (= o5 (store a1 i o4)))
...
(assert
;; WP_parameter_def
 ;; File "complex_arrays.adb", line 55, characters 0-0
  (not
  (= (to_rep (select (select a3 i) j)) (to_rep (select (select a3 j) i)))))
(check-sat)
(exit)
