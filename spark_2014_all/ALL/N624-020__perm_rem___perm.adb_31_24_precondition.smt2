...
(define-fun o17 () us_t (mk___t o9 o10))
...
;; H
  ...
  (and (= (to_rep (select (elts a) ia)) (to_rep (select (elts b) ib3)))
  (= (is_perm (remove a ia) (remove b ib3)) true))))))))
...
;; H
  ...
  (and (= (to_rep (select o ia1)) (to_rep (select o3 ib3)))
  (= (is_perm (remove o19 ia1) (remove o18 ib3)) true)))))))))))
...
;; H
  ...
  (=> (and (<= 1 kb1) (<= kb1 ib2))
  (not
  (and (= (to_rep (select (elts a) ia)) (to_rep (select (elts b) kb1)))
  (= (is_perm (remove a ia) (remove b kb1)) true))))))
...
;; H
  (assert
  (and (<= (to_rep1 (first (rt b))) kb) (<= kb (to_rep1 (last (rt b))))))
...
;; H
  (assert (= result3 (ite (= o16 o15) true false)))

;; H
  (assert (= result3 true))

;; H
  (assert (and (= o8 kb) (in_range2 kb)))
...
;; H
  ...
  (and (= (to_rep1 (last o10)) (- (to_rep1 (last (rt b))) 1))
  (and
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- o8 1)))
  (= (to_rep (select (elts b) k)) (to_rep (select o9 k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ o8 1) k) (<= k (to_rep1 (last (rt b)))))
  (= (to_rep (select (elts b) k)) (to_rep (select o9 (- k 1))))))
  (forall ((k Int))
  (=> (and (<= o8 k) (<= k (- (to_rep1 (last (rt b))) 1)))
  (= (to_rep (select (elts b) (+ k 1))) (to_rep (select o9 k))))))))))))
