...
;; H
  ...
  (and (= o1 (to_rep (select memory (bvadd addr ((_ int2bv 64) 1)))))
  (bvuge o1 ((_ int2bv 8) 0)))))
...
;; H
  (assert
  (and (= o (readmem8 addr memory))
  (and (= o (to_rep (select memory addr))) (bvuge o ((_ int2bv 8) 0)))))
...
(assert
;; WP_parameter_def
 ;; File "x86.ads", line 6, characters 0-0
  (not
  (= x86__readmem16__result4 (bvadd ((_ zero_extend 8) (to_rep
                                                       (select memory
                                                       addr))) (bvmul ((_ zero_extend 8)
  (to_rep (select memory (bvadd addr ((_ int2bv 64) 1))))) ((_ int2bv 16) 256))))))
(check-sat)
(exit)
