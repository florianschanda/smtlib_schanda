...
;; H
  ...
  (=> (or (<= x24 temp___255) (not (= x13 temp___254)))
...
;; H
  ...
  (=>
  (and (<= (to_rep1 (first (rt s2))) x23) (<= x23 (to_rep1 (last (rt s2)))))
  (=> (= (to_rep (select (elts s1) x13)) (to_rep (select (elts s2) x24)))
...
;; H
  ...
  (and (<= (to_rep1 (first (rt s2))) x23) (<= x23 (to_rep1 (last (rt s2)))))
  (=>
  (not (= (to_rep (select (elts s1) x13)) (to_rep (select (elts s2) x24))))
...
;; WP_parameter_def
 ;; File "system.ads", line 1, characters 0-0
  (not (dynamic_property2 first3 last3 last3)))
(check-sat)
(exit)
