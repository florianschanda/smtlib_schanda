...
(declare-fun i_v23 () Int)

(declare-fun a_v23 () map__ref)

(declare-fun i_v24 () int__ref)

(declare-fun a_v24 () (Array Int integer))

(declare-fun i_v25 () Int)
...
;; H
  ...
  (= (to_rep (select (map__content a_v1) m)) (to_rep (select (elts b_v1) m)))))
  (forall ((m Int))
  (=> (and (<= (+ k_1 1) m) (<= m (to_rep a_v1__last)))
  (= (to_rep (select (map__content a_v1) m)) (to_rep
                                             (select (elts c_v1) (- m 
                                             k_1)))))))
  (forall ((m Int))
  (=> (and (<= (to_rep a_v2__first) m) (<= m k_2))
  (= (to_rep (select (map__content a_v2) m)) (to_rep (select (elts b_v2) m)))))))
...
;; H
  (assert (= i_v21 temp___464))

;; H
  (assert (<= temp___464 i_v21))

;; H
  (assert (<= i_v21 (to_rep a_v2__last)))
...
;; H
  ...
  (forall ((m Int))
  (=> (and (<= (to_rep a_v2__first) m) (<= m k_2))
  (= (to_rep (select a_v21 m)) (to_rep (select (elts b_v2) m)))))
  (forall ((m Int))
  (=> (and (<= (+ k_2 1) m) (<= m (- i_v22 1)))
  (= (to_rep (select a_v21 m)) (to_rep (select (elts c_v2) (- m k_2))))))))
...
;; H
  ...
  (=>
  (and (<= (to_rep a_v2__first) temp___461)
  (<= temp___461 (to_rep a_v2__last)))
  (=> (<= i_v22 temp___461)
  (= (select a_v21 temp___461) (select (map__content a_v2) temp___461)))))
  (=> (<= (+ k_2 1) (to_rep a_v2__last)) (dynamic_property4 (+ k_2 1)
  (to_rep a_v2__last) i_v22)))
  (and (<= (+ k_2 1) i_v22) (<= i_v22 (to_rep a_v2__last)))))

;; H
  (assert (and (= temp___458 (- i_v22 k_2)) (in_range1 (- i_v22 k_2))))

;; H
  (assert
  (and (<= (to_rep (first (rt c_v2))) temp___458)
  (<= temp___458 (to_rep (last (rt c_v2))))))

;; H
  (assert (= o temp___458))

;; H
  (assert (= o1 (select (elts c_v2) o)))
...
;; H
  (assert (= o5 (store a_v21 o4 o3)))

;; H
  (assert (= result1 (mk_map__ref a_v21)))

;; H
  (assert (= a_v22 o5))

;; H
  (assert (not (= i_v22 (to_rep a_v2__last))))

;; H
  (assert (= result2 (mk_int__ref i_v22)))

;; H
  (assert (= i_v23 (+ i_v22 1)))

;; H
  (assert (= i_v24 (mk_int__ref i_v23)))

;; H
  (assert (= a_v23 (mk_map__ref a_v22)))
...
;; WP_parameter_def
 ;; File "infoflow.ads", line 89, characters 0-0
  (not
  (= (to_rep (select (map__content a_v23) m)) (to_rep (select (elts b_v2) m)))))
(check-sat)
