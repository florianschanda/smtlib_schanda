...
;; H
  ...
  (forall ((k Int))
  (=> (and (<= (+ i2 1) k) (<= k a_last))
  (= (to_rep1 (select a2 k)) (to_rep1 (select a k)))))))

;; H
  (assert (=> (<= a_first a_last) (dynamic_property1 a_first a_last i2)))
...
;; WP_parameter_def
 ;; File "system.ads", line 1, characters 0-0
  ...
  (= (oeq
     (osubtract
     (num_of_big_int (mk___t a (mk (to_rep a__first) (to_rep a__last)))
     a_first (- i3 a_first)) (num_of_big_int b b_first (- i3 a_first)))
     (osubtract
     (num_of_big_int (mk___t a2 (mk (to_rep a__first) (to_rep a__last)))
     a_first (- i3 a_first))
     (omultiply (oexpon (base Tuple0) (- i3 a_first))
     (num_of_boolean carry3)))) true)))
(check-sat)
(exit)
