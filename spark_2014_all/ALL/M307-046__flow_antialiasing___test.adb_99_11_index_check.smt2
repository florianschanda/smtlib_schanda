...
;; H
  (assert
  (and (<= (to_rep1 (first (rt y))) (to_rep1 (last (rt y))))
  (<= (to_rep1 (last (rt y))) (to_rep1 (last (rt y))))))

;; H
  (assert (= o (to_rep1 (last (rt y)))))

;; H
  (assert (= o1 (select (elts y) o)))
...
;; WP_parameter_def
 ;; File "test.adb", line 88, characters 0-0
  (not (<= (to_rep1 x__first) (to_rep1 x__first))))
(check-sat)
(exit)
