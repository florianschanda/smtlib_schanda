...
;; WP_parameter_def
 ;; File "lsc-types.ads", line 54, characters 0-0
  ...
  (= (oeq
     ...
     (+ a_first 1) (- i1 a_first))
     (omultiply (num_of_big_int b b_first (- i1 a_first)) (from_word32 x)))
     (omultiply (num_of_big_int c c_first (- i1 a_first)) (from_word32 y)))
     (from_word32 carry1)) (omultiply (base Tuple0) (from_word32 carry2)))
     (oadd
     (num_of_big_int (mk___t a (mk (to_rep a__first) (to_rep a__last)))
     a_first (- i1 a_first))
     (omultiply (oexpon (base Tuple0) (- i1 a_first))
     (oadd (from_word32 carry1)
     (omultiply (base Tuple0) (from_word32 carry2)))))) true)))
(check-sat)
(exit)
