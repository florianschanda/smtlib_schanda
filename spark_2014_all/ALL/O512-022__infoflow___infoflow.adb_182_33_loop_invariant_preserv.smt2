...
;; H
  (assert (= o5 (store a_v11 o4 o3)))

;; H
  (assert (= a_v11 result1))

;; H
  (assert (= a_v12 o5))

;; H
  (assert (not (= i_v12 (to_rep a_v1__last))))

;; H
  (assert (= result2 i_v12))

;; H
  (assert (= i_v13 (+ i_v12 1)))
...
;; WP_parameter_def
 ;; File "infoflow.ads", line 89, characters 0-0
  (not (= (to_rep (select a_v13 m)) (to_rep (select (elts b_v1) m)))))
(check-sat)
(exit)
