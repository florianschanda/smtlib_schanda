...
;; H
  (assert
  (= fileindex2 (to_rep4
                (select usedlogfiles__split_fields fileindexinusedlist2))))

;; H
  (assert (not (< (to_rep2 (select logfileentries fileindex2)) 1024)))
...
;; H
  (assert (= o3 (store logfilesstatus fileindex2 o2)))

;; H
  (assert (= result8 (mk_map__ref4 logfilesstatus)))

;; H
  (assert (= logfilesstatus1 o3))
...
;; H
  (assert
  (=> (not (= archivedfilecount2 0)) (= auditsystemfault auditsystemfault1)))
...
;; WP_parameter_def
 ;; File "auditlog.ads", line 33, characters 0-0
  (not (in_range7 (+ archivedfilecount2 1))))
(check-sat)
(exit)
