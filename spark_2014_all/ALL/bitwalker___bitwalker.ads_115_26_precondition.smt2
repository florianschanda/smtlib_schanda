...
;; H
  ...
  (= o35 (+ (- (to_rep addr__last) (to_rep addr__first)) 1))))
...
;; H
  (assert
  (=> (<= (to_rep addr__first) (to_rep addr__last))
  (= o20 (+ (- (to_rep addr__last) (to_rep addr__first)) 1))))
...
;; H
  ...
  (= (= result11 true)
  (forall ((i15 Int))
  (=> (and (<= start i15) (<= i15 (- (+ start len) 1)))
  (=
  (= (nth8_stream
     (mk___t (map__content addr10)
     (mk (to_rep addr__first) (to_rep addr__last))) i15) true)
  (= (nth value (+ (- (- len i15) 1) start)) true)))))))

;; H
  (assert (=> (not (= (int__content result__9) 0)) (= result11 (of_int 1))))
