...
;; fliteral_axiom
  (assert (and (tqtisFinite fliteral2) (= (tqtreal fliteral2) 7800.0)))
...
;; H
  (assert (and (le (neg fliteral2) y) (le y fliteral2)))

;; H
  (assert (lt (abs1 y) x))

;; H
  (assert (and (= o (mul RNE1 y y)) (tqtisFinite (mul RNE1 y y))))

(assert
;; WP_parameter_def
 ;; File "floating_point.adb", line 308, characters 0-0
  (not (tqtisFinite (mul RNE1 x x))))
(check-sat)
