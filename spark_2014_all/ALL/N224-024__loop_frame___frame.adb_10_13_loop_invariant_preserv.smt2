...
;; H
  ...
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k j2))
  (= (to_rep (select a3 k)) (ite (< 10 (to_rep (select a k))) 10
                            (to_rep (select a k)))))))
...
;; H
  (assert (=> (< 10 (to_rep (select a3 j3))) (= (to_rep o2) 10)))

;; H
  (assert (=> (< 10 (to_rep (select a3 j3))) (= o3 (store a3 j3 o2))))

;; H
  (assert (=> (< 10 (to_rep (select a3 j3))) (= result3 a3)))

;; H
  (assert (=> (< 10 (to_rep (select a3 j3))) (= a4 o3)))

;; H
  (assert (=> (not (< 10 (to_rep (select a3 j3)))) (= a4 a3)))
...
;; WP_parameter_def
 ;; File "frame.ads", line 14, characters 0-0
  (not
  (= (to_rep (select a4 k)) (ite (< 10 (to_rep (select a k))) 10
                            (to_rep (select a k))))))
(check-sat)
(exit)
