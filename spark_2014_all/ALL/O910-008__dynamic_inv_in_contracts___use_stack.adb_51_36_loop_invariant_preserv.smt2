...
(declare-fun o4 () (Array Int positive))
...
;; H
  (assert
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j i2)) (= (to_rep (select content2 j)) e))))
...
;; H
  (assert (not (= i2 (to_rep1 content__last))))
...
;; H
  (assert (= o4 (store content2 i3 o3)))

;; H
  (assert (= result4 content2))
...
;; WP_parameter_def
 ;; File "use_stack.adb", line 15, characters 0-0
  (not (= (to_rep (select content3 j)) e)))
(check-sat)
(exit)
