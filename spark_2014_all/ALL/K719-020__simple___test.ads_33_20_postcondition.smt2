...
(declare-const x2 (Array Int us_rep))

(declare-const x3 (Array Int us_rep))
...
;; H
  (assert
  (= o1 (store (rec__test__t__c (us_split_fields1 (select x 11))) 2 o)))

;; H
  (assert (= (rec__test__t__a (us_split_fields1 (select x 11))) o2))

;; H
  (assert (= (rec__test__t__b (us_split_fields1 (select x 11))) o3))
...
;; H
  (assert (= o8 (store x 11 (mk___rep (mk___split_fields o5 o6 o7)))))
...
(assert
;; WP_parameter_def
 ;; File "test.ads", line 30, characters 0-0
  (not
  (= (to_rep (select (rec__test__t__c (us_split_fields1 (select x2 11))) 2)) 1)))
(check-sat)
(exit)
