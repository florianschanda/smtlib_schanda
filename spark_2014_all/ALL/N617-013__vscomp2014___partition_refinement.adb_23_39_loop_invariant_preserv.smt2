...
(declare-const dummy tindex_countB)
...
(((mk_t5b__ref (t5b__content tindex_countB)))))
...
(declare-const temp___501 index)

(declare-const temp___5011 index)

(declare-const temp___5012 index_count)
...
(declare-const temp___502 index)

(declare-const temp___5021 index)

(declare-const temp___5022 index_count)
...
(declare-const o12 (Array Int partition_index))
...
;; H
  ...
  (and
  (<= (to_rep1
      (rec__partition_refinement__interval__first p_prime__split_fields3)) j3)
  (<= j3 i2)) (= (to_rep2 (select f2 j3)) p_prime_index2))))
...
;; H
  ...
  (<= j3 (- (to_rep1
            (rec__partition_refinement__interval__first
            p_prime__split_fields3)) 1)))
  (= (to_rep2 (select f2 j3)) (to_rep2 (select f j3))))))
...
;; H
  (assert
  (and
  (and (=> (<= r3b r4b) (dynamic_property r3b r4b i2))
  (forall ((temp___505 Int))
  (=> (and (<= 0 temp___505) (<= temp___505 5))
  (=> (< i2 temp___505) (= (select f2 temp___505) (select f temp___505))))))
  (and (<= r3b i2) (<= i2 r4b))))
...
;; H
  (assert (= (to_rep2 o11) p_prime_index2))

;; H
  (assert (= o12 (store f2 i3 o11)))

;; H
  (assert (= result9 (mk_map__ref f2)))
...
;; WP_parameter_def
 ;; File "a-cofove.ads", line 807, characters 0-0
  (not (= (to_rep2 (select f3 j3)) (to_rep2 (select f j3)))))
(check-sat)
(exit)
