...
;; H
  (assert (= perm__perm_symmetric__induction_hypothesis__B_4__w__assume2
  w))

;; H
  (assert
  (and
  (= o2 (remove b (to_rep1 (rec__perm__witnesses__ib (us_split_fields1 w)))))
  (and (dynamic_invariant1 o2 true false true)
  (and (= (invariant__ o2) true)
  (and (= (to_rep2 (last o1)) (- (to_rep2 (last (rt b))) 1))
  (and
  (forall ((k Int))
  (=>
  (and (<= 1 k)
  (<= k (- (to_rep1 (rec__perm__witnesses__ib (us_split_fields1 w))) 1)))
  (= (to_rep (select (elts b) k)) (to_rep (select o k)))))
  (and
  (forall ((k Int))
  (=>
  (and (<= (+ (to_rep1 (rec__perm__witnesses__ib (us_split_fields1 w))) 1) k)
  (<= k (to_rep2 (last (rt b)))))
  (= (to_rep (select (elts b) k)) (to_rep (select o (- k 1))))))
  (forall ((k Int))
  (=>
  (and (<= (to_rep1 (rec__perm__witnesses__ib (us_split_fields1 w))) k)
  (<= k (- (to_rep2 (last (rt b))) 1)))
  (= (to_rep (select (elts b) (+ k 1))) (to_rep (select o k))))))))))))
