...
;; H
  (assert
  (and (= temp___266 (remove (mk___t values2 (mk 1 100)) o47))
  (and (dynamic_invariant4 temp___266 true false true)
  (and (= (invariant__ temp___266) true)
  (and (= (to_rep1 (last (rt temp___266))) (- (to_rep1 (last (mk 1 100))) 1))
  (and
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- o47 1)))
  (= (to_rep (select values2 k)) (to_rep (select (elts temp___266) k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ o47 1) k) (<= k (to_rep1 (last (mk 1 100)))))
  (= (to_rep (select values2 k)) (to_rep (select (elts temp___266) (- k 1))))))
  (forall ((k Int))
  (=> (and (<= o47 k) (<= k (- (to_rep1 (last (mk 1 100))) 1)))
  (= (to_rep (select values2 (+ k 1))) (to_rep (select (elts temp___266) k))))))))))))

;; H
  (assert
  (and (<= (to_rep1 (first (rt temp___266))) (- x 1))
  (<= (- x 1) (to_rep1 (last (rt temp___266))))))

;; H
  (assert (= o48 (- x 1)))

;; H
  (assert (= o49 (select (elts temp___266) o48)))
