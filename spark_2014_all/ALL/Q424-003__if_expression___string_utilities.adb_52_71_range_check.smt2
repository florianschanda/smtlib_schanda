...
(declare-fun temp___260 () map1)

(declare-fun temp___253 () map1)
...
(define-fun x213 () int__ref (mk_int__ref x27))
...
;; H
  (assert
  (=>
  (and (<= (to_rep1 (first (rt s1))) x11) (<= x11 (to_rep1 (last (rt s1)))))
  (= temp___260 lengths)))
...
;; H
  ...
  (and (<= (to_rep1 (first (rt s1))) x11) (<= x11 (to_rep1 (last (rt s1)))))
  (=>
  (and (<= (to_rep1 (first (rt s2))) x23) (<= x23 (to_rep1 (last (rt s2)))))
  (= temp___253 lengths2))))
...
;; H
  ...
  (=> (or (<= x24 temp___255) (not (= x12 temp___254)))
...
;; H
  ...
  (=>
  (and (<= (to_rep1 (first (rt s2))) x23) (<= x23 (to_rep1 (last (rt s2)))))
  (=> (= (to_rep (select (elts s1) x12)) (to_rep (select (elts s2) x24)))
  (and (= o1 (- x24 1)) (in_range1 (- x24 1)))))))
...
;; H
  ...
  (and (<= (to_rep1 (first (rt s2))) x23) (<= x23 (to_rep1 (last (rt s2)))))
  (=>
  (not (= (to_rep (select (elts s1) x12)) (to_rep (select (elts s2) x24))))
  (and (= o10 (- x24 1)) (in_range1 (- x24 1)))))))
...
;; H
  ...
  (=>
  (not (= (to_rep (select (elts s1) x12)) (to_rep (select (elts s2) x24))))
  (and (= o12 (get_length x12 o11 lengths3))
  (and (dynamic_property3 0 last4 o12)
  (= o12 (ite (and (dynamic_property1 first2 last2 x12) (dynamic_property2
              first3 last3 o11))
         (to_rep2 (get lengths3 x12 o11)) 0))))))))
...
;; WP_parameter_def
 ;; File "system.ads", line 1, characters 0-0
  (not (in_range4 (to_rep1 (last (rt s2))))))
(check-sat)
(exit)
