...
(declare-fun wakeup_time (us_rep) us_rep)

(declare-fun wakeup_time__function_guard (us_rep us_rep) Bool)

;; wakeup_time__post_axiom
  (assert true)

;; wakeup_time__def_axiom
  (assert
  (forall ((p__wakeup us_rep))
  (! (= (wakeup_time p__wakeup) p__wakeup) :pattern ((wakeup_time p__wakeup)) )))

(assert
;; WP_parameter_def
 ;; File "a-reatim.ads", line 51, characters 0-0
  (not true))
(check-sat)
(exit)
