...
;; H
  ...
  (forall ((k1 Int))
  (=> (and (<= 0 k1) (<= k1 7))
  (ite (and (= (mod2 (+ k1 1) 8) 0) (= space2 8))
  (= (to_rep (select a1 k1)) (+ (+ (+ (+ (+ (+ (+ (to_rep (select a 0))
  (to_rep (select a 1))) (to_rep (select a 2))) (to_rep (select a 3)))
  (to_rep (select a 4))) (to_rep (select a 5))) (to_rep (select a 6)))
  (to_rep (select a 7))))
  (ite (and (= (mod2 (+ k1 1) 4) 0) (<= 4 space2))
  (= (to_rep (select a1 k1)) (+ (+ (+ (to_rep (select a k1)) (to_rep
                                                             (select
                                                             a (- k1 1))))
  (to_rep (select a (- k1 2)))) (to_rep (select a (- k1 3)))))
  (ite (and (= (mod2 (+ k1 1) 2) 0) (<= 2 space2))
  (= (to_rep (select a1 k1)) (+ (to_rep (select a k1)) (to_rep
                                                       (select a (- k1 1)))))
...
;; H
  (assert
  (and (= o1 (all_elements_in a1 o))
  (= (= o1 true)
  (forall ((k1 Int))
  (=> (and (<= 0 k1) (<= k1 7))
  (and (<= (- o) (to_rep (select a1 k1))) (<= (to_rep (select a1 k1)) o)))))))

;; H
  (assert
  (= result1 (ite (and (= o1 true)
                  (=> (not (= space2 1))
                  (=> (not (= space2 2)) (= space2 4))))
             true false)))
...
;; H
  (assert (not (and (= (mod2 (+ k 1) 4) 0) (<= 4 space2))))
