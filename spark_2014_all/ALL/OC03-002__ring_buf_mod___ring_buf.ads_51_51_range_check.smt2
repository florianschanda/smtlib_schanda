...
(declare-datatypes ((t32s__ref 0)) (((mk_t32s__ref (t32s__content us_t1)))))
...
;; H
  (assert
  (bvule (to_rep (first (rt m))) (bvsub (to_rep (last (rt m))) ((_ int2bv 16) 1))))

(assert
;; WP_parameter_def
 ;; File "ring_buf.ads", line 25, characters 0-0
  (not (bvule (to_rep (first (rt m))) (to_rep (first (rt m))))))
(check-sat)
(exit)
