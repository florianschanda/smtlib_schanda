...
;; H
  (assert
  (forall ((j Int))
  (=> (and (<= len j) (<= j (- (+ len (- 64 len)) 1)))
  (= (= (nth x j) true) (= (nth ((_ int2bv 64) 0) j) true)))))

;; H
  (assert
  (= (eq_sub__logic x ((_ int2bv 64) 0) (bv2nat len_bv)
     (bv2nat (bvsub ((_ int2bv 64) 64) len_bv))) true))
...
;; WP_parameter_def
 ;; File "bitspec.ads", line 16, characters 0-0
  (not (= (bvand x (bvsub (maxvalue len) ((_ int2bv 64) 1))) x)))
(check-sat)
(exit)
