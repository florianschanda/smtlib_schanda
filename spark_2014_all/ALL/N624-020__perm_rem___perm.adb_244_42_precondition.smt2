...
;; H
  (assert
  (and (= o16 (shift_perm_l o35 o34 o32))
  (and (in_range3 o16) (=> (= (of_int o16) true) (= (is_perm o32 o34) true)))))
...
;; H
  (assert
  (and (= o30 (remove b ib2))
  (and (dynamic_invariant2 o30 true false true)
  (and (= (invariant__ o30) true)
  (and (= (to_rep2 (last (rt o30))) (- (to_rep2 (last (rt b))) 1))
  (and
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- ib2 1)))
  (= (to_rep (select (elts b) k)) (to_rep (select (elts o30) k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ ib2 1) k) (<= k (to_rep2 (last (rt b)))))
  (= (to_rep (select (elts b) k)) (to_rep (select (elts o30) (- k 1))))))
  (forall ((k Int))
  (=> (and (<= ib2 k) (<= k (- (to_rep2 (last (rt b))) 1)))
  (= (to_rep (select (elts b) (+ k 1))) (to_rep (select (elts o30) k))))))))))))
...
;; H
  ...
  (and (= (to_rep2 (last (rt o31))) (- (to_rep2 (last (rt o30))) 1))
  (and
  (forall ((k Int))
  (=>
  (and (<= 1 k)
  (<= k (- (to_rep1 (rec__perm__witnesses__ib (us_split_fields1 w))) 1)))
  (= (to_rep (select (elts o30) k)) (to_rep (select (elts o31) k)))))
  (and
  (forall ((k Int))
  (=>
  (and (<= (+ (to_rep1 (rec__perm__witnesses__ib (us_split_fields1 w))) 1) k)
  (<= k (to_rep2 (last (rt o30)))))
  (= (to_rep (select (elts o30) k)) (to_rep (select (elts o31) (- k 1))))))
  (forall ((k Int))
  (=>
  (and (<= (to_rep1 (rec__perm__witnesses__ib (us_split_fields1 w))) k)
  (<= k (- (to_rep2 (last (rt o30))) 1)))
  (= (to_rep (select (elts o30) (+ k 1))) (to_rep (select (elts o31) k))))))))))))

;; H
  (assert
  (and
  (= o29 (- (to_rep1 (rec__perm__witnesses__ia (us_split_fields1 w))) 1))
  (in_range4
  (- (to_rep1 (rec__perm__witnesses__ia (us_split_fields1 w))) 1))))
