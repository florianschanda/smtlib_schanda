...
;; H
  ...
  (=> (and (<= 1 j) (<= j 2))
  (and
  (and (neq (to_rep (select d j)) fliteral2) (lt fliteral2
  (to_rep (select kv j)))) (lt fliteral2 (to_rep (select ka j)))))))
...
;; H
  ...
  (=> (and (<= 1 i1) (<= i1 2))
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j (- i2 1))) (lt fliteral2 (to_rep (select kvp2 j)))))))
...
;; H
  ...
  (= o (adjust2triangle (to_rep (select d i2)) (to_rep (select kv i2))
       (to_rep (select ka i2))))
  (and (tqtisFinite o) (and (lt fliteral2 o) (le o (to_rep (select kv i2))))))))
...
;; H
  (assert (= kvmax1 (temp___173 fliteral2)))

;; H
  (assert (= result6 (mk_map__ref kamax)))

;; H
  (assert (= kamax1 (temp___177 fliteral2)))
...
;; WP_parameter_def
 ;; File "homothetical.ads", line 5, characters 0-0
  (not (lt fliteral2 (to_rep (select ka i7)))))
(check-sat)
