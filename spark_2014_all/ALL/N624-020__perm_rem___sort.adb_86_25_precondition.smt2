...
;; H
  (assert
  (and (= temp___242 (remove (mk___t values2 (mk 1 100)) o48))
  (and (dynamic_invariant4 temp___242 true false true)
  (and (= (invariant__ temp___242) true)
  (and (= (to_rep1 (last (rt temp___242))) (- (to_rep1 (last (mk 1 100))) 1))
  (and
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- o48 1)))
  (= (to_rep (select values2 k)) (to_rep (select (elts temp___242) k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ o48 1) k) (<= k (to_rep1 (last (mk 1 100)))))
  (= (to_rep (select values2 k)) (to_rep (select (elts temp___242) (- k 1))))))
  (forall ((k Int))
  (=> (and (<= o48 k) (<= k (- (to_rep1 (last (mk 1 100))) 1)))
  (= (to_rep (select values2 (+ k 1))) (to_rep (select (elts temp___242) k))))))))))))

;; H
  (assert
  (and (<= (to_rep1 (first (rt temp___242))) x)
  (<= x (to_rep1 (last (rt temp___242))))))
