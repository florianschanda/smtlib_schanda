...
;; H
  (assert (and (= temp___141 (- y x)) (in_range1 (- y x))))
...
;; H
  (assert (<= v1 temp___141))

;; H
  (assert
  (and (=> (<= first last) (dynamic_property first last tmp_s2))
  (and (<= 1 v2) (<= v2 (- y x)))))

(assert
;; WP_parameter_def
 ;; File "p.adb", line 4, characters 0-0
  (not (in_range1 (+ tmp_s2 1))))
(check-sat)
(exit)
