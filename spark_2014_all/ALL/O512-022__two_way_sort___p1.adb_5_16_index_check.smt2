...
(declare-fun p1__swap__t__assume () Bool)
...
(declare-fun a2 () (Array Int Bool))

(declare-fun a3 () (Array Int Bool))

(declare-fun a4 () (Array Int Bool))
...
;; H
  (assert
  (and (and (<= (to_rep a__first) i) (<= i (to_rep a__last)))
...
;; H
  (assert (= p1__swap__t__assume (select a o)))

;; H
  (assert (= p1__swap__t__assume t1))
...
;; H
  ...
  (=> (and (not (= k i)) (not (= k j)))
  (= (= (select a3 k) true) (= (select a k) true)))))))
...
;; H
  (assert (= o10 (ite (= o9 (to_int1 (select a i))) true false)))

(assert
;; WP_parameter_def
 ;; File "p1.adb", line 17, characters 0-0
