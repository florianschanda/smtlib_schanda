...
;; H
  (assert (=> (= (= (length x) 0) true) (= arrays__sum__result4 0)))
...
;; H
  ...
  (forall ((j10 Int))
  (=>
  (and (<= (to_rep1 (first (rt x))) j10) (<= j10 (to_rep1 (last (rt x)))))
  (= (to_rep (select (elts x) j10)) 0))) (= arrays__sum__result4 0)))

;; H
  (assert (= result4 arrays__sum__result4))

(assert
;; WP_parameter_def
 ;; File "system.ads", line 1, characters 0-0
  (not (<= (length x) arrays__sum__result4)))
(check-sat)
(exit)
