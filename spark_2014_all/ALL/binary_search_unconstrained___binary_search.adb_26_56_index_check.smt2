...
;; H
  (assert
  (forall ((index2 Int))
  (=> (and (<= (to_rep1 (first (rt a))) index2) (<= index2 (- left2 1)))
  (< (to_rep (select (elts a) index2)) i))))

;; H
  (assert
  (forall ((index2 Int))
  (=>
  (and (<= (to_rep1 (first (rt a))) index2)
  (<= index2 (to_rep1 (last (rt a)))))
  (=> (< right2 index2) (< i (to_rep (select (elts a) index2)))))))
