...
;; WP_parameter_def
 ;; File "fixed_string.ads", line 31, characters 0-0
  ...
  (and
  (and
  (or (< (to_rep1 (first (rt new_val))) 0)
  (<= (- i (to_rep1 change__first)) (- 2147483647 (to_rep1
                                                  (first (rt new_val))))))
  (<= (+ (- i (to_rep1 change__first)) (to_rep1 (first (rt new_val))))
  (to_rep1 (last (rt new_val)))))
  (= (to_rep (select change3 i)) (to_rep
                                 (select (elts new_val) (+ (- i (to_rep1
                                                                change__first))
                                 (to_rep1 (first (rt new_val))))))))
  (= (to_rep (select change3 i)) 0))))
(check-sat)
(exit)
