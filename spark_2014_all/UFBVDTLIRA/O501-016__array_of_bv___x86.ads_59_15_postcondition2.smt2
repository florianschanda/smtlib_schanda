...
(assert (forall ((x86__rax (_ BitVec 64))) (! (=> (dynamic_invariant x86__rax true true true true) (let ((result (ah x86__rax))) (and (= result ((_ extract 7 0) (bvurem (bvudiv (bvand x86__rax #x000000000000FF00) #x0000000000000100) ((_ int2bv 64) 256)))) (dynamic_invariant1 result true false true true)))) :pattern ((ah x86__rax)))))
...
(assert (= result rax))
(assert (= rax1 (bvor (bvand rax #xFFFFFFFFFFFF00FF) ((_ zero_extend 48) (bvmul ((_ zero_extend 8) val__) #x0100)))))
(assert (= rax1 rax2))
(assert (= rax3 rax1))
(assert (not (= (ah rax2) val__)))
(check-sat)
(exit)
