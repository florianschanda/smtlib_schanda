...
(assert (forall ((x86__rbx (_ BitVec 64))) (! (=> (dynamic_invariant x86__rbx true true true) (let ((result (bh x86__rbx))) (and (= result ((_ extract 7 0) (bvurem (bvudiv (bvand x86__rbx ((_ int2bv 64) 65280)) ((_ int2bv 64) 256)) ((_ int2bv 64) 256)))) (dynamic_invariant1 result true false true)))) :pattern ((bh x86__rbx)))))
...
(assert true)
(assert (= result rbx))
(assert (= rbx1 (bvor (bvand rbx ((_ int2bv 64) 18446744073709486335)) ((_ zero_extend 48) (bvmul ((_ zero_extend 8) val__) ((_ int2bv 16) 256))))))
(assert (= rbx1 rbx2))
(assert (= rbx3 rbx1))
(assert (not (= (bh rbx2) val__)))
(check-sat)
(exit)
