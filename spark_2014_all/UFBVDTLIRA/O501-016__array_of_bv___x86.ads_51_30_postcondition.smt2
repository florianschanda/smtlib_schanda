...
(assert (forall ((x86__rax (_ BitVec 64))) (! (=> (dynamic_invariant x86__rax true true true true) (let ((result (al x86__rax))) (and (= result ((_ extract 7 0) (bvand x86__rax #x00000000000000FF))) (dynamic_invariant1 result true false true true)))) :pattern ((al x86__rax)))))
...
(assert (= result rax))
(assert (= rax1 (bvor (bvand rax #xFFFFFFFFFFFFFF00) ((_ zero_extend 56) val__))))
(assert (= rax1 rax2))
(assert (= rax3 rax1))
(assert (not (= (bvand rax2 #xFFFFFFFFFFFFFF00) (bvand rax #xFFFFFFFFFFFFFF00))))
(check-sat)
(exit)
