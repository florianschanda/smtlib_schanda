...
(assert (and (= o2 len) (uint_in_range len)))
(assert (= bitwalker__lemmafunction__len_bv__assume ((_ int2bv 64) o2)))
(assert (= bitwalker__lemmafunction__len_bv__assume len_bv))
(assert true)
(assert (forall ((j1 Int)) (=> (and (<= len j1) (<= j1 (- (+ len (- 64 len)) 1))) (= (= (nth x j1) true) (= (nth ((_ int2bv 64) 0) j1) true)))))
(assert (= (eq_sub__logic x ((_ int2bv 64) 0) (bv2nat len_bv) (bv2nat (bvsub ((_ int2bv 64) 64) len_bv))) true))
(assert (= (eq_sub_bv__logic x ((_ int2bv 64) 0) len_bv (bvsub ((_ int2bv 64) 64) len_bv)) true))
(assert (not (= (bvand x (bvsub (maxvalue len) ((_ int2bv 64) 1))) x)))
(check-sat)
(exit)
