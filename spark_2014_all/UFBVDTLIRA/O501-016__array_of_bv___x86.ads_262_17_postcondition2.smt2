...
(assert (forall ((x86__rsi (_ BitVec 64))) (! (=> (dynamic_invariant x86__rsi true true true true) (let ((result (esi x86__rsi))) (and (= result ((_ extract 31 0) (bvand x86__rsi #x00000000FFFFFFFF))) (dynamic_invariant1 result true false true true)))) :pattern ((esi x86__rsi)))))
...
(assert (= result rsi))
(assert (= rsi1 (bvor (bvand rsi #xFFFFFFFF00000000) ((_ zero_extend 32) val__))))
(assert (= rsi1 rsi2))
(assert (= rsi3 rsi1))
(assert (not (= (esi rsi2) val__)))
(check-sat)
(exit)
