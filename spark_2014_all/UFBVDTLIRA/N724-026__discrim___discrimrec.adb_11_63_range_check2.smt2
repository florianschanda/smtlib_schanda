...
(define-fun default_initial_assumption2 ((temp___expr_195 us_rep) (temp___skip_top_level_196 Bool)) Bool (and (= (attr__constrained temp___expr_195) false) (= (to_rep4 (rec__discrimrec__register_type__option (us_split_discrs1 temp___expr_195))) 0)))
(assert (and (= 0 (to_rep4 (rec__discrimrec__register_type__option (us_split_discrs1 reg)))) (= (attr__constrained reg) true)))
(assert (not (bvule ((_ zero_extend 32) #x00000000) (bvurem (to_rep (rec__discrimrec__register_type__value_64 (us_split_fields1 reg))) #x0000000100000000))))
(check-sat)
(exit)
