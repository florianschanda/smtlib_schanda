...
(assert (=> (bvule bottom (bvsub #xFFFFFFFFFFFFFFFF range_size)) (= o (ite (bvule bottom var) (ite (bvule var (bvadd bottom range_size)) true false) false))))
(assert (=> (not (bvule bottom (bvsub #xFFFFFFFFFFFFFFFF range_size))) (= o (ite (= (ite (bvule bottom var) (distinct 1 0) false) false) (ite (bvule var (bvsub (bvsub range_size (bvsub #xFFFFFFFFFFFFFFFF bottom)) #x0000000000000001)) true false) true))))
...
(assert (exists ((n (_ BitVec 64))) (and (and (bvule #x0000000000000000 n) (bvule n range_size)) (= var (bvadd bottom n)))))
(assert (not (= x86__inrange64__result4 true)))
(check-sat)
(exit)
