...
(assert (not (= bits1 ((_ int2bv 16) 0))))
(assert (not (= bits2 ((_ int2bv 16) 0))))
(assert (bvule m2 ((_ int2bv 16) 8)))
(assert (= bits_g2 (ite (= m2 ((_ int2bv 16) 0)) ((_ int2bv 16) 0) (pow21 (bvsub m2 ((_ int2bv 16) 1))))))
(assert (= bits2 (sqr bits_g2)))
(assert (= (bvand res_g2 (bvsub (pow21 m2) ((_ int2bv 16) 1))) ((_ int2bv 16) 0)))
(assert (bvult res_g2 (pow21 ((_ int2bv 16) 8))))
(assert (= res2 (bvmul res_g2 (pow21 m2))))
(assert (bvule num2 x))
(assert (= (bvsub x num2) (sqr res_g2)))
(assert (bvule x (bvsub (sqr (bvadd res_g2 (pow21 m2))) ((_ int2bv 16) 1))))
(assert true)
(assert (= temp___2361 result7))
(assert (= temp___2362 bits2))
(assert (not (not (= bits2 ((_ int2bv 16) 0)))))
(check-sat)
(exit)
