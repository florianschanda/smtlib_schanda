...
(assert (forall ((x86__rbx (_ BitVec 64))) (! (=> (dynamic_invariant x86__rbx true true true true) (let ((result (ebx x86__rbx))) (and (= result ((_ extract 31 0) (bvand x86__rbx #x00000000FFFFFFFF))) (dynamic_invariant1 result true false true true)))) :pattern ((ebx x86__rbx)))))
...
(assert (= result rbx))
(assert (= rbx1 (bvor (bvand rbx #xFFFFFFFF00000000) ((_ zero_extend 32) val__))))
(assert (= rbx1 rbx2))
(assert (= rbx3 rbx1))
(assert (not (= (ebx rbx2) val__)))
(check-sat)
(exit)
