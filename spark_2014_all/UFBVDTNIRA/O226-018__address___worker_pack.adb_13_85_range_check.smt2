...
(define-fun dynamic_invariant ((temp___expr_254 (_ BitVec 64)) (temp___is_init_250 Bool) (temp___skip_constant_251 Bool) (temp___do_toplevel_252 Bool) (temp___do_typ_inv_253 Bool)) Bool true)
(declare-const null_address (_ BitVec 64))
(declare-const attr__ATTRIBUTE_ADDRESS Int)
(declare-sort pvoid 0)
(define-fun in_range1 ((x (_ BitVec 64))) Bool (and (bvule #x0000000000000000 x) (bvule x #xFFFFFFFFFFFFFFFF)))
(define-fun in_range_int ((x Int)) Bool (and (<= 0 x) (<= x 18446744073709551615)))
...
(assert (forall ((x pvoid)) (! (= (of_rep (to_rep x)) x) :pattern ((to_rep x)))))
(assert (forall ((x pvoid)) (! (in_range1 (to_rep x)) :pattern ((to_rep x)))))
(assert (forall ((x (_ BitVec 64))) (! (let ((y (bvurem x #x0000000000000000))) (=> (in_range1 y) (= (to_rep (of_rep x)) y))) :pattern ((to_rep (of_rep x))))))
(define-fun to_int3 ((x pvoid)) Int (bv2nat (to_rep x)))
(assert (forall ((x pvoid)) (! (in_range_int (to_int3 x)) :pattern ((to_int3 x)))))
...
(assert (forall ((x action)) (! (= (of_rep1 (to_rep1 x)) x) :pattern ((to_rep1 x)))))
(assert (forall ((x action)) (! (in_range2 (to_rep1 x)) :pattern ((to_rep1 x)))))
(assert (forall ((x Int)) (! (=> (in_range2 x) (= (to_rep1 (of_rep1 x)) x)) :pattern ((to_rep1 (of_rep1 x))))))
(declare-datatypes () ((us_split_fields (mk___split_fields (rec__worker_pack__worker_work__func action) (rec__worker_pack__worker_work__arg pvoid)))))
(define-fun us_split_fields_Func__projection ((a us_split_fields)) action (rec__worker_pack__worker_work__func a))
(define-fun us_split_fields_Arg__projection ((a us_split_fields)) pvoid (rec__worker_pack__worker_work__arg a))
(declare-datatypes () ((us_split_fields__ref (mk___split_fields__ref (us_split_fields__content us_split_fields)))))
(define-fun us_split_fields__ref___projection ((a us_split_fields__ref)) us_split_fields (us_split_fields__content a))
(declare-datatypes () ((us_rep (mk___rep (us_split_fields1 us_split_fields)))))
(define-fun us_rep___projection ((a us_rep)) us_split_fields (us_split_fields1 a))
(define-fun bool_eq ((a us_rep) (b us_rep)) Bool (ite (and (= (to_rep1 (rec__worker_pack__worker_work__func (us_split_fields1 a))) (to_rep1 (rec__worker_pack__worker_work__func (us_split_fields1 b)))) (= (to_rep (rec__worker_pack__worker_work__arg (us_split_fields1 a))) (to_rep (rec__worker_pack__worker_work__arg (us_split_fields1 b))))) true false))
...
(assert (< worker_pack__worker_work__func__first__bit worker_pack__worker_work__func__last__bit))
...
(assert (< worker_pack__worker_work__arg__first__bit worker_pack__worker_work__arg__last__bit))
(assert (<= 0 worker_pack__worker_work__arg__position))
(declare-fun user_eq4 (us_rep us_rep) Bool)
(declare-const dummy4 us_rep)
(declare-datatypes () ((worker_work__ref (mk_worker_work__ref (worker_work__content us_rep)))))
(define-fun worker_work__ref___projection ((a worker_work__ref)) us_rep (worker_work__content a))
...
(define-fun dynamic_invariant2 ((temp___expr_212 (_ BitVec 32)) (temp___is_init_208 Bool) (temp___skip_constant_209 Bool) (temp___do_toplevel_210 Bool) (temp___do_typ_inv_211 Bool)) Bool true)
(declare-fun xqueue_create ((_ BitVec 32) Int) (_ BitVec 64))
(declare-fun xqueue_create__function_guard ((_ BitVec 64) (_ BitVec 32) Int) Bool)
(assert (forall ((queuelength (_ BitVec 32))) (forall ((itemsize Int)) (! (=> (and (dynamic_invariant2 queuelength true true true true) (dynamic_invariant1 itemsize true true true true)) (in_range1 (xqueue_create queuelength itemsize))) :pattern ((xqueue_create queuelength itemsize))))))
(define-fun dynamic_invariant3 ((temp___expr_510 Int) (temp___is_init_506 Bool) (temp___skip_constant_507 Bool) (temp___do_toplevel_508 Bool) (temp___do_typ_inv_509 Bool)) Bool (=> (or (= temp___is_init_506 true) (<= 0 2)) (in_range2 temp___expr_510)))
