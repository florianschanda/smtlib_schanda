(set-info :smt-lib-version 2.5)
(set-info :license |https://www.gnu.org/licenses/gpl-3.0.html|)
(set-logic UFDTNIRA)
(set-info :source |Benchmarks generated from industrial code by Florian Schanda using SPARK 2014 and an SMTLIB Obfuscation Script|)
(set-info :category industrial)
(set-info :status unsat)

(define-fun spoon_1 ((kitten_1 Int) (kitten_2 Int)) Int (ite (>= kitten_1 0) (div kitten_1 kitten_2) (- (div (- kitten_1) kitten_2))))
(define-fun spoon_2 ((kitten_3 Int) (kitten_4 Int)) Int (ite (>= kitten_3 0) (mod kitten_3 kitten_4) (- (mod (- kitten_3) kitten_4))))
(declare-datatypes () ((potato_1 (mk_potato_1))))
(declare-sort potato_2 0)
(declare-fun spoon_3 (potato_2 potato_2) Bool)
(declare-const cat_4 potato_2)
(declare-sort potato_3 0)
(declare-datatypes () ((potato_4 (mk_potato_4 (get_potato_4_a potato_3)))))
(declare-sort potato_5 0)
(declare-datatypes () ((potato_6 (mk_potato_6 (get_potato_6_a Int)))))
(declare-datatypes () ((potato_7 (mk_potato_7 (get_potato_7_a Bool)))))
(declare-datatypes () ((potato_8 (mk_potato_8 (get_potato_8_a Real)))))
(declare-datatypes () ((potato_9 (mk_potato_9 (get_potato_9_a potato_2)))))
(define-fun spoon_5 ((kitten_5 potato_6)) Int (get_potato_6_a kitten_5))
(define-fun spoon_6 ((kitten_6 potato_7)) Bool (get_potato_7_a kitten_6))
(define-fun spoon_7 ((kitten_7 potato_8)) Real (get_potato_8_a kitten_7))
(define-fun spoon_8 ((kitten_8 potato_9)) potato_2 (get_potato_9_a kitten_8))
(define-fun spoon_9 ((kitten_9 Int) (kitten_10 Int)) Int (ite (< 0 kitten_10) (mod kitten_9 kitten_10) (+ (mod kitten_9 kitten_10) kitten_10)))
(declare-sort potato_10 0)
(declare-fun spoon_10 (potato_10) Int)
(assert (forall ((kitten_11 potato_10)) (and (<= 0 (spoon_10 kitten_11)) (<= (spoon_10 kitten_11) 999))))
(define-fun spoon_11 ((kitten_12 Int)) Bool (and (<= 0 kitten_12) (<= kitten_12 999)))
(declare-fun spoon_12 (Int) potato_5)
(declare-fun spoon_13 (potato_5) Bool)
(declare-fun spoon_14 (potato_5) Int)
(declare-fun spoon_15 (potato_10 potato_10) Bool)
(declare-const cat_16 potato_10)
(declare-datatypes () ((potato_11 (mk_potato_11 (get_potato_11_a potato_10)))))
(define-fun spoon_17 ((kitten_13 potato_11)) potato_10 (get_potato_11_a kitten_13))
(define-fun spoon_18 ((kitten_14 potato_10)) Int (spoon_10 kitten_14))
(declare-fun spoon_19 (Int) potato_10)
(assert (forall ((kitten_15 potato_10)) (! (= (spoon_19 (spoon_18 kitten_15)) kitten_15) :pattern ((spoon_18 kitten_15)))))
(assert (forall ((kitten_16 potato_10)) (! (spoon_11 (spoon_18 kitten_16)) :pattern ((spoon_18 kitten_16)))))
(assert (forall ((kitten_17 Int)) (! (=> (spoon_11 kitten_17) (= (spoon_18 (spoon_19 kitten_17)) kitten_17)) :pattern ((spoon_18 (spoon_19 kitten_17))))))
(declare-datatypes () ((potato_12 (mk_potato_12 (get_potato_12_a potato_10) (get_potato_12_b potato_10)))))
(define-fun spoon_20 ((kitten_18 potato_12)) potato_10 (get_potato_12_a kitten_18))
(define-fun spoon_21 ((kitten_19 potato_12)) potato_10 (get_potato_12_b kitten_19))
(declare-datatypes () ((potato_13 (mk_potato_13 (get_potato_13_a potato_12)))))
(define-fun spoon_22 ((kitten_20 potato_13)) potato_12 (get_potato_13_a kitten_20))
(declare-datatypes () ((potato_14 (mk_potato_14 (get_potato_14_a potato_12)))))
(define-fun spoon_23 ((kitten_21 potato_14)) potato_12 (get_potato_14_a kitten_21))
(define-fun spoon_24 ((kitten_22 potato_14) (kitten_23 potato_14)) Bool (ite (and (= (spoon_18 (get_potato_12_a (get_potato_14_a kitten_22))) (spoon_18 (get_potato_12_a (get_potato_14_a kitten_23)))) (= (spoon_18 (get_potato_12_b (get_potato_14_a kitten_22))) (spoon_18 (get_potato_12_b (get_potato_14_a kitten_23))))) true false))
(declare-const cat_25 Int)
(declare-fun spoon_26 (potato_14) Int)
(declare-const cat_27 Int)
(declare-fun spoon_28 (potato_14) Int)
(assert (<= 0 cat_25))
(assert (forall ((kitten_24 potato_14)) (<= 0 (spoon_26 kitten_24))))
(assert (<= 0 cat_27))
(assert (forall ((kitten_25 potato_14)) (<= 0 (spoon_28 kitten_25))))
(declare-const cat_29 Int)
(declare-const cat_30 Int)
(declare-const cat_31 Int)
(assert (<= 0 cat_29))
(assert (< cat_29 cat_30))
(assert (<= 0 cat_31))
(declare-const cat_32 Int)
(declare-const cat_33 Int)
(declare-const cat_34 Int)
(assert (<= 0 cat_32))
(assert (< cat_32 cat_33))
(assert (<= 0 cat_34))
(declare-fun spoon_35 (potato_14 potato_14) Bool)
(declare-const cat_36 potato_14)
(declare-datatypes () ((potato_15 (mk_potato_15 (get_potato_15_a potato_14)))))
(define-fun spoon_37 ((kitten_26 potato_15)) potato_14 (get_potato_15_a kitten_26))
(declare-sort potato_16 0)
(declare-fun spoon_38 (potato_16) Int)
(assert (forall ((kitten_27 potato_16)) (and (<= 0 (spoon_38 kitten_27)) (<= (spoon_38 kitten_27) 150000))))
(define-fun spoon_39 ((kitten_28 Int)) Bool (and (<= 0 kitten_28) (<= kitten_28 150000)))
(declare-fun spoon_40 (Int) potato_5)
(declare-fun spoon_41 (potato_5) Bool)
(declare-fun spoon_42 (potato_5) Int)
(declare-fun spoon_43 (potato_16 potato_16) Bool)
(declare-const cat_44 potato_16)
(declare-datatypes () ((potato_17 (mk_potato_17 (get_potato_17_a potato_16)))))
(define-fun spoon_45 ((kitten_29 potato_17)) potato_16 (get_potato_17_a kitten_29))
(define-fun spoon_46 ((kitten_30 potato_16)) Int (spoon_38 kitten_30))
(declare-fun spoon_47 (Int) potato_16)
(assert (forall ((kitten_31 potato_16)) (! (= (spoon_47 (spoon_46 kitten_31)) kitten_31) :pattern ((spoon_46 kitten_31)))))
(assert (forall ((kitten_32 potato_16)) (! (spoon_39 (spoon_46 kitten_32)) :pattern ((spoon_46 kitten_32)))))
(assert (forall ((kitten_33 Int)) (! (=> (spoon_39 kitten_33) (= (spoon_46 (spoon_47 kitten_33)) kitten_33)) :pattern ((spoon_46 (spoon_47 kitten_33))))))
(declare-datatypes () ((potato_18 (mk_potato_18 (get_potato_18_a potato_16) (get_potato_18_b potato_16)))))
(define-fun spoon_48 ((kitten_34 potato_18)) potato_16 (get_potato_18_a kitten_34))
(define-fun spoon_49 ((kitten_35 potato_18)) potato_16 (get_potato_18_b kitten_35))
(declare-datatypes () ((potato_19 (mk_potato_19 (get_potato_19_a potato_18)))))
(define-fun spoon_50 ((kitten_36 potato_19)) potato_18 (get_potato_19_a kitten_36))
(declare-datatypes () ((potato_20 (mk_potato_20 (get_potato_20_a potato_18)))))
(define-fun spoon_51 ((kitten_37 potato_20)) potato_18 (get_potato_20_a kitten_37))
(define-fun spoon_52 ((kitten_38 potato_20) (kitten_39 potato_20)) Bool (ite (and (= (spoon_46 (get_potato_18_a (get_potato_20_a kitten_38))) (spoon_46 (get_potato_18_a (get_potato_20_a kitten_39)))) (= (spoon_46 (get_potato_18_b (get_potato_20_a kitten_38))) (spoon_46 (get_potato_18_b (get_potato_20_a kitten_39))))) true false))
(declare-const cat_53 Int)
(declare-fun spoon_54 (potato_20) Int)
(declare-const cat_55 Int)
(declare-fun spoon_56 (potato_20) Int)
(assert (<= 0 cat_53))
(assert (forall ((kitten_40 potato_20)) (<= 0 (spoon_54 kitten_40))))
(assert (<= 0 cat_55))
(assert (forall ((kitten_41 potato_20)) (<= 0 (spoon_56 kitten_41))))
(declare-const cat_57 Int)
(declare-const cat_58 Int)
(declare-const cat_59 Int)
(assert (<= 0 cat_57))
(assert (< cat_57 cat_58))
(assert (<= 0 cat_59))
(declare-const cat_60 Int)
(declare-const cat_61 Int)
(declare-const cat_62 Int)
(assert (<= 0 cat_60))
(assert (< cat_60 cat_61))
(assert (<= 0 cat_62))
(declare-fun spoon_63 (potato_20 potato_20) Bool)
(declare-const cat_64 potato_20)
(declare-datatypes () ((potato_21 (mk_potato_21 (get_potato_21_a potato_20)))))
(define-fun spoon_65 ((kitten_42 potato_21)) potato_20 (get_potato_21_a kitten_42))
(declare-const cat_66 potato_20)
(declare-const cat_67 Int)
(declare-const cat_68 Int)
(define-fun spoon_69 ((kitten_43 Int) (kitten_44 Bool) (kitten_45 Bool) (kitten_46 Bool) (kitten_47 Bool)) Bool (=> (or (= kitten_44 true) (<= 0 150000)) (spoon_39 kitten_43)))
(define-fun spoon_70 ((kitten_48 Int) (kitten_49 Bool) (kitten_50 Bool) (kitten_51 Bool) (kitten_52 Bool)) Bool (=> (or (= kitten_49 true) (<= 0 999)) (spoon_11 kitten_48)))
(declare-const cat_71 potato_16)
(assert (= (spoon_38 cat_71) 99801))
(declare-const cat_72 potato_16)
(assert (= (spoon_38 cat_72) 99900))
(declare-const cat_73 potato_16)
(declare-const cat_74 potato_16)
(declare-const cat_75 Int)
(declare-const cat_76 potato_10)
(declare-const cat_77 potato_16)
(declare-const cat_78 potato_16)
(declare-const cat_79 potato_16)
(declare-const cat_80 potato_16)
(declare-const cat_81 potato_16)
(declare-const cat_82 potato_16)
(declare-const cat_83 potato_16)
(declare-const cat_84 potato_16)
(declare-const cat_85 potato_16)
(declare-const cat_86 potato_16)
(declare-const cat_87 potato_16)
(declare-const cat_88 potato_16)
(define-fun spoon_89 () potato_18 (mk_potato_18 cat_83 cat_84))
(define-fun spoon_90 () potato_18 (mk_potato_18 cat_79 cat_80))
(assert (= cat_77 cat_73))
(assert (= cat_78 cat_74))
(assert (= spoon_90 (get_potato_20_a cat_66)))
(assert (=> (< 99801 (spoon_46 cat_79)) (= cat_81 cat_79)))
(assert (=> (< 99801 (spoon_46 cat_79)) (= cat_82 cat_80)))
(assert (=> (< 99801 (spoon_46 cat_79)) (= cat_71 cat_83)))
(assert (=> (< 99801 (spoon_46 cat_79)) (= cat_80 cat_84)))
(assert (=> (not (< 99801 (spoon_46 cat_79))) (= spoon_89 spoon_90)))
(assert (=> (< 99900 (spoon_46 cat_84)) (= cat_85 cat_83)))
(assert (=> (< 99900 (spoon_46 cat_84)) (= cat_86 cat_84)))
(assert (=> (< 99900 (spoon_46 cat_84)) (= cat_83 cat_87)))
(assert (=> (< 99900 (spoon_46 cat_84)) (= cat_72 cat_88)))
(assert (=> (not (< 99900 (spoon_46 cat_84))) (= (mk_potato_18 cat_87 cat_88) spoon_89)))
(assert (and (= cat_75 (spoon_1 (spoon_46 cat_88) 100)) (spoon_11 (spoon_1 (spoon_46 cat_88) 100))))
(assert (= (spoon_18 cat_76) cat_75))
(assert (not (not (= 100 0))))
(check-sat)
(exit)
