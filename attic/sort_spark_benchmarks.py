...
def needs_quoting(symbol):
    return re.match("^[a-zA-Z~!@$%^&*_+=<>.?/-][a-zA-Z0-9~!@$%^&*_+=<>.?/-]*$", symbol) is None
...
def process(task):
    ...
    if len(bench_status) == 0:
        bench_status = "unknown"
    elif len(bench_status) == 1:
    ...
    if task["dir"] is not None:
        ...
        rv.append("processing %s" % bench_name)
        bench_dir = os.path.dirname(task["file"])
...
def main():
    ...
    if options.file is None:
        src = ("Benchmarks generated by Florian Schanda from the public "
        ...
        src = ("Benchmarks generated by CBMC and post-processed to set "
               "appropriate logic and remove get-value commands.")
        for line in process({"file"   : options.file,
                             "dir"    : None,
                             "source" : src}):
            print line
