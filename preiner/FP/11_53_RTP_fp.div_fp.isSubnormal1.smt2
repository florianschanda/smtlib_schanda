...
(define-fun IC ((s FP)) Bool (or (fp.isNormal (fp.div RNE s posMaxSubnormal)) (and (fp.isNormal (fp.div RNE s s)) (not (fp.isNormal (fp.div RNE s posMaxNormal))))))

(assert
 (distinct
  (IC s)
  (exists ((x FP)) (fp.isSubnormal (fp.div RTP s x)))
 )
)
