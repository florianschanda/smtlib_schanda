...
(define-fun IC ((s FP)) Bool (and (not (fp.isZero s)) (not (fp.isInfinite s)) (not (fp.isNaN s)) (ite (fp.isNormal s) true (not ( fp.isSubnormal (fp.mul (ite (or (= R RTN) (= R RTP)) RNE R) s posMaxNormal))))))

(assert
 (distinct
  (IC s)
  (exists ((x FP)) (fp.isNormal (fp.mul RNA x s)))
 )
)
