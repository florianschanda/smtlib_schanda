...
(define-fun IC ((s FP) (t FP)) Bool (or (fp.isZero (fp.fma R s t (fp.neg (fp.mul RTP s t)))) (fp.isZero (fp.fma R s t (fp.neg (fp.mul RTN s t))))))

(assert
 (distinct
  (IC s u)
  (exists ((x FP)) (fp.isZero (fp.fma RTP s u x)))
 )
)
