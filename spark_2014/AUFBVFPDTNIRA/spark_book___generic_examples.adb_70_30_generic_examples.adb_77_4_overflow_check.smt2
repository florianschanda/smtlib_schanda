...
;; inversion_axiom
  (assert
  (forall ((x percent)) (! (= (of_rep (to_rep x)) x) :pattern ((to_rep x)) )))

;; range_axiom
  (assert
  (forall ((x percent)) (! (in_range3 (to_rep x)) :pattern ((to_rep x)) )))
...
(declare-datatypes ()
((map__ref (mk_map__ref (map__content (Array Int percent))))))
...
(declare-fun concat1 ((Array Int percent) Int Int (Array Int percent) Int
...
(define-fun bool_eq ((a (Array Int percent)) (a__first Int) (a__last Int)
...
(declare-fun compare ((Array Int percent) Int Int (Array Int percent) Int
  Int) Int)
...
(declare-fun dummy3 () component_type)
...
(define-fun of_array ((a (Array Int percent)) (f Int)
...
;; object__component__size_axiom
  (assert
  (forall ((a (Array Int percent))) (<= 0 (object__component__size a))))
...
;; H
  (assert
  (and (=> (<= 0 2147483647) (in_range2 result__2))
  (and (<= (to_rep1 (first (rt source))) index2)
  (<= index2 (to_rep1 (last (rt source)))))))

;; H
  (assert (< pattern (to_rep (select (elts source) index2))))
