...
;; H
  ...
  (forall ((j Int))
  (=>
  (and (<= (to_rep (first (rt list))) j) (<= j (to_rep (last (rt list)))))
  (forall ((k Int))
  (=>
  (and (<= (to_rep (first (rt list))) k) (<= k (to_rep (last (rt list)))))
  (=> (= (to_rep (select (elts list) j)) (to_rep (select (elts list) k)))
  (= j k)))))))))
...
;; H
  (assert (=> (< (to_rep (last (rt list))) position1) (= result1 true)))

;; H
  (assert
  (=> (not (< (to_rep (last (rt list))) position1))
  (and (<= (to_rep (first (rt list))) position1)
  (<= position1 (to_rep (last (rt list)))))))

;; H
  (assert
  (=> (not (< (to_rep (last (rt list))) position1)) (= o1 position1)))

;; H
  (assert
  (=> (not (< (to_rep (last (rt list))) position1))
  (= o2 (select (elts list) o1))))

;; H
  (assert
  (=> (not (< (to_rep (last (rt list))) position1)) (= o3 (to_rep o2))))

;; H
  (assert
  (=> (not (< (to_rep (last (rt list))) position1))
  (= result1 (ite (= o3 value) true false))))
...
;; WP_parameter_def
 ;; File "searches.ads", line 8, characters 0-0
  (not (in_range1 (+ (to_rep (last (rt list))) 1))))
(check-sat)
