...
(define-fun position12 () int__ref (mk_int__ref position8))

(define-fun position13 () int__ref (mk_int__ref position6))
...
;; H
  ...
  (exists ((index Int))
  (and
  (and (<= (to_rep (first (rt list))) index)
  (<= index (to_rep (last (rt list)))))
  (= (to_rep (select (elts list) index)) value))))
...
;; H
  ...
  (<= position3 (to_rep (last (rt list)))))
  (forall ((j Int))
  (=> (and (<= (to_rep (first (rt list))) j) (<= j (- position3 1)))
  (not (= (to_rep (select (elts list) j)) value)))))
...
;; H
  (assert (= position13 (mk_int__ref position4)))

;; H
  (assert (= position7 position5))

;; H
  (assert (= position12 position13))

;; H
  (assert (= position9 position7))

;; H
  (assert (= (mk_int__ref position10) position12))

;; H
  (assert (= position11 position9))

;; H
  (assert (<= (to_rep (first (rt list))) position10))

;; H
  (assert (<= position10 (to_rep (last (rt list)))))

(assert
;; WP_parameter_def
 ;; File "searches.ads", line 21, characters 0-0
  (not (<= (to_rep (first (rt list))) position10)))
(check-sat)
(exit)
