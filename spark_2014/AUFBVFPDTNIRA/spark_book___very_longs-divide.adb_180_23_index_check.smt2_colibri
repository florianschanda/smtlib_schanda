;; produced by colibri_gnatprove.drv ;;
(set-info :source |VC generated by SPARK 2014|)
(set-info :smt-lib-version 2.5)
(set-info :category industrial)
(set-info :status unknown)
;;; generated by SMT-LIB2 driver
;;; SMT-LIB2 driver: bit-vectors, common part
;;; SMT-LIB2: integer arithmetic
(define-fun uint_in_range ((i Int)) Bool (and (<= 0 i) (<= i 255)))

(declare-fun abstr () (_ BitVec 8))

(declare-fun abstr1 () (_ BitVec 8))

(declare-fun abstr2 () (_ BitVec 8))

(declare-fun abstr3 () (_ BitVec 8))

(declare-fun abstr4 () (_ BitVec 8))

(define-fun nth_bv ((x (_ BitVec 8))
  (i (_ BitVec 8))) Bool (ite (not (= (bvand (bvlshr x i) #x01) #x00)) true
                         false))

(declare-fun abstr5 () Bool)

(define-fun eq_sub ((a (_ BitVec 8)) (b (_ BitVec 8)) (i Int)
  (n Int)) Bool abstr5)

(define-fun bv_min ((x (_ BitVec 8))
  (y (_ BitVec 8))) (_ BitVec 8) (ite (bvule x y) x y))

(define-fun bv_max ((x (_ BitVec 8))
  (y (_ BitVec 8))) (_ BitVec 8) (ite (bvule x y) y x))

(define-fun uint_in_range1 ((i Int)) Bool (and (<= 0 i) (<= i 65535)))

(declare-fun abstr6 () (_ BitVec 16))

(declare-fun abstr7 () (_ BitVec 16))

(declare-fun abstr8 () (_ BitVec 16))

(declare-fun abstr9 () (_ BitVec 16))

(declare-fun abstr10 () (_ BitVec 16))

(define-fun nth_bv1 ((x (_ BitVec 16))
  (i (_ BitVec 16))) Bool (ite (not (= (bvand (bvlshr x i) #x0001) #x0000))
                          true false))

(declare-fun abstr11 () Bool)

(define-fun eq_sub1 ((a (_ BitVec 16)) (b (_ BitVec 16)) (i Int)
  (n Int)) Bool abstr11)

(define-fun bv_min1 ((x (_ BitVec 16))
  (y (_ BitVec 16))) (_ BitVec 16) (ite (bvule x y) x y))

(define-fun bv_max1 ((x (_ BitVec 16))
  (y (_ BitVec 16))) (_ BitVec 16) (ite (bvule x y) y x))

(define-fun to_int1 ((b Bool)) Int (ite (= b true) 1 0))

(define-fun of_int ((i Int)) Bool (ite (= i 0) false true))

(define-fun in_range ((x Int)) Bool (or (= x 0) (= x 1)))

(define-fun in_range1 ((x Int)) Bool (and (<= (- 2147483648) x)
                                     (<= x 2147483647)))

(define-fun in_range2 ((x Int)) Bool (and (<= 1 x) (<= x 65536)))

(define-fun dynamic_invariant ((temp___expr_204 Int)
  (temp___is_init_201 Bool) (temp___skip_constant_202 Bool)
  (temp___do_toplevel_203 Bool)) Bool (=>
                                      (or (= temp___is_init_201 true)
                                      (<= 1 65536)) (in_range2
                                      temp___expr_204)))

(declare-fun attr__ATTRIBUTE_MODULUS () (_ BitVec 8))

(define-fun dynamic_property ((first_int Int) (last_int Int)
  (x Int)) Bool (and (<= first_int x) (<= x last_int)))

(define-fun dynamic_property1 ((range_first Int) (range_last Int) (low Int)
  (high Int)) Bool (and (in_range1 low)
                   (and (in_range1 high)
                   (=> (<= low high)
                   (and (dynamic_property range_first range_last low)
                   (dynamic_property range_first range_last high))))))

(declare-fun value__size () Int)

(declare-fun value__component__size () Int)

(declare-fun value__alignment () Int)

;; value__size_axiom
  (assert (<= 0 value__size))

;; value__component__size_axiom
  (assert (<= 0 value__component__size))

;; value__alignment_axiom
  (assert (<= 0 value__alignment))

(declare-fun value__size1 () Int)

(declare-fun value__alignment1 () Int)

;; value__size_axiom
  (assert (<= 0 value__size1))

;; value__alignment_axiom
  (assert (<= 0 value__alignment1))

(declare-fun very_longs__very_long__length__first__bit () Int)

(declare-fun very_longs__very_long__length__last__bit () Int)

(declare-fun very_longs__very_long__length__position () Int)

;; very_longs__very_long__length__first__bit_axiom
  (assert (<= 0 very_longs__very_long__length__first__bit))

;; very_longs__very_long__length__last__bit_axiom
  (assert
  (< very_longs__very_long__length__first__bit very_longs__very_long__length__last__bit))

;; very_longs__very_long__length__position_axiom
  (assert (<= 0 very_longs__very_long__length__position))

(declare-fun very_longs__very_long__long_digits__first__bit () Int)

(declare-fun very_longs__very_long__long_digits__last__bit () Int)

(declare-fun very_longs__very_long__long_digits__position () Int)

;; very_longs__very_long__long_digits__first__bit_axiom
  (assert (<= 0 very_longs__very_long__long_digits__first__bit))

;; very_longs__very_long__long_digits__last__bit_axiom
  (assert
  (< very_longs__very_long__long_digits__first__bit very_longs__very_long__long_digits__last__bit))

;; very_longs__very_long__long_digits__position_axiom
  (assert (<= 0 very_longs__very_long__long_digits__position))

(declare-fun attr__ATTRIBUTE_ADDRESS () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS1 () Int)

(declare-fun attr__ATTRIBUTE_MODULUS1 () (_ BitVec 16))

(define-fun dynamic_invariant1 ((temp___expr_312 (_ BitVec 16))
  (temp___is_init_309 Bool) (temp___skip_constant_310 Bool)
  (temp___do_toplevel_311 Bool)) Bool true)

(define-fun in_range3 ((x Int)) Bool (and (<= 0 x) (<= x 2147483647)))

(define-fun dynamic_invariant2 ((temp___expr_33 Int) (temp___is_init_30 Bool)
  (temp___skip_constant_31 Bool)
  (temp___do_toplevel_32 Bool)) Bool (=>
                                     (or (= temp___is_init_30 true)
                                     (<= 0 2147483647)) (in_range3
                                     temp___expr_33)))

(declare-fun last () Int)

(define-fun dynamic_property2 ((first_int Int) (last_int Int)
  (x Int)) Bool (and (<= first_int x) (<= x last_int)))

(define-fun dynamic_invariant3 ((temp___expr_690 Int)
  (temp___is_init_687 Bool) (temp___skip_constant_688 Bool)
  (temp___do_toplevel_689 Bool)) Bool (=>
                                      (or (= temp___is_init_687 true)
                                      (<= 1 last)) (dynamic_property2 1
                                      last temp___expr_690)))

(declare-fun attr__ATTRIBUTE_ADDRESS2 () Int)

(define-fun dynamic_property3 ((first_int Int) (last_int Int)
  (x Int)) Bool (and (<= first_int x) (<= x last_int)))

(define-fun dynamic_property4 ((range_first Int) (range_last Int) (low Int)
  (high Int)) Bool (and (in_range1 low)
                   (and (in_range1 high)
                   (=> (<= low high)
                   (and (dynamic_property3 range_first range_last low)
                   (dynamic_property3 range_first range_last high))))))

(declare-fun value__size2 () Int)

(declare-fun value__component__size1 () Int)

(declare-fun value__alignment2 () Int)

;; value__size_axiom
  (assert (<= 0 value__size2))

;; value__component__size_axiom
  (assert (<= 0 value__component__size1))

;; value__alignment_axiom
  (assert (<= 0 value__alignment2))

(declare-fun value__size3 () Int)

(declare-fun value__alignment3 () Int)

;; value__size_axiom
  (assert (<= 0 value__size3))

;; value__alignment_axiom
  (assert (<= 0 value__alignment3))

(declare-fun very_longs__very_long__length__first__bit1 () Int)

(declare-fun very_longs__very_long__length__last__bit1 () Int)

(declare-fun very_longs__very_long__length__position1 () Int)

;; very_longs__very_long__length__first__bit_axiom
  (assert (<= 0 very_longs__very_long__length__first__bit1))

;; very_longs__very_long__length__last__bit_axiom
  (assert
  (< very_longs__very_long__length__first__bit1 very_longs__very_long__length__last__bit1))

;; very_longs__very_long__length__position_axiom
  (assert (<= 0 very_longs__very_long__length__position1))

(declare-fun very_longs__very_long__long_digits__first__bit1 () Int)

(declare-fun very_longs__very_long__long_digits__last__bit1 () Int)

(declare-fun very_longs__very_long__long_digits__position1 () Int)

;; very_longs__very_long__long_digits__first__bit_axiom
  (assert (<= 0 very_longs__very_long__long_digits__first__bit1))

;; very_longs__very_long__long_digits__last__bit_axiom
  (assert
  (< very_longs__very_long__long_digits__first__bit1 very_longs__very_long__long_digits__last__bit1))

;; very_longs__very_long__long_digits__position_axiom
  (assert (<= 0 very_longs__very_long__long_digits__position1))

(declare-fun attr__ATTRIBUTE_ADDRESS3 () Int)

(define-fun dynamic_property5 ((first_int Int) (last_int Int)
  (x Int)) Bool (and (<= first_int x) (<= x last_int)))

(define-fun dynamic_property6 ((range_first Int) (range_last Int) (low Int)
  (high Int)) Bool (and (in_range1 low)
                   (and (in_range1 high)
                   (=> (<= low high)
                   (and (dynamic_property5 range_first range_last low)
                   (dynamic_property5 range_first range_last high))))))

(declare-fun value__size4 () Int)

(declare-fun value__component__size2 () Int)

(declare-fun value__alignment4 () Int)

;; value__size_axiom
  (assert (<= 0 value__size4))

;; value__component__size_axiom
  (assert (<= 0 value__component__size2))

;; value__alignment_axiom
  (assert (<= 0 value__alignment4))

(declare-fun value__size5 () Int)

(declare-fun value__alignment5 () Int)

;; value__size_axiom
  (assert (<= 0 value__size5))

;; value__alignment_axiom
  (assert (<= 0 value__alignment5))

(declare-fun very_longs__very_long__length__first__bit2 () Int)

(declare-fun very_longs__very_long__length__last__bit2 () Int)

(declare-fun very_longs__very_long__length__position2 () Int)

;; very_longs__very_long__length__first__bit_axiom
  (assert (<= 0 very_longs__very_long__length__first__bit2))

;; very_longs__very_long__length__last__bit_axiom
  (assert
  (< very_longs__very_long__length__first__bit2 very_longs__very_long__length__last__bit2))

;; very_longs__very_long__length__position_axiom
  (assert (<= 0 very_longs__very_long__length__position2))

(declare-fun very_longs__very_long__long_digits__first__bit2 () Int)

(declare-fun very_longs__very_long__long_digits__last__bit2 () Int)

(declare-fun very_longs__very_long__long_digits__position2 () Int)

;; very_longs__very_long__long_digits__first__bit_axiom
  (assert (<= 0 very_longs__very_long__long_digits__first__bit2))

;; very_longs__very_long__long_digits__last__bit_axiom
  (assert
  (< very_longs__very_long__long_digits__first__bit2 very_longs__very_long__long_digits__last__bit2))

;; very_longs__very_long__long_digits__position_axiom
  (assert (<= 0 very_longs__very_long__long_digits__position2))

(declare-fun attr__ATTRIBUTE_ADDRESS4 () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS5 () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS6 () Int)

(declare-fun j () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS7 () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS8 () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS9 () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS10 () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS11 () Int)

(declare-fun r126b () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS12 () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS13 () Int)

(define-fun dynamic_invariant4 ((temp___expr_210 (_ BitVec 8))
  (temp___is_init_207 Bool) (temp___skip_constant_208 Bool)
  (temp___do_toplevel_209 Bool)) Bool true)

(declare-fun n () Int)

(declare-fun q_hat () (_ BitVec 16))

(declare-fun current_borrow () (_ BitVec 16))

(declare-fun carry () (_ BitVec 16))

(declare-fun product () (_ BitVec 16))

(declare-fun temporary_digit () (_ BitVec 16))

(declare-fun future_borrow () (_ BitVec 16))

(declare-fun divisor_index () Int)

(declare-fun temp___1281 () (_ BitVec 16))

(declare-fun temp___1277 () (_ BitVec 16))

(declare-fun temp___1276 () (_ BitVec 16))

(declare-fun temp___1275 () (_ BitVec 16))

(declare-fun temp___1274 () (_ BitVec 16))

(declare-fun o () Int)

(declare-fun o1 () (_ BitVec 8))

(declare-fun o2 () (_ BitVec 16))

(declare-fun o3 () (_ BitVec 16))

(declare-fun o4 () (_ BitVec 16))

(declare-fun o5 () (_ BitVec 16))

(declare-fun o6 () Int)

(declare-fun temp___1267 () Int)

(declare-fun o7 () Int)

(declare-fun o8 () (_ BitVec 8))

(declare-fun o9 () (_ BitVec 16))

(declare-fun o10 () Int)

(declare-fun temp___1271 () Int)

(declare-fun o11 () Int)

(declare-fun temp___1269 () Int)

(declare-fun o12 () Int)

(declare-fun o13 () (_ BitVec 8))

(declare-fun o14 () (_ BitVec 16))

(declare-fun o15 () (_ BitVec 16))

(declare-fun o16 () (_ BitVec 16))

(declare-fun o17 () (_ BitVec 16))

(declare-fun o18 () (_ BitVec 16))

(declare-fun o19 () (_ BitVec 8))

(declare-fun o20 () Int)

(declare-fun temp___1285 () Int)

(declare-fun o21 () Int)

(declare-fun o22 () (_ BitVec 8))

(declare-fun o23 () (_ BitVec 16))

(declare-fun temp___1289 () Int)

(declare-fun temp___1287 () Int)

(declare-fun o24 () Int)

(declare-fun o25 () (_ BitVec 8))

(declare-fun o26 () (_ BitVec 16))

(declare-fun o27 () (_ BitVec 16))

(declare-fun o28 () (_ BitVec 16))

(declare-fun o29 () (_ BitVec 16))

(declare-fun o30 () (_ BitVec 16))

(declare-fun o31 () (_ BitVec 8))

(declare-fun product1 () (_ BitVec 16))

(declare-fun temporary_digit1 () (_ BitVec 16))

(declare-fun future_borrow1 () (_ BitVec 16))

(declare-fun carry1 () (_ BitVec 16))

(declare-fun current_borrow1 () (_ BitVec 16))

(declare-fun divisor_index1 () Int)

(declare-fun current_borrow2 () (_ BitVec 16))

(declare-fun carry2 () (_ BitVec 16))

(declare-fun product2 () (_ BitVec 16))

(declare-fun temporary_digit2 () (_ BitVec 16))

(declare-fun future_borrow2 () (_ BitVec 16))

(declare-fun divisor_index2 () Int)

(declare-fun product3 () (_ BitVec 16))

(declare-fun carry3 () (_ BitVec 16))

(declare-fun temporary_digit3 () (_ BitVec 16))

(declare-fun result () Bool)

(declare-fun future_borrow3 () (_ BitVec 16))

(declare-fun future_borrow4 () (_ BitVec 16))

(declare-fun current_borrow3 () (_ BitVec 16))

(declare-fun current_borrow4 () (_ BitVec 16))

(declare-fun carry4 () (_ BitVec 16))

(declare-fun product4 () (_ BitVec 16))

(declare-fun temporary_digit4 () (_ BitVec 16))

(declare-fun future_borrow5 () (_ BitVec 16))

(declare-fun divisor_index3 () Int)

(declare-fun current_borrow5 () (_ BitVec 16))

(declare-fun carry5 () (_ BitVec 16))

(declare-fun product5 () (_ BitVec 16))

(declare-fun temporary_digit5 () (_ BitVec 16))

(declare-fun future_borrow6 () (_ BitVec 16))

(declare-fun divisor_index4 () Int)

(declare-fun result1 () Bool)

(declare-fun future_borrow7 () (_ BitVec 16))

(declare-fun future_borrow8 () (_ BitVec 16))

(declare-fun future_borrow9 () (_ BitVec 16))

;; H
  (assert (dynamic_property2 1 last n))

;; H
  (assert (in_range2 j))

(declare-fun abstr12 () (_ BitVec 16))

;; H
  (assert (= carry1 abstr12))

;; H
  (assert (= current_borrow1 abstr12))

;; H
  (assert (= n r126b))

;; H
  (assert (dynamic_property2 1 last r126b))

;; H
  (assert (= divisor_index1 1))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= temp___1281 carry1)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= temp___1277 current_borrow1)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= temp___1276 future_borrow)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= temp___1275 temporary_digit)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= temp___1274 product)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= o divisor_index2)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= o2 ((_ zero_extend 8) o1))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= o3 (bvmul q_hat o2))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= o4 (bvadd o3 carry2))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b)) (= product3 o4)))

(declare-fun abstr13 () (_ BitVec 16))

(declare-fun abstr14 () (_ BitVec 16))

(declare-fun abstr15 () (_ BitVec 16))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (and (= o5 abstr15)
  (= o5 (bvudiv product3 (ite (< 8 65536) (bvshl abstr13 abstr14) abstr12))))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b)) (= carry3 o5)))

(declare-fun abstr16 () (_ BitVec 16))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= temporary_digit3 (bvand product3 abstr16))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (and (= o6 (+ j divisor_index2)) (in_range1 (+ j divisor_index2)))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= temp___1267 (- o6 1))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= o7 temp___1267)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= o9 ((_ zero_extend 8) o8))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= result (ite (bvuge o9 (bvadd temporary_digit3 current_borrow2)) true
            false))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (=> (= result true) (= future_borrow3 abstr12))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (=> (not (= result true)) (= future_borrow4 abstr13))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (=> (not (= result true)) (= future_borrow3 future_borrow4))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (and (= o11 (+ j divisor_index2)) (in_range1 (+ j divisor_index2)))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= temp___1269 (- o11 1))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= o12 temp___1269)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= o14 ((_ zero_extend 8) o13))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= o15 (bvsub o14 temporary_digit3))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= o16 (bvsub o15 current_borrow2))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= o17 (bvand o16 abstr16))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b)) (= o18 o17)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= o19 ((_ extract 7 0) o18))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (and (= o10 (+ j divisor_index2)) (in_range1 (+ j divisor_index2)))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= temp___1271 (- o10 1))))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= o20 temp___1271)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= current_borrow3 future_borrow3)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= divisor_index2 r126b)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= divisor_index3 divisor_index2)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= future_borrow5 future_borrow3)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= temporary_digit4 temporary_digit3)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= product4 product3)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= carry4 carry3)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= current_borrow4 current_borrow3)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= divisor_index4 divisor_index3)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= future_borrow6 future_borrow5)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= temporary_digit5 temporary_digit4)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= product5 product4)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= carry5 carry4)))

;; H
  (assert
  (=> (and (<= 1 divisor_index1) (<= divisor_index1 r126b))
  (= current_borrow5 current_borrow4)))

;; H
  (assert
  (=> (not (and (<= 1 divisor_index1) (<= divisor_index1 r126b)))
  (= divisor_index4 divisor_index1)))

;; H
  (assert
  (=> (not (and (<= 1 divisor_index1) (<= divisor_index1 r126b)))
  (= future_borrow6 future_borrow1)))

;; H
  (assert
  (=> (not (and (<= 1 divisor_index1) (<= divisor_index1 r126b)))
  (= temporary_digit5 temporary_digit1)))

;; H
  (assert
  (=> (not (and (<= 1 divisor_index1) (<= divisor_index1 r126b)))
  (= product5 product1)))

;; H
  (assert
  (=> (not (and (<= 1 divisor_index1) (<= divisor_index1 r126b)))
  (= carry5 carry1)))

;; H
  (assert
  (=> (not (and (<= 1 divisor_index1) (<= divisor_index1 r126b)))
  (= current_borrow5 current_borrow1)))

;; H
  (assert (and (= temp___1285 (+ j n)) (in_range1 (+ j n))))

;; H
  (assert (= o21 temp___1285))

;; H
  (assert (= o23 ((_ zero_extend 8) o22)))

;; H
  (assert (=> (= result1 true) (= future_borrow7 abstr12)))

;; H
  (assert (=> (= result1 true) (= future_borrow9 future_borrow7)))

;; H
  (assert (=> (not (= result1 true)) (= future_borrow8 abstr13)))

;; H
  (assert (=> (not (= result1 true)) (= future_borrow9 future_borrow8)))

;; H
  (assert (and (= temp___1287 (+ j n)) (in_range1 (+ j n))))

;; H
  (assert (= o24 temp___1287))

;; H
  (assert (= o26 ((_ zero_extend 8) o25)))

;; H
  (assert (= o29 (bvand o28 abstr16)))

;; H
  (assert (= o30 o29))

;; H
  (assert (= o31 ((_ extract 7 0) o30)))

;; H
  (assert (and (= temp___1289 (+ j n)) (in_range1 (+ j n))))

(assert
;; WP_parameter_def
 ;; File "very_longs-divide.adb", line 109, characters 0-0
  (not false))
(check-sat)
(exit)
