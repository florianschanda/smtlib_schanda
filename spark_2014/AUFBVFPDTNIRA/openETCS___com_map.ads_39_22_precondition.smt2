...
(declare-fun user_eq (rbc_riu_id_t rbc_riu_id_t) Bool)

(declare-fun dummy () rbc_riu_id_t)
...
;; get__post_axiom
  ...
  (forall ((key Int))
  (! (=>
     (and (dynamic_invariant4 key true true true)
...
;; length__post_axiom
  (assert
  (forall ((container us_rep1)) (! (dynamic_invariant3 (length container)
...
;; H
  (assert
  (and (= result (has_key (model__ map1) id))
  (= (= result true) (= (has_key (model__ map1) id) true))))
