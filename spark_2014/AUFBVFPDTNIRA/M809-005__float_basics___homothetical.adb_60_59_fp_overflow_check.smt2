...
;; H
  (assert (= o6 (store lambda1 i1 o5)))

;; H
  (assert (= result5 (mk_map__ref lambda1)))
...
;; H
  (assert
  (and
  (= o7 (fp.mul RNE (to_rep (select ka i1)) (fp.abs (to_rep (select d j1)))))
...
;; WP_parameter_def
 ;; File "homothetical.ads", line 5, characters 0-0
  ...
  (to_rep (select d i1))))) (fp.isNaN (fp.mul RNE (to_rep (select ka j1)) (fp.abs
  (to_rep (select d i1)))))))))
(check-sat)
(exit)
