...
(declare-fun to_rep1 (element_type) Int)
...
(define-fun dynamic_invariant ((temp___expr_157 us_rep)
...
;; H
  ...
  (in_range
  (to_rep
  (rec__bounded_queue_v2__queue_type__max_size (us_split_discrs1 queue))))))

(assert
;; WP_parameter_def
 ;; File "bounded_queue_v2.ads", line 4, characters 0-0
  (not
  (<= (to_rep2
      (first
      (rt
      (rec__bounded_queue_v2__queue_type__items (us_split_fields1 queue)))))
  (to_rep (rec__bounded_queue_v2__queue_type__rear (us_split_fields1 queue))))))
(check-sat)
(exit)
