...
;; H
  (assert
  (=> (< 1024 (+ point (- characters_to_copy2 1)))
  (= result3 characters_to_copy2)))

;; H
  (assert
  (=> (< 1024 (+ point (- characters_to_copy2 1)))
  (= characters_to_copy3 (+ (- 1024 point) 1))))

;; H
  (assert
  (=> (not (< 1024 (+ point (- characters_to_copy2 1))))
  (= characters_to_copy3 characters_to_copy2)))
...
;; H
  (assert
  (and (= temp___266 (+ (to_rep1 (first (rt source))) (- i2 1))) (in_range1
  (+ (to_rep1 (first (rt source))) (- i2 1)))))

;; H
  (assert
  (and (<= (to_rep1 (first (rt source))) temp___266)
  (<= temp___266 (to_rep1 (last (rt source))))))

;; H
  (assert (= o2 temp___266))

;; H
  (assert (= o3 (select (elts source) o2)))
...
;; H
  (assert
  (and (= temp___267 (+ point (- i2 1))) (in_range1 (+ point (- i2 1)))))

(assert
;; WP_parameter_def
 ;; File "buffers.adb", line 100, characters 0-0
  (not (<= 1 temp___267)))
(check-sat)
