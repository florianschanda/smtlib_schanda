...
(define-fun in_range_int1 ((x Int)) Bool (and (<= 0 x) (<= x 49)))
...
(declare-fun user_eq2 (t3s t3s) Bool)

(declare-fun dummy2 () t3s)
...
(declare-fun failsafe__model__battery_level_at__aggregate_def (Float32) (Array (_ BitVec 8) battery_level_type))

;; def_axiom
  (assert
  (forall ((temp___259 Float32))
  (forall ((temp___260 (_ BitVec 8)))
  (= (select (failsafe__model__battery_level_at__aggregate_def temp___259) temp___260) 
  (of_rep temp___259)))))
...
(assert
;; WP_parameter_def
 ;; File "failsafe.ads", line 15, characters 0-0
  (not true))
(check-sat)
