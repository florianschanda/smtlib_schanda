...
;; H
  (assert (= result2 (ite (fp.lt o2 battery_threshold) true false)))
...
;; H
  ...
  (fp.lt (to_rep (select battery_level_at s4)) battery_threshold)))
  (and
  (forall ((s4 (_ BitVec 8)))
...
;; H
  (assert (= result5 (ite (fp.lt o6 battery_threshold) true false)))
...
;; H
  (assert
  (bvule (let ((temp___301 (bvsub current_time (bvsub res4 #x01))))
         (ite (bvult current_time (bvsub res4 #x01)) (bvadd temp___301 #x32)
         temp___301)) s4))
...
;; WP_parameter_def
 ;; File "failsafe.ads", line 16, characters 0-0
  (not (fp.lt (to_rep (select battery_level_at s4)) battery_threshold)))
(check-sat)
(exit)
