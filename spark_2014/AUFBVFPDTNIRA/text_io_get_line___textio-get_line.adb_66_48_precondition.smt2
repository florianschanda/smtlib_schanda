...
;; H
  ...
  (= (to_rep (select item2 idx)) (to_rep (select buf3 (+ (- idx o8) 1))))
  ...
  (forall ((idx Int))
  (=> (and (<= (to_rep1 item__first) idx) (<= idx (to_rep1 item__last)))
  (ite (and (<= o17 idx) (<= idx (- (+ o17 k3) 1)))
  (= (to_rep (select item3 idx)) (to_rep (select buf3 (+ (- idx o17) 1))))
...
;; H
  (assert (no_char_in_slice__function_guard
  (no_char_in_slice eof_ch cur_position (- o24 1) the_file) eof_ch
  cur_position (- o24 1) the_file))

;; H
  (assert
  (and (= o23 (- (+ cur_position n) 2)) (in_range2 (- (+ cur_position n) 2))))

;; H
  (assert
  (and
  (and (= o24 (find_char_in_slice eof_ch cur_position o23 the_file))
  (find_char_in_slice__function_guard o24 eof_ch cur_position o23 the_file))
  (and (in_range2 o24)
  (and (and (<= cur_position o24) (<= o24 o23))
  (and (= (no_char_in_slice eof_ch cur_position (- o24 1) the_file) true)
  (= (to_rep (select the_file o24)) eof_ch))))))

;; H
  (assert (= o25 (+ last2 o24)))

;; H
  (assert (= o26 (- o25 cur_position)))

;; H
  (assert (= result20 (ite (= (int__content last14) o26) true false)))
...
;; H
  (assert (has_char_in_slice__function_guard
  (has_char_in_slice eof_ch cur_position o22 the_file) eof_ch cur_position
  o22 the_file))

(assert
;; WP_parameter_def
 ;; File "textio.adb", line 102, characters 0-0
  (not (= (has_char_in_slice eof_ch cur_position o22 the_file) true)))
(check-sat)
