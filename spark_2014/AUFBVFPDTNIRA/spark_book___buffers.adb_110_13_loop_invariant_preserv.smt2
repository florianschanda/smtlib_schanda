...
;; H
  (assert
  (and (<= (to_rep1 (first (rt source))) temp___257)
  (<= temp___257 (to_rep1 (last (rt source))))))

;; H
  (assert (= o2 temp___257))
...
;; H
  (assert (= o6 (store buffer2 index2 o5)))

;; H
  (assert (= result5 buffer2))
...
;; WP_parameter_def
 ;; File "buffers.adb", line 69, characters 0-0
  (not (<= characters_to_copy2 (length source))))
(check-sat)
(exit)
