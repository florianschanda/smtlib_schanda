...
;; H
  (assert (<= (to_rep1 (first (rt text))) start))

;; H
  (assert (<= start (to_rep1 (last (rt text)))))
...
;; H
  (assert (<= current1 (to_rep1 (last (rt text)))))

;; H
  (assert
  (and (=> (<= 1 2147483647) (in_range2 position2))
  (<= current1 (to_rep1 (last (rt text))))))

(assert
;; WP_parameter_def
 ;; File "strings.ads", line 17, characters 0-0
  (not (<= (to_rep1 (first (rt text))) current1)))
(check-sat)
