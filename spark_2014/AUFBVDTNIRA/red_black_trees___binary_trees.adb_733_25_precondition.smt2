;; produced by cvc4_14.drv ;;
(set-logic AUFBVDTNIRA)
(set-info :source |VC generated by SPARK 2014|)
(set-info :smt-lib-version 2.0)
(set-info :category industrial)
(set-info :status unknown)
;;; generated by SMT-LIB2 driver
;;; SMT-LIB2 driver: bit-vectors, common part
;;; SMT-LIB2: integer arithmetic
(declare-datatypes () ((tuple0 (Tuple0))))
(declare-sort us_private 0)

(declare-fun private__bool_eq (us_private us_private) Bool)

(declare-fun us_null_ext__ () us_private)

(declare-sort us_type_of_heap 0)

(declare-datatypes ()
((us_type_of_heap__ref
 (mk___type_of_heap__ref (us_type_of_heap__content us_type_of_heap)))))
(declare-sort us_image 0)

(declare-datatypes () ((int__ref (mk_int__ref (int__content Int)))))
(declare-datatypes () ((bool__ref (mk_bool__ref (bool__content Bool)))))
(declare-datatypes () ((real__ref (mk_real__ref (real__content Real)))))
(declare-datatypes ()
((us_private__ref (mk___private__ref (us_private__content us_private)))))
(define-fun int__ref___projection ((a int__ref)) Int (int__content a))

(define-fun bool__ref___projection ((a bool__ref)) Bool (bool__content a))

(define-fun real__ref___projection ((a real__ref)) Real (real__content a))

(define-fun us_private__ref___projection ((a us_private__ref)) us_private 
  (us_private__content a))

(define-fun to_int1 ((b Bool)) Int (ite (= b true) 1 0))

(define-fun of_int ((i Int)) Bool (ite (= i 0) false true))

(define-fun in_range ((x Int)) Bool (or (= x 0) (= x 1)))

(declare-fun attr__ATTRIBUTE_IMAGE (Bool) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE (us_image) Bool)

(declare-sort index_type 0)

(define-fun in_range1 ((x Int)) Bool (and (<= 1 x) (<= x 100)))

(define-fun bool_eq ((x Int) (y Int)) Bool (ite (= x y) true false))

(declare-fun attr__ATTRIBUTE_IMAGE1 (Int) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check1 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE1 (us_image) Int)

(declare-fun user_eq (index_type index_type) Bool)

(declare-fun dummy () index_type)

(declare-datatypes ()
((index_type__ref (mk_index_type__ref (index_type__content index_type)))))
(define-fun index_type__ref___projection ((a index_type__ref)) index_type 
  (index_type__content a))

(define-fun dynamic_invariant ((temp___expr_150 Int)
  (temp___is_init_147 Bool) (temp___skip_constant_148 Bool)
  (temp___do_toplevel_149 Bool)) Bool (=>
                                      (or (= temp___is_init_147 true)
                                      (<= 1 100)) (in_range1
                                      temp___expr_150)))

(declare-sort extended_index_type 0)

(define-fun in_range2 ((x Int)) Bool (and (<= 0 x) (<= x 100)))

(define-fun bool_eq1 ((x Int) (y Int)) Bool (ite (= x y) true false))

(declare-fun attr__ATTRIBUTE_IMAGE2 (Int) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check2 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE2 (us_image) Int)

(declare-fun user_eq1 (extended_index_type extended_index_type) Bool)

(declare-fun dummy1 () extended_index_type)

(declare-datatypes ()
((extended_index_type__ref
 (mk_extended_index_type__ref
 (extended_index_type__content extended_index_type)))))
(define-fun extended_index_type__ref___projection ((a extended_index_type__ref)) extended_index_type 
  (extended_index_type__content a))

(define-fun dynamic_invariant1 ((temp___expr_156 Int)
  (temp___is_init_153 Bool) (temp___skip_constant_154 Bool)
  (temp___do_toplevel_155 Bool)) Bool (=>
                                      (or (= temp___is_init_153 true)
                                      (<= 0 100)) (in_range2
                                      temp___expr_156)))

(declare-fun to_rep (extended_index_type) Int)

(declare-fun of_rep (Int) extended_index_type)

;; inversion_axiom
  (assert
  (forall ((x extended_index_type))
  (! (= (of_rep (to_rep x)) x) :pattern ((to_rep x)) )))

;; range_axiom
  (assert
  (forall ((x extended_index_type)) (! (in_range2
  (to_rep x)) :pattern ((to_rep x)) )))

;; coerce_axiom
  (assert
  (forall ((x Int))
  (! (=> (in_range2 x) (= (to_rep (of_rep x)) x)) :pattern ((to_rep
                                                            (of_rep x))) )))

(declare-sort position_type 0)

(define-fun in_range3 ((x Int)) Bool (and (<= 0 x) (<= x 2)))

(define-fun bool_eq2 ((x Int) (y Int)) Bool (ite (= x y) true false))

(declare-fun attr__ATTRIBUTE_IMAGE3 (Int) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check3 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE3 (us_image) Int)

(declare-fun user_eq2 (position_type position_type) Bool)

(declare-fun dummy2 () position_type)

(declare-datatypes ()
((position_type__ref
 (mk_position_type__ref (position_type__content position_type)))))
(define-fun position_type__ref___projection ((a position_type__ref)) position_type 
  (position_type__content a))

(declare-fun to_rep1 (position_type) Int)

(declare-fun of_rep1 (Int) position_type)

;; inversion_axiom
  (assert
  (forall ((x position_type))
  (! (= (of_rep1 (to_rep1 x)) x) :pattern ((to_rep1 x)) )))

;; range_axiom
  (assert
  (forall ((x position_type)) (! (in_range3
  (to_rep1 x)) :pattern ((to_rep1 x)) )))

;; coerce_axiom
  (assert
  (forall ((x Int))
  (! (=> (in_range3 x) (= (to_rep1 (of_rep1 x)) x)) :pattern ((to_rep1
                                                              (of_rep1 x))) )))

(declare-sort direction 0)

(define-fun in_range4 ((x Int)) Bool (and (<= 0 x) (<= x 1)))

(define-fun bool_eq3 ((x Int) (y Int)) Bool (ite (= x y) true false))

(declare-fun attr__ATTRIBUTE_IMAGE4 (Int) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check4 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE4 (us_image) Int)

(declare-fun user_eq3 (direction direction) Bool)

(declare-fun dummy3 () direction)

(declare-datatypes ()
((direction__ref (mk_direction__ref (direction__content direction)))))
(define-fun direction__ref___projection ((a direction__ref)) direction 
  (direction__content a))

(declare-sort us_main_type 0)

(declare-fun us_main_eq (us_main_type us_main_type) Bool)

(declare-sort us_rep 0)

(declare-fun bool_eq4 (us_rep us_rep) Bool)

(declare-fun user_eq4 (us_rep us_rep) Bool)

(declare-fun value__size () Int)

(declare-fun object__size (us_rep) Int)

(declare-fun value__alignment () Int)

(declare-fun object__alignment (us_rep) Int)

;; value__size_axiom
  (assert (<= 0 value__size))

;; object__size_axiom
  (assert (forall ((a us_rep)) (<= 0 (object__size a))))

;; value__alignment_axiom
  (assert (<= 0 value__alignment))

;; object__alignment_axiom
  (assert (forall ((a us_rep)) (<= 0 (object__alignment a))))

(declare-fun dummy4 () us_rep)

(declare-datatypes ()
((sequence__ref (mk_sequence__ref (sequence__content us_rep)))))
(define-fun sequence__ref___projection ((a sequence__ref)) us_rep (sequence__content
                                                                  a))

(declare-fun length (us_rep) Int)

(declare-fun length__function_guard (Int us_rep) Bool)

(declare-sort count_type 0)

(define-fun in_range5 ((x Int)) Bool (and (<= 0 x) (<= x 2147483647)))

(define-fun bool_eq5 ((x Int) (y Int)) Bool (ite (= x y) true false))

(declare-fun attr__ATTRIBUTE_IMAGE5 (Int) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check5 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE5 (us_image) Int)

(declare-fun user_eq5 (count_type count_type) Bool)

(declare-fun dummy5 () count_type)

(declare-datatypes ()
((count_type__ref (mk_count_type__ref (count_type__content count_type)))))
(define-fun count_type__ref___projection ((a count_type__ref)) count_type 
  (count_type__content a))

(define-fun dynamic_invariant2 ((temp___expr_144 Int)
  (temp___is_init_141 Bool) (temp___skip_constant_142 Bool)
  (temp___do_toplevel_143 Bool)) Bool (=>
                                      (or (= temp___is_init_141 true)
                                      (<= 0 2147483647)) (in_range5
                                      temp___expr_144)))

(define-fun last ((container us_rep)) Int (+ 0 (length container)))

(declare-fun last__function_guard (Int us_rep) Bool)

(declare-fun get (us_rep Int) Int)

(declare-fun get__function_guard (Int us_rep Int) Bool)

(define-fun oeq ((left us_rep)
  (right us_rep)) Bool (and (= (length left) (length right)) (ite (forall
                                                                  ((n Int))
                                                                  (=>
                                                                  (and
                                                                  (<= 1 n)
                                                                  (<= n 
                                                                  (last left)))
                                                                  (= 
                                                                  (get left
                                                                  n) 
                                                                  (get right
                                                                  n))))
                                                             true false)))

(declare-fun oeq__function_guard (Bool us_rep us_rep) Bool)

;; user_eq__def_axiom
  (assert
  (forall ((a us_rep) (b us_rep))
  (! (= (user_eq4 a b) (oeq a b)) :pattern ((user_eq4 a b)) )))

(define-fun default_initial_assumption ((temp___expr_199 us_rep)
  (temp___skip_top_level_200 Bool)) Bool (=>
                                         (not
                                         (= temp___skip_top_level_200 true))
                                         (= (length temp___expr_199) 0)))

;; length__post_axiom
  (assert
  (forall ((container us_rep))
  (! (let ((result (length container)))
     (and (<= (+ 0 result) 2147483647) (dynamic_invariant2 result true false
     true))) :pattern ((length container)) )))

(define-fun ole ((left us_rep)
  (right us_rep)) Bool (and (<= (length left) (length right)) (ite (forall
                                                                   ((n Int))
                                                                   (=>
                                                                   (and
                                                                   (<= 1 n)
                                                                   (<= n 
                                                                   (last
                                                                   left)))
                                                                   (= 
                                                                   (get left
                                                                   n) 
                                                                   (get right
                                                                   n))))
                                                              true false)))

(declare-fun ole__function_guard (Bool us_rep us_rep) Bool)

(declare-datatypes ()
((us_split_fields
 (mk___split_fields
 (rec__tree_model__path_type__a us_rep)(rec__tree_model__path_type__k Bool)))))
(define-fun us_split_fields_A__projection ((a us_split_fields)) us_rep 
  (rec__tree_model__path_type__a a))

(define-fun us_split_fields_K__projection ((a us_split_fields)) Bool 
  (rec__tree_model__path_type__k a))

(declare-datatypes ()
((us_split_fields__ref
 (mk___split_fields__ref (us_split_fields__content us_split_fields)))))
(define-fun us_split_fields__ref___projection ((a us_split_fields__ref)) us_split_fields 
  (us_split_fields__content a))

(declare-datatypes ()
((us_rep1 (mk___rep (us_split_fields1 us_split_fields)))))
(define-fun us_rep___projection ((a us_rep1)) us_split_fields (us_split_fields1
                                                              a))

(define-fun bool_eq6 ((a us_rep1)
  (b us_rep1)) Bool (ite (and
                         (= (user_eq4
                            (rec__tree_model__path_type__a
                            (us_split_fields1 a))
                            (rec__tree_model__path_type__a
                            (us_split_fields1 b))) true)
                         (= (rec__tree_model__path_type__k
                            (us_split_fields1 a)) (rec__tree_model__path_type__k
                                                  (us_split_fields1 b))))
                    true false))

(declare-fun user_eq6 (us_rep1 us_rep1) Bool)

(declare-fun value__size1 () Int)

(declare-fun object__size1 (us_rep1) Int)

(declare-fun value__alignment1 () Int)

(declare-fun object__alignment1 (us_rep1) Int)

;; value__size_axiom
  (assert (<= 0 value__size1))

;; object__size_axiom
  (assert (forall ((a us_rep1)) (<= 0 (object__size1 a))))

;; value__alignment_axiom
  (assert (<= 0 value__alignment1))

;; object__alignment_axiom
  (assert (forall ((a us_rep1)) (<= 0 (object__alignment1 a))))

(declare-fun tree_model__path_type__a__first__bit () Int)

(declare-fun tree_model__path_type__a__last__bit () Int)

(declare-fun tree_model__path_type__a__position () Int)

;; tree_model__path_type__a__first__bit_axiom
  (assert (<= 0 tree_model__path_type__a__first__bit))

;; tree_model__path_type__a__last__bit_axiom
  (assert
  (< tree_model__path_type__a__first__bit tree_model__path_type__a__last__bit))

;; tree_model__path_type__a__position_axiom
  (assert (<= 0 tree_model__path_type__a__position))

(declare-fun tree_model__path_type__k__first__bit () Int)

(declare-fun tree_model__path_type__k__last__bit () Int)

(declare-fun tree_model__path_type__k__position () Int)

;; tree_model__path_type__k__first__bit_axiom
  (assert (<= 0 tree_model__path_type__k__first__bit))

;; tree_model__path_type__k__last__bit_axiom
  (assert
  (< tree_model__path_type__k__first__bit tree_model__path_type__k__last__bit))

;; tree_model__path_type__k__position_axiom
  (assert (<= 0 tree_model__path_type__k__position))

(declare-fun dummy6 () us_rep1)

(declare-datatypes ()
((path_type__ref (mk_path_type__ref (path_type__content us_rep1)))))
(define-fun path_type__ref___projection ((a path_type__ref)) us_rep1 
  (path_type__content a))

(declare-fun is_concat (us_rep us_rep us_rep) Bool)

(declare-fun is_concat__function_guard (Bool us_rep us_rep us_rep) Bool)

;; is_concat__post_axiom
  (assert true)

;; is_concat__def_axiom
  (assert
  (forall ((q us_rep) (v us_rep) (p us_rep))
  (! (=> (is_concat__function_guard (is_concat q v p) q v p)
     (= (= (is_concat q v p) true)
     (and
     (and
     (and (= (- (length p) (length v)) (length q))
     (forall ((i Int))
     (=> (and (<= 1 i) (<= i (length q))) (= (get p i) (get q i)))))
     (forall ((i Int))
     (=> (and (<= 1 i) (<= i (length v)))
     (= (get p (+ i (length q))) (get v i)))))
     (forall ((i Int))
     (=> (and (<= (+ (length q) 1) i) (<= i (length p)))
     (= (get v (- i (length q))) (get p i))))))) :pattern ((is_concat q v p)) )))

(declare-datatypes ()
((map__ref (mk_map__ref (map__content (Array Int us_rep1))))))
(declare-fun slide ((Array Int us_rep1) Int Int) (Array Int us_rep1))

;; slide_eq
  (assert
  (forall ((a (Array Int us_rep1)))
  (forall ((first Int))
  (! (= (slide a first first) a) :pattern ((slide a first first)) ))))

;; slide_def
  (assert
  (forall ((a (Array Int us_rep1)))
  (forall ((old_first Int))
  (forall ((new_first Int))
  (forall ((i Int))
  (! (= (select (slide a old_first new_first) i) (select a (- i (- new_first old_first)))) :pattern ((select 
  (slide a old_first new_first) i)) ))))))

(declare-fun concat1 ((Array Int us_rep1) Int Int (Array Int us_rep1) Int
  Int) (Array Int us_rep1))

;; concat_def
  (assert
  (forall ((a (Array Int us_rep1)) (b (Array Int us_rep1)))
  (forall ((a_first Int) (a_last Int) (b_first Int) (b_last Int))
  (forall ((i Int))
  (! (and
     (=> (and (<= a_first i) (<= i a_last))
     (= (select (concat1 a a_first a_last b b_first b_last) i) (select a i)))
     (=> (< a_last i)
     (= (select (concat1 a a_first a_last b b_first b_last) i) (select b (+ (- i a_last) (- b_first 1)))))) :pattern ((select 
  (concat1 a a_first a_last b b_first b_last) i)) )))))

(declare-fun singleton1 (us_rep1 Int) (Array Int us_rep1))

;; singleton_def
  (assert
  (forall ((v us_rep1))
  (forall ((i Int))
  (! (= (select (singleton1 v i) i) v) :pattern ((select (singleton1 v i) i)) ))))

(define-fun bool_eq7 ((a (Array Int us_rep1)) (a__first Int) (a__last Int)
  (b (Array Int us_rep1)) (b__first Int)
  (b__last Int)) Bool (ite (and
                           (ite (<= a__first a__last)
                           (and (<= b__first b__last)
                           (= (- a__last a__first) (- b__last b__first)))
                           (< b__last b__first))
                           (forall ((temp___idx_132 Int))
                           (=>
                           (and (<= a__first temp___idx_132)
                           (<= temp___idx_132 a__last))
                           (= (bool_eq6 (select a temp___idx_132)
                              (select b (+ (- b__first a__first) temp___idx_132))) true))))
                      true false))

;; bool_eq_rev
  (assert
  (forall ((a (Array Int us_rep1)) (b (Array Int us_rep1)))
  (forall ((a__first Int) (a__last Int) (b__first Int) (b__last Int))
  (=> (= (bool_eq7 b b__first b__last a a__first a__last) true)
  (and
  (ite (<= a__first a__last)
  (and (<= b__first b__last) (= (- a__last a__first) (- b__last b__first)))
  (< b__last b__first))
  (forall ((temp___idx_132 Int))
  (=> (and (<= a__first temp___idx_132) (<= temp___idx_132 a__last))
  (= (bool_eq6 (select a temp___idx_132)
     (select b (+ (- b__first a__first) temp___idx_132))) true))))))))

(declare-fun oeq1 ((Array Int us_rep1) (Array Int us_rep1)) Bool)

(declare-fun oeq__function_guard1 (Bool (Array Int us_rep1)
  (Array Int us_rep1)) Bool)

(define-fun dynamic_invariant3 ((temp___expr_216 (Array Int us_rep1))
  (temp___is_init_213 Bool) (temp___skip_constant_214 Bool)
  (temp___do_toplevel_215 Bool)) Bool (forall ((temp___217 Int))
                                      (=>
                                      (and (<= 1 temp___217)
                                      (<= temp___217 100))
                                      (=> (= temp___is_init_213 true)
                                      (< (length
                                         (rec__tree_model__path_type__a
                                         (us_split_fields1
                                         (select temp___expr_216 temp___217)))) 100)))))

(define-fun default_initial_assumption1 ((temp___expr_219 (Array Int us_rep1))
  (temp___skip_top_level_220 Bool)) Bool (forall ((temp___221 Int))
                                         (=>
                                         (and (<= 1 temp___221)
                                         (<= temp___221 100))
                                         (and
                                         (= (length
                                            (rec__tree_model__path_type__a
                                            (us_split_fields1
                                            (select temp___expr_219 temp___221)))) 0)
                                         (= (rec__tree_model__path_type__k
                                            (us_split_fields1
                                            (select temp___expr_219 temp___221))) 
                                         (of_int 0))))))

;; oeq__post_axiom
  (assert true)

;; oeq__def_axiom
  (assert
  (forall ((m1 (Array Int us_rep1)) (m2 (Array Int us_rep1)))
  (! (=> (oeq__function_guard1 (oeq1 m1 m2) m1 m2)
     (= (= (oeq1 m1 m2) true)
     (forall ((i Int))
     (=> (and (<= 1 i) (<= i 100))
     (and
     (= (oeq (rec__tree_model__path_type__a (us_split_fields1 (select m1 i)))
        (rec__tree_model__path_type__a (us_split_fields1 (select m2 i)))) true)
     (=
     (= (rec__tree_model__path_type__k (us_split_fields1 (select m1 i))) true)
     (= (rec__tree_model__path_type__k (us_split_fields1 (select m2 i))) true))))))) :pattern (
  (oeq1 m1 m2)) )))

(declare-fun is_add (us_rep Int us_rep) Bool)

(declare-fun is_add__function_guard (Bool us_rep Int us_rep) Bool)

(declare-datatypes ()
((us_split_fields2
 (mk___split_fields1
 (rec__binary_trees__cell__left extended_index_type)(rec__binary_trees__cell__right extended_index_type)(rec__binary_trees__cell__parent extended_index_type)(rec__binary_trees__cell__position position_type)))))
(define-fun us_split_fields_Left__projection ((a us_split_fields2)) extended_index_type 
  (rec__binary_trees__cell__left a))

(define-fun us_split_fields_Right__projection ((a us_split_fields2)) extended_index_type 
  (rec__binary_trees__cell__right a))

(define-fun us_split_fields_Parent__projection ((a us_split_fields2)) extended_index_type 
  (rec__binary_trees__cell__parent a))

(define-fun us_split_fields_Position__projection ((a us_split_fields2)) position_type 
  (rec__binary_trees__cell__position a))

(declare-datatypes ()
((us_split_fields__ref1
 (mk___split_fields__ref1 (us_split_fields__content1 us_split_fields2)))))
(define-fun us_split_fields__ref_2__projection ((a us_split_fields__ref1)) us_split_fields2 
  (us_split_fields__content1 a))

(declare-datatypes ()
((us_rep2 (mk___rep1 (us_split_fields3 us_split_fields2)))))
(define-fun us_rep_2__projection ((a us_rep2)) us_split_fields2 (us_split_fields3
                                                                a))

(define-fun bool_eq8 ((a us_rep2)
  (b us_rep2)) Bool (ite (and
                         (and
                         (and
                         (= (to_rep
                            (rec__binary_trees__cell__left
                            (us_split_fields3 a))) (to_rep
                                                   (rec__binary_trees__cell__left
                                                   (us_split_fields3 b))))
                         (= (to_rep
                            (rec__binary_trees__cell__right
                            (us_split_fields3 a))) (to_rep
                                                   (rec__binary_trees__cell__right
                                                   (us_split_fields3 b)))))
                         (= (to_rep
                            (rec__binary_trees__cell__parent
                            (us_split_fields3 a))) (to_rep
                                                   (rec__binary_trees__cell__parent
                                                   (us_split_fields3 b)))))
                         (= (to_rep1
                            (rec__binary_trees__cell__position
                            (us_split_fields3 a))) (to_rep1
                                                   (rec__binary_trees__cell__position
                                                   (us_split_fields3 b)))))
                    true false))

(declare-fun user_eq7 (us_rep2 us_rep2) Bool)

(declare-fun value__size2 () Int)

(declare-fun object__size2 (us_rep2) Int)

(declare-fun value__alignment2 () Int)

(declare-fun object__alignment2 (us_rep2) Int)

;; value__size_axiom
  (assert (<= 0 value__size2))

;; object__size_axiom
  (assert (forall ((a us_rep2)) (<= 0 (object__size2 a))))

;; value__alignment_axiom
  (assert (<= 0 value__alignment2))

;; object__alignment_axiom
  (assert (forall ((a us_rep2)) (<= 0 (object__alignment2 a))))

(declare-fun binary_trees__cell__left__first__bit () Int)

(declare-fun binary_trees__cell__left__last__bit () Int)

(declare-fun binary_trees__cell__left__position () Int)

;; binary_trees__cell__left__first__bit_axiom
  (assert (<= 0 binary_trees__cell__left__first__bit))

;; binary_trees__cell__left__last__bit_axiom
  (assert
  (< binary_trees__cell__left__first__bit binary_trees__cell__left__last__bit))

;; binary_trees__cell__left__position_axiom
  (assert (<= 0 binary_trees__cell__left__position))

(declare-fun binary_trees__cell__right__first__bit () Int)

(declare-fun binary_trees__cell__right__last__bit () Int)

(declare-fun binary_trees__cell__right__position () Int)

;; binary_trees__cell__right__first__bit_axiom
  (assert (<= 0 binary_trees__cell__right__first__bit))

;; binary_trees__cell__right__last__bit_axiom
  (assert
  (< binary_trees__cell__right__first__bit binary_trees__cell__right__last__bit))

;; binary_trees__cell__right__position_axiom
  (assert (<= 0 binary_trees__cell__right__position))

(declare-fun binary_trees__cell__parent__first__bit () Int)

(declare-fun binary_trees__cell__parent__last__bit () Int)

(declare-fun binary_trees__cell__parent__position () Int)

;; binary_trees__cell__parent__first__bit_axiom
  (assert (<= 0 binary_trees__cell__parent__first__bit))

;; binary_trees__cell__parent__last__bit_axiom
  (assert
  (< binary_trees__cell__parent__first__bit binary_trees__cell__parent__last__bit))

;; binary_trees__cell__parent__position_axiom
  (assert (<= 0 binary_trees__cell__parent__position))

(declare-fun binary_trees__cell__position__first__bit () Int)

(declare-fun binary_trees__cell__position__last__bit () Int)

(declare-fun binary_trees__cell__position__position () Int)

;; binary_trees__cell__position__first__bit_axiom
  (assert (<= 0 binary_trees__cell__position__first__bit))

;; binary_trees__cell__position__last__bit_axiom
  (assert
  (< binary_trees__cell__position__first__bit binary_trees__cell__position__last__bit))

;; binary_trees__cell__position__position_axiom
  (assert (<= 0 binary_trees__cell__position__position))

(declare-fun dummy7 () us_rep2)

(declare-datatypes () ((cell__ref (mk_cell__ref (cell__content us_rep2)))))
(define-fun cell__ref___projection ((a cell__ref)) us_rep2 (cell__content a))

(declare-datatypes ()
((map__ref1 (mk_map__ref1 (map__content1 (Array Int us_rep2))))))
(declare-fun slide1 ((Array Int us_rep2) Int Int) (Array Int us_rep2))

;; slide_eq
  (assert
  (forall ((a (Array Int us_rep2)))
  (forall ((first Int))
  (! (= (slide1 a first first) a) :pattern ((slide1 a first first)) ))))

;; slide_def
  (assert
  (forall ((a (Array Int us_rep2)))
  (forall ((old_first Int))
  (forall ((new_first Int))
  (forall ((i Int))
  (! (= (select (slide1 a old_first new_first) i) (select a (- i (- new_first old_first)))) :pattern ((select 
  (slide1 a old_first new_first) i)) ))))))

(declare-fun concat2 ((Array Int us_rep2) Int Int (Array Int us_rep2) Int
  Int) (Array Int us_rep2))

;; concat_def
  (assert
  (forall ((a (Array Int us_rep2)) (b (Array Int us_rep2)))
  (forall ((a_first Int) (a_last Int) (b_first Int) (b_last Int))
  (forall ((i Int))
  (! (and
     (=> (and (<= a_first i) (<= i a_last))
     (= (select (concat2 a a_first a_last b b_first b_last) i) (select a i)))
     (=> (< a_last i)
     (= (select (concat2 a a_first a_last b b_first b_last) i) (select b (+ (- i a_last) (- b_first 1)))))) :pattern ((select 
  (concat2 a a_first a_last b b_first b_last) i)) )))))

(declare-fun singleton2 (us_rep2 Int) (Array Int us_rep2))

;; singleton_def
  (assert
  (forall ((v us_rep2))
  (forall ((i Int))
  (! (= (select (singleton2 v i) i) v) :pattern ((select (singleton2 v i) i)) ))))

(define-fun bool_eq9 ((a (Array Int us_rep2)) (a__first Int) (a__last Int)
  (b (Array Int us_rep2)) (b__first Int)
  (b__last Int)) Bool (ite (and
                           (ite (<= a__first a__last)
                           (and (<= b__first b__last)
                           (= (- a__last a__first) (- b__last b__first)))
                           (< b__last b__first))
                           (forall ((temp___idx_133 Int))
                           (=>
                           (and (<= a__first temp___idx_133)
                           (<= temp___idx_133 a__last))
                           (= (bool_eq8 (select a temp___idx_133)
                              (select b (+ (- b__first a__first) temp___idx_133))) true))))
                      true false))

;; bool_eq_rev
  (assert
  (forall ((a (Array Int us_rep2)) (b (Array Int us_rep2)))
  (forall ((a__first Int) (a__last Int) (b__first Int) (b__last Int))
  (=> (= (bool_eq9 b b__first b__last a a__first a__last) true)
  (and
  (ite (<= a__first a__last)
  (and (<= b__first b__last) (= (- a__last a__first) (- b__last b__first)))
  (< b__last b__first))
  (forall ((temp___idx_133 Int))
  (=> (and (<= a__first temp___idx_133) (<= temp___idx_133 a__last))
  (= (bool_eq8 (select a temp___idx_133)
     (select b (+ (- b__first a__first) temp___idx_133))) true))))))))

(declare-fun dummy8 () (Array Int us_rep2))

(declare-fun value__size3 () Int)

(declare-fun object__size3 ((Array Int us_rep2)) Int)

(declare-fun value__component__size () Int)

(declare-fun object__component__size ((Array Int us_rep2)) Int)

(declare-fun value__alignment3 () Int)

(declare-fun object__alignment3 ((Array Int us_rep2)) Int)

;; value__size_axiom
  (assert (<= 0 value__size3))

;; object__size_axiom
  (assert (forall ((a (Array Int us_rep2))) (<= 0 (object__size3 a))))

;; value__component__size_axiom
  (assert (<= 0 value__component__size))

;; object__component__size_axiom
  (assert
  (forall ((a (Array Int us_rep2))) (<= 0 (object__component__size a))))

;; value__alignment_axiom
  (assert (<= 0 value__alignment3))

;; object__alignment_axiom
  (assert (forall ((a (Array Int us_rep2))) (<= 0 (object__alignment3 a))))

(declare-fun user_eq8 ((Array Int us_rep2) (Array Int us_rep2)) Bool)

(declare-datatypes ()
((us_split_fields4
 (mk___split_fields2
 (rec__binary_trees__forest__s extended_index_type)(rec__binary_trees__forest__c (Array Int us_rep2))))))
(define-fun us_split_fields_S__projection ((a us_split_fields4)) extended_index_type 
  (rec__binary_trees__forest__s a))

(define-fun us_split_fields_C__projection ((a us_split_fields4)) (Array Int us_rep2) 
  (rec__binary_trees__forest__c a))

(declare-datatypes ()
((us_split_fields__ref2
 (mk___split_fields__ref2 (us_split_fields__content2 us_split_fields4)))))
(define-fun us_split_fields__ref_3__projection ((a us_split_fields__ref2)) us_split_fields4 
  (us_split_fields__content2 a))

(declare-datatypes ()
((us_rep3 (mk___rep2 (us_split_fields5 us_split_fields4)))))
(define-fun us_rep_3__projection ((a us_rep3)) us_split_fields4 (us_split_fields5
                                                                a))

(define-fun bool_eq10 ((a us_rep3)
  (b us_rep3)) Bool (ite (and
                         (= (to_rep
                            (rec__binary_trees__forest__s
                            (us_split_fields5 a))) (to_rep
                                                   (rec__binary_trees__forest__s
                                                   (us_split_fields5 b))))
                         (= (bool_eq9
                            (rec__binary_trees__forest__c
                            (us_split_fields5 a)) 1 100
                            (rec__binary_trees__forest__c
                            (us_split_fields5 b)) 1 100) true))
                    true false))

(declare-fun user_eq9 (us_rep3 us_rep3) Bool)

(declare-fun value__size4 () Int)

(declare-fun object__size4 (us_rep3) Int)

(declare-fun value__alignment4 () Int)

(declare-fun object__alignment4 (us_rep3) Int)

;; value__size_axiom
  (assert (<= 0 value__size4))

;; object__size_axiom
  (assert (forall ((a us_rep3)) (<= 0 (object__size4 a))))

;; value__alignment_axiom
  (assert (<= 0 value__alignment4))

;; object__alignment_axiom
  (assert (forall ((a us_rep3)) (<= 0 (object__alignment4 a))))

(declare-fun binary_trees__forest__s__first__bit () Int)

(declare-fun binary_trees__forest__s__last__bit () Int)

(declare-fun binary_trees__forest__s__position () Int)

;; binary_trees__forest__s__first__bit_axiom
  (assert (<= 0 binary_trees__forest__s__first__bit))

;; binary_trees__forest__s__last__bit_axiom
  (assert
  (< binary_trees__forest__s__first__bit binary_trees__forest__s__last__bit))

;; binary_trees__forest__s__position_axiom
  (assert (<= 0 binary_trees__forest__s__position))

(declare-fun binary_trees__forest__c__first__bit () Int)

(declare-fun binary_trees__forest__c__last__bit () Int)

(declare-fun binary_trees__forest__c__position () Int)

;; binary_trees__forest__c__first__bit_axiom
  (assert (<= 0 binary_trees__forest__c__first__bit))

;; binary_trees__forest__c__last__bit_axiom
  (assert
  (< binary_trees__forest__c__first__bit binary_trees__forest__c__last__bit))

;; binary_trees__forest__c__position_axiom
  (assert (<= 0 binary_trees__forest__c__position))

(declare-fun dummy9 () us_rep3)

(declare-datatypes ()
((forest__ref (mk_forest__ref (forest__content us_rep3)))))
(define-fun forest__ref___projection ((a forest__ref)) us_rep3 (forest__content
                                                               a))

(declare-fun size (us_rep3) Int)

(declare-fun size__function_guard (Int us_rep3) Bool)

(declare-fun tree_structure (us_rep3) Bool)

(declare-fun tree_structure__function_guard (Bool us_rep3) Bool)

;; temp___result_287_def
  (assert
  (forall ((temp___286 us_rep3)) (size__function_guard (size temp___286)
  temp___286)))

(define-fun default_initial_assumption2 ((temp___expr_281 us_rep3)
  (temp___skip_top_level_282 Bool)) Bool (and
                                         (and
                                         (= (to_rep
                                            (rec__binary_trees__forest__s
                                            (us_split_fields5
                                            temp___expr_281))) 0)
                                         (forall ((temp___284 Int))
                                         (=>
                                         (and (<= 1 temp___284)
                                         (<= temp___284 100))
                                         (and
                                         (and
                                         (and
                                         (= (to_rep
                                            (rec__binary_trees__cell__left
                                            (us_split_fields3
                                            (select (rec__binary_trees__forest__c
                                                    (us_split_fields5
                                                    temp___expr_281)) temp___284)))) 0)
                                         (= (to_rep
                                            (rec__binary_trees__cell__right
                                            (us_split_fields3
                                            (select (rec__binary_trees__forest__c
                                                    (us_split_fields5
                                                    temp___expr_281)) temp___284)))) 0))
                                         (= (to_rep
                                            (rec__binary_trees__cell__parent
                                            (us_split_fields3
                                            (select (rec__binary_trees__forest__c
                                                    (us_split_fields5
                                                    temp___expr_281)) temp___284)))) 0))
                                         (= (to_rep1
                                            (rec__binary_trees__cell__position
                                            (us_split_fields3
                                            (select (rec__binary_trees__forest__c
                                                    (us_split_fields5
                                                    temp___expr_281)) temp___284)))) 2)))))
                                         (=>
                                         (not
                                         (= temp___skip_top_level_282 true))
                                         (= (size temp___expr_281) 0))))

;; temp___result_290_def
  (assert
  (forall ((temp___289 us_rep3)) (tree_structure__function_guard
  (tree_structure temp___289) temp___289)))

(define-fun type_invariant ((temp___288 us_rep3)) Bool (= (tree_structure
                                                          temp___288) true))

(declare-fun model__ (us_rep3 Int) (Array Int us_rep1))

(declare-fun model____function_guard ((Array Int us_rep1) us_rep3 Int) Bool)

(declare-fun valid_root (us_rep3 Int) Bool)

(declare-fun valid_root__function_guard (Bool us_rep3 Int) Bool)

(declare-fun parent (us_rep3 Int) Int)

(declare-fun parent__function_guard (Int us_rep3 Int) Bool)

(declare-fun position (us_rep3 Int) Int)

(declare-fun position__function_guard (Int us_rep3 Int) Bool)

;; model____post_axiom
  (assert
  (forall ((f us_rep3))
  (forall ((root Int))
  (! (and (valid_root__function_guard (valid_root f root) f root)
     (=>
     (and (and (dynamic_invariant root true true true) (type_invariant f))
     (= (valid_root f root) true))
     (let ((result (model__ f root)))
     (and (forall ((i Int)) (parent__function_guard (parent f i) f i))
     (and (forall ((i Int)) (parent__function_guard (parent f i) f i))
     (and (forall ((i Int)) (parent__function_guard (parent f i) f i))
     (and (forall ((i Int)) (position__function_guard (position f i) f i))
     (and (forall ((i Int)) (parent__function_guard (parent f i) f i))
     (and (forall ((i Int)) (position__function_guard (position f i) f i))
     (and
     (forall ((i Int)) (is_add__function_guard
     (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1 (select result (parent f i)))) (position f i)
     (rec__tree_model__path_type__a (us_split_fields1 (select result i))))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select result (parent f i)))) (position f i)
     (rec__tree_model__path_type__a (us_split_fields1 (select result i)))))
     (=> (model____function_guard result f root)
     (and
     (and
     (= (rec__tree_model__path_type__k
        (us_split_fields1 (select result root))) true)
     (and
     (= (length
        (rec__tree_model__path_type__a
        (us_split_fields1 (select result root)))) 0)
     (and
     (forall ((i Int))
     (=> (and (<= 1 i) (<= i 100))
     (=> (not (= i root))
     (ite (and (not (= (parent f i) 0))
          (= (rec__tree_model__path_type__k
             (us_split_fields1 (select result (parent f i)))) true))
     (= (rec__tree_model__path_type__k (us_split_fields1 (select result i))) true)
     (not
     (= (rec__tree_model__path_type__k (us_split_fields1 (select result i))) true))))))
     (and
     (forall ((i Int))
     (=> (and (<= 1 i) (<= i 100))
     (ite (and
          (= (rec__tree_model__path_type__k
             (us_split_fields1 (select result i))) true)
          (not (= i root)))
     (= (is_add
        (rec__tree_model__path_type__a
        (us_split_fields1 (select result (parent f i)))) (position f i)
        (rec__tree_model__path_type__a (us_split_fields1 (select result i)))) true)
     (= (length
        (rec__tree_model__path_type__a (us_split_fields1 (select result i)))) 0))))
     (forall ((i Int))
     (=> (and (<= 1 i) (<= i 100))
     (=>
     (= (rec__tree_model__path_type__k (us_split_fields1 (select result i))) true)
     (forall ((j Int))
     (=> (and (<= 1 j) (<= j 100))
     (=>
     (and
     (= (rec__tree_model__path_type__k (us_split_fields1 (select result j))) true)
     (= (oeq
        (rec__tree_model__path_type__a (us_split_fields1 (select result i)))
        (rec__tree_model__path_type__a (us_split_fields1 (select result j)))) true))
     (= j i))))))))))) (dynamic_invariant3 result true false true))))))))))))) :pattern (
  (model__ f root)) ))))

(declare-fun attr__ATTRIBUTE_ADDRESS () Int)

(declare-fun root () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS1 () Int)

(declare-fun v () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS2 () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS3 () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS4 () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS5 () Int)

(declare-fun r246b () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS6 () Int)

(declare-sort tcount_typeB 0)

(define-fun in_range6 ((x Int)) Bool (and (<= (- 2147483648) x)
                                     (<= x 2147483647)))

(define-fun bool_eq11 ((x Int) (y Int)) Bool (ite (= x y) true false))

(declare-fun attr__ATTRIBUTE_IMAGE6 (Int) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check6 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE6 (us_image) Int)

(declare-fun user_eq10 (tcount_typeB tcount_typeB) Bool)

(declare-fun dummy10 () tcount_typeB)

(declare-datatypes ()
((tcount_typeB__ref
 (mk_tcount_typeB__ref (tcount_typeB__content tcount_typeB)))))
(define-fun tcount_typeB__ref___projection ((a tcount_typeB__ref)) tcount_typeB 
  (tcount_typeB__content a))

(declare-fun to_rep2 (tcount_typeB) Int)

(declare-fun of_rep2 (Int) tcount_typeB)

;; inversion_axiom
  (assert
  (forall ((x tcount_typeB))
  (! (= (of_rep2 (to_rep2 x)) x) :pattern ((to_rep2 x)) )))

;; range_axiom
  (assert
  (forall ((x tcount_typeB)) (! (in_range6
  (to_rep2 x)) :pattern ((to_rep2 x)) )))

;; coerce_axiom
  (assert
  (forall ((x Int))
  (! (=> (in_range6 x) (= (to_rep2 (of_rep2 x)) x)) :pattern ((to_rep2
                                                              (of_rep2 x))) )))

(declare-fun last1 () Int)

(define-fun dynamic_property ((first_int Int) (last_int Int)
  (x Int)) Bool (and (<= first_int x) (<= x last_int)))

(define-fun bool_eq12 ((x Int) (y Int)) Bool (ite (= x y) true false))

(declare-fun attr__ATTRIBUTE_IMAGE7 (Int) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check7 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE7 (us_image) Int)

(declare-fun user_eq11 (tcount_typeB tcount_typeB) Bool)

(declare-fun dummy11 () tcount_typeB)

(declare-datatypes ()
((t247b__ref (mk_t247b__ref (t247b__content tcount_typeB)))))
(define-fun t247b__ref___projection ((a t247b__ref)) tcount_typeB (t247b__content
                                                                  a))

(declare-fun attr__ATTRIBUTE_ADDRESS7 () Int)

;; size__post_axiom
  (assert
  (forall ((f us_rep3))
  (! (=> (type_invariant f)
     (let ((result (size f)))
     (=> (size__function_guard result f) (dynamic_invariant1 result true
     false true)))) :pattern ((size f)) )))

;; size__def_axiom
  (assert
  (forall ((f us_rep3))
  (! (=> (size__function_guard (size f) f)
     (= (size f) (to_rep (rec__binary_trees__forest__s (us_split_fields5 f))))) :pattern (
  (size f)) )))

;; valid_root__post_axiom
  (assert
  (forall ((f us_rep3))
  (forall ((i Int))
  (! (=> (and (dynamic_invariant i true true true) (type_invariant f))
     (let ((result (valid_root f i)))
     (and (size__function_guard (size f) f)
     (=> (valid_root__function_guard result f i)
     (=> (< (size f) i) (not (= result true))))))) :pattern ((valid_root f i)) ))))

;; valid_root__def_axiom
  (assert
  (forall ((f us_rep3))
  (forall ((i Int))
  (! (=> (valid_root__function_guard (valid_root f i) f i)
     (= (= (valid_root f i) true)
     (and (<= i (to_rep (rec__binary_trees__forest__s (us_split_fields5 f))))
     (= (to_rep1
        (rec__binary_trees__cell__position
        (us_split_fields3
        (let ((temp___298 (rec__binary_trees__forest__c (us_split_fields5 f))))
        (select temp___298 i))))) 2)))) :pattern ((valid_root f i)) ))))

;; parent__post_axiom
  (assert
  (forall ((f us_rep3))
  (forall ((i Int))
  (! (=> (and (dynamic_invariant i true true true) (type_invariant f))
     (let ((result (parent f i)))
     (and (valid_root__function_guard (valid_root f i) f i)
     (and (size__function_guard (size f) f)
     (=> (parent__function_guard result f i)
     (and
     (and (=> (= (valid_root f i) true) (= result 0))
     (=> (= (size f) 0) (= result 0))) (dynamic_invariant1 result true false
     true))))))) :pattern ((parent f i)) ))))

;; parent__def_axiom
  (assert
  (forall ((f us_rep3))
  (forall ((i Int))
  (! (=>
     (and (dynamic_invariant i true true true) (parent__function_guard
     (parent f i) f i))
     (= (parent f i) (to_rep
                     (rec__binary_trees__cell__parent
                     (us_split_fields3
                     (let ((temp___306 (rec__binary_trees__forest__c
                                       (us_split_fields5 f))))
                     (select temp___306 i))))))) :pattern ((parent f i)) ))))

(define-fun dynamic_invariant4 ((temp___expr_168 Int)
  (temp___is_init_165 Bool) (temp___skip_constant_166 Bool)
  (temp___do_toplevel_167 Bool)) Bool (=>
                                      (or (= temp___is_init_165 true)
                                      (<= 0 1)) (in_range4 temp___expr_168)))

;; position__post_axiom
  (assert
  (forall ((f us_rep3))
  (forall ((i Int))
  (! (and (parent__function_guard (parent f i) f i)
     (=>
     (and (and (dynamic_invariant i true true true) (type_invariant f))
     (not (= (parent f i) 0)))
     (let ((result (position f i)))
     (=> (position__function_guard result f i) (dynamic_invariant4 result
     true false true))))) :pattern ((position f i)) ))))

;; position__def_axiom
  (assert
  (forall ((f us_rep3))
  (forall ((i Int))
  (! (=>
     (and (dynamic_invariant i true true true) (position__function_guard
     (position f i) f i))
     (= (position f i) (to_rep1
                       (rec__binary_trees__cell__position
                       (us_split_fields3
                       (let ((temp___312 (rec__binary_trees__forest__c
                                         (us_split_fields5 f))))
                       (select temp___312 i))))))) :pattern ((position f i)) ))))

(define-fun default_initial_assumption3 ((temp___expr_265 us_rep2)
  (temp___skip_top_level_266 Bool)) Bool (and
                                         (and
                                         (and
                                         (= (to_rep
                                            (rec__binary_trees__cell__left
                                            (us_split_fields3
                                            temp___expr_265))) 0)
                                         (= (to_rep
                                            (rec__binary_trees__cell__right
                                            (us_split_fields3
                                            temp___expr_265))) 0))
                                         (= (to_rep
                                            (rec__binary_trees__cell__parent
                                            (us_split_fields3
                                            temp___expr_265))) 0))
                                         (= (to_rep1
                                            (rec__binary_trees__cell__position
                                            (us_split_fields3
                                            temp___expr_265))) 2)))

(define-fun default_initial_assumption4 ((temp___expr_272 (Array Int us_rep2))
  (temp___skip_top_level_273 Bool)) Bool (forall ((temp___274 Int))
                                         (=>
                                         (and (<= 1 temp___274)
                                         (<= temp___274 100))
                                         (and
                                         (and
                                         (and
                                         (= (to_rep
                                            (rec__binary_trees__cell__left
                                            (us_split_fields3
                                            (select temp___expr_272 temp___274)))) 0)
                                         (= (to_rep
                                            (rec__binary_trees__cell__right
                                            (us_split_fields3
                                            (select temp___expr_272 temp___274)))) 0))
                                         (= (to_rep
                                            (rec__binary_trees__cell__parent
                                            (us_split_fields3
                                            (select temp___expr_272 temp___274)))) 0))
                                         (= (to_rep1
                                            (rec__binary_trees__cell__position
                                            (us_split_fields3
                                            (select temp___expr_272 temp___274)))) 2)))))

;; tree_structure__post_axiom
  (assert true)

;; tree_structure__def_axiom
  (assert
  (forall ((f us_rep3))
  (! (=> (tree_structure__function_guard (tree_structure f) f)
     (= (= (tree_structure f) true)
     (and
     (and
     (and
     (and
     (and
     (and
     (and
     (and
     (forall ((i Int))
     (=>
     (and
     (<= (+ (to_rep (rec__binary_trees__forest__s (us_split_fields5 f))) 1) i)
     (<= i 100))
     (= (bool_eq8
        (let ((temp___580 (rec__binary_trees__forest__c (us_split_fields5 f))))
        (select temp___580 i))
        (mk___rep1
        (mk___split_fields1 (of_rep 0) (of_rep 0) (of_rep 0) (of_rep1 2)))) true)))
     (forall ((i Int))
     (=> (and (<= 1 i) (<= i 100))
     (and
     (<= 0 (to_rep
           (rec__binary_trees__cell__parent
           (us_split_fields3
           (let ((temp___581 (rec__binary_trees__forest__c
                             (us_split_fields5 f))))
           (select temp___581 i))))))
     (<= (to_rep
         (rec__binary_trees__cell__parent
         (us_split_fields3
         (let ((temp___581 (rec__binary_trees__forest__c
                           (us_split_fields5 f))))
         (select temp___581 i))))) (to_rep
                                   (rec__binary_trees__forest__s
                                   (us_split_fields5 f))))))))
     (forall ((i Int))
     (=> (and (<= 1 i) (<= i 100))
     (and
     (<= 0 (to_rep
           (rec__binary_trees__cell__left
           (us_split_fields3
           (let ((temp___582 (rec__binary_trees__forest__c
                             (us_split_fields5 f))))
           (select temp___582 i))))))
     (<= (to_rep
         (rec__binary_trees__cell__left
         (us_split_fields3
         (let ((temp___582 (rec__binary_trees__forest__c
                           (us_split_fields5 f))))
         (select temp___582 i))))) (to_rep
                                   (rec__binary_trees__forest__s
                                   (us_split_fields5 f))))))))
     (forall ((i Int))
     (=> (and (<= 1 i) (<= i 100))
     (and
     (<= 0 (to_rep
           (rec__binary_trees__cell__right
           (us_split_fields3
           (let ((temp___583 (rec__binary_trees__forest__c
                             (us_split_fields5 f))))
           (select temp___583 i))))))
     (<= (to_rep
         (rec__binary_trees__cell__right
         (us_split_fields3
         (let ((temp___583 (rec__binary_trees__forest__c
                           (us_split_fields5 f))))
         (select temp___583 i))))) (to_rep
                                   (rec__binary_trees__forest__s
                                   (us_split_fields5 f))))))))
     (forall ((i Int))
     (=> (and (<= 1 i) (<= i 100))
     (=>
     (= (to_rep1
        (rec__binary_trees__cell__position
        (us_split_fields3
        (let ((temp___585 (rec__binary_trees__forest__c (us_split_fields5 f))))
        (select temp___585 i))))) 2)
     (= (to_rep
        (rec__binary_trees__cell__parent
        (us_split_fields3
        (let ((temp___584 (rec__binary_trees__forest__c (us_split_fields5 f))))
        (select temp___584 i))))) 0)))))
     (forall ((i Int))
     (=> (and (<= 1 i) (<= i 100))
     (=>
     (not
     (= (to_rep
        (rec__binary_trees__cell__left
        (us_split_fields3
        (let ((temp___590 (rec__binary_trees__forest__c (us_split_fields5 f))))
        (select temp___590 i))))) 0))
     (and
     (= (to_rep1
        (rec__binary_trees__cell__position
        (us_split_fields3
        (let ((temp___586 (rec__binary_trees__forest__c (us_split_fields5 f))))
        (select temp___586 (to_rep
                           (rec__binary_trees__cell__left
                           (us_split_fields3
                           (let ((temp___587 (rec__binary_trees__forest__c
                                             (us_split_fields5 f))))
                           (select temp___587 i)))))))))) 0)
     (= (to_rep
        (rec__binary_trees__cell__parent
        (us_split_fields3
        (let ((temp___588 (rec__binary_trees__forest__c (us_split_fields5 f))))
        (select temp___588 (to_rep
                           (rec__binary_trees__cell__left
                           (us_split_fields3
                           (let ((temp___589 (rec__binary_trees__forest__c
                                             (us_split_fields5 f))))
                           (select temp___589 i)))))))))) i))))))
     (forall ((i Int))
     (=> (and (<= 1 i) (<= i 100))
     (=>
     (not
     (= (to_rep
        (rec__binary_trees__cell__right
        (us_split_fields3
        (let ((temp___595 (rec__binary_trees__forest__c (us_split_fields5 f))))
        (select temp___595 i))))) 0))
     (and
     (= (to_rep1
        (rec__binary_trees__cell__position
        (us_split_fields3
        (let ((temp___591 (rec__binary_trees__forest__c (us_split_fields5 f))))
        (select temp___591 (to_rep
                           (rec__binary_trees__cell__right
                           (us_split_fields3
                           (let ((temp___592 (rec__binary_trees__forest__c
                                             (us_split_fields5 f))))
                           (select temp___592 i)))))))))) 1)
     (= (to_rep
        (rec__binary_trees__cell__parent
        (us_split_fields3
        (let ((temp___593 (rec__binary_trees__forest__c (us_split_fields5 f))))
        (select temp___593 (to_rep
                           (rec__binary_trees__cell__right
                           (us_split_fields3
                           (let ((temp___594 (rec__binary_trees__forest__c
                                             (us_split_fields5 f))))
                           (select temp___594 i)))))))))) i))))))
     (forall ((i Int))
     (=> (and (<= 1 i) (<= i 100))
     (=>
     (and
     (not
     (= (to_rep
        (rec__binary_trees__cell__parent
        (us_split_fields3
        (let ((temp___598 (rec__binary_trees__forest__c (us_split_fields5 f))))
        (select temp___598 i))))) 0))
     (= (to_rep1
        (rec__binary_trees__cell__position
        (us_split_fields3
        (let ((temp___599 (rec__binary_trees__forest__c (us_split_fields5 f))))
        (select temp___599 i))))) 0))
     (= (to_rep
        (rec__binary_trees__cell__left
        (us_split_fields3
        (let ((temp___596 (rec__binary_trees__forest__c (us_split_fields5 f))))
        (select temp___596 (to_rep
                           (rec__binary_trees__cell__parent
                           (us_split_fields3
                           (let ((temp___597 (rec__binary_trees__forest__c
                                             (us_split_fields5 f))))
                           (select temp___597 i)))))))))) i)))))
     (forall ((i Int))
     (=> (and (<= 1 i) (<= i 100))
     (=>
     (and
     (not
     (= (to_rep
        (rec__binary_trees__cell__parent
        (us_split_fields3
        (let ((temp___602 (rec__binary_trees__forest__c (us_split_fields5 f))))
        (select temp___602 i))))) 0))
     (= (to_rep1
        (rec__binary_trees__cell__position
        (us_split_fields3
        (let ((temp___603 (rec__binary_trees__forest__c (us_split_fields5 f))))
        (select temp___603 i))))) 1))
     (= (to_rep
        (rec__binary_trees__cell__right
        (us_split_fields3
        (let ((temp___600 (rec__binary_trees__forest__c (us_split_fields5 f))))
        (select temp___600 (to_rep
                           (rec__binary_trees__cell__parent
                           (us_split_fields3
                           (let ((temp___601 (rec__binary_trees__forest__c
                                             (us_split_fields5 f))))
                           (select temp___601 i)))))))))) i))))))) :pattern (
  (tree_structure f)) )))

(define-fun dynamic_invariant5 ((temp___expr_162 Int)
  (temp___is_init_159 Bool) (temp___skip_constant_160 Bool)
  (temp___do_toplevel_161 Bool)) Bool (=>
                                      (or (= temp___is_init_159 true)
                                      (<= 0 2)) (in_range3 temp___expr_162)))

(define-fun dynamic_invariant6 ((temp___expr_205 us_rep1)
  (temp___is_init_202 Bool) (temp___skip_constant_203 Bool)
  (temp___do_toplevel_204 Bool)) Bool (=> (= temp___do_toplevel_204 true)
                                      (=> (= temp___is_init_202 true)
                                      (< (length
                                         (rec__tree_model__path_type__a
                                         (us_split_fields1 temp___expr_205))) 100))))

(define-fun default_initial_assumption5 ((temp___expr_207 us_rep1)
  (temp___skip_top_level_208 Bool)) Bool (and
                                         (= (length
                                            (rec__tree_model__path_type__a
                                            (us_split_fields1
                                            temp___expr_207))) 0)
                                         (= (rec__tree_model__path_type__k
                                            (us_split_fields1
                                            temp___expr_207)) (of_int 0))))

(define-fun dynamic_predicate ((temp___211 us_rep1)) Bool (< (length
                                                             (rec__tree_model__path_type__a
                                                             (us_split_fields1
                                                             temp___211))) 100))

(define-fun olt ((left us_rep)
  (right us_rep)) Bool (and (< (length left) (length right)) (ite (forall
                                                                  ((n Int))
                                                                  (=>
                                                                  (and
                                                                  (<= 1 n)
                                                                  (<= n 
                                                                  (last left)))
                                                                  (= 
                                                                  (get left
                                                                  n) 
                                                                  (get right
                                                                  n))))
                                                             true false)))

(declare-fun olt__function_guard (Bool us_rep us_rep) Bool)

;; is_add__post_axiom
  (assert true)

;; is_add__def_axiom
  (assert
  (forall ((s1 us_rep) (s2 us_rep))
  (forall ((d Int))
  (! (=> (is_add__function_guard (is_add s1 d s2) s1 d s2)
     (= (= (is_add s1 d s2) true)
     (and (and (= (- (length s2) 1) (length s1)) (= (olt s1 s2) true))
     (= (get s2 (length s2)) d)))) :pattern ((is_add s1 d s2)) ))))

(declare-sort extended_index 0)

(define-fun in_range7 ((x Int)) Bool (and (<= 0 x) (<= x 2147483647)))

(define-fun bool_eq13 ((x Int) (y Int)) Bool (ite (= x y) true false))

(declare-fun attr__ATTRIBUTE_IMAGE8 (Int) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check8 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE8 (us_image) Int)

(declare-fun user_eq12 (extended_index extended_index) Bool)

(declare-fun dummy12 () extended_index)

(declare-datatypes ()
((extended_index__ref
 (mk_extended_index__ref (extended_index__content extended_index)))))
(define-fun extended_index__ref___projection ((a extended_index__ref)) extended_index 
  (extended_index__content a))

(define-fun dynamic_invariant7 ((temp___expr_192 Int)
  (temp___is_init_189 Bool) (temp___skip_constant_190 Bool)
  (temp___do_toplevel_191 Bool)) Bool (=>
                                      (or (= temp___is_init_189 true)
                                      (<= 0 2147483647)) (in_range7
                                      temp___expr_192)))

(declare-sort element_type 0)

(define-fun in_range8 ((x Int)) Bool (and (<= 0 x) (<= x 1)))

(define-fun bool_eq14 ((x Int) (y Int)) Bool (ite (= x y) true false))

(declare-fun attr__ATTRIBUTE_IMAGE9 (Int) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check9 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE9 (us_image) Int)

(declare-fun user_eq13 (element_type element_type) Bool)

(declare-fun dummy13 () element_type)

(declare-datatypes ()
((element_type__ref
 (mk_element_type__ref (element_type__content element_type)))))
(define-fun element_type__ref___projection ((a element_type__ref)) element_type 
  (element_type__content a))

(define-fun dynamic_invariant8 ((temp___expr_186 Int)
  (temp___is_init_183 Bool) (temp___skip_constant_184 Bool)
  (temp___do_toplevel_185 Bool)) Bool (=>
                                      (or (= temp___is_init_183 true)
                                      (<= 0 1)) (in_range8 temp___expr_186)))

;; get__post_axiom
  (assert
  (forall ((container us_rep))
  (forall ((position1 Int))
  (! (=>
     (and (dynamic_invariant7 position1 true true true)
     (and (<= 1 position1) (<= position1 (last container))))
     (dynamic_invariant8 (get container position1) true false true)) :pattern (
  (get container position1)) ))))

(declare-fun f__split_fields () extended_index_type)

(declare-fun f__split_fields1 () (Array Int us_rep2))

(declare-fun f_old__split_fields () extended_index_type)

(declare-fun f_old__split_fields1 () (Array Int us_rep2))

(declare-fun n () Int)

(declare-fun ki () Int)

(declare-fun r () Int)

(declare-fun temp___2339 () Int)

(declare-fun temp___2279 () Int)

(declare-fun temp___2256 () (Array Int us_rep1))

(declare-fun o () us_rep1)

(declare-fun o1 () us_split_fields)

(declare-fun temp___2255 () (Array Int us_rep1))

(declare-fun o2 () us_rep1)

(declare-fun o3 () us_split_fields)

(declare-fun o4 () us_rep)

(declare-fun o5 () Int)

(declare-fun temp___2254 () (Array Int us_rep1))

(declare-fun o6 () us_rep1)

(declare-fun o7 () us_split_fields)

(declare-fun temp___2257 () (Array Int us_rep1))

(declare-fun o8 () Int)

(declare-fun o9 () us_rep1)

(declare-fun o10 () us_split_fields)

(declare-fun o11 () us_rep)

(declare-fun temp___2260 () (Array Int us_rep1))

(declare-fun o12 () Int)

(declare-fun o13 () us_rep1)

(declare-fun o14 () us_split_fields)

(declare-fun o15 () us_rep)

(declare-fun temp___2263 () (Array Int us_rep1))

(declare-fun o16 () us_rep1)

(declare-fun o17 () us_split_fields)

(declare-fun o18 () us_rep)

(declare-fun temp___2264 () (Array Int us_rep1))

(declare-fun o19 () us_rep1)

(declare-fun o20 () us_split_fields)

(declare-fun o21 () us_rep)

(declare-fun o22 () Int)

(declare-fun o23 () Int)

(declare-fun o24 () Int)

(declare-fun temp___2268 () (Array Int us_rep1))

(declare-fun o25 () us_rep1)

(declare-fun o26 () us_split_fields)

(declare-fun temp___2267 () (Array Int us_rep1))

(declare-fun o27 () us_rep1)

(declare-fun o28 () us_split_fields)

(declare-fun o29 () us_rep)

(declare-fun o30 () Int)

(declare-fun temp___2266 () (Array Int us_rep1))

(declare-fun o31 () us_rep1)

(declare-fun o32 () us_split_fields)

(declare-fun o33 () Bool)

(declare-fun o34 () Int)

(declare-fun o35 () Int)

(declare-fun temp___2269 () (Array Int us_rep1))

(declare-fun o36 () Int)

(declare-fun o37 () us_rep1)

(declare-fun o38 () us_split_fields)

(declare-fun o39 () us_rep)

(declare-fun temp___2272 () (Array Int us_rep1))

(declare-fun o40 () Int)

(declare-fun o41 () us_rep1)

(declare-fun o42 () us_split_fields)

(declare-fun o43 () us_rep)

(declare-fun o44 () Int)

(declare-fun o45 () Int)

(declare-fun temp___2275 () (Array Int us_rep1))

(declare-fun o46 () us_rep1)

(declare-fun o47 () us_split_fields)

(declare-fun o48 () us_rep)

(declare-fun temp___2276 () (Array Int us_rep1))

(declare-fun o49 () us_rep1)

(declare-fun o50 () us_split_fields)

(declare-fun o51 () us_rep)

(declare-fun temp___2277 () (Array Int us_rep1))

(declare-fun o52 () Int)

(declare-fun o53 () us_rep1)

(declare-fun o54 () us_split_fields)

(declare-fun o55 () us_rep)

(declare-fun o56 () Int)

(declare-fun temp___2509 () Int)

(declare-fun o57 () Int)

(declare-fun o58 () Int)

(declare-fun r1 () Int)

(declare-fun o59 () Int)

(declare-fun o60 () us_rep2)

(declare-fun o61 () us_split_fields2)

(declare-fun o62 () position_type)

(declare-fun o63 () Int)

(declare-fun o64 () Bool)

(declare-fun o65 () Int)

(declare-fun o66 () Int)

(declare-fun o67 () Int)

(declare-fun o68 () Int)

(declare-fun o69 () (Array Int us_rep1))

(declare-fun temp___2575 () (Array Int us_rep1))

(declare-fun o70 () Int)

(declare-fun o71 () us_rep1)

(declare-fun o72 () us_split_fields)

(declare-fun ki1 () Int)

(declare-fun result () int__ref)

(declare-fun n1 () Int)

(declare-fun n2 () Int)

(declare-fun ki2 () Int)

(declare-fun result1 () int__ref)

(declare-fun ki3 () Int)

(declare-fun result2 () Bool)

(declare-fun result3 () Bool)

(declare-fun result4 () Bool)

(declare-fun result5 () Bool)

(declare-fun result6 () Bool)

(declare-fun result7 () Bool)

(declare-fun ki4 () Int)

(declare-fun ki5 () int__ref)

(declare-fun ki6 () Int)

(declare-fun ki7 () int__ref)

(declare-fun ki8 () Int)

(declare-fun n3 () int__ref)

(declare-fun ki9 () int__ref)

(declare-fun n4 () Int)

(declare-fun ki10 () Int)

(declare-fun n5 () int__ref)

(declare-fun ki11 () int__ref)

(declare-fun n6 () Int)

(declare-fun ki12 () Int)

(declare-fun result8 () int__ref)

(declare-fun r2 () Int)

(declare-fun r3 () Int)

(declare-fun r4 () int__ref)

(declare-fun r5 () Int)

(declare-fun r6 () int__ref)

(declare-fun r7 () Int)

(declare-fun n7 () int__ref)

(declare-fun ki13 () int__ref)

(declare-fun r8 () int__ref)

(declare-fun n8 () Int)

(declare-fun ki14 () Int)

(declare-fun r9 () Int)

(declare-fun result9 () Bool)

(declare-fun result10 () Bool)

(declare-fun result11 () Bool)

(declare-fun result12 () Bool)

(declare-fun result13 () Bool)

(declare-fun result14 () Bool)

(declare-fun result15 () Bool)

;; H
  (assert (in_range1 root))

;; H
  (assert (in_range2 v))

;; H
  (assert
  (and (type_invariant
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1)))
  (type_invariant
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)))))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert
  (and (= (to_rep f__split_fields) (to_rep f_old__split_fields))
  (and (<= root (to_rep f__split_fields))
  (and
  (= (to_rep1
     (rec__binary_trees__cell__position
     (us_split_fields3 (select f_old__split_fields1 root)))) 2)
  (and
  (= (to_rep1
     (rec__binary_trees__cell__position
     (us_split_fields3 (select f__split_fields1 root)))) 2)
  (and (not (= v root))
  (and (and (<= 1 v) (<= v (to_rep f__split_fields)))
  (and
  (= (to_rep1
     (rec__binary_trees__cell__position
     (us_split_fields3 (select f_old__split_fields1 v)))) 2)
  (and
  (not
  (= (to_rep
     (rec__binary_trees__cell__parent
     (us_split_fields3 (select f__split_fields1 v)))) 0))
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             root) (to_rep
                   (rec__binary_trees__cell__parent
                   (us_split_fields3 (select f__split_fields1 v))))))) true)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i v))
  (= (to_rep
     (rec__binary_trees__cell__parent
     (us_split_fields3 (select f__split_fields1 i)))) (to_rep
                                                      (rec__binary_trees__cell__parent
                                                      (us_split_fields3
                                                      (select f_old__split_fields1 i))))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i v))
  (= (to_rep1
     (rec__binary_trees__cell__position
     (us_split_fields3 (select f__split_fields1 i)))) (to_rep1
                                                      (rec__binary_trees__cell__position
                                                      (us_split_fields3
                                                      (select f_old__split_fields1 i))))))))
  (and
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             root) j))) true))
  (= (to_rep
     (rec__binary_trees__cell__left
     (us_split_fields3 (select f__split_fields1 j)))) (to_rep
                                                      (rec__binary_trees__cell__left
                                                      (us_split_fields3
                                                      (select f_old__split_fields1 j))))))))
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             root) j))) true))
  (= (to_rep
     (rec__binary_trees__cell__right
     (us_split_fields3 (select f__split_fields1 j)))) (to_rep
                                                      (rec__binary_trees__cell__right
                                                      (us_split_fields3
                                                      (select f_old__split_fields1 j)))))))))))))))))))))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert
  (forall ((i Int)) (is_concat__function_guard
  (is_concat
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) v)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2
          (mk___split_fields2 f_old__split_fields f_old__split_fields1)) 
          v) i)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) v)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2
          (mk___split_fields2 f_old__split_fields f_old__split_fields1)) 
          v) i)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2254 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2254 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2254 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2254 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2255 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2255 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2255 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2255 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2256 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2256 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2256 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2256 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2264 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2264 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2264 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2264 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2263 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2263 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2263 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2263 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2260 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2260 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2260 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2260 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2257 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2257 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2257 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2257 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2266 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2266 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2266 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2266 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2267 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2267 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2267 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2267 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2268 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2268 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2268 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2268 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2277 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2277 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2277 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2277 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2276 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2276 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2276 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2276 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2275 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2275 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2275 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2275 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2272 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2272 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2272 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2272 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2269 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2269 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2269 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2269 i))))))

;; H
  (assert (is_concat__function_guard (is_concat o55 o48 o51) o55 o48 o51))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert
  (forall ((i Int)) (is_concat__function_guard
  (is_concat
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) v)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2
          (mk___split_fields2 f_old__split_fields f_old__split_fields1)) 
          v) i)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) v)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2
          (mk___split_fields2 f_old__split_fields f_old__split_fields1)) 
          v) i)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) i))))))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert
  (forall ((i Int)) (is_concat__function_guard
  (is_concat
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) v)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2
          (mk___split_fields2 f_old__split_fields f_old__split_fields1)) 
          v) i)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) v)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2
          (mk___split_fields2 f_old__split_fields f_old__split_fields1)) 
          v) i)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2254 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2254 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2254 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2254 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2255 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2255 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2255 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2255 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2256 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2256 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2256 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2256 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2264 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2264 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2264 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2264 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2263 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2263 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2263 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2263 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2260 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2260 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2260 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2260 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2257 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2257 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2257 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2257 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2266 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2266 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2266 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2266 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2267 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2267 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2267 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2267 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2268 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2268 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2268 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2268 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2277 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2277 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2277 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2277 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2276 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2276 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2276 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2276 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2275 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2275 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2275 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2275 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2272 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2272 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2272 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2272 i))))))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2269 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2269 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2269 (parent
                      (mk___rep2
                      (mk___split_fields2 f_old__split_fields
                      f_old__split_fields1)) i))))
  (position
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2269 i))))))

;; H
  (assert (is_concat__function_guard (is_concat o55 o48 o51) o55 o48 o51))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert
  (forall ((i Int)) (is_concat__function_guard
  (is_concat
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) v)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2
          (mk___split_fields2 f_old__split_fields f_old__split_fields1)) 
          v) i)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) v)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2
          (mk___split_fields2 f_old__split_fields f_old__split_fields1)) 
          v) i)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) i))))))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  o58)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  o58))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  r2) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) 
  r2))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  r2) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) 
  r2))

;; H
  (assert (oeq__function_guard1
  (oeq1
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2)
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  r2))
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2)
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  r2)))

;; H
  (assert
  (forall ((p Int)) (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  p) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) p)))

;; H
  (assert
  (forall ((p Int)) (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  p)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  p)))

;; H
  (assert
  (forall ((p Int)) (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  p) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) p)))

;; H
  (assert
  (forall ((p Int)) (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  p)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  p)))

;; H
  (assert
  (forall ((p Int)) (oeq__function_guard1
  (oeq1
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  p)
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  p))
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  p)
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  p))))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  o58)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  o58))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  r2) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) 
  r2))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  r2) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) 
  r2))

;; H
  (assert (oeq__function_guard1
  (oeq1
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2)
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  r2))
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  r2)
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  r2)))

;; H
  (assert
  (forall ((p Int)) (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  p) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) p)))

;; H
  (assert
  (forall ((p Int)) (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  p)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  p)))

;; H
  (assert
  (forall ((p Int)) (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  p) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) p)))

;; H
  (assert
  (forall ((p Int)) (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  p)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  p)))

;; H
  (assert
  (forall ((p Int)) (oeq__function_guard1
  (oeq1
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  p)
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  p))
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  p)
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  p))))

;; H
  (assert
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (and
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i))) true)
  (= (ole
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) 
     v)))
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i)))) true))
  (not
  (= (to_rep
     (rec__binary_trees__cell__parent
     (us_split_fields3 (select f__split_fields1 i)))) 0)))
  (or (= i v)
  (= (ole
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) 
     v)))
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) 
     (to_rep
     (rec__binary_trees__cell__parent
     (us_split_fields3 (select f__split_fields1 i)))))))) true))))))

;; H
  (assert (= result (mk_int__ref n)))

;; H
  (assert (= n1 1))

;; H
  (assert (=> (and (<= 1 n1) (<= n1 100)) (= temp___2339 n1)))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (and
  (and
  (< 0 (length
       (rec__tree_model__path_type__a
       (us_split_fields1
       (select (model__
               (mk___rep2
               (mk___split_fields2 f__split_fields f__split_fields1)) 
               root) v)))))
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i))) true))
  (<= (length
      (rec__tree_model__path_type__a
      (us_split_fields1
      (select (model__
              (mk___rep2
              (mk___split_fields2 f__split_fields f__split_fields1)) 
              root) i)))) (- n2 1)))
  (ite (= (ole
          (rec__tree_model__path_type__a
          (us_split_fields1
          (select (model__
                  (mk___rep2
                  (mk___split_fields2 f__split_fields f__split_fields1))
                  root) v)))
          (rec__tree_model__path_type__a
          (us_split_fields1
          (select (model__
                  (mk___rep2
                  (mk___split_fields2 f__split_fields f__split_fields1))
                  root) i)))) true)
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             v) i))) true)
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             root) i))) true)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             root) i))) true)
  (<= (length
      (rec__tree_model__path_type__a
      (us_split_fields1
      (select (model__
              (mk___rep2
              (mk___split_fields2 f_old__split_fields f_old__split_fields1))
              root) i)))) (- n2 1)))
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i))) true))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (and
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             v) i))) true)
  (<= (length
      (rec__tree_model__path_type__a
      (us_split_fields1
      (select (model__
              (mk___rep2
              (mk___split_fields2 f_old__split_fields f_old__split_fields1))
              v) i)))) (- n2 1)))
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) 
     (to_rep
     (rec__binary_trees__cell__parent
     (us_split_fields3 (select f__split_fields1 v))))))) true))
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i))) true))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i))) true)
  (<= (length
      (rec__tree_model__path_type__a
      (us_split_fields1
      (select (model__
              (mk___rep2
              (mk___split_fields2 f__split_fields f__split_fields1)) 
              root) i)))) (- n2 1)))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             root) i))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i)))
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             root) i)))) true)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i))) true)
  (<= (length
      (rec__tree_model__path_type__a
      (us_split_fields1
      (select (model__
              (mk___rep2
              (mk___split_fields2 f__split_fields f__split_fields1)) 
              root) i)))) (- n2 1)))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             v) i))) true)
  (= (is_concat
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) 
     v)))
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             v) i)))
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i)))) true)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (and (=> (<= 1 100) (in_range1 n2)) (and (<= 1 n2) (<= n2 100)))))

;; H
  (assert (=> (and (<= 1 n1) (<= n1 100)) (= result1 (mk_int__ref ki2))))

;; H
  (assert (=> (and (<= 1 n1) (<= n1 100)) (= ki3 1)))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100)) (= temp___2279 ki3))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100)) (type_invariant
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (and
  (and
  (= temp___2254 (model__
                 (mk___rep2
                 (mk___split_fields2 f__split_fields f__split_fields1)) 
                 root))
  (model____function_guard temp___2254
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) root))
  (and (dynamic_invariant3 temp___2254 true false true)
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2254 root))) true)
  (and
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2254 root)))) 0)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i root))
  (ite (and
       (not
       (= (parent
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          i) 0))
       (= (rec__tree_model__path_type__k
          (us_split_fields1
          (select temp___2254 (parent
                              (mk___rep2
                              (mk___split_fields2 f__split_fields
                              f__split_fields1)) i)))) true))
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2254 i))) true)
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2254 i))) true))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (ite (and
       (= (rec__tree_model__path_type__k
          (us_split_fields1 (select temp___2254 i))) true)
       (not (= i root)))
  (= (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select temp___2254 (parent
                         (mk___rep2
                         (mk___split_fields2 f__split_fields
                         f__split_fields1)) i))))
     (position
     (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2254 i)))) true)
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2254 i)))) 0))))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2254 i))) true)
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2254 j))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2254 i)))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2254 j)))) true))
  (= j i))))))))))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100)) (= o6 (select temp___2254 ki3)))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100)) (= o7 (us_split_fields1 o6)))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (= result4 (rec__tree_model__path_type__k o7)))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result4 true) (type_invariant
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result4 true)
  (and
  (and
  (= temp___2255 (model__
                 (mk___rep2
                 (mk___split_fields2 f__split_fields f__split_fields1)) 
                 root))
  (model____function_guard temp___2255
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) root))
  (and (dynamic_invariant3 temp___2255 true false true)
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2255 root))) true)
  (and
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2255 root)))) 0)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i root))
  (ite (and
       (not
       (= (parent
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          i) 0))
       (= (rec__tree_model__path_type__k
          (us_split_fields1
          (select temp___2255 (parent
                              (mk___rep2
                              (mk___split_fields2 f__split_fields
                              f__split_fields1)) i)))) true))
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2255 i))) true)
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2255 i))) true))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (ite (and
       (= (rec__tree_model__path_type__k
          (us_split_fields1 (select temp___2255 i))) true)
       (not (= i root)))
  (= (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select temp___2255 (parent
                         (mk___rep2
                         (mk___split_fields2 f__split_fields
                         f__split_fields1)) i))))
     (position
     (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2255 i)))) true)
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2255 i)))) 0))))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2255 i))) true)
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2255 j))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2255 i)))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2255 j)))) true))
  (= j i)))))))))))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result4 true) (= o2 (select temp___2255 ki3))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result4 true) (= o3 (us_split_fields1 o2))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result4 true) (= o4 (rec__tree_model__path_type__a o3))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result4 true)
  (and (= o5 (length o4)) (and (in_range5 o5) (<= (+ 0 o5) 2147483647)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result4 true) (= result3 (ite (= o5 n2) true false))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (not (= result4 true)) (= result3 false)))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result3 true) (type_invariant
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result3 true)
  (and
  (and
  (= temp___2256 (model__
                 (mk___rep2
                 (mk___split_fields2 f_old__split_fields
                 f_old__split_fields1)) root))
  (model____function_guard temp___2256
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))
  (and (dynamic_invariant3 temp___2256 true false true)
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2256 root))) true)
  (and
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2256 root)))) 0)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i root))
  (ite (and
       (not
       (= (parent
          (mk___rep2
          (mk___split_fields2 f_old__split_fields f_old__split_fields1)) i) 0))
       (= (rec__tree_model__path_type__k
          (us_split_fields1
          (select temp___2256 (parent
                              (mk___rep2
                              (mk___split_fields2 f_old__split_fields
                              f_old__split_fields1)) i)))) true))
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2256 i))) true)
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2256 i))) true))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (ite (and
       (= (rec__tree_model__path_type__k
          (us_split_fields1 (select temp___2256 i))) true)
       (not (= i root)))
  (= (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select temp___2256 (parent
                         (mk___rep2
                         (mk___split_fields2 f_old__split_fields
                         f_old__split_fields1)) i))))
     (position
     (mk___rep2
     (mk___split_fields2 f_old__split_fields f_old__split_fields1)) i)
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2256 i)))) true)
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2256 i)))) 0))))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2256 i))) true)
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2256 j))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2256 i)))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2256 j)))) true))
  (= j i)))))))))))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result3 true) (= o (select temp___2256 ki3))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result3 true) (= o1 (us_split_fields1 o))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result3 true) (= result2 (rec__tree_model__path_type__k o1))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (not (= result3 true)) (= result2 false)))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true)
  (and
  (= o22 (to_rep1
         (rec__binary_trees__cell__position
         (us_split_fields3 (select f__split_fields1 ki3)))))
  (in_range4
  (to_rep1
  (rec__binary_trees__cell__position
  (us_split_fields3 (select f__split_fields1 ki3))))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true) (type_invariant
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true)
  (and
  (and
  (= temp___2264 (model__
                 (mk___rep2
                 (mk___split_fields2 f_old__split_fields
                 f_old__split_fields1)) root))
  (model____function_guard temp___2264
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))
  (and (dynamic_invariant3 temp___2264 true false true)
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2264 root))) true)
  (and
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2264 root)))) 0)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i root))
  (ite (and
       (not
       (= (parent
          (mk___rep2
          (mk___split_fields2 f_old__split_fields f_old__split_fields1)) i) 0))
       (= (rec__tree_model__path_type__k
          (us_split_fields1
          (select temp___2264 (parent
                              (mk___rep2
                              (mk___split_fields2 f_old__split_fields
                              f_old__split_fields1)) i)))) true))
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2264 i))) true)
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2264 i))) true))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (ite (and
       (= (rec__tree_model__path_type__k
          (us_split_fields1 (select temp___2264 i))) true)
       (not (= i root)))
  (= (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select temp___2264 (parent
                         (mk___rep2
                         (mk___split_fields2 f_old__split_fields
                         f_old__split_fields1)) i))))
     (position
     (mk___rep2
     (mk___split_fields2 f_old__split_fields f_old__split_fields1)) i)
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2264 i)))) true)
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2264 i)))) 0))))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2264 i))) true)
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2264 j))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2264 i)))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2264 j)))) true))
  (= j i)))))))))))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true) (= o19 (select temp___2264 ki3))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true) (= o20 (us_split_fields1 o19))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true) (= o21 (rec__tree_model__path_type__a o20))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true) (type_invariant
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true)
  (and
  (and
  (= temp___2263 (model__
                 (mk___rep2
                 (mk___split_fields2 f__split_fields f__split_fields1)) 
                 root))
  (model____function_guard temp___2263
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) root))
  (and (dynamic_invariant3 temp___2263 true false true)
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2263 root))) true)
  (and
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2263 root)))) 0)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i root))
  (ite (and
       (not
       (= (parent
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          i) 0))
       (= (rec__tree_model__path_type__k
          (us_split_fields1
          (select temp___2263 (parent
                              (mk___rep2
                              (mk___split_fields2 f__split_fields
                              f__split_fields1)) i)))) true))
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2263 i))) true)
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2263 i))) true))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (ite (and
       (= (rec__tree_model__path_type__k
          (us_split_fields1 (select temp___2263 i))) true)
       (not (= i root)))
  (= (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select temp___2263 (parent
                         (mk___rep2
                         (mk___split_fields2 f__split_fields
                         f__split_fields1)) i))))
     (position
     (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2263 i)))) true)
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2263 i)))) 0))))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2263 i))) true)
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2263 j))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2263 i)))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2263 j)))) true))
  (= j i)))))))))))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true) (= o16 (select temp___2263 ki3))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true) (= o17 (us_split_fields1 o16))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true) (= o18 (rec__tree_model__path_type__a o17))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true) (type_invariant
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true)
  (and
  (and
  (= temp___2260 (model__
                 (mk___rep2
                 (mk___split_fields2 f_old__split_fields
                 f_old__split_fields1)) root))
  (model____function_guard temp___2260
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))
  (and (dynamic_invariant3 temp___2260 true false true)
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2260 root))) true)
  (and
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2260 root)))) 0)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i root))
  (ite (and
       (not
       (= (parent
          (mk___rep2
          (mk___split_fields2 f_old__split_fields f_old__split_fields1)) i) 0))
       (= (rec__tree_model__path_type__k
          (us_split_fields1
          (select temp___2260 (parent
                              (mk___rep2
                              (mk___split_fields2 f_old__split_fields
                              f_old__split_fields1)) i)))) true))
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2260 i))) true)
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2260 i))) true))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (ite (and
       (= (rec__tree_model__path_type__k
          (us_split_fields1 (select temp___2260 i))) true)
       (not (= i root)))
  (= (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select temp___2260 (parent
                         (mk___rep2
                         (mk___split_fields2 f_old__split_fields
                         f_old__split_fields1)) i))))
     (position
     (mk___rep2
     (mk___split_fields2 f_old__split_fields f_old__split_fields1)) i)
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2260 i)))) true)
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2260 i)))) 0))))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2260 i))) true)
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2260 j))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2260 i)))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2260 j)))) true))
  (= j i)))))))))))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true)
  (and
  (<= 1 (to_rep
        (rec__binary_trees__cell__parent
        (us_split_fields3 (select f__split_fields1 ki3)))))
  (<= (to_rep
      (rec__binary_trees__cell__parent
      (us_split_fields3 (select f__split_fields1 ki3)))) 100))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true)
  (= o12 (to_rep
         (rec__binary_trees__cell__parent
         (us_split_fields3 (select f__split_fields1 ki3)))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true) (= o13 (select temp___2260 o12))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true) (= o14 (us_split_fields1 o13))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true) (= o15 (rec__tree_model__path_type__a o14))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true) (type_invariant
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true)
  (and
  (and
  (= temp___2257 (model__
                 (mk___rep2
                 (mk___split_fields2 f__split_fields f__split_fields1)) 
                 root))
  (model____function_guard temp___2257
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) root))
  (and (dynamic_invariant3 temp___2257 true false true)
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2257 root))) true)
  (and
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2257 root)))) 0)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i root))
  (ite (and
       (not
       (= (parent
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          i) 0))
       (= (rec__tree_model__path_type__k
          (us_split_fields1
          (select temp___2257 (parent
                              (mk___rep2
                              (mk___split_fields2 f__split_fields
                              f__split_fields1)) i)))) true))
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2257 i))) true)
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2257 i))) true))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (ite (and
       (= (rec__tree_model__path_type__k
          (us_split_fields1 (select temp___2257 i))) true)
       (not (= i root)))
  (= (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select temp___2257 (parent
                         (mk___rep2
                         (mk___split_fields2 f__split_fields
                         f__split_fields1)) i))))
     (position
     (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2257 i)))) true)
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2257 i)))) 0))))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2257 i))) true)
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2257 j))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2257 i)))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2257 j)))) true))
  (= j i)))))))))))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true)
  (and
  (<= 1 (to_rep
        (rec__binary_trees__cell__parent
        (us_split_fields3 (select f__split_fields1 ki3)))))
  (<= (to_rep
      (rec__binary_trees__cell__parent
      (us_split_fields3 (select f__split_fields1 ki3)))) 100))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true)
  (= o8 (to_rep
        (rec__binary_trees__cell__parent
        (us_split_fields3 (select f__split_fields1 ki3)))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true) (= o9 (select temp___2257 o8))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true) (= o10 (us_split_fields1 o9))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true) (= o11 (rec__tree_model__path_type__a o10))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result2 true) (= (oeq o18 o21) true)))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100)) (type_invariant
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (and
  (and
  (= temp___2266 (model__
                 (mk___rep2
                 (mk___split_fields2 f__split_fields f__split_fields1)) 
                 root))
  (model____function_guard temp___2266
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) root))
  (and (dynamic_invariant3 temp___2266 true false true)
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2266 root))) true)
  (and
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2266 root)))) 0)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i root))
  (ite (and
       (not
       (= (parent
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          i) 0))
       (= (rec__tree_model__path_type__k
          (us_split_fields1
          (select temp___2266 (parent
                              (mk___rep2
                              (mk___split_fields2 f__split_fields
                              f__split_fields1)) i)))) true))
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2266 i))) true)
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2266 i))) true))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (ite (and
       (= (rec__tree_model__path_type__k
          (us_split_fields1 (select temp___2266 i))) true)
       (not (= i root)))
  (= (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select temp___2266 (parent
                         (mk___rep2
                         (mk___split_fields2 f__split_fields
                         f__split_fields1)) i))))
     (position
     (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2266 i)))) true)
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2266 i)))) 0))))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2266 i))) true)
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2266 j))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2266 i)))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2266 j)))) true))
  (= j i))))))))))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100)) (= o31 (select temp___2266 ki3)))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100)) (= o32 (us_split_fields1 o31)))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (= result7 (rec__tree_model__path_type__k o32)))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result7 true) (type_invariant
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result7 true)
  (and
  (and
  (= temp___2267 (model__
                 (mk___rep2
                 (mk___split_fields2 f__split_fields f__split_fields1)) 
                 root))
  (model____function_guard temp___2267
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) root))
  (and (dynamic_invariant3 temp___2267 true false true)
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2267 root))) true)
  (and
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2267 root)))) 0)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i root))
  (ite (and
       (not
       (= (parent
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          i) 0))
       (= (rec__tree_model__path_type__k
          (us_split_fields1
          (select temp___2267 (parent
                              (mk___rep2
                              (mk___split_fields2 f__split_fields
                              f__split_fields1)) i)))) true))
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2267 i))) true)
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2267 i))) true))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (ite (and
       (= (rec__tree_model__path_type__k
          (us_split_fields1 (select temp___2267 i))) true)
       (not (= i root)))
  (= (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select temp___2267 (parent
                         (mk___rep2
                         (mk___split_fields2 f__split_fields
                         f__split_fields1)) i))))
     (position
     (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2267 i)))) true)
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2267 i)))) 0))))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2267 i))) true)
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2267 j))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2267 i)))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2267 j)))) true))
  (= j i)))))))))))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result7 true) (= o27 (select temp___2267 ki3))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result7 true) (= o28 (us_split_fields1 o27))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result7 true) (= o29 (rec__tree_model__path_type__a o28))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result7 true)
  (and (= o30 (length o29)) (and (in_range5 o30) (<= (+ 0 o30) 2147483647)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result7 true) (= result6 (ite (= o30 n2) true false))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (not (= result7 true)) (= result6 false)))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result6 true) (and (= o23 v) (in_range1 v))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result6 true) (type_invariant
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result6 true) (and (= o24 v) (in_range1 v))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result6 true)
  (and
  (and
  (= temp___2268 (model__
                 (mk___rep2
                 (mk___split_fields2 f_old__split_fields
                 f_old__split_fields1)) o24))
  (model____function_guard temp___2268
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  o24))
  (and (dynamic_invariant3 temp___2268 true false true)
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2268 o24))) true)
  (and
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2268 o24)))) 0)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i o24))
  (ite (and
       (not
       (= (parent
          (mk___rep2
          (mk___split_fields2 f_old__split_fields f_old__split_fields1)) i) 0))
       (= (rec__tree_model__path_type__k
          (us_split_fields1
          (select temp___2268 (parent
                              (mk___rep2
                              (mk___split_fields2 f_old__split_fields
                              f_old__split_fields1)) i)))) true))
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2268 i))) true)
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2268 i))) true))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (ite (and
       (= (rec__tree_model__path_type__k
          (us_split_fields1 (select temp___2268 i))) true)
       (not (= i o24)))
  (= (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select temp___2268 (parent
                         (mk___rep2
                         (mk___split_fields2 f_old__split_fields
                         f_old__split_fields1)) i))))
     (position
     (mk___rep2
     (mk___split_fields2 f_old__split_fields f_old__split_fields1)) i)
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2268 i)))) true)
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2268 i)))) 0))))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2268 i))) true)
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2268 j))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2268 i)))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2268 j)))) true))
  (= j i)))))))))))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result6 true) (= o25 (select temp___2268 ki3))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result6 true) (= o26 (us_split_fields1 o25))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result6 true) (= o33 (rec__tree_model__path_type__k o26))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (not (= result6 true)) (= o33 false)))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (= result5 (ite (and (= o33 true) (not (= ki3 v))) true false)))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true)
  (and
  (= o56 (to_rep1
         (rec__binary_trees__cell__position
         (us_split_fields3 (select f__split_fields1 ki3)))))
  (in_range4
  (to_rep1
  (rec__binary_trees__cell__position
  (us_split_fields3 (select f__split_fields1 ki3))))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (type_invariant
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true)
  (and
  (and
  (= temp___2277 (model__
                 (mk___rep2
                 (mk___split_fields2 f__split_fields f__split_fields1)) 
                 root))
  (model____function_guard temp___2277
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) root))
  (and (dynamic_invariant3 temp___2277 true false true)
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2277 root))) true)
  (and
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2277 root)))) 0)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i root))
  (ite (and
       (not
       (= (parent
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          i) 0))
       (= (rec__tree_model__path_type__k
          (us_split_fields1
          (select temp___2277 (parent
                              (mk___rep2
                              (mk___split_fields2 f__split_fields
                              f__split_fields1)) i)))) true))
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2277 i))) true)
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2277 i))) true))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (ite (and
       (= (rec__tree_model__path_type__k
          (us_split_fields1 (select temp___2277 i))) true)
       (not (= i root)))
  (= (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select temp___2277 (parent
                         (mk___rep2
                         (mk___split_fields2 f__split_fields
                         f__split_fields1)) i))))
     (position
     (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2277 i)))) true)
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2277 i)))) 0))))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2277 i))) true)
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2277 j))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2277 i)))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2277 j)))) true))
  (= j i)))))))))))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (and (<= 1 v) (<= v 100))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100)) (=> (= result5 true) (= o52 v)))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (= o53 (select temp___2277 o52))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (= o54 (us_split_fields1 o53))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (= o55 (rec__tree_model__path_type__a o54))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (type_invariant
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true)
  (and
  (and
  (= temp___2276 (model__
                 (mk___rep2
                 (mk___split_fields2 f__split_fields f__split_fields1)) 
                 root))
  (model____function_guard temp___2276
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) root))
  (and (dynamic_invariant3 temp___2276 true false true)
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2276 root))) true)
  (and
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2276 root)))) 0)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i root))
  (ite (and
       (not
       (= (parent
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          i) 0))
       (= (rec__tree_model__path_type__k
          (us_split_fields1
          (select temp___2276 (parent
                              (mk___rep2
                              (mk___split_fields2 f__split_fields
                              f__split_fields1)) i)))) true))
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2276 i))) true)
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2276 i))) true))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (ite (and
       (= (rec__tree_model__path_type__k
          (us_split_fields1 (select temp___2276 i))) true)
       (not (= i root)))
  (= (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select temp___2276 (parent
                         (mk___rep2
                         (mk___split_fields2 f__split_fields
                         f__split_fields1)) i))))
     (position
     (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2276 i)))) true)
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2276 i)))) 0))))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2276 i))) true)
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2276 j))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2276 i)))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2276 j)))) true))
  (= j i)))))))))))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (= o49 (select temp___2276 ki3))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (= o50 (us_split_fields1 o49))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (= o51 (rec__tree_model__path_type__a o50))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (and (= o44 v) (in_range1 v))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (type_invariant
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (and (= o45 v) (in_range1 v))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true)
  (and
  (and
  (= temp___2275 (model__
                 (mk___rep2
                 (mk___split_fields2 f_old__split_fields
                 f_old__split_fields1)) o45))
  (model____function_guard temp___2275
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  o45))
  (and (dynamic_invariant3 temp___2275 true false true)
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2275 o45))) true)
  (and
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2275 o45)))) 0)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i o45))
  (ite (and
       (not
       (= (parent
          (mk___rep2
          (mk___split_fields2 f_old__split_fields f_old__split_fields1)) i) 0))
       (= (rec__tree_model__path_type__k
          (us_split_fields1
          (select temp___2275 (parent
                              (mk___rep2
                              (mk___split_fields2 f_old__split_fields
                              f_old__split_fields1)) i)))) true))
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2275 i))) true)
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2275 i))) true))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (ite (and
       (= (rec__tree_model__path_type__k
          (us_split_fields1 (select temp___2275 i))) true)
       (not (= i o45)))
  (= (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select temp___2275 (parent
                         (mk___rep2
                         (mk___split_fields2 f_old__split_fields
                         f_old__split_fields1)) i))))
     (position
     (mk___rep2
     (mk___split_fields2 f_old__split_fields f_old__split_fields1)) i)
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2275 i)))) true)
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2275 i)))) 0))))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2275 i))) true)
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2275 j))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2275 i)))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2275 j)))) true))
  (= j i)))))))))))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (= o46 (select temp___2275 ki3))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (= o47 (us_split_fields1 o46))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (= o48 (rec__tree_model__path_type__a o47))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (type_invariant
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true)
  (and
  (and
  (= temp___2272 (model__
                 (mk___rep2
                 (mk___split_fields2 f__split_fields f__split_fields1)) 
                 root))
  (model____function_guard temp___2272
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) root))
  (and (dynamic_invariant3 temp___2272 true false true)
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2272 root))) true)
  (and
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2272 root)))) 0)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i root))
  (ite (and
       (not
       (= (parent
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          i) 0))
       (= (rec__tree_model__path_type__k
          (us_split_fields1
          (select temp___2272 (parent
                              (mk___rep2
                              (mk___split_fields2 f__split_fields
                              f__split_fields1)) i)))) true))
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2272 i))) true)
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2272 i))) true))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (ite (and
       (= (rec__tree_model__path_type__k
          (us_split_fields1 (select temp___2272 i))) true)
       (not (= i root)))
  (= (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select temp___2272 (parent
                         (mk___rep2
                         (mk___split_fields2 f__split_fields
                         f__split_fields1)) i))))
     (position
     (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2272 i)))) true)
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2272 i)))) 0))))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2272 i))) true)
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2272 j))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2272 i)))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2272 j)))) true))
  (= j i)))))))))))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true)
  (and
  (<= 1 (to_rep
        (rec__binary_trees__cell__parent
        (us_split_fields3 (select f__split_fields1 ki3)))))
  (<= (to_rep
      (rec__binary_trees__cell__parent
      (us_split_fields3 (select f__split_fields1 ki3)))) 100))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true)
  (= o40 (to_rep
         (rec__binary_trees__cell__parent
         (us_split_fields3 (select f__split_fields1 ki3)))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (= o41 (select temp___2272 o40))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (= o42 (us_split_fields1 o41))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (= o43 (rec__tree_model__path_type__a o42))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (and (= o34 v) (in_range1 v))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (type_invariant
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1)))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (and (= o35 v) (in_range1 v))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true)
  (and
  (and
  (= temp___2269 (model__
                 (mk___rep2
                 (mk___split_fields2 f_old__split_fields
                 f_old__split_fields1)) o35))
  (model____function_guard temp___2269
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  o35))
  (and (dynamic_invariant3 temp___2269 true false true)
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2269 o35))) true)
  (and
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2269 o35)))) 0)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i o35))
  (ite (and
       (not
       (= (parent
          (mk___rep2
          (mk___split_fields2 f_old__split_fields f_old__split_fields1)) i) 0))
       (= (rec__tree_model__path_type__k
          (us_split_fields1
          (select temp___2269 (parent
                              (mk___rep2
                              (mk___split_fields2 f_old__split_fields
                              f_old__split_fields1)) i)))) true))
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2269 i))) true)
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2269 i))) true))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (ite (and
       (= (rec__tree_model__path_type__k
          (us_split_fields1 (select temp___2269 i))) true)
       (not (= i o35)))
  (= (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select temp___2269 (parent
                         (mk___rep2
                         (mk___split_fields2 f_old__split_fields
                         f_old__split_fields1)) i))))
     (position
     (mk___rep2
     (mk___split_fields2 f_old__split_fields f_old__split_fields1)) i)
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2269 i)))) true)
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2269 i)))) 0))))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2269 i))) true)
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2269 j))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2269 i)))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2269 j)))) true))
  (= j i)))))))))))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true)
  (and
  (<= 1 (to_rep
        (rec__binary_trees__cell__parent
        (us_split_fields3 (select f__split_fields1 ki3)))))
  (<= (to_rep
      (rec__binary_trees__cell__parent
      (us_split_fields3 (select f__split_fields1 ki3)))) 100))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true)
  (= o36 (to_rep
         (rec__binary_trees__cell__parent
         (us_split_fields3 (select f__split_fields1 ki3)))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (= o37 (select temp___2269 o36))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (= o38 (us_split_fields1 o37))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (= o39 (rec__tree_model__path_type__a o38))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (=> (= result5 true) (= (is_concat o55 o48 o51) true)))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i ki4))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i))) true)
  (<= (length
      (rec__tree_model__path_type__a
      (us_split_fields1
      (select (model__
              (mk___rep2
              (mk___split_fields2 f__split_fields f__split_fields1)) 
              root) i)))) n2))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             root) i))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i)))
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             root) i)))) true))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i ki4))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i))) true)
  (<= (length
      (rec__tree_model__path_type__a
      (us_split_fields1
      (select (model__
              (mk___rep2
              (mk___split_fields2 f__split_fields f__split_fields1)) 
              root) i)))) n2))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             v) i))) true)
  (= (is_concat
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) 
     v)))
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             v) i)))
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i)))) true))))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100))
  (and (=> (<= 1 100) (in_range1 ki4)) (and (<= 1 ki4) (<= ki4 100))))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100)) (= ki4 100))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100)) (= ki5 (mk_int__ref ki4)))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100)) (= ki6 ki4))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100)) (= ki7 ki5))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (and (<= 1 ki3) (<= ki3 100)) (= ki8 ki6))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (not (and (<= 1 ki3) (<= ki3 100))) (= ki7 (mk_int__ref ki3)))))

;; H
  (assert
  (=> (and (<= 1 n1) (<= n1 100))
  (=> (not (and (<= 1 ki3) (<= ki3 100))) (= ki8 ki3))))

;; H
  (assert (=> (and (<= 1 n1) (<= n1 100)) (= n2 100)))

;; H
  (assert (=> (and (<= 1 n1) (<= n1 100)) (= ki9 ki7)))

;; H
  (assert (=> (and (<= 1 n1) (<= n1 100)) (= n3 (mk_int__ref n2))))

;; H
  (assert (=> (and (<= 1 n1) (<= n1 100)) (= ki10 ki8)))

;; H
  (assert (=> (and (<= 1 n1) (<= n1 100)) (= n4 n2)))

;; H
  (assert (=> (and (<= 1 n1) (<= n1 100)) (= ki11 ki9)))

;; H
  (assert (=> (and (<= 1 n1) (<= n1 100)) (= n5 n3)))

;; H
  (assert (=> (and (<= 1 n1) (<= n1 100)) (= ki12 ki10)))

;; H
  (assert (=> (and (<= 1 n1) (<= n1 100)) (= n6 n4)))

;; H
  (assert (=> (not (and (<= 1 n1) (<= n1 100))) (= ki11 (mk_int__ref ki))))

;; H
  (assert (=> (not (and (<= 1 n1) (<= n1 100))) (= n5 (mk_int__ref n1))))

;; H
  (assert (=> (not (and (<= 1 n1) (<= n1 100))) (= ki12 ki1)))

;; H
  (assert (=> (not (and (<= 1 n1) (<= n1 100))) (= n6 n1)))

;; H
  (assert
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             v) i))) true)
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i))) true)))))

;; H
  (assert
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i))) true)
  (ite (= (ole
          (rec__tree_model__path_type__a
          (us_split_fields1
          (select (model__
                  (mk___rep2
                  (mk___split_fields2 f__split_fields f__split_fields1))
                  root) v)))
          (rec__tree_model__path_type__a
          (us_split_fields1
          (select (model__
                  (mk___rep2
                  (mk___split_fields2 f__split_fields f__split_fields1))
                  root) i)))) true)
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             v) i))) true)
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             root) i))) true))))))

;; H
  (assert (= (to_rep f_old__split_fields) r246b))

;; H
  (assert (in_range2 r246b))

;; H
  (assert (= result8 (mk_int__ref r)))

;; H
  (assert (= r2 1))

;; H
  (assert (=> (and (<= 1 r2) (<= r2 r246b)) (= temp___2509 r2)))

;; H
  (assert
  (=> (and (<= 1 r2) (<= r2 r246b))
  (=>
  (and (not (= r2 root))
  (and (not (= r2 v))
  (= (to_rep1
     (rec__binary_trees__cell__position
     (us_split_fields3 (select f_old__split_fields1 r2)))) 2)))
  (type_invariant
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))))))

;; H
  (assert
  (=> (and (<= 1 r2) (<= r2 r246b))
  (=>
  (and (not (= r2 root))
  (and (not (= r2 v))
  (= (to_rep1
     (rec__binary_trees__cell__position
     (us_split_fields3 (select f_old__split_fields1 r2)))) 2)))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (or
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             root) i))) true))
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             r2) i))) true))))))))

;; H
  (assert
  (=> (and (<= 1 r2) (<= r2 r246b))
  (=>
  (and (not (= r2 root))
  (and (not (= r2 v))
  (= (to_rep1
     (rec__binary_trees__cell__position
     (us_split_fields3 (select f_old__split_fields1 r2)))) 2)))
  (and (= o57 v) (in_range1 v)))))

;; H
  (assert
  (=> (and (<= 1 r2) (<= r2 r246b))
  (=>
  (and (not (= r2 root))
  (and (not (= r2 v))
  (= (to_rep1
     (rec__binary_trees__cell__position
     (us_split_fields3 (select f_old__split_fields1 r2)))) 2)))
  (type_invariant
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))))))

;; H
  (assert
  (=> (and (<= 1 r2) (<= r2 r246b))
  (=>
  (and (not (= r2 root))
  (and (not (= r2 v))
  (= (to_rep1
     (rec__binary_trees__cell__position
     (us_split_fields3 (select f_old__split_fields1 r2)))) 2)))
  (and (= o58 v) (in_range1 v)))))

;; H
  (assert
  (=> (and (<= 1 r2) (<= r2 r246b))
  (=>
  (and (not (= r2 root))
  (and (not (= r2 v))
  (= (to_rep1
     (rec__binary_trees__cell__position
     (us_split_fields3 (select f_old__split_fields1 r2)))) 2)))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (or
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             o58) i))) true))
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             r2) i))) true))))))))

;; H
  (assert
  (=> (and (<= 1 r2) (<= r2 r246b))
  (=>
  (and (not (= r2 root))
  (and (not (= r2 v))
  (= (to_rep1
     (rec__binary_trees__cell__position
     (us_split_fields3 (select f_old__split_fields1 r2)))) 2)))
  (and
  (= (to_rep1
     (rec__binary_trees__cell__position
     (us_split_fields3 (select f__split_fields1 r2)))) 2)
  (= (oeq1
     (model__
     (mk___rep2
     (mk___split_fields2 f_old__split_fields f_old__split_fields1)) r2)
     (model__
     (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) 
     r2)) true)))))

;; H
  (assert
  (=> (and (<= 1 r2) (<= r2 r246b))
  (forall ((p Int))
  (=> (and (<= 1 p) (<= p r3))
  (=>
  (and (and (not (= p root)) (not (= p v)))
  (= (to_rep1
     (rec__binary_trees__cell__position
     (us_split_fields3 (select f_old__split_fields1 p)))) 2))
  (= (oeq1
     (model__
     (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) p)
     (model__
     (mk___rep2
     (mk___split_fields2 f_old__split_fields f_old__split_fields1)) p)) true))))))

;; H
  (assert
  (=> (and (<= 1 r2) (<= r2 r246b))
  (and (=> (<= 1 r246b) (dynamic_property 1 r246b r3))
  (and (<= 1 r3) (<= r3 r246b)))))

;; H
  (assert (=> (and (<= 1 r2) (<= r2 r246b)) (= r3 r246b)))

;; H
  (assert (=> (and (<= 1 r2) (<= r2 r246b)) (= r4 (mk_int__ref r3))))

;; H
  (assert (=> (and (<= 1 r2) (<= r2 r246b)) (= r5 r3)))

;; H
  (assert (=> (and (<= 1 r2) (<= r2 r246b)) (= r6 r4)))

;; H
  (assert (=> (and (<= 1 r2) (<= r2 r246b)) (= r7 r5)))

;; H
  (assert (=> (not (and (<= 1 r2) (<= r2 r246b))) (= r6 (mk_int__ref r2))))

;; H
  (assert (=> (not (and (<= 1 r2) (<= r2 r246b))) (= r7 r2)))

;; H
  (assert (= r8 r6))

;; H
  (assert (= ki13 ki11))

;; H
  (assert (= n7 n5))

;; H
  (assert (= r9 r7))

;; H
  (assert (= ki14 ki12))

;; H
  (assert (= n8 n6))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2575 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2575 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select temp___2575 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i)
  (rec__tree_model__path_type__a (us_split_fields1 (select temp___2575 i))))))

;; H
  (assert (type_invariant
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))))

;; H
  (assert
  (and
  (and
  (= temp___2575 (model__
                 (mk___rep2
                 (mk___split_fields2 f__split_fields f__split_fields1)) 
                 root))
  (model____function_guard temp___2575
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) root))
  (and (dynamic_invariant3 temp___2575 true false true)
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2575 root))) true)
  (and
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2575 root)))) 0)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i root))
  (ite (and
       (not
       (= (parent
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          i) 0))
       (= (rec__tree_model__path_type__k
          (us_split_fields1
          (select temp___2575 (parent
                              (mk___rep2
                              (mk___split_fields2 f__split_fields
                              f__split_fields1)) i)))) true))
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2575 i))) true)
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2575 i))) true))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (ite (and
       (= (rec__tree_model__path_type__k
          (us_split_fields1 (select temp___2575 i))) true)
       (not (= i root)))
  (= (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select temp___2575 (parent
                         (mk___rep2
                         (mk___split_fields2 f__split_fields
                         f__split_fields1)) i))))
     (position
     (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2575 i)))) true)
  (= (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2575 i)))) 0))))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2575 i))) true)
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select temp___2575 j))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2575 i)))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select temp___2575 j)))) true))
  (= j i))))))))))))))

;; H
  (assert (and (<= 1 v) (<= v 100)))

;; H
  (assert (= o70 v))

;; H
  (assert (= o71 (select temp___2575 o70)))

;; H
  (assert (= o72 (us_split_fields1 o71)))

;; H
  (assert (= result9 (rec__tree_model__path_type__k o72)))

;; H
  (assert (= result9 true))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert
  (= (= result10 true)
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i))) true)
  (ite (= (ole
          (rec__tree_model__path_type__a
          (us_split_fields1
          (select (model__
                  (mk___rep2
                  (mk___split_fields2 f__split_fields f__split_fields1))
                  root) v)))
          (rec__tree_model__path_type__a
          (us_split_fields1
          (select (model__
                  (mk___rep2
                  (mk___split_fields2 f__split_fields f__split_fields1))
                  root) i)))) true)
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             v) i))) true)
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             root) i))) true)))))))

;; H
  (assert (= result10 true))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert
  (= (= result11 true)
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             root) i))) true)
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i))) true))))))

;; H
  (assert (= result11 true))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert
  (= (= result12 true)
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             v) i))) true)
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i))) true))))))

;; H
  (assert (= result12 true))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  root))

;; H
  (assert
  (= (= result13 true)
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             root) i))) true)
  (= (oeq
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i)))
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             root) i)))) true))))))

;; H
  (assert (= result13 true))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert (model____function_guard
  (model__
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  v))

;; H
  (assert (model____function_guard
  (model__ (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  root))

;; H
  (assert
  (forall ((i Int)) (is_concat__function_guard
  (is_concat
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) v)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2
          (mk___split_fields2 f_old__split_fields f_old__split_fields1)) 
          v) i)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) v)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2
          (mk___split_fields2 f_old__split_fields f_old__split_fields1)) 
          v) i)))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select (model__
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          root) i))))))

;; H
  (assert
  (= (= result14 true)
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             v) i))) true)
  (= (is_concat
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) 
     v)))
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f_old__split_fields f_old__split_fields1))
             v) i)))
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select (model__
             (mk___rep2
             (mk___split_fields2 f__split_fields f__split_fields1)) root) i)))) true))))))

;; H
  (assert (= result14 true))

;; H
  (assert (<= 1 r1))

;; H
  (assert (<= r1 (to_rep f__split_fields)))

;; H
  (assert (and (<= 1 r1) (<= r1 100)))

;; H
  (assert (= o59 r1))

;; H
  (assert (= o60 (select f_old__split_fields1 o59)))

;; H
  (assert (= o61 (us_split_fields3 o60)))

;; H
  (assert (= o62 (rec__binary_trees__cell__position o61)))

;; H
  (assert (= o63 (to_rep1 o62)))

;; H
  (assert (= o64 (ite (= o63 2) true false)))

;; H
  (assert
  (= result15 (ite (not (= r1 root)) (ite (not (= r1 v)) o64 false) false)))

;; H
  (assert (= result15 true))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (parent__function_guard
  (parent (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (position__function_guard
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)))

;; H
  (assert
  (forall ((i Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select o69 (parent
              (mk___rep2
              (mk___split_fields2 f__split_fields f__split_fields1)) i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (rec__tree_model__path_type__a (us_split_fields1 (select o69 i))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select o69 (parent
              (mk___rep2
              (mk___split_fields2 f__split_fields f__split_fields1)) i))))
  (position (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
  i) (rec__tree_model__path_type__a (us_split_fields1 (select o69 i))))))

;; H
  (assert (and (= o67 r1) (in_range1 r1)))

;; H
  (assert (type_invariant
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))))

;; H
  (assert (and (= o68 r1) (in_range1 r1)))

;; H
  (assert
  (and
  (and
  (= o69 (model__
         (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
         o68))
  (model____function_guard o69
  (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) o68))
  (and (dynamic_invariant3 o69 true false true)
  (and
  (= (rec__tree_model__path_type__k (us_split_fields1 (select o69 o68))) true)
  (and
  (= (length
     (rec__tree_model__path_type__a (us_split_fields1 (select o69 o68)))) 0)
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=> (not (= i o68))
  (ite (and
       (not
       (= (parent
          (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1))
          i) 0))
       (= (rec__tree_model__path_type__k
          (us_split_fields1
          (select o69 (parent
                      (mk___rep2
                      (mk___split_fields2 f__split_fields f__split_fields1))
                      i)))) true))
  (= (rec__tree_model__path_type__k (us_split_fields1 (select o69 i))) true)
  (not
  (= (rec__tree_model__path_type__k (us_split_fields1 (select o69 i))) true))))))
  (and
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (ite (and
       (= (rec__tree_model__path_type__k (us_split_fields1 (select o69 i))) true)
       (not (= i o68)))
  (= (is_add
     (rec__tree_model__path_type__a
     (us_split_fields1
     (select o69 (parent
                 (mk___rep2
                 (mk___split_fields2 f__split_fields f__split_fields1)) i))))
     (position
     (mk___rep2 (mk___split_fields2 f__split_fields f__split_fields1)) i)
     (rec__tree_model__path_type__a (us_split_fields1 (select o69 i)))) true)
  (= (length
     (rec__tree_model__path_type__a (us_split_fields1 (select o69 i)))) 0))))
  (forall ((i Int))
  (=> (and (<= 1 i) (<= i 100))
  (=>
  (= (rec__tree_model__path_type__k (us_split_fields1 (select o69 i))) true)
  (forall ((j Int))
  (=> (and (<= 1 j) (<= j 100))
  (=>
  (and
  (= (rec__tree_model__path_type__k (us_split_fields1 (select o69 j))) true)
  (= (oeq (rec__tree_model__path_type__a (us_split_fields1 (select o69 i)))
     (rec__tree_model__path_type__a (us_split_fields1 (select o69 j)))) true))
  (= j i))))))))))))))

;; H
  (assert (and (= o65 r1) (in_range1 r1)))

;; H
  (assert (type_invariant
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))))

;; H
  (assert (and (= o66 r1) (in_range1 r1)))

;; H
  (assert (valid_root__function_guard
  (valid_root
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  o66)
  (mk___rep2 (mk___split_fields2 f_old__split_fields f_old__split_fields1))
  o66))

(assert
;; WP_parameter_def
 ;; File "system.ads", line 1, characters 0-0
  (not
  (= (valid_root
     (mk___rep2
     (mk___split_fields2 f_old__split_fields f_old__split_fields1)) o66) true)))
(check-sat)
