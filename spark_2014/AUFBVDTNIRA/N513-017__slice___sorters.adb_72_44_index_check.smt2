...
;; H
  ...
  (forall ((j2 Int))
  (=> (and (<= (to_rep (first o2)) j2) (<= j2 (to_rep (last o2))))
  (<= (to_rep (select o1 o3)) (to_rep (select o1 j2)))))))))
...
;; H
  ...
  (forall ((j2 Int))
  (=> (and (<= (to_rep values__first) j2) (<= j2 (to_rep values__last)))
  (=> (and (not (= j2 o4)) (not (= j2 smallest1)))
  (= (to_rep (select values2 j2)) (to_rep (select values j2))))))))))
...
;; H
  ...
  (forall ((j2 Int))
  (=> (and (<= (to_rep values__first) j2) (<= j2 (- current2 1)))
  (<= (to_rep (select values3 j2)) (to_rep (select values3 (+ j2 1)))))))
  (forall ((j2 Int))
  (=> (and (<= current2 j2) (<= j2 (to_rep values__last)))
  (<= (to_rep (select values3 current2)) (to_rep (select values3 j2)))))))
...
;; H
  (assert
  (=> (<= (to_rep values__first) current2)
  (= result3 (ite (< current2 (to_rep values__last)) true false))))
...
;; H
  (assert
  (=> (= result3 true)
  (and (= o5 (- current2 1)) (in_range1 (- current2 1)))))

;; H
  (assert (=> (= result3 true) (= o6 (ite (<= j1 o5) true false))))

;; H
  (assert
  (=> (= result3 true)
  (= result4 (ite (<= (to_rep values__first) j1) o6 false))))
