...
;; H
  (assert (= (to_rep o) (to_rep (select inkeys_v1 i1))))

;; H
  (assert (= o1 (store outkeys_v1 i1 o)))

;; H
  (assert (= (mk_map__ref result1) (mk_map__ref outkeys_v1)))

;; H
  (assert (= outkeys_v11 o1))
...
;; H
  ...
  (=> (and (<= 1 k) (<= k 10))
  (=> (<= k i2)
  (= (to_rep (select outkeys_v12 k)) (to_rep (select inkeys_v1 k)))))))
...
;; H
  ...
  (=> (< i2 temp___363)
  (= (select outkeys_v12 temp___363) (select outkeys_v1 temp___363)))))
...
;; H
  (assert (= (to_rep o2) (to_rep (select inkeys_v1 i3))))

;; H
  (assert (= o3 (store outkeys_v12 i3 o2)))

;; H
  (assert (= result3 outkeys_v12))

;; H
  (assert (= outkeys_v13 o3))
...
;; WP_parameter_def
 ;; File "infoflow.ads", line 43, characters 0-0
  (not (= (to_rep (select outkeys_v13 k)) (to_rep (select inkeys_v1 k)))))
(check-sat)
