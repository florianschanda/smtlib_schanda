...
;; H
  ...
  (nth8_stream addr (- (- (+ start length1) j1) 1)) addr
  (- (- (+ start length1) j1) 1))))

;; H
  (assert
  (forall ((j1 Int))
  (=> (and (<= (- length1 i2) j1) (<= j1 (- length1 1)))
  (= (= (nth8_stream addr (- (- (+ start length1) j1) 1)) true)
  (= (nth retval2 j1) true)))))

;; H
  (assert
  (forall ((j1 Int))
  (=> (and (<= length1 j1) (<= j1 63)) (not (= (nth retval2 j1) true)))))
...
;; H
  (assert (and (= o4 j) (in_range2 j)))

;; H
  (assert (= o5 (nth retval2 o4)))

;; H
  (assert (= o6 (to_int2 o5)))

(assert
;; WP_parameter_def
 ;; File "interfac.ads", line 77, characters 0-0
  (not (in_range2 (- (- (+ start length1) j) 1))))
(check-sat)
