...
;; round_bound_ne
  ...
  (! (=> (no_overflow RNE2 x)
     ...
     (round RNE2 x))
     (<= (round RNE2 x) (+ (+ x (* (/ 1.0 9007199254740992.0) (ite (>= x 0.0) x (- x)))) (/ 1.0 .0))))) :pattern (
  (round RNE2 x)) )))
...
;; low_bound__def_axiom
  ...
  (! (=>
     ...
     (low_bound n) n))
     (= (low_bound n) (mul RNE2 (of_int RNE2 n) (neg fliteral3)))) :pattern (
  (low_bound n)) )))
...
;; high_bound__def_axiom
  ...
  (! (=>
     ...
     (high_bound n) n))
     (= (high_bound n) (mul RNE2 (of_int RNE2 n) fliteral3))) :pattern (
  (high_bound n)) )))
...
;; invariant____def_axiom
  ...
  (! (=> (invariant____function_guard (invariant__ n speed distance) n speed
     ...
     (and (le
     (mul RNE2
     (mul RNE2 (mul RNE2 (of_int RNE2 n) (of_int RNE2 (+ n 1)))
     (neg fliteral3)) fliteral5) distance) (le distance
     (mul RNE2
     (mul RNE2 (mul RNE2 (of_int RNE2 n) (of_int RNE2 (+ n 1))) fliteral3)
...
;; t__def_axiom
  ...
  (forall ((n1 Int))
  (! (=>
     (and (dynamic_invariant5 n1 true true true) (t__function_guard (t1 n1)
     n1)) (= (t1 n1) (of_int RNE2 n1))) :pattern ((t1 n1)) )))
...
;; delta_speed__def_axiom
  (assert
  (= delta_speed (add RNE2 drag
                 (mul RNE2 (mul RNE2 factor fliteral6) fliteral7))))
...
;; H
  (assert
  (= (add RNE2 drag (mul RNE2 (mul RNE2 factor fliteral6) fliteral7)) 
  delta_speed))
...
;; H
  (assert
  (and (= o (add RNE2 speed delta_speed)) (tqtisFinite
  (add RNE2 speed delta_speed))))
...
;; H
  (assert (eq (add RNE2 (mul RNE2 (of_ubv16 RNA2 n_bv) fliteral3) fliteral3)
  (mul RNE2 (add RNE2 (of_ubv16 RNA2 n_bv) fliteral) fliteral3)))

;; H
  (assert (eq (add RNE2 (mul RNE2 (of_int RNE2 n) fliteral3) fliteral3)
  (mul RNE2 (add RNE2 (of_int RNE2 n) fliteral) fliteral3)))

;; H
  (assert (eq (sub RNE2 (mul RNE2 (of_int RNE2 n) (neg fliteral3)) fliteral3)
  (mul RNE2 (add RNE2 (of_int RNE2 n) fliteral) (neg fliteral3))))

;; H
  (assert (t__function_guard (t1 1) 1))

;; H
  (assert (eq (t1 1) fliteral))

;; H
  (assert (eq (add RNE2 (of_int RNE2 n) fliteral) (of_int RNE2 (+ n 1))))

;; H
  (assert (le (sub RNE2 (mul RNE2 (of_int RNE2 n) (neg fliteral3)) fliteral3)
  speed1))

;; H
  (assert (le (mul RNE2 (of_int RNE2 (+ n 1)) (neg fliteral3)) speed1))

;; H
  (assert (le speed1
  (add RNE2 (mul RNE2 (of_int RNE2 n) fliteral3) fliteral3)))

;; H
  (assert (le speed1 (mul RNE2 (of_int RNE2 (+ n 1)) fliteral3)))

;; H
  (assert
  (and (= o1 (add RNE2 old_speed speed1)) (tqtisFinite
  (add RNE2 old_speed speed1))))

;; H
  (assert (= o2 (div1 RNE2 o1 fliteral8)))
...
;; WP_parameter_def
 ;; File "complex_trajectory.ads", line 29, characters 0-0
  (not (le (low_bound (+ n 1)) old_speed)))
(check-sat)
