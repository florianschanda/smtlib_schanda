...
;; H
  (assert (dynamic_invariant1 addr true false true))
...
;; H
  (assert
  (and (= (to_rep1 (first (rt addr))) 0) (< left (* 8 (length addr)))))

;; H
  (assert
  (and (<= (to_rep1 (first (rt addr))) (div1 left 8))
  (<= (div1 left 8) (to_rep1 (last (rt addr))))))
...
(assert
;; WP_parameter_def
 ;; File "bitwalker.ads", line 14, characters 0-0
  (not (< (mod1 left 8) 8)))
(check-sat)
