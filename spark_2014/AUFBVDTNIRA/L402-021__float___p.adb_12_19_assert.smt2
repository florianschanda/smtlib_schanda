...
;; H
  (assert (= c1 (roundToIntegral RTP2 x)))

;; H
  (assert (tqtisFinite c1))

;; H
  (assert (= result1 f))

;; H
  (assert (= f1 (roundToIntegral RTN2 x)))

;; H
  (assert (tqtisFinite f1))

;; H
  (assert (= result2 r))

;; H
  (assert (= r1 (roundToIntegral RNA2 x)))

;; H
  (assert (tqtisFinite r1))

;; H
  (assert (= result3 t1))

;; H
  (assert (= t2 (roundToIntegral RTZ2 x)))
...
;; H
  (assert (lt (sub RNE2 c1 fliteral) x))

;; H
  (assert (le f1 x))

;; H
  (assert (lt x (add RNE2 f1 fliteral)))

;; H
  (assert (or (eq r1 c1) (eq r1 f1)))

(assert
;; WP_parameter_def
 ;; File "system.ads", line 1, characters 0-0
  (not (or (eq t2 c1) (eq t2 f1))))
(check-sat)
