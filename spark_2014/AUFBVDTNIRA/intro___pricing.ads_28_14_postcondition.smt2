...
;; H
  (assert
  (=>
  (and (<= (to_rep1 (first (rt bk))) it1) (<= it1 (to_rep1 (last (rt bk)))))
  (= temp___173 it1)))
...
;; H
  (assert (price_of_item__function_guard
  (price_of_item (select (elts bk) it10)) (select (elts bk) it10)))

(assert
;; WP_parameter_def
 ;; File "sat.ads", line 12, characters 0-0
  (not
  (<= (price_of_item (select (elts bk) it10)) pricing__price_of_basket__result4)))
(check-sat)
