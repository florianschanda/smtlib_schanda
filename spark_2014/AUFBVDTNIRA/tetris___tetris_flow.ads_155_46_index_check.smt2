...
;; H
  (assert
  (not (= (to_rep (rec__tetris_flow__piece__s (us_split_fields1 p))) 2)))

;; H
  (assert
  (not (= (to_rep (rec__tetris_flow__piece__s (us_split_fields1 p))) 1)))
...
(assert
;; WP_parameter_def
 ;; File "tetris_flow.ads", line 144, characters 0-0
  (not (<= 3 (to_rep (rec__tetris_flow__piece__s (us_split_fields1 p))))))
(check-sat)
