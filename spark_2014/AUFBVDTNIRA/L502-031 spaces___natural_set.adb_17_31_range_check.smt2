...
(define-fun bool_eq5 ((a (Array Int element_t)) (a__first Int) (a__last Int)
...
(declare-fun dummy5 () (Array Int element_t))
...
;; contains__def_axiom
  ...
  (! (=> (contains__function_guard (contains s value) s value)
     ...
     (= (to_rep1
        (let ((temp___192 (rec__natural_set__t__m (us_split_fields1 s))))
        (select temp___192 i))) value)))))) :pattern ((contains s value)) ))))
...
;; members__post_axiom
  ...
  (! (let ((result (members s)))
     ...
     (and (and (<= 0 result) (<= result 10)) (dynamic_invariant1 result true
...
;; H
  (assert (members__function_guard
  (members (mk___rep (mk___split_fields s__split_fields s__split_fields1)))
  (mk___rep (mk___split_fields s__split_fields s__split_fields1))))
...
(assert
;; WP_parameter_def
 ;; File "natural_set.ads", line 49, characters 0-0
  (not (in_range2 (+ (to_rep s__split_fields) 1))))
(check-sat)
