...
;; H
  ...
  (=> (<= 1 1024) (in_range3 index2)))
  (forall ((temp___230 Int))
  (=> (and (<= 1 temp___230) (<= temp___230 1024))
  (=> (<= index2 temp___230)
  (= (select buffer1 temp___230) (select buffer temp___230))))))
  (and (<= 1 index2) (<= index2 1024))))

;; H
  (assert
  (=> (= (to_rep (select buffer1 index2)) ch)
  (and (= o (+ count2 1)) (in_range2 (+ count2 1)))))

;; H
  (assert (=> (= (to_rep (select buffer1 index2)) ch) (= result2 count2)))

;; H
  (assert (=> (= (to_rep (select buffer1 index2)) ch) (= count3 o)))

;; H
  (assert (=> (= (to_rep (select buffer1 index2)) ch) (= (to_rep o1) 32)))

;; H
  (assert
  (=> (= (to_rep (select buffer1 index2)) ch)
  (= o2 (store buffer1 index2 o1))))

;; H
  (assert (=> (= (to_rep (select buffer1 index2)) ch) (= result3 buffer1)))

;; H
  (assert (=> (= (to_rep (select buffer1 index2)) ch) (= buffer2 o2)))

;; H
  (assert
  (=> (not (= (to_rep (select buffer1 index2)) ch)) (= count3 count2)))

;; H
  (assert
  (=> (not (= (to_rep (select buffer1 index2)) ch)) (= buffer2 buffer1)))
