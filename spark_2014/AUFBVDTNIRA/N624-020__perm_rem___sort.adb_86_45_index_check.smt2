...
;; H
  (assert
  (and (= temp___240 (remove (mk___t init (mk 1 100)) o47))
  (and (dynamic_invariant4 temp___240 true false true)
  (and (= (invariant__ temp___240) true)
  (and (= (to_rep1 (last (rt temp___240))) (- (to_rep1 (last (mk 1 100))) 1))
  (and
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- o47 1)))
  (= (to_rep (select init k)) (to_rep (select (elts temp___240) k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ o47 1) k) (<= k (to_rep1 (last (mk 1 100)))))
  (= (to_rep (select init k)) (to_rep (select (elts temp___240) (- k 1))))))
  (forall ((k Int))
  (=> (and (<= o47 k) (<= k (- (to_rep1 (last (mk 1 100))) 1)))
  (= (to_rep (select init (+ k 1))) (to_rep (select (elts temp___240) k))))))))))))

(assert
;; WP_parameter_def
 ;; File "sort.ads", line 4, characters 0-0
  (not (<= (to_rep1 (first (rt temp___240))) (- y 1))))
(check-sat)
