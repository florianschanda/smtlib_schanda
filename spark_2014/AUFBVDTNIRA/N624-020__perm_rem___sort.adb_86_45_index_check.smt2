...
;; H
  (assert
  (and
  (and (= temp___335 (remove (mk___t values2 (mk 1 100)) o48))
  (remove__function_guard temp___335 (mk___t values2 (mk 1 100)) o48))
...
;; H
  (assert
  (and (<= (to_rep1 (first (rt temp___335))) x)
  (<= x (to_rep1 (last (rt temp___335))))))

;; H
  (assert (= o49 x))

;; H
  (assert (= o50 (select (elts temp___335) o49)))

;; H
  (assert (= o51 (to_rep o50)))

;; H
  (assert (invariant____function_guard (invariant__ temp___333) temp___333))
...
;; H
  (assert
  (and
  (and (= temp___333 (remove (mk___t init (mk 1 100)) o47))
  (remove__function_guard temp___333 (mk___t init (mk 1 100)) o47))
  (and (dynamic_invariant4 temp___333 true false true)
  (and (= (invariant__ temp___333) true)
  (and (= (to_rep1 (last (rt temp___333))) (- (to_rep1 (last (mk 1 100))) 1))
  (and
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- o47 1)))
  (= (to_rep (select init k)) (to_rep (select (elts temp___333) k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ o47 1) k) (<= k (to_rep1 (last (mk 1 100)))))
  (= (to_rep (select init k)) (to_rep (select (elts temp___333) (- k 1))))))
  (forall ((k Int))
  (=> (and (<= o47 k) (<= k (- (to_rep1 (last (mk 1 100))) 1)))
  (= (to_rep (select init (+ k 1))) (to_rep (select (elts temp___333) k))))))))))))

(assert
;; WP_parameter_def
 ;; File "sort.ads", line 4, characters 0-0
  (not (<= (to_rep1 (first (rt temp___333))) (- y 1))))
(check-sat)
