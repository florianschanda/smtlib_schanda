;; produced by cvc4_14.drv ;;
(set-logic AUFBVDTNIRA)
(set-info :source |VC generated by SPARK 2014|)
(set-info :smt-lib-version 2.0)
(set-info :category industrial)
(set-info :status unknown)
;;; generated by SMT-LIB2 driver
;;; SMT-LIB2 driver: bit-vectors, common part
;;; SMT-LIB2: integer arithmetic
(declare-datatypes () ((tuple0 (Tuple0))))
(declare-sort us_private 0)

(declare-fun private__bool_eq (us_private us_private) Bool)

(declare-fun us_null_ext__ () us_private)

(declare-sort us_type_of_heap 0)

(declare-datatypes ()
((us_type_of_heap__ref
 (mk___type_of_heap__ref (us_type_of_heap__content us_type_of_heap)))))
(declare-sort us_image 0)

(declare-datatypes () ((int__ref (mk_int__ref (int__content Int)))))
(declare-datatypes () ((bool__ref (mk_bool__ref (bool__content Bool)))))
(declare-datatypes () ((real__ref (mk_real__ref (real__content Real)))))
(declare-datatypes ()
((us_private__ref (mk___private__ref (us_private__content us_private)))))
(define-fun int__ref___projection ((a int__ref)) Int (int__content a))

(define-fun bool__ref___projection ((a bool__ref)) Bool (bool__content a))

(define-fun real__ref___projection ((a real__ref)) Real (real__content a))

(define-fun us_private__ref___projection ((a us_private__ref)) us_private 
  (us_private__content a))

(declare-fun div1 (Int Int) Int)

(declare-fun mod1 (Int Int) Int)

;; Div_mod
  (assert
  (forall ((x Int) (y Int))
  (=> (not (= y 0)) (= x (+ (* y (div1 x y)) (mod1 x y))))))

;; Div_bound
  (assert
  (forall ((x Int) (y Int))
  (=> (and (<= 0 x) (< 0 y)) (and (<= 0 (div1 x y)) (<= (div1 x y) x)))))

;; Mod_bound
  (assert
  (forall ((x Int) (y Int))
  (=> (not (= y 0)) (and (< (- (abs y)) (mod1 x y)) (< (mod1 x y) (abs y))))))

;; Div_sign_pos
  (assert
  (forall ((x Int) (y Int)) (=> (and (<= 0 x) (< 0 y)) (<= 0 (div1 x y)))))

;; Div_sign_neg
  (assert
  (forall ((x Int) (y Int)) (=> (and (<= x 0) (< 0 y)) (<= (div1 x y) 0))))

;; Mod_sign_pos
  (assert
  (forall ((x Int) (y Int))
  (=> (and (<= 0 x) (not (= y 0))) (<= 0 (mod1 x y)))))

;; Mod_sign_neg
  (assert
  (forall ((x Int) (y Int))
  (=> (and (<= x 0) (not (= y 0))) (<= (mod1 x y) 0))))

;; Rounds_toward_zero
  (assert
  (forall ((x Int) (y Int))
  (=> (not (= y 0)) (<= (abs (* (div1 x y) y)) (abs x)))))

;; Div_1
  (assert (forall ((x Int)) (= (div1 x 1) x)))

;; Mod_1
  (assert (forall ((x Int)) (= (mod1 x 1) 0)))

;; Div_inf
  (assert
  (forall ((x Int) (y Int)) (=> (and (<= 0 x) (< x y)) (= (div1 x y) 0))))

;; Mod_inf
  (assert
  (forall ((x Int) (y Int)) (=> (and (<= 0 x) (< x y)) (= (mod1 x y) x))))

;; Div_mult
  (assert
  (forall ((x Int) (y Int) (z Int))
  (! (=> (and (< 0 x) (and (<= 0 y) (<= 0 z)))
     (= (div1 (+ (* x y) z) x) (+ y (div1 z x)))) :pattern ((div1
                                                            (+ (* x y) z) x)) )))

;; Mod_mult
  (assert
  (forall ((x Int) (y Int) (z Int))
  (! (=> (and (< 0 x) (and (<= 0 y) (<= 0 z)))
     (= (mod1 (+ (* x y) z) x) (mod1 z x))) :pattern ((mod1 (+ (* x y) z) x)) )))

(define-fun mod2 ((x Int)
  (y Int)) Int (ite (< 0 y) (mod x y) (+ (mod x y) y)))

(define-fun to_int1 ((b Bool)) Int (ite (= b true) 1 0))

(define-fun of_int ((i Int)) Bool (ite (= i 0) false true))

(define-fun in_range ((x Int)) Bool (or (= x 0) (= x 1)))

(declare-fun attr__ATTRIBUTE_IMAGE (Bool) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE (us_image) Bool)

(declare-sort integer 0)

(define-fun in_range1 ((x Int)) Bool (and (<= (- 2147483648) x)
                                     (<= x 2147483647)))

(define-fun bool_eq ((x Int) (y Int)) Bool (ite (= x y) true false))

(declare-fun attr__ATTRIBUTE_IMAGE1 (Int) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check1 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE1 (us_image) Int)

(declare-fun user_eq (integer integer) Bool)

(declare-fun dummy () integer)

(declare-datatypes ()
((integer__ref (mk_integer__ref (integer__content integer)))))
(define-fun integer__ref___projection ((a integer__ref)) integer (integer__content
                                                                 a))

(define-fun dynamic_invariant ((temp___expr_15 Int) (temp___is_init_12 Bool)
  (temp___skip_constant_13 Bool)
  (temp___do_toplevel_14 Bool)) Bool (=>
                                     (or (= temp___is_init_12 true)
                                     (<= (- 2147483648) 2147483647))
                                     (in_range1 temp___expr_15)))

(declare-sort controlangle 0)

(define-fun in_range2 ((x Int)) Bool (and (<= (- 45) x) (<= x 45)))

(define-fun bool_eq1 ((x Int) (y Int)) Bool (ite (= x y) true false))

(declare-fun attr__ATTRIBUTE_IMAGE2 (Int) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check2 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE2 (us_image) Int)

(declare-fun user_eq1 (controlangle controlangle) Bool)

(declare-fun dummy1 () controlangle)

(declare-datatypes ()
((controlangle__ref
 (mk_controlangle__ref (controlangle__content controlangle)))))
(define-fun controlangle__ref___projection ((a controlangle__ref)) controlangle 
  (controlangle__content a))

(define-fun dynamic_invariant1 ((temp___expr_135 Int)
  (temp___is_init_132 Bool) (temp___skip_constant_133 Bool)
  (temp___do_toplevel_134 Bool)) Bool (=>
                                      (or (= temp___is_init_132 true)
                                      (<= (- 45) 45)) (in_range2
                                      temp___expr_135)))

(declare-sort machnumber 0)

(define-fun in_range3 ((x Int)) Bool (and (<= 0 x) (<= x 100)))

(define-fun bool_eq2 ((x Int) (y Int)) Bool (ite (= x y) true false))

(declare-fun attr__ATTRIBUTE_IMAGE3 (Int) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check3 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE3 (us_image) Int)

(declare-fun user_eq2 (machnumber machnumber) Bool)

(declare-fun dummy2 () machnumber)

(declare-datatypes ()
((machnumber__ref (mk_machnumber__ref (machnumber__content machnumber)))))
(define-fun machnumber__ref___projection ((a machnumber__ref)) machnumber 
  (machnumber__content a))

(define-fun dynamic_invariant2 ((temp___expr_171 Int)
  (temp___is_init_168 Bool) (temp___skip_constant_169 Bool)
  (temp___do_toplevel_170 Bool)) Bool (=>
                                      (or (= temp___is_init_168 true)
                                      (<= 0 100)) (in_range3
                                      temp___expr_171)))

(declare-sort scaledata 0)

(define-fun in_range4 ((x Int)) Bool (and (<= (- 100) x) (<= x 100)))

(define-fun bool_eq3 ((x Int) (y Int)) Bool (ite (= x y) true false))

(declare-fun attr__ATTRIBUTE_IMAGE4 (Int) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check4 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE4 (us_image) Int)

(declare-fun user_eq3 (scaledata scaledata) Bool)

(declare-fun dummy3 () scaledata)

(declare-datatypes ()
((scaledata__ref (mk_scaledata__ref (scaledata__content scaledata)))))
(define-fun scaledata__ref___projection ((a scaledata__ref)) scaledata 
  (scaledata__content a))

(define-fun dynamic_invariant3 ((temp___expr_189 Int)
  (temp___is_init_186 Bool) (temp___skip_constant_187 Bool)
  (temp___do_toplevel_188 Bool)) Bool (=>
                                      (or (= temp___is_init_186 true)
                                      (<= (- 100) 100)) (in_range4
                                      temp___expr_189)))

(declare-fun mach () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS () Int)

(declare-fun present () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS1 () Int)

(declare-fun target () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS2 () Int)

(declare-fun max () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS3 () Int)

(declare-sort percent 0)

(define-fun in_range5 ((x Int)) Bool (and (<= 0 x) (<= x 100)))

(define-fun bool_eq4 ((x Int) (y Int)) Bool (ite (= x y) true false))

(declare-fun attr__ATTRIBUTE_IMAGE5 (Int) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check5 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE5 (us_image) Int)

(declare-fun user_eq4 (percent percent) Bool)

(declare-fun dummy4 () percent)

(declare-datatypes ()
((percent__ref (mk_percent__ref (percent__content percent)))))
(define-fun percent__ref___projection ((a percent__ref)) percent (percent__content
                                                                 a))

(define-fun dynamic_invariant4 ((temp___expr_201 Int)
  (temp___is_init_198 Bool) (temp___skip_constant_199 Bool)
  (temp___do_toplevel_200 Bool)) Bool (=>
                                      (or (= temp___is_init_198 true)
                                      (<= 0 100)) (in_range5
                                      temp___expr_201)))

(declare-fun inverse (Int Int) Int)

(declare-fun inverse__function_guard (Int Int Int) Bool)

;; inverse__post_axiom
  (assert
  (forall ((val__ Int) (flatness Int))
  (! (=>
     (and
     (and (dynamic_invariant3 val__ true true true) (dynamic_invariant
     flatness true true true))
     (and (and (<= 0 val__) (< 0 flatness)) (<= flatness 100)))
     (dynamic_invariant4 (inverse val__ flatness) true false true)) :pattern (
  (inverse val__ flatness)) )))

(declare-fun attr__ATTRIBUTE_ADDRESS4 () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS5 () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS6 () Int)

(declare-fun attr__ATTRIBUTE_ADDRESS7 () Int)

(declare-fun target_angle () Int)

(declare-fun k1 () Int)

(declare-fun k2 () Int)

(declare-fun gap () Int)

(declare-fun scale__scale_movement__result () Int)

(declare-fun o () Int)

(declare-fun o1 () Int)

(declare-fun o2 () Int)

(declare-fun o3 () Int)

(declare-fun k11 () Int)

(declare-fun k21 () Int)

(declare-fun gap1 () Int)

(declare-fun result () Int)

(declare-fun target_angle1 () Int)

(declare-fun gap2 () Int)

(declare-fun k12 () Int)

(declare-fun k22 () Int)

(declare-fun result1 () Int)

(declare-fun target_angle2 () Int)

(declare-fun result2 () Int)

(declare-fun target_angle3 () Int)

(declare-fun result3 () Int)

(declare-fun scale__scale_movement__result1 () Int)

(declare-fun scale__scale_movement__result2 () Int)

(declare-fun scale__scale_movement__result3 () Int)

(declare-fun target_angle4 () Int)

(declare-fun k13 () Int)

(declare-fun k23 () Int)

(declare-fun gap3 () Int)

(declare-fun scale__scale_movement__result4 () Int)

(declare-fun target_angle5 () Int)

(declare-fun k14 () Int)

(declare-fun k24 () Int)

(declare-fun gap4 () Int)

(declare-fun scale__scale_movement__result5 () Int)

(declare-fun result4 () Int)

;; H
  (assert (in_range3 mach))

;; H
  (assert (in_range4 present))

;; H
  (assert (in_range4 target))

;; H
  (assert (in_range2 max))

;; H
  (assert (< 0 max))

;; H
  (assert (=> (<= (- 45) 45) (in_range2 target_angle)))

;; H
  (assert (=> (<= (- 2147483648) 2147483647) (in_range1 k1)))

;; H
  (assert (=> (<= (- 2147483648) 2147483647) (in_range1 k2)))

;; H
  (assert (=> (<= (- 2147483648) 2147483647) (in_range1 gap)))

;; H
  (assert (=> (= present target) (= result target_angle)))

;; H
  (assert (=> (= present target) (= target_angle1 0)))

;; H
  (assert (=> (= present target) (= gap2 gap)))

;; H
  (assert (=> (= present target) (= k22 k2)))

;; H
  (assert (=> (= present target) (= k12 k1)))

;; H
  (assert (=> (= present target) (= gap2 gap1)))

;; H
  (assert (=> (= present target) (= k22 k21)))

;; H
  (assert (=> (= present target) (= k12 k11)))

;; H
  (assert
  (=> (not (= present target))
  (and (and (in_range5 k12) (< 0 k22)) (<= k12 k22))))

;; H
  (assert
  (=> (not (= present target))
  (=> (<= (- 2147483648) 2147483647) (in_range1 k12))))

;; H
  (assert
  (=> (not (= present target))
  (=> (<= (- 2147483648) 2147483647) (in_range1 k22))))

;; H
  (assert
  (=> (not (= present target))
  (and (= o (* max k12)) (in_range1 (* max k12)))))

;; H
  (assert (=> (not (= present target)) (= o1 (div1 o k22))))

;; H
  (assert (=> (not (= present target)) (and (= o2 o1) (in_range1 o1))))

;; H
  (assert (=> (not (= present target)) (and (= o3 o2) (in_range2 o2))))

;; H
  (assert (=> (not (= present target)) (= result1 target_angle)))

;; H
  (assert (=> (not (= present target)) (= target_angle2 o3)))

;; H
  (assert
  (=> (not (= present target))
  (=> (< target present) (= result2 target_angle2))))

;; H
  (assert
  (=> (not (= present target))
  (=> (< target present) (= target_angle3 (- target_angle2)))))

;; H
  (assert
  (=> (not (= present target))
  (=> (not (< target present)) (= target_angle3 target_angle2))))

;; H
  (assert (=> (not (= present target)) (= target_angle1 target_angle3)))

;; H
  (assert (= scale__scale_movement__result1 scale__scale_movement__result2))

;; H
  (assert (= scale__scale_movement__result3 scale__scale_movement__result1))

;; H
  (assert (= result3 scale__scale_movement__result))

;; H
  (assert (= scale__scale_movement__result1 target_angle1))

;; H
  (assert
  (= (mk_int__ref scale__scale_movement__result4) (mk_int__ref
                                                  scale__scale_movement__result2)))

;; H
  (assert (= gap3 gap2))

;; H
  (assert (= k23 k22))

;; H
  (assert (= k13 k12))

;; H
  (assert (= target_angle4 target_angle1))

;; H
  (assert (= scale__scale_movement__result5 scale__scale_movement__result3))

;; H
  (assert (= gap4 gap2))

;; H
  (assert (= k24 k22))

;; H
  (assert (= k14 k12))

;; H
  (assert (= target_angle5 target_angle1))

;; H
  (assert (= result4 scale__scale_movement__result4))

(assert
;; WP_parameter_def
 ;; File "surfaces.ads", line 7, characters 0-0
  (not (<= (- max) scale__scale_movement__result4)))
(check-sat)
