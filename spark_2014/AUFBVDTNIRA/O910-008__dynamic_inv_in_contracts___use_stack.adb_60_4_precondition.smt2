...
;; mk_def
  ...
  (! (=> (in_range1 f)
     (=> (in_range1 l)
     (and (= (to_rep (first (mk f l))) f) (= (to_rep (last2 (mk f l))) l)))) :pattern (
  (mk f l)) )))
...
(define-fun bool_eq5 ((x us_t)
  (y us_t)) Bool (bool_eq2 (elts x) (to_rep (first (rt x)))
                 (to_rep (last2 (rt x))) (elts y) (to_rep (first (rt y)))
                 (to_rep (last2 (rt y)))))
...
;; peek__def_axiom
  ...
  (! (=>
     ...
     (= (peek use_stack__stacks__top use_stack__stacks__content) (to_rep1
...
;; all_eq__def_axiom
  ...
  (! (=> (all_eq__function_guard (all_eq e use_stack__stacks__content) e
     ...
     (forall ((i Int))
     (=> (and (<= 1 i) (<= i (to_rep content__last)))
     (= (to_rep1 (select use_stack__stacks__content i)) e))))) :pattern (
...
;; H
  (assert
  (and (dynamic_property2 1 max (to_rep content__first)
  (to_rep content__last))
  (and (= (to_rep content__first) 1) (= (to_rep content__last) max))))
...
;; H
  (assert (is_full__function_guard (is_full top1) top1))

;; H
  (assert (all_eq__function_guard (all_eq e1 content) e1 content))

;; H
  (assert
  (and (and (= (is_full top1) true) (= (all_eq e1 content) true))
  (dynamic_property 0 last top1)))

;; H
  (assert (is_empty__function_guard (is_empty top1) top1))

(assert
;; WP_parameter_def
 ;; File "use_stack.adb", line 26, characters 0-0
  (not (not (= (is_empty top1) true))))
(check-sat)
