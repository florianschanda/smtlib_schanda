...
(declare-fun complete_lines4 () (_ BitVec 32))
...
;; H
  ...
  (= (select cur_board3 temp___2248) (select cur_board temp___2248)))))
...
;; H
  (assert (= result7 (mk_int__ref del_line2)))
...
;; H
  (assert
  (and
  (and (= result8 (is_complete_line (select cur_board3 del_line3)))
  (is_complete_line__function_guard result8 (select cur_board3 del_line3)))
  (= (= result8 true)
  (forall ((x Int))
  (=> (and (<= 1 x) (<= x 10))
  (not (= (to_rep (select (select cur_board3 del_line3) x)) 0)))))))

;; H
  (assert (=> (= result8 true) (= result9 (mk_map__ref1 cur_board3))))

;; H
  (assert
  (=> (= result8 true)
  (= cur_board4 (store cur_board3 del_line3 empty_line))))

;; H
  (assert (=> (= result8 true) (= result10 (mk_int__ref to_line3))))

;; H
  (assert (=> (= result8 true) (= to_line4 del_line3)))

;; H
  (assert
  (=> (= result8 true)
  (=>
  (bvule (bvsub ((_ int2bv 32) 100000) complete_lines3) ((_ int2bv 32) 1))
  (= o1 (bvsub (bvadd complete_lines3 ((_ int2bv 32) 1)) ((_ int2bv 32) 100000))))))
...
;; H
  ...
  (not
  (bvule (bvsub ((_ int2bv 32) 100000) complete_lines3) ((_ int2bv 32) 1)))
  (= o1 (bvadd complete_lines3 ((_ int2bv 32) 1))))))

;; H
  (assert (=> (= result8 true) (= result11 (mk_t__ref complete_lines3))))

;; H
  (assert (=> (= result8 true) (= complete_lines4 o1)))

;; H
  (assert
  (=> (= result8 true)
  (= (to_rep (select (select cur_board4 del_line3) 1)) 0)))

;; H
  (assert (=> (not (= result8 true)) (= complete_lines4 complete_lines3)))

;; H
  (assert (=> (not (= result8 true)) (= to_line4 to_line3)))

;; H
  (assert (=> (not (= result8 true)) (= cur_board4 cur_board3)))
...
;; H
  (assert (is_complete_line__function_guard
  (is_complete_line (select cur_board4 y)) (select cur_board4 y)))

(assert
;; WP_parameter_def
 ;; File "patris.ads", line 196, characters 0-0
  (not (not (= (is_complete_line (select cur_board4 y)) true))))
(check-sat)
