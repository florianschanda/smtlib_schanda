...
;; H
  ...
  (< (to_rep
     (rec__red_black_trees__black_count__depth
     (us_split_fields11 (select (nb_blacks c99b) c101b)))) n5))
...
;; H
  ...
  (eq (select (nb_blacks c100b) y2) (select (nb_blacks c99b) y2))
  (select (nb_blacks c100b) y2) (select (nb_blacks c99b) y2)))

(assert
;; WP_parameter_def
 ;; File "red_black_trees.ads", line 59, characters 0-0
  (not
  (= (eq (select (nb_blacks c100b) y2) (select (nb_blacks c99b) y2)) true)))
(check-sat)
