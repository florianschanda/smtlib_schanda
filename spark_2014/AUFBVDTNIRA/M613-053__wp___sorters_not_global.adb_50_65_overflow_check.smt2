...
;; H
  (assert
  (and (and (<= starting_at min2) (<= min2 limit))
  (forall ((j Int))
  (=> (and (<= starting_at j) (<= j (- index2 1)))
  (<= (to_rep (select values min2)) (to_rep (select values j)))))))

;; H
  (assert
  (and
  (and (=> (<= 1 64) (in_range2 min2))
  (=> (<= (+ starting_at 1) limit) (dynamic_property (+ starting_at 1) 
  limit index2))) (and (<= (+ starting_at 1) index2) (<= index2 limit))))

;; H
  (assert (<= starting_at min2))
...
(assert
;; WP_parameter_def
 ;; File "sorters_not_global.ads", line 8, characters 0-0
