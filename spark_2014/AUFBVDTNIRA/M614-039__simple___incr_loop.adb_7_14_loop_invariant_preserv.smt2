...
(declare-datatypes ()
((arr_type__ref (mk_arr_type__ref (arr_type__content us_t)))))
(define-fun arr_type__ref___projection ((a arr_type__ref)) us_t (arr_type__content
                                                                a))
...
;; H
  (assert (dynamic_property (- 2147483648) 2147483647 (to_rep a__first)
  (to_rep a__last)))

;; H
  (assert
  (forall ((i4 Int))
  (=> (and (<= (to_rep a__first) i4) (<= i4 (to_rep a__last)))
  (< (to_rep (select a i4)) 2147483647))))
...
;; H
  ...
  (= (to_rep (select a1 k)) (+ (to_rep (select a k)) 1))))
  (forall ((k Int))
  (=> (and (<= i2 k) (<= k (to_rep a__last)))
...
;; H
  ...
  (=>
  (and (<= (to_rep a__first) temp___154) (<= temp___154 (to_rep a__last)))
  (=> (<= i2 temp___154) (= (select a1 temp___154) (select a temp___154))))))
  (and (<= (to_rep a__first) i2) (<= i2 (to_rep a__last)))))

;; H
  (assert
  (and (= o (+ (to_rep (select a1 i2)) 1)) (in_range1
  (+ (to_rep (select a1 i2)) 1))))

;; H
  (assert (= (to_rep o1) o))

;; H
  (assert (= o2 (store a1 i2 o1)))
...
;; WP_parameter_def
 ;; File "incr_loop.ads", line 5, characters 0-0
  (not (= (to_rep (select a2 k)) (+ (to_rep (select a k)) 1))))
(check-sat)
