...
;; H
  (assert
  (forall ((index2 Int)) (is_digit__function_guard
  (is_digit (to_rep (select (elts source) index2)))
  (to_rep (select (elts source) index2)))))
...
;; H
  ...
  (= (is_digit (to_rep (select (elts source) index2))) true))))))
...
;; WP_parameter_def
 ;; File "aida-conversion.adb", line 1538, characters 0-0
  (not (<= (to_rep1 (select n2 j)) (to_rep1 (select int_array1 j)))))
(check-sat)
