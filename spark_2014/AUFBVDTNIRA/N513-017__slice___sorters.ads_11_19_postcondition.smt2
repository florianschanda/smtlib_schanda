...
;; H
  (assert
  (=> (and (<= (to_rep values__first) current1) (<= current1 temp___180))
  (= temp___177 values)))
...
;; H
  (assert
  (=> (and (<= (to_rep values__first) current1) (<= current1 temp___180))
  (= (mk1 current1 (to_rep values__last)) temp___1731)))
...
;; H
  ...
  (=> (and (<= (to_rep values__first) current1) (<= current1 temp___180))
  (=> (not (= smallest2 current1))
  (and (= (to_rep (select values2 o4)) (to_rep (select values smallest2)))
  (and (= (to_rep (select values2 smallest2)) (to_rep (select values o4)))
  (forall ((j Int))
  (=> (and (<= (to_rep values__first) j) (<= j (to_rep values__last)))
  (=> (and (not (= j o4)) (not (= j smallest2)))
...
;; H
  (assert
  (=>
  (not (and (<= (to_rep values__first) current1) (<= current1 temp___180)))
  (= current1 current5)))
...
;; WP_parameter_def
 ;; File "sorters.adb", line 29, characters 0-0
  (not (<= (to_rep (select values8 j)) (to_rep (select values8 (+ j 1))))))
(check-sat)
