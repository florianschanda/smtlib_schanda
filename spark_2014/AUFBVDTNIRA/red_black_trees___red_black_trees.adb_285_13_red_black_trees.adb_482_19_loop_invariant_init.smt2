...
;; H
  (assert (= (mk___rep4 t_old__split_fields7) c99b))

;; H
  (assert (dynamic_invariant6 c99b true false true))

;; H
  (assert (= (mk___rep4 t__split_fields8) c100b))
...
;; H
  (assert (= red_black_trees__insert__L_8__C101b__assume c101b))

;; H
  (assert (in_range2 c101b))
...
;; H
  (assert (= red_black_trees__insert__L_8__B102b__y__assume y2))
...
;; H
  (assert (model____function_guard
  (model__ (rec__red_black_trees__rbt__struct (us_split_fields9 c99b)))
  (rec__red_black_trees__rbt__struct (us_split_fields9 c99b))))

;; H
  (assert (nb_blacks__function_guard (nb_blacks c99b) c99b))
...
;; H
  ...
  (< (to_rep
     (rec__red_black_trees__black_count__depth
     (us_split_fields11 (select (nb_blacks c99b) i)))) n5))

;; H
  (assert (not (= i y2)))

;; H
  (assert (nb_blacks__function_guard (nb_blacks c100b) c100b))
...
;; H
  (assert (eq__function_guard
  (eq (select (nb_blacks c100b) i) (select (nb_blacks c99b) i))
  (select (nb_blacks c100b) i) (select (nb_blacks c99b) i)))

(assert
;; WP_parameter_def
 ;; File "red_black_trees.ads", line 59, characters 0-0
  (not
  (= (eq (select (nb_blacks c100b) i) (select (nb_blacks c99b) i)) true)))
(check-sat)
