...
;; H
  (assert (remove__function_guard
  (remove (remove (mk___t values2 (mk 1 100)) x) (- y 1))
  (remove (mk___t values2 (mk 1 100)) x) (- y 1)))
...
;; H
  (assert
  (= (let ((temp___316 (remove (remove (mk___t values2 (mk 1 100)) x)
                       (- y 1))))
     (let ((temp___315 (remove (remove (mk___t interm1 (mk 1 100)) x)
                       (- y 1))))
     (bool_eq5 (elts temp___315) (to_rep1 (first (rt temp___315)))
     (to_rep1 (last (rt temp___315))) (elts temp___316)
     (to_rep1 (first (rt temp___316))) (to_rep1 (last (rt temp___316)))))) true))

;; H
  (assert (and (= o26 (- y 1)) (in_range5 (- y 1))))
