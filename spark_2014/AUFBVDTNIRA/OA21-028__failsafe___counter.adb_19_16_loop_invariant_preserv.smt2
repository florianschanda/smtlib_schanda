...
;; H
  ...
  (= (= (select inputs1 k) true) (= (select inputs k) true))))))
...
;; H
  (assert (= inputs1 result1))

;; H
  (assert
  (= inputs2 (store inputs1 i2 (select inputs1 (bvadd i2 ((_ int2bv 8) 1))))))
...
;; H
  (assert (bvule k (bvsub i3 ((_ int2bv 8) 1))))

;; H
  (assert (= (select inputs2 k) true))
