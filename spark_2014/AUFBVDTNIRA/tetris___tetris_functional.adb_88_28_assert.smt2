...
;; H
  (assert (= (mk_map__ref1 result4) (mk_map__ref1 cur_board)))

;; H
  (assert (= cur_board1 (store cur_board del_line1 empty_line)))

;; H
  (assert (= has_complete_lines1 result5))

;; H
  (assert (= has_complete_lines2 (of_int 1)))

;; H
  (assert (= to_line1 result6))

;; H
  (assert (= to_line2 del_line1))

(assert
;; WP_parameter_def
 ;; File "tetris_functional.ads", line 170, characters 0-0
  (not (= (to_rep (select (select cur_board1 del_line1) 1)) 0)))
(check-sat)
