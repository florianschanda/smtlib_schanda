...
;; H
  (assert
  (and
  (and (<= last2 (to_rep1 (last (rt str))))
  (= (to_rep1 (last (rt str))) (to_rep1 (last (rt res)))))
  (= (to_rep1 (first (rt str))) (to_rep1 (first (rt res))))))
...
;; H
  (assert
  (=> (= result1 true) (= result (ite (not (= o8 true)) true false))))
...
;; H
  (assert
  (and (<= (to_rep1 (first (rt str))) j) (<= j (to_rep1 (last (rt str))))))
...
;; H
  (assert (and (= o21 (to_upper o20)) (in_range3 o21)))

(assert
;; WP_parameter_def
 ;; File "hello.adb", line 24, characters 0-0
  (not (<= (to_rep1 (first (rt res))) j)))
(check-sat)
