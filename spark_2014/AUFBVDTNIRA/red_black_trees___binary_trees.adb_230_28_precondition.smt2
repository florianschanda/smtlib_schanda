...
;; H
  ...
  (and
  (forall ((i3 Int))
  (=> (and (<= 1 i3) (<= i3 100)) (= (contains o1 i3) true)))
  (= (length1 o1) 100))))
...
;; H
  (assert
  (forall ((j2 Int)) (is_add__function_guard
  (is_add
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select r2 (to_rep
             (rec__binary_trees__cell__parent
             (us_split_fields3
             (select (rec__binary_trees__forest__c (us_split_fields5 f)) j2)))))))
  (to_rep1
  (rec__binary_trees__cell__position
  (us_split_fields3
  (select (rec__binary_trees__forest__c (us_split_fields5 f)) j2))))
  (rec__tree_model__path_type__a (us_split_fields1 (select r2 j2))))
  (rec__tree_model__path_type__a
  (us_split_fields1
  (select r2 (to_rep
             (rec__binary_trees__cell__parent
...
;; H
  ...
  (= (rec__tree_model__path_type__k (us_split_fields1 (select r2 j2))) true)))))
...
;; H
  ...
  (not
  (= (olt (rec__tree_model__path_type__a (us_split_fields1 (select r2 j2)))
...
;; H
  ...
  (= (to_rep
     (rec__binary_trees__cell__parent
     (us_split_fields3
     (select (rec__binary_trees__forest__c (us_split_fields5 f)) j2)))) 0))
  (= (rec__tree_model__path_type__k
     (us_split_fields1
     (select r2 (to_rep
                (rec__binary_trees__cell__parent
                (us_split_fields3
                (select (rec__binary_trees__forest__c (us_split_fields5 f)) j2))))))) true))))))
...
;; H
  ...
  (= (is_add
     ...
     (us_split_fields3
     (select (rec__binary_trees__forest__c (us_split_fields5 f)) j2))))
     (rec__tree_model__path_type__a (us_split_fields1 (select r2 j2)))) true)
  (= (length
     (rec__tree_model__path_type__a (us_split_fields1 (select r2 j2)))) 0)))))
...
;; H
  ...
  (=> (and (<= 1 j2) (<= j2 100))
  (<= (length
      (rec__tree_model__path_type__a (us_split_fields1 (select r2 j2)))) (- 100 
  (length1 unseen2))))))
...
;; H
  (assert (= o22 (store r2 o21 (mk___rep (mk___split_fields o19 o20)))))

;; H
  (assert (= result8 (mk_map__ref r2)))
...
;; H
  (assert (= o28 (mk___split_fields1 o24 o25 o26 o27)))

;; H
  (assert (= o29 (rec__binary_trees__cell__parent o28)))

;; H
  (assert (= temp___880 (to_rep o29)))

;; H
  (assert (and (<= 1 temp___880) (<= temp___880 100)))

;; H
  (assert (= o30 temp___880))

;; H
  (assert (= o31 (select r3 o30)))

;; H
  (assert (= o32 (us_split_fields1 o31)))

;; H
  (assert (= o33 (rec__tree_model__path_type__a o32)))

(assert
;; WP_parameter_def
 ;; File "tree_model.ads", line 49, characters 0-0
  (not (< (length o33) 2147483647)))
(check-sat)
