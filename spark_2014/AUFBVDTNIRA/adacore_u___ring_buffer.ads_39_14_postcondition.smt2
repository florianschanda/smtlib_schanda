...
(declare-fun dummy6 () tTcontentSP1)
...
;; H
  ...
  (and (= (get o (last o)) e) (= (ole model__ o) true)))))
...
;; H
  (assert (=> (<= first (- 100 length1)) (= (to_rep o2) e)))
...
;; H
  (assert (=> (<= first (- 100 length1)) (= o3 (+ first length1))))

;; H
  (assert (=> (<= first (- 100 length1)) (= o4 (store content o3 o2))))

;; H
  (assert (=> (<= first (- 100 length1)) (= (mk_map__ref result1) content5)))

;; H
  (assert (=> (<= first (- 100 length1)) (= content1 o4)))

;; H
  (assert (=> (not (<= first (- 100 length1))) (= (to_rep o5) e)))
...
;; H
  (assert
  (=> (not (<= first (- 100 length1))) (= o6 (+ (- length1 100) first))))

;; H
  (assert (=> (not (<= first (- 100 length1))) (= o7 (store content o6 o5))))

;; H
  (assert
  (=> (not (<= first (- 100 length1))) (= (mk_map__ref result2) content5)))

;; H
  (assert (=> (not (<= first (- 100 length1))) (= content2 o7)))

;; H
  (assert (=> (not (<= first (- 100 length1))) (= content1 content2)))

;; H
  (assert (and (= o8 (+ length1 1)) (in_range2 (+ length1 1))))

;; H
  (assert (= result3 length1))
...
;; WP_parameter_def
 ;; File "ring_buffer.ads", line 33, characters 0-0
  (not (= (valid_model content3 first length3 model__2) true)))
(check-sat)
