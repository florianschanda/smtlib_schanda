...
(declare-fun perm__remove_eq__result () Int)
...
(declare-fun perm__remove_eq__result1 () Int)

(declare-fun perm__remove_eq__result2 () Int)

(declare-fun perm__remove_eq__result3 () Int)

(declare-fun perm__remove_eq__result4 () Int)

(declare-fun perm__remove_eq__result5 () Int)
...
;; H
  (assert
  (forall ((j Int))
  (=> (and (<= (to_rep1 (first (rt a))) j) (<= j (to_rep1 (last (rt a)))))
  (= (to_rep (select (elts a) j)) (to_rep (select (elts b) j))))))

;; H
  (assert
  (forall ((j Int))
  (=> (and (<= (to_rep1 (first (rt aa))) j) (<= j (to_rep1 (last (rt aa)))))
  (ite (< j i)
  (and (= (to_rep (select (elts aa) j)) (to_rep (select (elts a) j)))
  (= (to_rep (select (elts aa) j)) (to_rep (select (elts b) j))))
  (and (= (to_rep (select (elts aa) j)) (to_rep (select (elts a) (+ j 1))))
  (= (to_rep (select (elts aa) j)) (to_rep (select (elts b) (+ j 1)))))))))
...
;; H
  (assert
  (= (mk_int__ref perm__remove_eq__result4) (mk_int__ref
                                            perm__remove_eq__result2)))

;; H
  (assert (= perm__remove_eq__result5 perm__remove_eq__result3))

;; H
  (assert (= result1 perm__remove_eq__result4))

;; H
  (assert (= (of_int perm__remove_eq__result4) true))

;; H
  (assert (remove__function_guard (remove b i) b i))

;; H
  (assert (remove__function_guard (remove a i) a i))

(assert
;; WP_parameter_def
 ;; File "perm.ads", line 2, characters 0-0
  (not
  (= (let ((temp___747 (remove b i)))
     (let ((temp___746 (remove a i)))
     (bool_eq3 (elts temp___746) (to_rep1 (first (rt temp___746)))
     (to_rep1 (last (rt temp___746))) (elts temp___747)
     (to_rep1 (first (rt temp___747))) (to_rep1 (last (rt temp___747)))))) true)))
(check-sat)
