...
(define-fun winners23 () map__ref1 (mk_map__ref1 winners8))
...
;; H
  ...
  (< (to_rep (select counters latest_highest_score2)) (to_rep
                                                      (select counters 
                                                      i3)))
  (=> (and (<= 0 j3) (<= j3 (- i3 1))) (= winners23 (mk_map__ref1 winners6))))))
...
;; H
  ...
  (< (to_rep (select counters latest_highest_score2)) (to_rep
                                                      (select counters 
                                                      i3)))
  (= (mk_map__ref1 result6) winners23))))
...
;; H
  (assert (= result12 (mk_map__ref1 winners19)))

;; H
  (assert (= winners20 (store winners19 i9 (of_int 0))))
...
;; H
  (assert (= winners21 (mk_map__ref1 winners20)))

;; H
  (assert (= i12 i10))

;; H
  (assert (= winners22 winners20))
...
;; H
  (assert (<= j16 (- (int__content i11) 1)))

(assert
;; WP_parameter_def
 ;; File "evoting.ads", line 48, characters 0-0
  (not (not (= (select (map__content1 winners21) j16) true))))
(check-sat)
