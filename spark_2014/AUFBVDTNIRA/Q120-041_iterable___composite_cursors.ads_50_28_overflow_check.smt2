...
;; has_element__2__def_axiom
  ...
  (forall ((c1 us_rep1))
  (forall ((p1 us_rep))
  (! (=> (has_element__2__function_guard (has_element__2 c1 p1) c1 p1)
     (= (= (has_element__2 c1 p1) true)
     (and
     (<= 1 (to_rep
           (rec__composite_cursors__rec_cursor__content
           (us_split_fields1 p1))))
...
(assert
;; WP_parameter_def
 ;; File "composite_cursors.ads", line 31, characters 0-0
  (not (in_range
  (+ (to_rep
     (rec__composite_cursors__rec_cursor__content (us_split_fields1 p))) 1))))
(check-sat)
