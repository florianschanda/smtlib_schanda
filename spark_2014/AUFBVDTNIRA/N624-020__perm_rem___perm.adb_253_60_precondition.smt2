...
;; H
  (assert (and (= o6 ia1) (in_range4 ia1)))

;; H
  (assert
  (and (and (= o7 (remove a o6)) (remove__function_guard o7 a o6))
  (and (dynamic_invariant2 o7 true false true)
  (and (= (invariant__ o7) true)
  (and (= (to_rep2 (last (rt o7))) (- (to_rep2 (last (rt a))) 1))
  (and
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- o6 1)))
  (= (to_rep (select (elts a) k)) (to_rep (select (elts o7) k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ o6 1) k) (<= k (to_rep2 (last (rt a)))))
  (= (to_rep (select (elts a) k)) (to_rep (select (elts o7) (- k 1))))))
  (forall ((k Int))
  (=> (and (<= o6 k) (<= k (- (to_rep2 (last (rt a))) 1)))
  (= (to_rep (select (elts a) (+ k 1))) (to_rep (select (elts o7) k))))))))))))

;; H
  (assert
  (and
  (and
  (= o8 (remove o7 (to_rep1 (rec__perm__witnesses__ia (us_split_fields1 w)))))
  (remove__function_guard o8 o7
  (to_rep1 (rec__perm__witnesses__ia (us_split_fields1 w)))))
  (and (dynamic_invariant2 o8 true false true)
  (and (= (invariant__ o8) true)
  (and (= (to_rep2 (last (rt o8))) (- (to_rep2 (last (rt o7))) 1))
  (and
  (forall ((k Int))
  (=>
  (and (<= 1 k)
  (<= k (- (to_rep1 (rec__perm__witnesses__ia (us_split_fields1 w))) 1)))
  (= (to_rep (select (elts o7) k)) (to_rep (select (elts o8) k)))))
  (and
  (forall ((k Int))
  (=>
  (and (<= (+ (to_rep1 (rec__perm__witnesses__ia (us_split_fields1 w))) 1) k)
  (<= k (to_rep2 (last (rt o7)))))
  (= (to_rep (select (elts o7) k)) (to_rep (select (elts o8) (- k 1))))))
  (forall ((k Int))
  (=>
  (and (<= (to_rep1 (rec__perm__witnesses__ia (us_split_fields1 w))) k)
  (<= k (- (to_rep2 (last (rt o7))) 1)))
  (= (to_rep (select (elts o7) (+ k 1))) (to_rep (select (elts o8) k))))))))))))
