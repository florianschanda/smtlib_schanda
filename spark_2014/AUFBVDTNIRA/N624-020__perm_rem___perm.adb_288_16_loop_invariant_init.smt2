...
;; H
  (assert (forall ((ib Int)) (remove__function_guard (remove bb ib) bb ib)))
...
;; H
  ...
  (is_perm (remove aa ia2) (remove bb ib)) (remove aa ia2) (remove bb ib))))
...
;; H
  ...
  (exists ((ib Int))
  (and
  (and (<= (to_rep2 (first (rt bb))) ib) (<= ib (to_rep2 (last (rt bb)))))
  (and (= (to_rep (select (elts aa) ia2)) (to_rep (select (elts bb) ib)))
  (= (is_perm (remove aa ia2) (remove bb ib)) true))))))))))
...
;; H
  (assert (remove__function_guard (remove a k) a k))
...
;; WP_parameter_def
 ;; File "perm.ads", line 2, characters 0-0
  ...
  (exists ((ib Int))
  (and (and (<= (to_rep2 (first (rt b))) ib) (<= ib (to_rep2 (last (rt b)))))
  (and (= (to_rep (select (elts a) k)) (to_rep (select (elts b) ib)))
