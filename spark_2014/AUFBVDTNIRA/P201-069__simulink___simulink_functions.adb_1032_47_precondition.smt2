...
(declare-fun arcsin (t) t)

(declare-fun arcsin__function_guard (t t) Bool)
...
;; arcsin__post_axiom
  (assert
  (forall ((x t))
  (! (=> (and (dynamic_invariant x true true true) (le (abs1 x) fliteral))
     (let ((result (arcsin x)))
     (and (=> (eq x fliteral2) (eq result fliteral2)) (dynamic_invariant
     result true false true)))) :pattern ((arcsin x)) )))
...
;; H
  (assert (tqtisFinite f))

(assert
;; WP_parameter_def
 ;; File "system.ads", line 1, characters 0-0
  (not (le (abs1 f) fliteral)))
(check-sat)
