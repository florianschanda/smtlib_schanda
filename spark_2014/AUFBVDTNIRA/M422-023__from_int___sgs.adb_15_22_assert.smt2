...
;; fliteral_axiom
  (assert (and (tqtisFinite fliteral3) (= (tqtreal fliteral3) 14.0)))
...
(declare-fun a__function_guard (t Int) Bool)

(declare-fun c (Int) t)

(declare-fun c__function_guard (t Int) Bool)
...
;; a__post_axiom
  ...
  (! (=> (dynamic_invariant1 x true true true)
     (let ((result (a x)))
     (and (and (le fliteral2 result) (le result fliteral3))
...
;; c__def_axiom
  (assert
  (forall ((x Int))
  (! (=> (dynamic_invariant3 x true true true) (= (c x) (of_int RNE1 x))) :pattern (
  (c x)) )))
...
;; H
  (assert (=> (le (neg fliteral1) fliteral1) (tqtisFinite r1)))

;; H
  (assert (=> (le fliteral2 fliteral3) (in_range2 r2)))

;; H
  (assert (le fliteral2 (a x1)))

;; H
  (assert (le (a x1) fliteral3))

;; H
  (assert (le (a x2) fliteral3))

;; H
  (assert (le fliteral2 (a x2)))

(assert
;; WP_parameter_def
 ;; File "sgs.ads", line 3, characters 0-0
  (not (le fliteral2 (a x3))))
(check-sat)
