...
;; H
  (assert (and (= o8 (+ last2 1)) (in_range1 (+ last2 1))))

;; H
  (assert
  (ite (= (= (- n 1) 0) true)
  (= (bool_eq3 item1 (to_rep1 item__first) (to_rep1 item__last) item
     (to_rep1 item__first) (to_rep1 item__last)) true)
  (=> (= (not (= (- n 1) 0)) true)
  (forall ((idx Int))
  (=> (and (<= (to_rep1 item__first) idx) (<= idx (to_rep1 item__last)))
  (ite (and (<= o8 idx) (<= idx (- (+ o8 (- n 1)) 1)))
  (= (to_rep (select item1 idx)) (to_rep (select buf3 (+ (- idx o8) 1))))
  (= (to_rep (select item1 idx)) (to_rep (select item idx)))))))))

(assert
;; WP_parameter_def
 ;; File "textio.adb", line 102, characters 0-0
  (not (in_range1 (+ (- last2 1) n))))
(check-sat)
