...
;; H
  (assert (= temp1 (to_rep (select values x))))

;; H
  (assert (= (to_rep o) (to_rep (select values y))))

;; H
  (assert (= o1 (store values x o)))
...
;; H
  (assert (= o3 (store values1 y o2)))

;; H
  (assert (= result2 values1))
...
(assert
;; WP_parameter_def
 ;; File "sorters_not_global.ads", line 20, characters 0-0
  (not (= (to_rep (select values3 x)) (to_rep (select values y)))))
(check-sat)
