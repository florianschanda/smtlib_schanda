...
;; H
  ...
  (mk___t (elts1 init)
  (mk (to_rep (first3 (rt1 init))) (to_rep (last3 (rt1 init))))) x o5
  (mk___t interm (mk (to_rep interm__first) (to_rep interm__last)))))
  (= (= result true)
  (and
  (and
  ...
  (to_rep
  (last (mk (to_rep (first3 (rt1 init))) (to_rep (last3 (rt1 init))))))))
  (= (to_rep (select interm x)) o5))
  (forall ((j Int))
  (=>
  (and
  (<= (to_rep
      (first (mk (to_rep (first3 (rt1 init))) (to_rep (last3 (rt1 init)))))) j)
  (<= j (to_rep
        (last (mk (to_rep (first3 (rt1 init))) (to_rep (last3 (rt1 init))))))))
  (=> (not (= x j))
