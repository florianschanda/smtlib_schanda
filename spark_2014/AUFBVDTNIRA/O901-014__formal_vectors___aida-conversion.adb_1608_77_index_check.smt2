...
;; H
  (assert
  (forall ((index3 Int)) (is_digit__function_guard
  (is_digit (to_rep (select (elts source) index3)))
  (to_rep (select (elts source) index3)))))
...
;; H
  ...
  (forall ((index3 Int))
  (=>
  (and (<= (+ (to_rep1 (first (rt source))) 5) index3)
  (<= index3 (+ (to_rep1 (first (rt source))) 10)))
...
;; H
  (assert (<= temp___2323 index1))

;; H
  (assert (<= index1 temp___2324))
...
;; H
  (assert
  (forall ((j1 Int))
  (=> (and (<= (+ (to_rep1 (first (rt source))) 5) j1) (<= j1 index2))
  (<= (to_rep1 (select n3 j1)) (to_rep1 (select int_array1 j1))))))
...
;; H
  ...
  (=> (< index2 temp___2321)
  (= (select n3 temp___2321) (select n1 temp___2321)))))
  (=>
...
;; H
  (assert
  (and (= o9 (+ (to_rep1 (first (rt source))) 5)) (in_range1
  (+ (to_rep1 (first (rt source))) 5))))
...
;; H
  (assert
  (= result4 (ite (= o10 true) (ite (<= j index2) true false) false)))
...
;; H
  (assert
  (and (<= (to_rep1 int_array__first) j) (<= j (to_rep1 int_array__last))))
