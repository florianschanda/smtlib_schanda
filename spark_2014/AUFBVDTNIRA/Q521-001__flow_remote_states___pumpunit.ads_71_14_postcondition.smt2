...
;; H
  (assert (is_pumping__function_guard (is_pumping cur_state) cur_state))

;; H
  (assert (in_range cur_state))

;; H
  (assert (= (is_pumping cur_state) true))

;; H
  (assert (=> (= cur_state 0) (= result cur_state)))

;; H
  (assert (=> (= cur_state 0) (= cur_state2 1)))

;; H
  (assert (=> (not (= cur_state 0)) (= cur_state2 cur_state)))

;; H
  (assert (=> (not (= cur_state 0)) (= cur_state2 cur_state1)))
...
;; H
  (assert (is_ready__function_guard (is_ready cur_state3) cur_state3))

(assert
;; WP_parameter_def
 ;; File "pumpunit.ads", line 65, characters 0-0
  (not (= (is_ready cur_state3) true)))
(check-sat)
