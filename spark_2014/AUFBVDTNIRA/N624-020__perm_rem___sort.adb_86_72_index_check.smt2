...
;; H
  (assert (invariant____function_guard (invariant__ temp___335) temp___335))
...
;; H
  (assert
  (and
  (and (= temp___335 (remove (mk___t values2 (mk 1 100)) o47))
  (remove__function_guard temp___335 (mk___t values2 (mk 1 100)) o47))
  (and (dynamic_invariant4 temp___335 true false true)
  (and (= (invariant__ temp___335) true)
  (and (= (to_rep1 (last (rt temp___335))) (- (to_rep1 (last (mk 1 100))) 1))
  (and
  (forall ((k Int))
  (=> (and (<= 1 k) (<= k (- o47 1)))
  (= (to_rep (select values2 k)) (to_rep (select (elts temp___335) k)))))
  (and
  (forall ((k Int))
  (=> (and (<= (+ o47 1) k) (<= k (to_rep1 (last (mk 1 100)))))
  (= (to_rep (select values2 k)) (to_rep (select (elts temp___335) (- k 1))))))
  (forall ((k Int))
  (=> (and (<= o47 k) (<= k (- (to_rep1 (last (mk 1 100))) 1)))
  (= (to_rep (select values2 (+ k 1))) (to_rep (select (elts temp___335) k))))))))))))

(assert
;; WP_parameter_def
 ;; File "sort.ads", line 4, characters 0-0
  (not (<= (to_rep1 (first (rt temp___335))) x)))
(check-sat)
