...
(declare-fun dummy4 () extended_index_type)
...
;; size__post_axiom
  ...
  (! (=> (type_invariant f1)
     (let ((result (size f1)))
     (=> (size__function_guard result f1) (dynamic_invariant3 result true
...
;; parent__post_axiom
  ...
  (! (=> (and (dynamic_invariant i true true true) (type_invariant f1))
     ...
     (=> (= (size f1) 0) (= result 0))) (dynamic_invariant3 result true false
...
;; position__post_axiom
  ...
  (! (and (parent__function_guard (parent f1 i) f1 i)
     ...
     (=> (position__function_guard result f1 i) (dynamic_invariant4 result
...
;; H
  (assert (valid_root__function_guard (valid_root f t1) f t1))

;; H
  (assert (valid_root__function_guard (valid_root f t2) f t2))

;; H
  (assert
  (and (not (= t1 t2))
  (and (= (valid_root f t1) true) (= (valid_root f t2) true))))
...
;; H
  (assert (model____function_guard (model__ f t1) f t1))
...
;; H
  (assert
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select (model__ f t1) i))) true))

;; H
  (assert
  (< (length
     (rec__tree_model__path_type__a
     (us_split_fields1 (select (model__ f t1) i)))) n1))

;; H
  (assert (model____function_guard (model__ f t2) f t2))
...
;; WP_parameter_def
 ;; File "system.ads", line 1, characters 0-0
  ...
  (not
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select (model__ f t2) i))) true))))
(check-sat)
