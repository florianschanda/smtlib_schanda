...
;; size__post_axiom
  (assert
  (forall ((f us_rep3))
  (! (=> (dynamic_invariant5 f true true true)
     (let ((result (size f)))
     (=> (size__function_guard result f) (dynamic_invariant7 result true
...
;; H
  (assert (ordered_leafs__function_guard (ordered_leafs f2 root values) 
  f2 root values))
...
;; H
  (assert (oeq__function_guard1 (oeq1 (model__ f2 root) (model__ f1 root))
  (model__ f2 root) (model__ f1 root)))
...
;; H
  (assert (dynamic_invariant5 f1 true false true))

;; H
  (assert (dynamic_invariant5 f2 true false true))
...
;; H
  ...
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select (model__ f1 root) j))) true))
  (= (olt
     (rec__tree_model__path_type__a
     (us_split_fields1 (select (model__ f1 root) i)))
     (rec__tree_model__path_type__a
     (us_split_fields1 (select (model__ f1 root) j)))) true))
  (and
  (and
  ...
  (= (rec__tree_model__path_type__k
     (us_split_fields1 (select (model__ f2 root) j))) true))
  (= (olt
     (rec__tree_model__path_type__a
  ...
  (= (get
     (rec__tree_model__path_type__a
     (us_split_fields1 (select (model__ f1 root) j)))
     (+ (length
        (rec__tree_model__path_type__a
        (us_split_fields1 (select (model__ f1 root) i)))) 1)) (get
                                                              (rec__tree_model__path_type__a
                                                              (us_split_fields1
                                                              (select 
                                                              (model__ 
...
;; H
  ...
  (ite (= (get
          ...
          (+ (length
             (rec__tree_model__path_type__a
             (us_split_fields1 (select (model__ f1 root) i)))) 1)) 0)
  (< (to_rep (select values j)) (to_rep (select values i)))
  (< (to_rep (select values i)) (to_rep (select values j))))))))))

;; H
  (assert (ordered_leafs__function_guard (ordered_leafs f1 root values) 
  f1 root values))

(assert
;; WP_parameter_def
 ;; File "search_trees.adb", line 329, characters 0-0
  (not (= (ordered_leafs f1 root values) true)))
(check-sat)
