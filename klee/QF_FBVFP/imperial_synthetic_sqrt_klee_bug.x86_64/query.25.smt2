...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee_bug.x86_64/query.25.smt2
|)
...
(not (not (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x91 ?x19)) ?x37)))))))))))))))))))))))))))
(check-sat)
(exit)
