...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee_bug.x86_64/query.18.smt2
|)
...
(assert
 ...
 (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x63 ?x19)) ?x37))))))))))))))))
(assert
 (let ((?x32 ((_ to_fp 8 24) (_ bv1056964608 32))))
 (let ((?x22 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x15 (concat (select x0 (_ bv2 32)) (concat (select x0 (_ bv1 32)) (select x0 (_ bv0 32))))))
...
(let ((?x60 ((_ to_fp 8 24) (ite $x55 fresh_to_ieee_bv_!1 (ite $x41 fresh_to_ieee_bv_!0 ?x18)))))
(let ((?x63 (fp.mul roundNearestTiesToEven (fp.add roundNearestTiesToEven ?x60 (ite $x55 ?x47 ?x49)) ?x32)))
(let ((?x68 (fp.sub roundNearestTiesToEven ?x19 (fp.mul roundNearestTiesToEven ?x63 ?x63))))
(not (not (fp.lt (fp.abs ?x68) ?x37)))))))))))))))))))
(check-sat)
(exit)
