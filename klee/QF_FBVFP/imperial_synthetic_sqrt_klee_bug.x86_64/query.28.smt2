...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee_bug.x86_64/query.28.smt2
|)
...
(let ((?x89 (ite $x83 ?x75 ?x77)))
(let ((?x98 (ite $x97 ?x89 ?x91)))
(let (($x108 (and (not (fp.isNaN ?x98)) (not (fp.isNaN ((_ to_fp 8 24) (ite $x97 fresh_to_ieee_bv_!4 ?x87)))))))
(let (($x111 (and $x108 (not (fp.gt ?x98 ((_ to_fp 8 24) (ite $x97 fresh_to_ieee_bv_!4 ?x87)))))))
(not $x111))))))))))))))))))))))))))))
(check-sat)
(exit)
