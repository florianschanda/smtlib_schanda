...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_gsl_benchmarks_sort_smallest_klee.x86_64/query.1461.smt2
|)
...
(let ((?x48 (concat (select x0 (_ bv5 32)) (concat (select x0 (_ bv4 32)) (concat (select x0 (_ bv3 32)) ?x39)))))
(let ((?x55 ((_ to_fp 11 53) (concat (select x0 (_ bv7 32)) (concat (select x0 (_ bv6 32)) ?x48)))))
(let (($x182 (and (not (fp.isNaN ?x168)) (or (or (fp.isNaN ?x55) (fp.isNaN ?x168)) (fp.gt ?x55 ?x168)))))
(not (not $x182))))))))))
(check-sat)
(exit)
