...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_rlim_invariant.x86_64/query.657.smt2
|)
...
(let ((?x152 ((_ to_fp 8 24) ?x151)))
(let ((?x167 (fp.sub roundNearestTiesToEven ?x152 (fp.sub roundNearestTiesToEven ?x135 ?x123))))
(not (or (or (fp.isNaN ?x167) (fp.isNaN ?x163)) (fp.lt ?x167 ?x163)))))))))))))))))))))))))
(check-sat)
(exit)
