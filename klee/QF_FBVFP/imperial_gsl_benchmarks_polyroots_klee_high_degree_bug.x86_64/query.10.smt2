...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_gsl_benchmarks_polyroots_klee_high_degree_bug.x86_64/query.10.smt2
|)
...
(let ((?x77 (fp.add roundNearestTiesToEven (fp.abs (fp.sub roundNearestTiesToEven ?x39 ?x68)) (fp.abs (fp.sub roundNearestTiesToEven ?x45 ?x74)))))
(not (not (fp.eq (fp.add roundNearestTiesToEven ?x77 ?x45) ?x45))))))))))))))))))))))))))
(check-sat)
(exit)
