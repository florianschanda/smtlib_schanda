...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_gsl_benchmarks_vectors_klee_bug.x86_64/query.3.smt2
|)
...
(let ((?x24 (concat (select temp0 (_ bv5 32)) (concat (select temp0 (_ bv4 32)) (concat (select temp0 (_ bv3 32)) ?x15)))))
(let ((?x31 ((_ to_fp 11 53) (concat (select temp0 (_ bv7 32)) (concat (select temp0 (_ bv6 32)) ?x24)))))
(let ((?x40 (fp.sub roundNearestTiesToEven ((_ to_fp 11 53) (_ bv4612451630364040888 64)) (fp.sub roundNearestTiesToEven ((_ to_fp 11 53) (_ bv0 64)) ?x31))))
(let ((?x35 (fp.add roundNearestTiesToEven ?x31 ((_ to_fp 11 53) (_ bv4612451630364040888 64)))))
(not (not (fp.eq ?x35 ?x40)))))))))
(check-sat)
(exit)
