...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/aachen_real_wxpro_tostr.x86_64/query.01.smt2
|)
...
(let ((?x24 (concat (select meters0 (_ bv5 32)) (concat (select meters0 (_ bv4 32)) (concat (select meters0 (_ bv3 32)) ?x15)))))
(let ((?x31 ((_ to_fp 11 53) (concat (select meters0 (_ bv7 32)) (concat (select meters0 (_ bv6 32)) ?x24)))))
(let (($x44 (not (bvult (_ bv9223372036854775807 64) ((_ sign_extend 32) ((_ fp.to_sbv 32) roundTowardZero (fp.abs ?x31)))))))
(not $x44))))))
(check-sat)
(exit)
