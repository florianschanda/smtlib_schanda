...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/aachen_syn_nan_double.x86_64/query.11.smt2
|)
...
(let ((?x24 (concat (select f_10 (_ bv5 32)) (concat (select f_10 (_ bv4 32)) (concat (select f_10 (_ bv3 32)) ?x15)))))
(let ((?x31 ((_ to_fp 11 53) (concat (select f_10 (_ bv7 32)) (concat (select f_10 (_ bv6 32)) ?x24)))))
(not (not (not (fp.isNaN (fp.sqrt roundNearestTiesToEven ?x31)))))))))
(check-sat)
(exit)
