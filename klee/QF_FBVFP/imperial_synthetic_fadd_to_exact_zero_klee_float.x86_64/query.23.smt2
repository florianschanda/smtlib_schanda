...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_fadd_to_exact_zero_klee_float.x86_64/query.23.smt2
|)
...
(let ((?x18 (concat (select f0 (_ bv3 32)) ?x15)))
(let ((?x19 ((_ to_fp 8 24) ?x18)))
(not (fp.isNaN (fp.add roundTowardZero ?x19 ?x30))))))))))
(check-sat)
(exit)
