...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_interval_klee_bug_symbolic_increment.x86_64/query.12.smt2
|)
...
(let ((?x15 (concat (select initial0 (_ bv2 32)) (concat (select initial0 (_ bv1 32)) (select initial0 (_ bv0 32))))))
(let ((?x19 ((_ to_fp 8 24) (concat (select initial0 (_ bv3 32)) ?x15))))
(let ((?x46 (fp.add roundTowardNegative ?x19 ?x30)))
(not (and (not (fp.isNaN ?x46)) (fp.leq ?x46 ?x46)))))))))
(check-sat)
(exit)
