...
(let ((?x168 (bvadd (ite $x165 (ite $x144 (ite $x123 (ite $x103 (_ bv0 32) (_ bv2 32)) ?x107) ?x127) ?x150) (ite $x165 ?x150 ?x146))))
(let ((?x172 ((_ extract 31 0) (bvmul (_ bv4 64) ((_ sign_extend 32) (bvsdiv ?x168 (_ bv2 32)))))))
(let ((?x179 (concat (select data0 (bvadd (_ bv2 32) ?x172)) (concat (select data0 (bvadd (_ bv1 32) ?x172)) (select data0 ?x172)))))
(let (($x185 (not (fp.eq ((_ to_fp 8 24) (concat (select data0 (bvadd (_ bv3 32) ?x172)) ?x179)) ?x94))))
(not $x185)))))))))))))))))))))))))))))
(check-sat)
(exit)
