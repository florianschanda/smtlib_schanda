...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_non_terminating_klee_bug.x86_64/query.08.smt2
|)
...
(let ((?x36 (fp.sub roundNearestTiesToEven ?x34 ?x19)))
(let ((?x38 (fp.sub roundNearestTiesToEven ?x36 ?x19)))
(not (fp.gt (fp.sub roundNearestTiesToEven ?x38 ?x19) ?x30)))))))))))
(check-sat)
(exit)
