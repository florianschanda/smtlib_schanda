...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_non_terminating_klee_bug.x86_64/query.22.smt2
|)
...
(let ((?x64 (fp.sub roundNearestTiesToEven ?x62 ?x19)))
(let ((?x66 (fp.sub roundNearestTiesToEven ?x64 ?x19)))
(not (fp.gt (fp.sub roundNearestTiesToEven ?x66 ?x19) ?x30)))))))))))))))))))))))))
(check-sat)
(exit)
