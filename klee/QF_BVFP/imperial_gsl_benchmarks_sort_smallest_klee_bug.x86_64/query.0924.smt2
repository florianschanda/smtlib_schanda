...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_gsl_benchmarks_sort_smallest_klee_bug.x86_64/query.0924.smt2
|)
...
(assert
 (let ((?x13 ((_ to_fp 11 53) x_ackermann!0)))
(let (($x44 (fp.isNaN ?x13)))
(not (not (and (not $x44) $x44))))))
(check-sat)
(exit)
