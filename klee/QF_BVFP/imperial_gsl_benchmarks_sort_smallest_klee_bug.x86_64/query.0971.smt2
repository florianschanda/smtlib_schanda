...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_gsl_benchmarks_sort_smallest_klee_bug.x86_64/query.0971.smt2
|)
...
(assert
 ...
 (let (($x42 (fp.isNaN ?x29)))
 (not $x42))))
(assert
 (let ((?x29 ((_ to_fp 11 53) x_ackermann!4)))
 (let (($x42 (fp.isNaN ?x29)))
 (or (or (fp.isNaN ((_ to_fp 11 53) x_ackermann!0)) $x42) (fp.gt ((_ to_fp 11 53) x_ackermann!0) ?x29)))))
(assert
 (not false))
(check-sat)
(exit)
