...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_gsl_benchmarks_sort_smallest_klee_bug.x86_64/query.1461.smt2
|)
...
(assert
 ...
 (let (($x43 (fp.isNaN ?x36)))
 (not $x43))))
(assert
 (let ((?x36 ((_ to_fp 11 53) x_ackermann!5)))
 (let (($x43 (fp.isNaN ?x36)))
 (or (or (fp.isNaN ((_ to_fp 11 53) x_ackermann!1)) $x43) (fp.gt ((_ to_fp 11 53) x_ackermann!1) ?x36)))))
(assert
 (not false))
(check-sat)
(exit)
