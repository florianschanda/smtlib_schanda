...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee_bug.x86_64/query.13.smt2
|)
...
(assert
 ...
 (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x39 ?x8)) ?x27)))))))))
(assert
...
(let ((?x38 (fp.add roundNearestTiesToEven ((_ to_fp 8 24) (ite $x31 fresh_to_ieee_bv_!1 x_ackermann!0)) (ite $x31 ?x12 ?x23))))
(let ((?x39 (fp.mul roundNearestTiesToEven ?x38 ((_ to_fp 8 24) (_ bv1056964608 32)))))
(let ((?x44 (fp.sub roundNearestTiesToEven ?x8 (fp.mul roundNearestTiesToEven ?x39 ?x39))))
(not (not (fp.lt (fp.abs ?x44) ?x27))))))))))))
(check-sat)
(exit)
