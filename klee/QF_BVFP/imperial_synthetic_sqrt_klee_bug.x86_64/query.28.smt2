...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee_bug.x86_64/query.28.smt2
|)
...
(let ((?x79 (ite $x73 ?x65 ?x67)))
(let ((?x88 (ite $x87 ?x79 ?x81)))
(let (($x98 (and (not (fp.isNaN ?x88)) (not (fp.isNaN ((_ to_fp 8 24) (ite $x87 fresh_to_ieee_bv_!5 ?x77)))))))
(let (($x101 (and $x98 (not (fp.gt ?x88 ((_ to_fp 8 24) (ite $x87 fresh_to_ieee_bv_!5 ?x77)))))))
(not $x101))))))))))))))))))))))))))
(check-sat)
(exit)
