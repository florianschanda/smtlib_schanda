...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee_bug.x86_64/query.18.smt2
|)
...
(let ((?x52 (fp.add roundNearestTiesToEven ((_ to_fp 8 24) (ite $x45 fresh_to_ieee_bv_!2 ?x35)) (ite $x45 ?x37 ?x39))))
(let ((?x53 (fp.mul roundNearestTiesToEven ?x52 ?x22)))
(let ((?x58 (fp.sub roundNearestTiesToEven ?x8 (fp.mul roundNearestTiesToEven ?x53 ?x53))))
(not (not (fp.lt (fp.abs ?x58) ?x27))))))))))))))))))
(check-sat)
(exit)
