...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee_bug.x86_64/query.20.smt2
|)
...
(let ((?x68 (fp.sub roundNearestTiesToEven (fp.mul roundNearestTiesToEven ?x66 ?x22) ?x8)))
(not (not (fp.lt (fp.abs ?x68) ?x27)))))))))))))))))))))
(check-sat)
(exit)
