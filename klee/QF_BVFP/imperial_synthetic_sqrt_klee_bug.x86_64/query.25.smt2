...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee_bug.x86_64/query.25.smt2
|)
...
(not (not (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x81 ?x8)) ?x27)))))))))))))))))))))))))
(check-sat)
(exit)
