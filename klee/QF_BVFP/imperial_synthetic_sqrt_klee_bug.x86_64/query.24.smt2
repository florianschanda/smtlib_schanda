...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee_bug.x86_64/query.24.smt2
|)
...
(assert
 ...
 (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x67 ?x8)) ?x27)))))))))))))))))))
(assert
 (let ((?x22 ((_ to_fp 8 24) (_ bv1056964608 32))))
...
(let ((?x66 (fp.add roundNearestTiesToEven ((_ to_fp 8 24) (ite $x59 fresh_to_ieee_bv_!3 ?x49)) (ite $x59 ?x51 ?x53))))
(let ((?x67 (fp.mul roundNearestTiesToEven ?x66 ?x22)))
(let ((?x72 (fp.sub roundNearestTiesToEven ?x8 (fp.mul roundNearestTiesToEven ?x67 ?x67))))
(not (not (fp.lt (fp.abs ?x72) ?x27))))))))))))))))))))))
(check-sat)
(exit)
