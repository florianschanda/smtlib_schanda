...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee_bug.x86_64/query.34.smt2
|)
...
(let ((?x94 (fp.add roundNearestTiesToEven ((_ to_fp 8 24) (ite $x87 fresh_to_ieee_bv_!5 ?x77)) (ite $x87 ?x79 ?x81))))
(let ((?x95 (fp.mul roundNearestTiesToEven ?x94 ?x22)))
(let ((?x100 (fp.sub roundNearestTiesToEven ?x8 (fp.mul roundNearestTiesToEven ?x95 ?x95))))
(not (not (fp.lt (fp.abs ?x100) ?x27)))))))))))))))))))))))))))))))
(check-sat)
(exit)
