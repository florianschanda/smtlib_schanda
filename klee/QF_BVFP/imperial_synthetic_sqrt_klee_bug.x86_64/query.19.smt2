...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee_bug.x86_64/query.19.smt2
|)
...
(let ((?x51 (ite $x45 ?x37 ?x39)))
(let ((?x60 (ite $x59 ?x51 ?x53)))
(let (($x70 (and (not (fp.isNaN ?x60)) (not (fp.isNaN ((_ to_fp 8 24) (ite $x59 fresh_to_ieee_bv_!3 ?x49)))))))
(let (($x73 (and $x70 (not (fp.gt ?x60 ((_ to_fp 8 24) (ite $x59 fresh_to_ieee_bv_!3 ?x49)))))))
(not $x73))))))))))))))))))
(check-sat)
(exit)
