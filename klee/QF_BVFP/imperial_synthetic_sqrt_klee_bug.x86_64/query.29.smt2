...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee_bug.x86_64/query.29.smt2
|)
...
(let ((?x78 ((_ to_fp 8 24) (ite $x73 fresh_to_ieee_bv_!4 (ite $x59 fresh_to_ieee_bv_!3 ?x49)))))
(let ((?x81 (fp.mul roundNearestTiesToEven (fp.add roundNearestTiesToEven ?x78 (ite $x73 ?x65 ?x67)) ?x22)))
(let ((?x86 (fp.sub roundNearestTiesToEven ?x8 (fp.mul roundNearestTiesToEven ?x81 ?x81))))
(not (not (fp.lt (fp.abs ?x86) ?x27))))))))))))))))))))))))))
(check-sat)
(exit)
