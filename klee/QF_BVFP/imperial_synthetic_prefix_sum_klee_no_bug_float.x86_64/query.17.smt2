...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_prefix_sum_klee_no_bug_float.x86_64/query.17.smt2
|)
...
(let ((?x35 (fp.add roundNearestTiesToEven ?x23 (fp.add roundNearestTiesToEven ?x11 ?x18))))
(let ((?x28 ((_ to_fp 8 24) data_in_ackermann!3)))
(let ((?x36 (fp.add roundNearestTiesToEven ?x28 ?x35)))
(let (($x44 (and (and (not (fp.isNaN ?x36)) (not (fp.isNaN ?x35))) (not (fp.lt ?x36 ?x35)))))
(not $x44)))))))))
(check-sat)
(exit)
