...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_interval_klee_bug.x86_64/query.19.smt2
|)
...
(assert
 ...
 (fp.geq ((_ to_fp 11 53) roundNearestTiesToEven ?x31) (fp.add roundNearestTiesToEven ?x29 ((_ to_fp 11 53) (_ bv4707126720094797824 64))))))))))
(assert
 (let ((?x21 ((_ to_fp 8 24) (_ bv1251513984 32))))
(let ((?x8 ((_ to_fp 8 24) initial_ackermann!0)))
(let ((?x23 (fp.add roundTowardNegative ?x8 ?x21)))
...
(not (and (not (fp.isNaN ?x35)) (fp.leq ?x35 ?x35)))))))))
(check-sat)
(exit)
