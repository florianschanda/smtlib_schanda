...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_interval_klee_bug.x86_64/query.29.smt2
|)
...
(let ((?x35 (fp.add roundTowardNegative ?x31 ?x21)))
(let ((?x39 (fp.add roundTowardNegative ?x35 ?x21)))
(let ((?x43 (fp.add roundTowardNegative ?x39 ?x21)))
(not (and (not (fp.isNaN ?x43)) (fp.leq ?x43 ?x43)))))))))))
(check-sat)
(exit)
