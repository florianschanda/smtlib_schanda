...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_interval_klee_bug.x86_64/query.14.smt2
|)
...
(assert
 (let ((?x8 ((_ to_fp 8 24) initial_ackermann!0)))
(let ((?x23 (fp.add roundTowardNegative ?x8 ((_ to_fp 8 24) (_ bv1251513984 32)))))
(let ((?x31 (fp.add roundTowardNegative ?x23 ((_ to_fp 8 24) (_ bv1251513984 32)))))
(not (and (not (fp.isNaN ?x31)) (fp.leq ?x31 ?x31)))))))
(check-sat)
(exit)
