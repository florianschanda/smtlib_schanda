...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_interval_klee_bug.x86_64/query.27.smt2
|)
...
(assert
 ...
 (let ((?x31 (fp.add roundTowardNegative ?x23 ?x21)))
 (let ((?x35 (fp.add roundTowardNegative ?x31 ?x21)))
 (fp.geq ((_ to_fp 11 53) roundNearestTiesToEven (fp.add roundTowardNegative ?x35 ?x21)) ?x41))))))))))))
(assert
 (let ((?x28 ((_ to_fp 11 53) (_ bv4707126720094797824 64))))
(let ((?x29 (fp.add roundNearestTiesToEven ((_ to_fp 11 53) roundNearestTiesToEven ((_ to_fp 8 24) initial_ackermann!0)) ?x28)))
(let ((?x33 (fp.add roundNearestTiesToEven ?x29 ?x28)))
(let ((?x37 (fp.add roundNearestTiesToEven ?x33 ?x28)))
(let ((?x41 (fp.add roundNearestTiesToEven ?x37 ?x28)))
(not (not (fp.isNaN (fp.add roundNearestTiesToEven ?x41 ?x28))))))))))
(check-sat)
(exit)
