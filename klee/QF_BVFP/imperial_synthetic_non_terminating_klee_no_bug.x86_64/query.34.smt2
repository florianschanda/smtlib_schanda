...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_non_terminating_klee_no_bug.x86_64/query.34.smt2
|)
...
(let ((?x41 (fp.sub roundNearestTiesToEven (fp.sub roundNearestTiesToEven ?x39 ?x8) ?x8)))
(let ((?x43 (fp.sub roundNearestTiesToEven (fp.sub roundNearestTiesToEven ?x41 ?x8) ?x8)))
(let ((?x45 (fp.sub roundNearestTiesToEven (fp.sub roundNearestTiesToEven ?x43 ?x8) ?x8)))
(let ((?x47 (fp.sub roundNearestTiesToEven (fp.sub roundNearestTiesToEven ?x45 ?x8) ?x8)))
(let ((?x49 (fp.sub roundNearestTiesToEven (fp.sub roundNearestTiesToEven ?x47 ?x8) ?x8)))
(not (fp.gt ?x49 ((_ to_fp 8 24) (_ bv0 32))))))))))))))))))))))
(check-sat)
(exit)
