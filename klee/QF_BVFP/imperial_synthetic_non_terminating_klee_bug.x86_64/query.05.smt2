...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_non_terminating_klee_bug.x86_64/query.05.smt2
|)
...
(let ((?x18 (fp.sub roundNearestTiesToEven ((_ to_fp 8 24) (_ bv1120403456 32)) ?x8)))
(let ((?x22 (fp.sub roundNearestTiesToEven ?x18 ?x8)))
(not (fp.gt (fp.sub roundNearestTiesToEven ?x22 ?x8) ?x20)))))))
(check-sat)
(exit)
