...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_non_terminating_klee_bug.x86_64/query.06.smt2
|)
...
(let ((?x22 (fp.sub roundNearestTiesToEven ?x18 ?x8)))
(let ((?x24 (fp.sub roundNearestTiesToEven ?x22 ?x8)))
(not (fp.gt (fp.sub roundNearestTiesToEven ?x24 ?x8) ?x20))))))))
(check-sat)
(exit)
