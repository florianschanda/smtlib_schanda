...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_non_terminating_klee_bug.x86_64/query.28.smt2
|)
...
(let ((?x66 (fp.sub roundNearestTiesToEven ?x64 ?x8)))
(let ((?x68 (fp.sub roundNearestTiesToEven ?x66 ?x8)))
(not (fp.gt (fp.sub roundNearestTiesToEven ?x68 ?x8) ?x20))))))))))))))))))))))))))))))
(check-sat)
(exit)
