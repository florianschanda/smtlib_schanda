...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_non_terminating_klee_bug.x86_64/query.30.smt2
|)
...
(assert
 ...
 (let ((?x68 (fp.sub roundNearestTiesToEven ?x66 ?x8)))
 (let ((?x70 (fp.sub roundNearestTiesToEven ?x68 ?x8)))
 (let ((?x72 (fp.sub roundNearestTiesToEven ?x70 ?x8)))
 (fp.gt ?x72 ?x20)))))))))))))))))))))))))))))))
(assert
 (let ((?x20 ((_ to_fp 8 24) (_ bv0 32))))
(let ((?x8 ((_ to_fp 8 24) delta_ackermann!0)))
...
(let ((?x70 (fp.sub roundNearestTiesToEven ?x68 ?x8)))
(let ((?x72 (fp.sub roundNearestTiesToEven ?x70 ?x8)))
(not (fp.gt (fp.sub roundNearestTiesToEven ?x72 ?x8) ?x20))))))))))))))))))))))))))))))))
(check-sat)
(exit)
