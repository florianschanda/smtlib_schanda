...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_rlim_invariant.x86_64/query.185.smt2
|)
...
(assert
 ...
 (let ((?x36 (fp.sub roundNearestTiesToEven ?x17 ?x15)))
 (let ((?x51 (fp.sub roundNearestTiesToEven ?x36 ?x32)))
 (let ((?x65 (fp.add roundNearestTiesToEven ?x51 ?x45)))
...
(let ((?x51 (fp.sub roundNearestTiesToEven ?x36 ?x32)))
(let ((?x65 (fp.add roundNearestTiesToEven ?x51 ?x45)))
(let ((?x88 (fp.sub roundNearestTiesToEven ?x65 ?x61)))
