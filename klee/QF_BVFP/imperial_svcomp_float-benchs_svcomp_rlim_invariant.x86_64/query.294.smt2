...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_rlim_invariant.x86_64/query.294.smt2
|)
...
(assert
 ...
 (let ((?x38 (fp.sub roundNearestTiesToEven ?x17 ?x15)))
 (let ((?x54 (fp.add roundNearestTiesToEven ?x38 ?x32)))
 (let ((?x68 (fp.add roundNearestTiesToEven ?x54 ?x48)))
...
(let ((?x54 (fp.add roundNearestTiesToEven ?x38 ?x32)))
(let ((?x68 (fp.add roundNearestTiesToEven ?x54 ?x48)))
(let ((?x91 (fp.sub roundNearestTiesToEven ?x68 ?x64)))
