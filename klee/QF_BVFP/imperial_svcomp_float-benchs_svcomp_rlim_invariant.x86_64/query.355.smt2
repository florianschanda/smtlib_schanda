(set-info :smt-lib-version 2.6)
(set-logic QF_BVFP)
(set-info :source |
Generated by: Daniel Liew, Daniel Schemmel, Cristian Cadar, Alastair Donaldson, and Rafael Zähl
Generated on: 2017-4-28
Generator: KLEE
Application: Branch satisfiability check for symbolic execution of C programs
Target solver: Z3 or MathSAT5
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_rlim_invariant.x86_64/query.355.smt2
|)
(set-info :license "https://creativecommons.org/licenses/by/4.0/")
(set-info :category "industrial")
(set-info :status unsat)
(declare-fun symbolic_0_float_8_ackermann!9 () (_ BitVec 32))
(declare-fun symbolic_0_float_9_ackermann!1 () (_ BitVec 32))
(declare-fun symbolic_0_float_7_ackermann!6 () (_ BitVec 32))
(declare-fun symbolic_0_float_5_ackermann!4 () (_ BitVec 32))
(declare-fun symbolic_0_float_2_ackermann!3 () (_ BitVec 32))
(declare-fun symbolic_0_float_3_ackermann!0 () (_ BitVec 32))
(declare-fun symbolic_0_float_1_ackermann!7 () (_ BitVec 32))
(declare-fun symbolic_0_float_4_ackermann!5 () (_ BitVec 32))
(declare-fun symbolic_0_float_6_ackermann!2 () (_ BitVec 32))
(declare-fun symbolic_0_float_ackermann!8 () (_ BitVec 32))
(assert
 (let ((?x19 ((_ to_fp 8 24) (_ bv3271557120 32))))
 (let ((?x17 ((_ to_fp 8 24) symbolic_0_float_8_ackermann!9)))
 (fp.geq ?x17 ?x19))))
(assert
 (let ((?x22 ((_ to_fp 8 24) (_ bv1124073472 32))))
 (let ((?x17 ((_ to_fp 8 24) symbolic_0_float_8_ackermann!9)))
 (fp.leq ?x17 ?x22))))
(assert
 (let ((?x34 ((_ to_fp 8 24) symbolic_0_float_9_ackermann!1)))
 (let ((?x33 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x35 (fp.sub roundNearestTiesToEven ?x33 ?x34)))
 (let ((?x28 ((_ to_fp 8 24) symbolic_0_float_7_ackermann!6)))
 (let ((?x25 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!4)))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (let ((?x27 (fp.sub roundNearestTiesToEven ?x24 ?x25)))
 (let ((?x17 ((_ to_fp 8 24) symbolic_0_float_8_ackermann!9)))
 (let ((?x30 (fp.sub roundNearestTiesToEven ?x17 (fp.sub roundNearestTiesToEven ?x27 ?x28))))
 (or (or (fp.isNaN ?x30) (fp.isNaN ?x35)) (fp.gt ?x30 ?x35))))))))))))
(assert
 (let ((?x34 ((_ to_fp 8 24) symbolic_0_float_9_ackermann!1)))
 (let ((?x28 ((_ to_fp 8 24) symbolic_0_float_7_ackermann!6)))
 (let ((?x25 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!4)))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (let ((?x27 (fp.sub roundNearestTiesToEven ?x24 ?x25)))
 (let ((?x17 ((_ to_fp 8 24) symbolic_0_float_8_ackermann!9)))
 (let ((?x30 (fp.sub roundNearestTiesToEven ?x17 (fp.sub roundNearestTiesToEven ?x27 ?x28))))
 (or (or (fp.isNaN ?x30) (fp.isNaN ?x34)) (fp.lt ?x30 ?x34))))))))))
(assert
 (let ((?x19 ((_ to_fp 8 24) (_ bv3271557120 32))))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (fp.geq ?x24 ?x19))))
(assert
 (let ((?x22 ((_ to_fp 8 24) (_ bv1124073472 32))))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (fp.leq ?x24 ?x22))))
(assert
 (let ((?x52 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
 (let ((?x33 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x53 (fp.sub roundNearestTiesToEven ?x33 ?x52)))
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!7)))
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (let ((?x50 (fp.sub roundNearestTiesToEven ?x24 (fp.sub roundNearestTiesToEven ?x47 ?x48))))
 (or (or (fp.isNaN ?x50) (fp.isNaN ?x53)) (fp.gt ?x50 ?x53))))))))))
(assert
 (let ((?x52 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!7)))
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (let ((?x50 (fp.sub roundNearestTiesToEven ?x24 (fp.sub roundNearestTiesToEven ?x47 ?x48))))
 (or (or (fp.isNaN ?x50) (fp.isNaN ?x52)) (fp.lt ?x50 ?x52))))))))
(assert
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x25 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!4)))
 (fp.geq ?x25 ?x47))))
(assert
 (let ((?x64 ((_ to_fp 8 24) (_ bv1098907648 32))))
 (let ((?x25 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!4)))
 (fp.leq ?x25 ?x64))))
(assert
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (let ((?x66 ((_ to_fp 8 24) symbolic_0_float_4_ackermann!5)))
 (let ((?x67 (fp.sub roundNearestTiesToEven ?x66 ?x24)))
 (not (fp.isNaN ?x67))))))
(assert
 (let ((?x25 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!4)))
 (let ((?x33 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x70 (fp.sub roundNearestTiesToEven ?x33 ?x25)))
 (not (fp.isNaN ?x70))))))
(assert
 (let ((?x25 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!4)))
 (let ((?x33 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x70 (fp.sub roundNearestTiesToEven ?x33 ?x25)))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (let ((?x66 ((_ to_fp 8 24) symbolic_0_float_4_ackermann!5)))
 (let ((?x67 (fp.sub roundNearestTiesToEven ?x66 ?x24)))
 (not (fp.gt ?x67 ?x70)))))))))
(assert
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x28 ((_ to_fp 8 24) symbolic_0_float_7_ackermann!6)))
 (fp.geq ?x28 ?x47))))
(assert
 (let ((?x64 ((_ to_fp 8 24) (_ bv1098907648 32))))
 (let ((?x28 ((_ to_fp 8 24) symbolic_0_float_7_ackermann!6)))
 (fp.leq ?x28 ?x64))))
(assert
 (let ((?x25 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!4)))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (let ((?x27 (fp.sub roundNearestTiesToEven ?x24 ?x25)))
 (let ((?x77 ((_ to_fp 8 24) symbolic_0_float_6_ackermann!2)))
 (let ((?x78 (fp.sub roundNearestTiesToEven ?x77 ?x27)))
 (not (fp.isNaN ?x78))))))))
(assert
 (let ((?x28 ((_ to_fp 8 24) symbolic_0_float_7_ackermann!6)))
 (let ((?x33 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x81 (fp.sub roundNearestTiesToEven ?x33 ?x28)))
 (not (fp.isNaN ?x81))))))
(assert
 (let ((?x28 ((_ to_fp 8 24) symbolic_0_float_7_ackermann!6)))
 (let ((?x33 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x81 (fp.sub roundNearestTiesToEven ?x33 ?x28)))
 (let ((?x25 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!4)))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (let ((?x27 (fp.sub roundNearestTiesToEven ?x24 ?x25)))
 (let ((?x77 ((_ to_fp 8 24) symbolic_0_float_6_ackermann!2)))
 (let ((?x78 (fp.sub roundNearestTiesToEven ?x77 ?x27)))
 (not (fp.gt ?x78 ?x81)))))))))))
(assert
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x34 ((_ to_fp 8 24) symbolic_0_float_9_ackermann!1)))
 (fp.geq ?x34 ?x47))))
(assert
 (let ((?x64 ((_ to_fp 8 24) (_ bv1098907648 32))))
 (let ((?x34 ((_ to_fp 8 24) symbolic_0_float_9_ackermann!1)))
 (fp.leq ?x34 ?x64))))
(assert
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!7)))
 (fp.geq ?x48 ?x47))))
(assert
 (let ((?x64 ((_ to_fp 8 24) (_ bv1098907648 32))))
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!7)))
 (fp.leq ?x48 ?x64))))
(assert
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!7)))
 (let ((?x33 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x90 (fp.sub roundNearestTiesToEven ?x33 ?x48)))
 (not (fp.isNaN ?x90))))))
(assert
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!7)))
 (let ((?x33 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x90 (fp.sub roundNearestTiesToEven ?x33 ?x48)))
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x93 ((_ to_fp 8 24) symbolic_0_float_ackermann!8)))
 (let ((?x94 (fp.sub roundNearestTiesToEven ?x93 ?x47)))
 (not (fp.gt ?x94 ?x90)))))))))
(assert
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x52 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
 (fp.geq ?x52 ?x47))))
(assert
 (let ((?x64 ((_ to_fp 8 24) (_ bv1098907648 32))))
 (let ((?x52 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
 (fp.leq ?x52 ?x64))))
(assert
 (let ((?x19 ((_ to_fp 8 24) (_ bv3271557120 32))))
 (let ((?x66 ((_ to_fp 8 24) symbolic_0_float_4_ackermann!5)))
 (fp.geq ?x66 ?x19))))
(assert
 (let ((?x22 ((_ to_fp 8 24) (_ bv1124073472 32))))
 (let ((?x66 ((_ to_fp 8 24) symbolic_0_float_4_ackermann!5)))
 (fp.leq ?x66 ?x22))))
(assert
 (let ((?x19 ((_ to_fp 8 24) (_ bv3271557120 32))))
 (let ((?x77 ((_ to_fp 8 24) symbolic_0_float_6_ackermann!2)))
 (fp.geq ?x77 ?x19))))
(assert
 (let ((?x22 ((_ to_fp 8 24) (_ bv1124073472 32))))
 (let ((?x77 ((_ to_fp 8 24) symbolic_0_float_6_ackermann!2)))
 (fp.leq ?x77 ?x22))))
(assert
 (let ((?x19 ((_ to_fp 8 24) (_ bv3271557120 32))))
 (let ((?x93 ((_ to_fp 8 24) symbolic_0_float_ackermann!8)))
 (fp.geq ?x93 ?x19))))
(assert
 (let ((?x22 ((_ to_fp 8 24) (_ bv1124073472 32))))
 (let ((?x93 ((_ to_fp 8 24) symbolic_0_float_ackermann!8)))
 (fp.leq ?x93 ?x22))))
(assert
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x93 ((_ to_fp 8 24) symbolic_0_float_ackermann!8)))
 (let ((?x94 (fp.sub roundNearestTiesToEven ?x93 ?x47)))
 (not (fp.isNaN ?x94))))))
(assert
 (let (($x113 (and (fp.geq ((_ to_fp 8 24) symbolic_0_float_8_ackermann!9) ((_ to_fp 8 24) (_ bv3271622656 32))) (fp.leq ((_ to_fp 8 24) symbolic_0_float_8_ackermann!9) ((_ to_fp 8 24) (_ bv1124139008 32))))))
(not $x113)))
(check-sat)
(exit)
