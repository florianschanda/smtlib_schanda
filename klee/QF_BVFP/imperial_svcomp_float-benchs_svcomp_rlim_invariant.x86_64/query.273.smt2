(set-info :smt-lib-version 2.6)
(set-logic QF_BVFP)
(set-info :source |
Generated by: Daniel Liew, Daniel Schemmel, Cristian Cadar, Alastair Donaldson, and Rafael Zähl
Generated on: 2017-4-28
Generator: KLEE
Application: Branch satisfiability check for symbolic execution of C programs
Target solver: Z3 or MathSAT5
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_rlim_invariant.x86_64/query.273.smt2
|)
(set-info :license "https://creativecommons.org/licenses/by/4.0/")
(set-info :category "industrial")
(set-info :status unsat)
(declare-fun symbolic_0_float_1_ackermann!6 () (_ BitVec 32))
(declare-fun symbolic_0_float_ackermann!7 () (_ BitVec 32))
(declare-fun symbolic_0_float_3_ackermann!0 () (_ BitVec 32))
(declare-fun symbolic_0_float_2_ackermann!2 () (_ BitVec 32))
(declare-fun symbolic_0_float_5_ackermann!3 () (_ BitVec 32))
(declare-fun symbolic_0_float_4_ackermann!4 () (_ BitVec 32))
(declare-fun symbolic_0_float_7_ackermann!5 () (_ BitVec 32))
(declare-fun symbolic_0_float_6_ackermann!1 () (_ BitVec 32))
(assert
 (let ((?x17 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x15 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!6)))
 (fp.geq ?x15 ?x17))))
(assert
 (let ((?x20 ((_ to_fp 8 24) (_ bv1098907648 32))))
 (let ((?x15 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!6)))
 (fp.leq ?x15 ?x20))))
(assert
 (let ((?x15 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!6)))
 (let ((?x23 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x25 (fp.sub roundNearestTiesToEven ?x23 ?x15)))
 (not (fp.isNaN ?x25))))))
(assert
 (let ((?x15 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!6)))
 (let ((?x23 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x25 (fp.sub roundNearestTiesToEven ?x23 ?x15)))
 (let ((?x17 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x28 ((_ to_fp 8 24) symbolic_0_float_ackermann!7)))
 (let ((?x29 (fp.sub roundNearestTiesToEven ?x28 ?x17)))
 (not (fp.gt ?x29 ?x25)))))))))
(assert
 (let ((?x17 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x32 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
 (fp.geq ?x32 ?x17))))
(assert
 (let ((?x20 ((_ to_fp 8 24) (_ bv1098907648 32))))
 (let ((?x32 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
 (fp.leq ?x32 ?x20))))
(assert
 (let ((?x32 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
 (let ((?x23 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x35 (fp.sub roundNearestTiesToEven ?x23 ?x32)))
 (let ((?x15 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!6)))
 (let ((?x17 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x38 (fp.sub roundNearestTiesToEven ?x17 ?x15)))
 (let ((?x37 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!2)))
 (let ((?x39 (fp.sub roundNearestTiesToEven ?x37 ?x38)))
 (or (fp.isNaN ?x35) (fp.gt ?x39 ?x35)))))))))))
(assert
 (let ((?x15 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!6)))
 (let ((?x17 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x38 (fp.sub roundNearestTiesToEven ?x17 ?x15)))
 (let ((?x37 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!2)))
 (let ((?x39 (fp.sub roundNearestTiesToEven ?x37 ?x38)))
 (not (fp.isNaN ?x39))))))))
(assert
 (not (fp.isNaN ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0))))
(assert
 (let ((?x32 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
 (let ((?x15 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!6)))
 (let ((?x17 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x38 (fp.sub roundNearestTiesToEven ?x17 ?x15)))
 (let ((?x37 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!2)))
 (let ((?x39 (fp.sub roundNearestTiesToEven ?x37 ?x38)))
 (not (fp.lt ?x39 ?x32)))))))))
(assert
 (let ((?x17 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!3)))
 (fp.geq ?x48 ?x17))))
(assert
 (let ((?x20 ((_ to_fp 8 24) (_ bv1098907648 32))))
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!3)))
 (fp.leq ?x48 ?x20))))
(assert
 (let ((?x32 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
 (let ((?x15 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!6)))
 (let ((?x17 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x38 (fp.sub roundNearestTiesToEven ?x17 ?x15)))
 (let ((?x52 (fp.add roundNearestTiesToEven ?x38 ?x32)))
 (let ((?x51 ((_ to_fp 8 24) symbolic_0_float_4_ackermann!4)))
 (let ((?x53 (fp.sub roundNearestTiesToEven ?x51 ?x52)))
 (not (fp.isNaN ?x53))))))))))
(assert
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!3)))
 (let ((?x23 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x56 (fp.sub roundNearestTiesToEven ?x23 ?x48)))
 (not (fp.isNaN ?x56))))))
(assert
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!3)))
 (let ((?x23 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x56 (fp.sub roundNearestTiesToEven ?x23 ?x48)))
 (let ((?x32 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
 (let ((?x15 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!6)))
 (let ((?x17 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x38 (fp.sub roundNearestTiesToEven ?x17 ?x15)))
 (let ((?x52 (fp.add roundNearestTiesToEven ?x38 ?x32)))
 (let ((?x51 ((_ to_fp 8 24) symbolic_0_float_4_ackermann!4)))
 (let ((?x53 (fp.sub roundNearestTiesToEven ?x51 ?x52)))
 (not (fp.gt ?x53 ?x56)))))))))))))
(assert
 (let ((?x17 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x61 ((_ to_fp 8 24) symbolic_0_float_7_ackermann!5)))
 (fp.geq ?x61 ?x17))))
(assert
 (let ((?x20 ((_ to_fp 8 24) (_ bv1098907648 32))))
 (let ((?x61 ((_ to_fp 8 24) symbolic_0_float_7_ackermann!5)))
 (fp.leq ?x61 ?x20))))
(assert
 (let ((?x61 ((_ to_fp 8 24) symbolic_0_float_7_ackermann!5)))
 (let ((?x23 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x64 (fp.sub roundNearestTiesToEven ?x23 ?x61)))
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!3)))
 (let ((?x32 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
 (let ((?x15 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!6)))
 (let ((?x17 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x38 (fp.sub roundNearestTiesToEven ?x17 ?x15)))
 (let ((?x52 (fp.add roundNearestTiesToEven ?x38 ?x32)))
 (let ((?x67 (fp.sub roundNearestTiesToEven ?x52 ?x48)))
 (let ((?x66 ((_ to_fp 8 24) symbolic_0_float_6_ackermann!1)))
 (let ((?x68 (fp.sub roundNearestTiesToEven ?x66 ?x67)))
 (or (fp.isNaN ?x64) (fp.gt ?x68 ?x64)))))))))))))))
(assert
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!3)))
 (let ((?x32 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
 (let ((?x15 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!6)))
 (let ((?x17 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x38 (fp.sub roundNearestTiesToEven ?x17 ?x15)))
 (let ((?x52 (fp.add roundNearestTiesToEven ?x38 ?x32)))
 (let ((?x67 (fp.sub roundNearestTiesToEven ?x52 ?x48)))
 (let ((?x66 ((_ to_fp 8 24) symbolic_0_float_6_ackermann!1)))
 (let ((?x68 (fp.sub roundNearestTiesToEven ?x66 ?x67)))
 (not (fp.isNaN ?x68))))))))))))
(assert
 (not (fp.isNaN ((_ to_fp 8 24) symbolic_0_float_7_ackermann!5))))
(assert
 (let ((?x61 ((_ to_fp 8 24) symbolic_0_float_7_ackermann!5)))
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!3)))
 (let ((?x32 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
 (let ((?x15 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!6)))
 (let ((?x17 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x38 (fp.sub roundNearestTiesToEven ?x17 ?x15)))
 (let ((?x52 (fp.add roundNearestTiesToEven ?x38 ?x32)))
 (let ((?x67 (fp.sub roundNearestTiesToEven ?x52 ?x48)))
 (let ((?x66 ((_ to_fp 8 24) symbolic_0_float_6_ackermann!1)))
 (let ((?x68 (fp.sub roundNearestTiesToEven ?x66 ?x67)))
 (not (fp.lt ?x68 ?x61)))))))))))))
(assert
 (let ((?x78 ((_ to_fp 8 24) (_ bv3271557120 32))))
 (let ((?x28 ((_ to_fp 8 24) symbolic_0_float_ackermann!7)))
 (fp.geq ?x28 ?x78))))
(assert
 (let ((?x81 ((_ to_fp 8 24) (_ bv1124073472 32))))
 (let ((?x28 ((_ to_fp 8 24) symbolic_0_float_ackermann!7)))
 (fp.leq ?x28 ?x81))))
(assert
 (let ((?x17 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x28 ((_ to_fp 8 24) symbolic_0_float_ackermann!7)))
 (let ((?x29 (fp.sub roundNearestTiesToEven ?x28 ?x17)))
 (not (fp.isNaN ?x29))))))
(assert
 (let ((?x78 ((_ to_fp 8 24) (_ bv3271557120 32))))
 (let ((?x37 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!2)))
 (fp.geq ?x37 ?x78))))
(assert
 (let ((?x81 ((_ to_fp 8 24) (_ bv1124073472 32))))
 (let ((?x37 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!2)))
 (fp.leq ?x37 ?x81))))
(assert
 (let ((?x78 ((_ to_fp 8 24) (_ bv3271557120 32))))
 (let ((?x51 ((_ to_fp 8 24) symbolic_0_float_4_ackermann!4)))
 (fp.geq ?x51 ?x78))))
(assert
 (let ((?x81 ((_ to_fp 8 24) (_ bv1124073472 32))))
 (let ((?x51 ((_ to_fp 8 24) symbolic_0_float_4_ackermann!4)))
 (fp.leq ?x51 ?x81))))
(assert
 (let ((?x78 ((_ to_fp 8 24) (_ bv3271557120 32))))
 (let ((?x66 ((_ to_fp 8 24) symbolic_0_float_6_ackermann!1)))
 (fp.geq ?x66 ?x78))))
(assert
 (let ((?x81 ((_ to_fp 8 24) (_ bv1124073472 32))))
 (let ((?x66 ((_ to_fp 8 24) symbolic_0_float_6_ackermann!1)))
 (fp.leq ?x66 ?x81))))
(assert
 (let ((?x61 ((_ to_fp 8 24) symbolic_0_float_7_ackermann!5)))
(let ((?x48 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!3)))
(let ((?x32 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
(let ((?x15 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!6)))
(let ((?x17 ((_ to_fp 8 24) (_ bv0 32))))
(let ((?x38 (fp.sub roundNearestTiesToEven ?x17 ?x15)))
(let ((?x52 (fp.add roundNearestTiesToEven ?x38 ?x32)))
(let ((?x67 (fp.sub roundNearestTiesToEven ?x52 ?x48)))
(let ((?x91 (fp.add roundNearestTiesToEven ?x67 ?x61)))
(not (and (fp.geq ?x91 ((_ to_fp 8 24) (_ bv3271622656 32))) (fp.leq ?x91 ((_ to_fp 8 24) (_ bv1124139008 32)))))))))))))))
(check-sat)
(exit)
