...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_rlim_invariant.x86_64/query.174.smt2
|)
...
(let ((?x69 ((_ to_fp 8 24) symbolic_0_float_6_ackermann!1)))
(let ((?x76 (fp.sub roundNearestTiesToEven ?x69 (fp.add roundNearestTiesToEven ?x59 ?x53))))
(let (($x95 (and (and (not (fp.isNaN ?x76)) (not (fp.isNaN ?x72))) (not (fp.lt ?x76 ?x72)))))
(not $x95))))))))))
(check-sat)
(exit)
