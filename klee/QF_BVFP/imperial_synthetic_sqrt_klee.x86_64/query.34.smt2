...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee.x86_64/query.34.smt2
|)
...
(assert
 ...
 (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x96 ?x8)) ?x28))))))))))))))))))))))))))))))))))
(assert
 (let ((?x22 ((_ to_fp 8 24) (_ bv1056964608 32))))
...
(let ((?x95 (fp.mul roundNearestTiesToEven ?x94 ?x22)))
(let ((?x96 (fp.mul roundNearestTiesToEven ?x95 ?x95)))
(not (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x8 ?x96)) ?x28)))))))))))))))))))))))))))))))))))
(check-sat)
(exit)
