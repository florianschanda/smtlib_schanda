...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_count_klee.x86_64/query.05.smt2
|)
...
(let ((?x15 (concat (select start0 (_ bv2 32)) (concat (select start0 (_ bv1 32)) (select start0 (_ bv0 32))))))
(let ((?x18 (concat (select start0 (_ bv3 32)) ?x15)))
(not (not (fp.eq ((_ to_fp 8 24) (bvadd (_ bv2 32) ?x18)) ?x30))))))))
(check-sat)
(exit)
