...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee_bug.x86_64/query.30.smt2
|)
...
(let ((?x106 (fp.sub roundNearestTiesToEven (fp.mul roundNearestTiesToEven ?x104 ?x32) ?x19)))
(not (not (fp.lt (fp.abs ?x106) ?x37))))))))))))))))))))))))))))))))
(check-sat)
(exit)
