...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee_bug.x86_64/query.20.smt2
|)
...
(let ((?x78 (fp.sub roundNearestTiesToEven (fp.mul roundNearestTiesToEven ?x76 ?x32) ?x19)))
(not (not (fp.lt (fp.abs ?x78) ?x37)))))))))))))))))))))))
(check-sat)
(exit)
