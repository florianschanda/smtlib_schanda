...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_prefix_sum_klee_bug_float.x86_64/query.16.smt2
|)
...
(let ((?x33 (concat (select data_in0 (_ bv10 32)) (concat (select data_in0 (_ bv9 32)) (select data_in0 (_ bv8 32))))))
(let ((?x37 ((_ to_fp 8 24) (concat (select data_in0 (_ bv11 32)) ?x33))))
(let ((?x80 (fp.add roundNearestTiesToEven ?x53 (fp.add roundNearestTiesToEven ?x37 ?x74))))
(not (and (not (fp.isNaN ?x80)) (not (fp.lt ?x80 ?x53)))))))))))))))
(check-sat)
(exit)
