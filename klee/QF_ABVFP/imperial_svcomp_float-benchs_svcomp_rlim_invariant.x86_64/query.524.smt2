...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_rlim_invariant.x86_64/query.524.smt2
|)
...
(let ((?x133 ((_ to_fp 8 24) ?x132)))
(let ((?x148 (fp.sub roundNearestTiesToEven ?x133 (fp.add roundNearestTiesToEven ?x115 ?x101))))
(let (($x191 (and (and (not (fp.isNaN ?x148)) (not (fp.isNaN ?x144))) (not (fp.lt ?x148 ?x144)))))
(not $x191))))))))))))))))))))
(check-sat)
(exit)
