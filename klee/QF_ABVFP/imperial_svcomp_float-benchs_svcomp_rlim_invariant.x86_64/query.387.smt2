...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_rlim_invariant.x86_64/query.387.smt2
|)
...
(assert
 ...
 (let ((?x169 (fp.sub roundNearestTiesToEven ?x168 ?x88)))
...
(let ((?x76 ((_ to_fp 8 24) ?x75)))
(let ((?x93 (fp.sub roundNearestTiesToEven ?x76 ?x19)))
(let (($x190 (and (and (not (fp.isNaN ?x93)) (not (fp.isNaN ?x87))) (not (fp.lt ?x93 ?x87)))))
(not $x190)))))))))))))
(check-sat)
(exit)
