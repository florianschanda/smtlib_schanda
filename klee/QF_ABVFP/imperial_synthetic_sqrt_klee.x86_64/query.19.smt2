...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee.x86_64/query.19.smt2
|)
...
(assert
 ...
 (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x64 ?x19)) ?x38)))))))))))))))))))
(assert
 (let ((?x32 ((_ to_fp 8 24) (_ bv1056964608 32))))
 (let ((?x22 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x15 (concat (select x0 (_ bv2 32)) (concat (select x0 (_ bv1 32)) (select x0 (_ bv0 32))))))
...
(let ((?x63 (fp.mul roundNearestTiesToEven (fp.add roundNearestTiesToEven ?x60 (ite $x55 ?x47 ?x49)) ?x32)))
(let ((?x64 (fp.mul roundNearestTiesToEven ?x63 ?x63)))
(not (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x19 ?x64)) ?x38))))))))))))))))))))
(check-sat)
(exit)
