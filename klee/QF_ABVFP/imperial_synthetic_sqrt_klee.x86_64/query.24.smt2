...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee.x86_64/query.24.smt2
|)
...
(assert
 ...
 (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x78 ?x19)) ?x38)))))))))))))))))))))))))
(assert
 (let ((?x32 ((_ to_fp 8 24) (_ bv1056964608 32))))
 (let ((?x22 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x15 (concat (select x0 (_ bv2 32)) (concat (select x0 (_ bv1 32)) (select x0 (_ bv0 32))))))
...
(let ((?x77 (fp.mul roundNearestTiesToEven ?x76 ?x32)))
(let ((?x78 (fp.mul roundNearestTiesToEven ?x77 ?x77)))
(not (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x19 ?x78)) ?x38))))))))))))))))))))))))))
(check-sat)
(exit)
