...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee.x86_64/query.18.smt2
|)
...
(let ((?x61 (ite $x55 ?x47 ?x49)))
(let ((?x70 (ite $x69 ?x61 ?x63)))
(let (($x80 (and (not (fp.isNaN ?x70)) (not (fp.isNaN ((_ to_fp 8 24) (ite $x69 fresh_to_ieee_bv_!2 ?x59)))))))
(let (($x83 (and $x80 (not (fp.gt ?x70 ((_ to_fp 8 24) (ite $x69 fresh_to_ieee_bv_!2 ?x59)))))))
(not $x83)))))))))))))))))))))))
(check-sat)
(exit)
