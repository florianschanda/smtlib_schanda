...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee.x86_64/query.09.smt2
|)
...
(assert
 ...
 (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x34 ?x19)) ((_ to_fp 8 24) (_ bv869711765 32))))))))
(assert
 (let ((?x15 (concat (select x0 (_ bv2 32)) (concat (select x0 (_ bv1 32)) (select x0 (_ bv0 32))))))
(let ((?x19 ((_ to_fp 8 24) (concat (select x0 (_ bv3 32)) ?x15))))
(let ((?x33 (fp.mul roundNearestTiesToEven (fp.add roundNearestTiesToEven ?x19 ((_ to_fp 8 24) (_ bv0 32))) ((_ to_fp 8 24) (_ bv1056964608 32)))))
(let ((?x34 (fp.mul roundNearestTiesToEven ?x33 ?x33)))
(let (($x42 (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x19 ?x34)) ((_ to_fp 8 24) (_ bv869711765 32)))))
(not $x42)))))))
(check-sat)
(exit)
