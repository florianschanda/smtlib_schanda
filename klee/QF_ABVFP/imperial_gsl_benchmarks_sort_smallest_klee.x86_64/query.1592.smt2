...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_gsl_benchmarks_sort_smallest_klee.x86_64/query.1592.smt2
|)
...
(let ((?x24 (concat (select x0 (_ bv21 32)) (concat (select x0 (_ bv20 32)) (concat (select x0 (_ bv19 32)) ?x15)))))
(let ((?x31 ((_ to_fp 11 53) (concat (select x0 (_ bv23 32)) (concat (select x0 (_ bv22 32)) ?x24)))))
(let (($x181 (and (not (fp.isNaN ?x164)) (or (or (fp.isNaN ?x31) (fp.isNaN ?x164)) (fp.gt ?x31 ?x164)))))
(not (not $x181))))))))))
(check-sat)
(exit)
