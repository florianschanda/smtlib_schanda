...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/aachen_real_libmatheval_sqrt.x86_64/query.5.smt2
|)
...
(let ((?x24 (concat (select x0 (_ bv5 32)) (concat (select x0 (_ bv4 32)) (concat (select x0 (_ bv3 32)) ?x15)))))
(let ((?x31 ((_ to_fp 11 53) (concat (select x0 (_ bv7 32)) (concat (select x0 (_ bv6 32)) ?x24)))))
(let ((?x68 (fp.sqrt roundNearestTiesToEven (fp.sub roundNearestTiesToEven ?x31 ?x51))))
(not (fp.isNaN ?x68))))))))))
(check-sat)
(exit)
