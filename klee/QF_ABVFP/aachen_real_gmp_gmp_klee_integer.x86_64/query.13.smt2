...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/aachen_real_gmp_gmp_klee_integer.x86_64/query.13.smt2
|)
...
(assert
 ...
 (not (= (_ bv0 64) (ite (bvult ?x54 (_ bv64 64)) ?x431 (_ bv0 64)))))))))))))))))))))))))))))))))))))))))))
(assert
 (let ((?x15 (concat (select d0 (_ bv2 32)) (concat (select d0 (_ bv1 32)) (select d0 (_ bv0 32))))))
(let ((?x24 (concat (select d0 (_ bv5 32)) (concat (select d0 (_ bv4 32)) (concat (select d0 (_ bv3 32)) ?x15)))))
(let ((?x30 (concat (select d0 (_ bv7 32)) (concat (select d0 (_ bv6 32)) ?x24))))
(let ((?x31 ((_ to_fp 11 53) ?x30)))
(not (not (fp.eq ?x31 ((_ to_fp 11 53) (_ bv4611686018427387904 64))))))))))
(check-sat)
(exit)
