...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/aachen_real_gmp_gmp_klee_inv_arg.x86_64/query.07.smt2
|)
...
(assert
 ...
 (fp.isNaN ((_ to_fp 11 53) ?x30))))))
(assert
 (let ((?x15 (concat (select d0 (_ bv2 32)) (concat (select d0 (_ bv1 32)) (select d0 (_ bv0 32))))))
(let ((?x24 (concat (select d0 (_ bv5 32)) (concat (select d0 (_ bv4 32)) (concat (select d0 (_ bv3 32)) ?x15)))))
(let ((?x30 (concat (select d0 (_ bv7 32)) (concat (select d0 (_ bv6 32)) ?x24))))
(not (= (_ bv9218868437227405312 64) (bvand ?x30 (_ bv9218868437227405312 64))))))))
(check-sat)
(exit)
