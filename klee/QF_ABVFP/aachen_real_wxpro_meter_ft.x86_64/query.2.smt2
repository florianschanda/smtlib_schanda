...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/aachen_real_wxpro_meter_ft.x86_64/query.2.smt2
|)
...
(let ((?x24 (concat (select meters0 (_ bv5 32)) (concat (select meters0 (_ bv4 32)) (concat (select meters0 (_ bv3 32)) ?x15)))))
(let ((?x31 ((_ to_fp 11 53) (concat (select meters0 (_ bv7 32)) (concat (select meters0 (_ bv6 32)) ?x24)))))
(let ((?x46 (fp.mul roundNearestTiesToEven ?x31 ((_ to_fp 11 53) (_ bv4614613358185178726 64)))))
(let ((?x41 (fp.mul roundNearestTiesToEven ?x31 ((_ to_fp 11 53) (_ bv4614570213700748517 64)))))
(let (($x52 (and (and (not (fp.isNaN ?x41)) (not (fp.isNaN ?x46))) (not (fp.gt ?x41 ?x46)))))
(not $x52))))))))
(check-sat)
(exit)
