...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_prefix_sum_klee_bug_double.x86_64/query.12.smt2
|)
...
(assert
 ...
 (let ((?x63 (fp.add roundNearestTiesToEven ?x61 ?x31)))
 (or (fp.isNaN ?x63) (fp.lt ?x63 ?x31))))))))))))
(assert
 (let ((?x45 (concat (select data_in0 (_ bv2 32)) (concat (select data_in0 (_ bv1 32)) (select data_in0 (_ bv0 32))))))
 (let ((?x51 (concat (select data_in0 (_ bv4 32)) (concat (select data_in0 (_ bv3 32)) ?x45))))
 (let ((?x57 (concat (select data_in0 (_ bv6 32)) (concat (select data_in0 (_ bv5 32)) ?x51))))
