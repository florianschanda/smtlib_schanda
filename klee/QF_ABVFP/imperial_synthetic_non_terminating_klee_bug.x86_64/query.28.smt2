...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_non_terminating_klee_bug.x86_64/query.28.smt2
|)
...
(let ((?x76 (fp.sub roundNearestTiesToEven ?x74 ?x19)))
(let ((?x78 (fp.sub roundNearestTiesToEven ?x76 ?x19)))
(not (fp.gt (fp.sub roundNearestTiesToEven ?x78 ?x19) ?x30)))))))))))))))))))))))))))))))
(check-sat)
(exit)
