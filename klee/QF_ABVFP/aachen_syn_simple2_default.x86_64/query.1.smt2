...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/aachen_syn_simple2_default.x86_64/query.1.smt2
|)
...
(assert
 (let ((?x15 (concat (select d0 (_ bv2 32)) (concat (select d0 (_ bv1 32)) (select d0 (_ bv0 32))))))
(let ((?x24 (concat (select d0 (_ bv5 32)) (concat (select d0 (_ bv4 32)) (concat (select d0 (_ bv3 32)) ?x15)))))
(let (($x36 (fp.eq ((_ to_fp 11 53) (concat (select d0 (_ bv7 32)) (concat (select d0 (_ bv6 32)) ?x24))) ((_ to_fp 11 53) (_ bv4602678819172646912 64)))))
(not (not $x36))))))
(check-sat)
(exit)
