...
;; H
  ...
  (=>
  (not
  (= s (let ((temp___316 (bvadd current_time #x01)))
       (ite (bvule (bvsub #x32 current_time) #x01) (bvsub temp___316 #x32)
       temp___316))))
  (fp.lt (to_rep (select battery_level_at s)) battery_threshold))))
  (bvuge counter #x31)))
...
;; H
  (assert
  (= result3 (ite (fp.lt (to_rep (select battery_level_at1 current_time1))
             battery_threshold) o5 false)))
...
;; H
  (assert (= temp___318 (bvsub current_time1 o6)))
...
;; H
  (assert (=> (= result5 true) (= o8 (bvadd temp___318 #x32))))

;; H
  (assert (=> (not (= result5 true)) (= o8 temp___318)))
...
;; H
  (assert (= result4 (ite (fp.leq battery_threshold o10) true false)))
