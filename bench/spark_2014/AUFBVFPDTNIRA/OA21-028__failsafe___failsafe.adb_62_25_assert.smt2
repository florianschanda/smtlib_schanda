...
;; H
  (assert (bvuge current_time1 (bvsub counter1 #x01)))

(declare-const s (_ BitVec 8))

;; H
  (assert
  (bvule (let ((temp___324 (bvsub current_time1 (bvsub counter1 #x01))))
         (ite (bvult current_time1 (bvsub counter1 #x01))
         (bvadd temp___324 #x32) temp___324)) s))

;; H
  (assert (bvule s current_time1))

(assert
;; WP_parameter_def
 ;; File "failsafe.adb", line 31, characters 0-0
  (not (fp.lt (to_rep (select battery_level_at1 s)) battery_threshold)))
(check-sat)
(exit)
