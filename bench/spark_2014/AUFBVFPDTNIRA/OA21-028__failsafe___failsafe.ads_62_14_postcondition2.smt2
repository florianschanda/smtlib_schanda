...
(define-fun dynamic_invariant3 ((temp___expr_236 Float32)
...
;; H
  (assert (= failsafe__is_raised__result failsafe__is_raised__result1))

;; H
  (assert
  (= failsafe__is_raised__result (ite (bvuge counter #x32) true false)))

;; H
  (assert
  (= (mk_bool__ref failsafe__is_raised__result2) (mk_bool__ref
                                                 failsafe__is_raised__result1)))

(assert
;; WP_parameter_def
 ;; File "failsafe.ads", line 60, characters 0-0
  (not (= (is_valid battery_level_at current_time counter) true)))
(check-sat)
(exit)
