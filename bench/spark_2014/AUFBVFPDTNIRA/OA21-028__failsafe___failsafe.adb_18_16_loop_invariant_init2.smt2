...
;; H
  (assert
  (bvule (let ((temp___301 (bvsub current_time (bvsub res2 #x01))))
         (ite (bvult current_time (bvsub res2 #x01)) (bvadd temp___301 #x32)
         temp___301)) s2))
...
;; WP_parameter_def
 ;; File "failsafe.ads", line 16, characters 0-0
  (not (fp.lt (to_rep (select battery_level_at s2)) battery_threshold)))
(check-sat)
(exit)
