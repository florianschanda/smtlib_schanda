...
;; H
  (assert
  (=> (fp.lt battery_level battery_threshold)
...
;; H
  (assert (=> (fp.lt battery_level battery_threshold) (= counter1 o3)))
...
;; H
  ...
  (=>
  (and
  (bvule (let ((temp___325 (bvsub current_time1 (bvsub counter1 #x01))))
         (ite (bvult current_time1 (bvsub counter1 #x01))
         (bvadd temp___325 #x32) temp___325)) s)
  (bvule s #x31))
  (fp.lt (to_rep (select battery_level_at1 s)) battery_threshold)))))))))
...
;; H
  (assert
  (=> (not (fp.lt battery_level battery_threshold)) (= result3 counter)))

;; H
  (assert
  (=> (not (fp.lt battery_level battery_threshold)) (= counter2 #x00)))

;; H
  (assert
  (=> (not (fp.lt battery_level battery_threshold)) (= counter1 counter2)))
...
;; WP_parameter_def
 ;; File "failsafe.adb", line 31, characters 0-0
  (not
  (= current_time2 (let ((temp___331 (bvadd current_time #x01)))
                   (ite (bvule (bvsub #x32 current_time) #x01)
                   (bvsub temp___331 #x32) temp___331)))))
(check-sat)
(exit)
