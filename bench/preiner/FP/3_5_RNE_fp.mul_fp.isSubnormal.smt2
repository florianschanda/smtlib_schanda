...
(define-fun IC ((s FP)) Bool (and (or (fp.isNormal s) (fp.isSubnormal s)) (or (not (fp.isNormal (fp.mul R s posMinSubnormal))) (not (fp.isNormal (fp.mul R s negMinSubnormal))))))

(assert
 (distinct
  (IC s)
  (exists ((x FP)) (fp.isSubnormal (fp.mul RNE x s)))
 )
)
