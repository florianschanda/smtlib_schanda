...
(declare-const digitval2 t__ref)
...
(assert (<= r83b i1))
(assert (<= i1 r84b))
(assert (and (<= 1 begidx11) (bvule tmpval2 #x7FFF)))
(assert (and (=> (<= r83b r84b) (dynamic_property r83b r84b i2)) (and (<= r83b i2) (<= i2 r84b))))
(assert (= result9 (mk_t__ref digitval)))
(assert (= digitval1 (bvsub ((_ int2bv 16) (to_rep (select zonefileline i2))) #x0030)))
(assert (= result10 (mk_t__ref tmpval2)))
(assert (= tmpval3 (bvadd (bvmul #x000A tmpval2) digitval1)))
(assert (not (bvugt tmpval3 #x7FFF)))
(assert (not (= i2 r84b)))
(assert (= result11 (mk_int__ref i2)))
(assert (= i3 (+ i2 1)))
(assert (= i4 (mk_int__ref i3)))
(assert (= tmpval4 (mk_t__ref tmpval3)))
(assert (= digitval2 (mk_t__ref digitval1)))
(assert (= i5 i3))
(assert (= tmpval5 tmpval3))
(assert (= digitval3 digitval1))
(assert (not (bvule (t__content tmpval4) #x7FFF)))
(check-sat)
(exit)
