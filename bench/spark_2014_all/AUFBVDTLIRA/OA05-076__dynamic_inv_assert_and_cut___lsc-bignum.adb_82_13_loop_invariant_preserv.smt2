...
(assert (forall ((k Int)) (=> (and (<= (to_rep b__first) k) (<= k (to_rep b__last))) (and (=> (and (<= b_first k) (<= k (- (- (+ b_first (+ i2 1)) a_first) 1))) (= (to_rep1 (select b2 k)) (to_rep1 (select (elts a) (- (+ a_first k) b_first))))) (=> (=> (<= b_first k) (not (<= k (- (- (+ b_first (+ i2 1)) a_first) 1)))) (= (to_rep1 (select b2 k)) (to_rep1 (select b k))))))))
...
(assert (=> (<= b_first k) (not (<= k (- (- (+ b_first i3) a_first) 1)))))
