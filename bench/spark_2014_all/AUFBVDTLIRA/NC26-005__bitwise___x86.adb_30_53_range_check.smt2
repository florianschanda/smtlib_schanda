...
(assert (forall ((addr1 (_ BitVec 64))) (forall ((x86__memory (Array (_ BitVec 64) unsigned8))) (! (=> (dynamic_invariant addr1 true true true true) (let ((result (readmem8 addr1 x86__memory))) (and (= result (to_rep (select x86__memory addr1))) (dynamic_invariant2 result true false true true)))) :pattern ((readmem8 addr1 x86__memory))))))
...
(assert (and (= (readmem8 addr memory1) ((_ extract 7 0) (bvand val__ #x00FF))) (forall ((x (_ BitVec 64))) (=> (and (bvule #x0000000000000000 x) (bvule x #xFFFFFFFFFFFFFFFF)) (or (= x addr) (= (to_rep (select memory1 x)) (to_rep (select memory x))))))))
