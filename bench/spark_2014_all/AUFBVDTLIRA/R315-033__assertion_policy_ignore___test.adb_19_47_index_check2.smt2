...
(assert (and (= temp___202 (+ src_first i1)) (in_range1 (+ src_first i1))))
(assert (and (<= (to_rep (first (rt src))) temp___202) (<= temp___202 (to_rep (last (rt src))))))
(assert (= o temp___202))
(assert (= o1 (select (elts src) o)))
(assert (= o2 (to_rep1 o1)))
(assert (= (to_rep1 o3) o2))
(assert (and (= temp___203 (+ index i1)) (in_range1 (+ index i1))))
(assert (and (<= (to_rep dest__first) temp___203) (<= temp___203 (to_rep dest__last))))
(assert (= o4 temp___203))
(assert (= o5 (store dest o4 o3)))
(assert (= (mk_map__ref result1) (mk_map__ref dest)))
(assert (= dest1 o5))
(assert (forall ((j1 Int)) (=> (and (<= (to_rep dest__first) j1) (<= j1 (to_rep dest__last))) (ite (and (<= index j1) (<= j1 (+ index i2))) (= (to_rep1 (select dest2 j1)) (to_rep1 (select (elts src) (- (+ src_first j1) index)))) (= (to_rep1 (select dest2 j1)) (to_rep1 (select dest j1)))))))
(assert (and (=> (<= 0 (- src_length 1)) (dynamic_property1 0 (- src_length 1) i2)) (and (<= 0 i2) (<= i2 (- src_length 1)))))
(assert (<= (to_rep dest__first) j))
(assert (<= j (to_rep dest__last)))
(assert (not (and (<= index j) (<= j (+ index i2)))))
(assert (not (<= (to_rep dest__first) j)))
(check-sat)
(exit)
