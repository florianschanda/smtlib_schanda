...
(assert (forall ((m us_t)) (forall ((x Int)) (! (=> (and (dynamic_invariant1 m true true true true) (dynamic_invariant x true true true true)) (dynamic_invariant1 (func_push m x) true false true true)) :pattern ((func_push m x))))))
(assert (forall ((m us_t)) (forall ((x Int)) (! (=> (and (dynamic_invariant1 m true true true true) (dynamic_invariant x true true true true)) (= (func_push m x) (ite (= (length m) 0) (of_array (singleton1 (of_rep x) #x0000) #x0000 #x0000) (of_array (concat_singleton_right (to_array m) (first1 m) (last1 m) (of_rep x)) (first1 m) ((_ int2bv 16) (- (+ (bv2nat (first1 m)) (+ (ite (<= (bv2nat (first1 m)) (bv2nat (last1 m))) (+ (- (bv2nat (last1 m)) (bv2nat (first1 m))) 1) 0) 1)) 1)))))) :pattern ((func_push m x))))))
