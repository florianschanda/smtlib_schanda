...
(assert (fp.lt (to_rep (select battery_level_at1 current_time1)) battery_threshold))
(assert (not (forall ((s (_ BitVec 8))) (=> (and (bvule #x00 s) (bvule s #x31)) (fp.lt (to_rep (select battery_level_at1 s)) battery_threshold)))))
(assert (not (fp.leq battery_threshold (to_rep (select battery_level_at1 (let ((temp___323 (bvsub current_time1 counter1))) (ite (bvult current_time1 counter1) (bvadd temp___323 #x32) temp___323)))))))
(check-sat)
(exit)
