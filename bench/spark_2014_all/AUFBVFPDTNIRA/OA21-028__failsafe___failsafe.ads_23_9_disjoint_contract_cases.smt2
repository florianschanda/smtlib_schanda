...
(assert (not (<= (+ (+ 0 (ite (= (fp.geq (to_rep (select battery_level_at current_time)) battery_threshold) true) 1 0)) (ite (forall ((s (_ BitVec 8))) (=> (and (bvule #x00 s) (bvule s #x31)) (fp.lt (to_rep (select battery_level_at s)) battery_threshold))) 1 0)) 1)))
(check-sat)
(exit)
