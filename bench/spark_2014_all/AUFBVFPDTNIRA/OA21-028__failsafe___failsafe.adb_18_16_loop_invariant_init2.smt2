...
(assert (bvule #x00 s1))
(assert (bvule s1 #x31))
(assert (=> (bvult current_time s1) (= o (bvadd (bvsub current_time s1) #x32))))
...
(assert (bvuge current_time (bvsub res2 #x01)))
(declare-const s2 (_ BitVec 8))
(assert (bvule (let ((temp___301 (bvsub current_time (bvsub res2 #x01)))) (ite (bvult current_time (bvsub res2 #x01)) (bvadd temp___301 #x32) temp___301)) s2))
(assert (bvule s2 current_time))
(assert (not (fp.lt (to_rep (select battery_level_at s2)) battery_threshold)))
(check-sat)
(exit)
