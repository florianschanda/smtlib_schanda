...
(assert (= (= o true) (forall ((index Int)) (=> (and (<= (to_rep1 (first (rt source))) index) (<= index (to_rep1 (last (rt source))))) (= (is_digit (to_rep (select (elts source) index))) true)))))
(assert (= (length source) 10))
(assert (and (= temp___1384 (+ (to_rep1 (first (rt source))) 7)) (in_range1 (+ (to_rep1 (first (rt source))) 7))))
(assert (not (<= temp___1384 (to_rep1 (last (rt source))))))
(check-sat)
(exit)
