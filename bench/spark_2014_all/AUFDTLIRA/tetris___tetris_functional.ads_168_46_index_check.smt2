...
(assert true)
(assert (forall ((b (Array Int (Array Int cell)))) (forall ((y Int) (x Int)) (! (= (= (is_empty b y x) true) (and (and (in_range3 x) (in_range4 y)) (= (to_rep (let ((temp___673 (select b y))) (select temp___673 x))) 0))) :pattern ((is_empty b y x))))))
...
(assert (not (= (to_rep1 (rec__tetris_functional__piece__s (us_split_fields1 p))) 2)))
(assert (not (= (to_rep1 (rec__tetris_functional__piece__s (us_split_fields1 p))) 1)))
(assert (<= 0 y))
(assert (<= y 2))
(assert (<= 0 x))
(assert (<= x 2))
(assert (not (<= (to_rep1 (rec__tetris_functional__piece__s (us_split_fields1 p))) 7)))
(check-sat)
(exit)
