...
(assert (<= i_v21 k_2))
(assert (forall ((m1 Int)) (=> (and (<= (to_rep a_v2__first) m1) (<= m1 (- i_v22 1))) (= (to_rep (select a_v21 m1)) (to_rep (select (elts b_v2) m1))))))
...
(assert (<= m (- i_v22 1)))
(assert (and (<= (to_rep (first (rt b_v2))) m) (<= m (to_rep (last (rt b_v2))))))
(assert (= o m))
(assert (= o1 (select (elts b_v2) o)))
(assert (= o2 (to_rep o1)))
(assert (not (<= (to_rep a_v2__first) m)))
(check-sat)
(exit)
