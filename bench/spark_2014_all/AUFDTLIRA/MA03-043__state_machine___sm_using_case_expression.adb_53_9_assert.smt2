...
(assert (forall ((final_state Int)) (=> (and (<= 0 final_state) (<= final_state 3)) (exists ((initial_state Int)) (and (and (<= 0 initial_state) (<= initial_state 3)) (exists ((trigger2 Int)) (and (and (<= 0 trigger2) (<= trigger2 4)) (= final_state (my_sm initial_state trigger2)))))))))
(declare-const state2 Int)
(assert (<= 0 state2))
(assert (<= state2 3))
(declare-const trigger1 Int)
(assert (<= 0 trigger1))
(assert (<= trigger1 4))
(assert (not (or (not (= (my_sm state2 trigger1) 3)) (exists ((idx Int)) (and (and (<= 1 idx) (<= idx 20)) (and (= (to_rep (rec__sm_using_case_expression__invalid_transition_record_t__initial_state (us_split_fields1 (select invalid_transition_array idx)))) state2) (= (to_rep1 (rec__sm_using_case_expression__invalid_transition_record_t__trigger (us_split_fields1 (select invalid_transition_array idx)))) trigger1)))))))
(check-sat)
(exit)
