...
(assert (=> (not (not (= first_available 0))) (= result (distinct 1 0))))
(assert (= result true))
(assert (<= 1 r))
(assert (<= r 10000))
(assert (= (to_rep1 (rec__list_mod_allocator__cell__stat (us_split_fields1 (select data r)))) 0))
(assert (not (= (to_rep (rec__list_mod_allocator__cell__next (us_split_fields1 (select data r)))) 0)))
(assert (not (<= 1 (to_rep (rec__list_mod_allocator__cell__next (us_split_fields1 (select data r)))))))
(check-sat)
(exit)
