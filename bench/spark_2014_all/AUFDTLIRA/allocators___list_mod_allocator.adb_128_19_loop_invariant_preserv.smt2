...
(declare-const j Int)
(assert (<= 2 j))
(assert (<= j (length (sequence__content avail4))))
(assert (not (= (get (sequence__content avail4) j) (to_rep (rec__list_mod_allocator__cell__next (us_split_fields1 (select data (get (sequence__content avail4) (- j 1)))))))))
(check-sat)
(exit)
