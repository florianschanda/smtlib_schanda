...
(define-fun dynamic_invariant ((temp___expr_168 Int) (temp___is_init_164 Bool) (temp___skip_constant_165 Bool) (temp___do_toplevel_166 Bool) (temp___do_typ_inv_167 Bool)) Bool (=> (or (= temp___is_init_164 true) (<= 0 2)) (in_range2 temp___expr_168)))
(assert true)
(assert (forall ((p us_rep1)) (forall ((c Int)) (forall ((d us_rep)) (! (= (= (plain_is_dot p c d) true) (= (bool_eq (get_plain p c) d) true)) :pattern ((plain_is_dot p c d)))))))
(declare-fun some_plain_is_dot (us_rep1 us_rep) Bool)
(declare-fun some_plain_is_dot__function_guard (Bool us_rep1 us_rep) Bool)
(assert true)
(assert (forall ((p us_rep1)) (forall ((d us_rep)) (! (= (= (some_plain_is_dot p d) true) (exists ((c Int)) (and (and (<= 0 c) (<= c 2)) (= (plain_is_dot p c d) true)))) :pattern ((some_plain_is_dot p d))))))
...
(assert true)
(assert (forall ((p us_rep1)) (forall ((c Int)) (! (=> (dynamic_invariant c true true true true) (= (get_plain p c) (let ((temp___214 (rec__p__painting__plain (us_split_fields3 p)))) (select temp___214 c)))) :pattern ((get_plain p c))))))
...
(assert (= (some_plain_is_dot (mk___rep1 (mk___split_fields1 p__split_fields p__split_fields1)) d) true))
(assert (forall ((c Int)) (=> (and (<= 0 c) (<= c 2)) (=> (= (bool_eq (select p__split_fields c) d) true) (= (bool_eq (select p__split_fields3 c) d) true)))))
(assert (= p__split_fields4 p__split_fields2))
(assert (= p__split_fields5 p__split_fields3))
(assert (= (mk___split_fields1 p__split_fields6 p__split_fields7) (mk___split_fields1 p__split_fields2 p__split_fields3)))
(declare-const c Int)
(assert (<= 0 c))
(assert (<= c 2))
(assert (= (plain_is_dot (mk___rep1 (mk___split_fields1 p__split_fields p__split_fields1)) c d) true))
(assert (not (= (bool_eq (select p__split_fields5 c) d) true)))
(check-sat)
(exit)
