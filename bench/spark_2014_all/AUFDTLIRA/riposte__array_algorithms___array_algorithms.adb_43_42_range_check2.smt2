...
(assert (and (forall ((j Int)) (=> (and (<= (to_rep (first (rt a))) j) (<= j i2)) (not (= (to_rep1 (select (elts a) j)) val__)))) (not (= found3 true))))
(assert (and (and (=> (<= (- 2147483648) 2147483647) (in_range3 location3)) (=> (<= (to_rep (first (rt a))) (to_rep (last (rt a)))) (dynamic_property1 (to_rep (first (rt a))) (to_rep (last (rt a))) i2))) (and (<= (to_rep (first (rt a))) i2) (<= i2 (to_rep (last (rt a)))))))
(assert (<= (to_rep (first (rt a))) i2))
(assert (not (<= (- 2147483648) (to_rep (first (rt a))))))
(check-sat)
(exit)
