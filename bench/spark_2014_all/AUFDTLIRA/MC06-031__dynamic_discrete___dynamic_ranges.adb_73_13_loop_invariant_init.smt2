...
(assert (and (= (to_rep (select o1 o)) 0) (forall ((j Int)) (=> (and (<= (to_rep a__first) j) (<= j (to_rep a__last))) (=> (not (= o j)) (= (to_rep (select o1 j)) (to_rep (select a j))))))))
(declare-const j Int)
(assert (<= (to_rep a__first) j))
(assert (<= j i1))
(assert (not (= (to_rep (select o1 j)) 0)))
(check-sat)
(exit)
