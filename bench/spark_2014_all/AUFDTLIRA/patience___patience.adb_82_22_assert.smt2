...
(declare-const j Int)
(assert (<= 0 j))
(assert (<= j (- (to_rep (select (rec__patience__state__stacksizes (us_split_fields__content s__split_fields20)) i13)) 1)))
(assert (not (<= 0 (to_rep (select (select (rec__patience__state__stacks (us_split_fields__content s__split_fields20)) i13) j)))))
(check-sat)
(exit)
