...
(declare-fun ascending (us_t) Bool)
(declare-fun ascending__function_guard (Bool us_t) Bool)
...
(assert true)
(assert (forall ((p us_t)) (! (= (= (ascending p) true) (or (< (length p) 2) (forall ((i Int)) (=> (and (<= (first1 p) i) (<= i (- (last1 p) 1))) (< (to_rep (select (to_array p) i)) (to_rep (select (to_array p) (+ i 1)))))))) :pattern ((ascending p)))))
...
(define-fun dynamic_invariant1 ((temp___expr_574 us_t1) (temp___is_init_570 Bool) (temp___skip_constant_571 Bool) (temp___do_toplevel_572 Bool) (temp___do_typ_inv_573 Bool)) Bool (=> (not (= temp___skip_constant_571 true)) (dynamic_property1 1 2147483647 (first3 temp___expr_574) (last3 temp___expr_574))))
...
(assert (=> (< (length p) (length1 s)) (and (= result (ascending p)) (= (= result true) (or (< (length p) 2) (forall ((i1 Int)) (=> (and (<= (to_rep1 (first (rt p))) i1) (<= i1 (- (to_rep1 (last (rt p))) 1))) (< (to_rep (select (elts p) i1)) (to_rep (select (elts p) (+ i1 1)))))))))))
(assert (=> (not (< (length p) (length1 s))) (= result false)))
(assert (= result true))
(assert (<= (to_rep1 (first (rt p))) i))
(assert (<= i (to_rep1 (last (rt p)))))
