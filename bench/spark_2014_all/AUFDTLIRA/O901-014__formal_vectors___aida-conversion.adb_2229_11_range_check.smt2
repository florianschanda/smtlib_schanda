...
(assert (and (= (length source) 3) (and (= (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 0))) 45) (forall ((index Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 1) index) (<= index (+ (to_rep1 (first (rt source))) 2))) (= (is_digit (to_rep (select (elts source) index))) true))))))
...
(assert (<= (+ (to_rep1 (first (rt source))) 1) (+ (to_rep1 (first (rt source))) 2)))
(assert (not (<= (+ (to_rep1 (first (rt source))) 2) 2147483647)))
(check-sat)
(exit)
