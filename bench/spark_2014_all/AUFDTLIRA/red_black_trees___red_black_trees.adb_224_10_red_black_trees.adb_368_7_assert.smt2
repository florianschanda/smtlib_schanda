...
(assert (and (= o4 (size (rec__red_black_trees__rbt__struct (us_split_fields9 c67b)))) (and (in_range2 o4) (= o4 (size1 (rec__search_trees__search_tree__struct (us_split_fields5 (rec__red_black_trees__rbt__struct (us_split_fields9 c67b)))))))))
(assert (= result3 (ite (< 0 o4) true false)))
(assert (= result3 true))
(assert (= result4 (mk_int__ref n1)))
(assert (= n5 1))
(assert (=> (and (<= 1 n5) (<= n5 100)) (= temp___656 n5)))
(assert (=> (and (<= 1 n5) (<= n5 100)) (forall ((i Int)) (=> (and (<= 1 i) (<= i 100)) (=> (and (and (= (rec__tree_model__path_type__k (us_split_fields7 (select (model__ (rec__red_black_trees__rbt__struct (us_split_fields9 c67b))) i))) true) (< (to_rep (rec__red_black_trees__black_count__depth (us_split_fields11 (select (nb_blacks c67b) i)))) n6)) (not (= i (root (rec__red_black_trees__rbt__struct (us_split_fields9 c68b)))))) (= (eq (select (nb_blacks c68b) i) (select (nb_blacks c67b) i)) true))))))
