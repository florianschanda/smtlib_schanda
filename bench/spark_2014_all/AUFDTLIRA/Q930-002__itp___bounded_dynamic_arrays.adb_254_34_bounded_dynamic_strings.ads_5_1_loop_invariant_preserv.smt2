...
(assert (forall ((j Int)) (=> (and (<= 1 j) (<= j k2)) (not (= (to_rep (select (elts (rec__bounded_dynamic_strings__sequence__content (us_split_fields1 within))) j)) fragment)))))
(assert (and (=> (<= 1 r171b) (dynamic_property2 1 r171b k2)) (and (<= 1 k2) (<= k2 r171b))))
(assert (not (= k2 r171b)))
(assert (= k2 result2))
(assert (= k3 (+ k2 1)))
(assert (and (<= (to_rep2 (first (rt (rec__bounded_dynamic_strings__sequence__content (us_split_fields1 within))))) k3) (<= k3 (to_rep2 (last (rt (rec__bounded_dynamic_strings__sequence__content (us_split_fields1 within))))))))
(assert (= o3 k3))
(assert (= o4 (select (elts (rec__bounded_dynamic_strings__sequence__content (us_split_fields1 within))) o3)))
(assert (= o5 (to_rep o4)))
(assert (= result3 (ite (= o5 fragment) true false)))
(assert (not (= result3 true)))
(assert (= bounded_dynamic_strings__location__3__result4 bounded_dynamic_strings__location__3__result3))
(declare-const j Int)
(assert (<= 1 j))
(assert (<= j k3))
(assert (not (not (= (to_rep (select (elts (rec__bounded_dynamic_strings__sequence__content (us_split_fields1 within))) j)) fragment))))
(check-sat)
(exit)
