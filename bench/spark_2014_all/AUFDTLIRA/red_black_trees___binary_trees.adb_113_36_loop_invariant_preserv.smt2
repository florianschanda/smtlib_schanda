...
(assert (<= 1 i1))
(assert (<= i1 100))
(assert (forall ((j Int)) (=> (and (<= 1 j) (<= j (- i2 1))) (not (= (select todo j) true)))))
(assert (and (=> (<= 1 100) (in_range2 i2)) (and (<= 1 i2) (<= i2 100))))
(assert (not (= (select todo i2) true)))
(assert (= binary_trees__model__next__result1 binary_trees__model__next__result))
(assert (not (= i2 100)))
(assert (= result1 i2))
(assert (= i3 (+ i2 1)))
(declare-const j Int)
(assert (<= 1 j))
(assert (<= j (- i3 1)))
(assert (not (not (= (select todo j) true))))
(check-sat)
(exit)
