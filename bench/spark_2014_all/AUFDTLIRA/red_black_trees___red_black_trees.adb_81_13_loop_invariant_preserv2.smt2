...
(assert (<= 1 i9))
(assert (<= i9 100))
(assert (= (rec__tree_model__path_type__k (us_split_fields7 (select m i9))) true))
(assert (< (int__content n4) (to_rep (rec__red_black_trees__black_count__depth (us_split_fields11 (select (map__content4 res12) i9))))))
(assert (not (< (length (rec__tree_model__path_type__a (us_split_fields7 (select m i9)))) (- 100 (int__content n4)))))
(check-sat)
(exit)
