...
(declare-const x14 (Array Int natural))
(assert (= result x))
(assert (= x1 (iter_array__x__aggregate_def 0)))
(assert (forall ((j4 Int)) (=> (and (<= 1 j4) (<= j4 10)) (= (to_rep (select x1 j4)) 0))))
...
(assert (forall ((k Int)) (=> (and (<= 1 k) (<= k j2)) (= (to_rep (select x13 k)) 2))))
...
(assert (not (= j2 10)))
(assert (= result23 (mk_int__ref j2)))
(assert (= j3 (+ j2 1)))
(assert (= result24 (mk_map__ref x13)))
(assert (= x14 (store x13 j3 rliteral2)))
(declare-const k Int)
(assert (<= 1 k))
(assert (<= k j3))
(assert (not (= (to_rep (select x14 k)) 2)))
(check-sat)
(exit)
