...
(declare-const j Int)
(assert (<= 1 j))
(assert (<= j (- (length (rec__list_allocator__m__t__available model____split_fields3)) 1)))
(assert (not (= (to_rep (rec__list_allocator__cell__next (us_split_fields3 (select data3 (get ma1 (+ j 1)))))) (get ma1 (+ j 2)))))
(check-sat)
(exit)
