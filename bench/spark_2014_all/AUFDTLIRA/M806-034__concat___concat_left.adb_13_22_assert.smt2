...
(assert (or (< (to_rep (last (rt y))) 2147483647) (< (- 2147483648) (to_rep (first (rt y))))))
(assert (=> (<= (to_rep (first (rt y))) (to_rep (last (rt y)))) (= o1 (+ (- (to_rep (last (rt y))) (to_rep (first (rt y)))) 1))))
(assert (=> (not (<= (to_rep (first (rt y))) (to_rep (last (rt y))))) (= o1 0)))
(assert (= o2 (+ 1 o1)))
(assert (= o3 (+ (- 2147483648) o2)))
(assert (= o4 (- o3 1)))
(assert (and (= o5 o4) (in_range o4)))
(assert (= (to_rep o) x))
(assert (= o6 (concat_singleton_left o (- 2147483648) (elts y) (to_rep (first (rt y))) (to_rep (last (rt y))))))
(assert (= o6 concat_left__one__z__assume))
(assert (= (mk (- 2147483648) o5) concat_left__one__z__assume1))
(assert (= result z))
(assert (= z1 concat_left__one__z__assume))
(assert (= (to_rep z__first) (to_rep (first concat_left__one__z__assume1))))
...
(assert (= (to_rep (select z1 (to_rep z__first))) x))
(assert (not (= (ite (<= (to_rep z__first) (to_rep z__last)) (+ (- (to_rep z__last) (to_rep z__first)) 1) 0) (+ (length y) 1))))
(check-sat)
(exit)
