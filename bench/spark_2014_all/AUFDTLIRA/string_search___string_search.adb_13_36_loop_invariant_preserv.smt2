...
(declare-const diff18 bool__ref)
(declare-const j16 int__ref)
(declare-const diff19 Bool)
(declare-const j17 Int)
(define-fun j18 () int__ref (mk_int__ref j8))
(define-fun diff20 () bool__ref (mk_bool__ref diff9))
(define-fun j19 () int__ref (mk_int__ref j6))
(define-fun diff21 () bool__ref (mk_bool__ref diff7))
...
(assert (=> (and (<= (to_rep1 (first (rt needle))) j1) (<= j1 (to_rep1 (last (rt needle))))) (= (mk_int__ref j10) j18)))
(assert (=> (and (<= (to_rep1 (first (rt needle))) j1) (<= j1 (to_rep1 (last (rt needle))))) (= (mk_bool__ref diff11) diff20)))
...
(assert (not (= j14 (to_rep1 (last (rt needle))))))
(assert (= result10 (mk_int__ref j14)))
(assert (= j15 (+ j14 1)))
(assert (and (= temp___2993 (+ j15 (- i3 1))) (in_range1 (+ j15 (- i3 1)))))
(assert (and (<= (to_rep1 (first (rt haystack))) temp___2993) (<= temp___2993 (to_rep1 (last (rt haystack))))))
(assert (= o20 temp___2993))
(assert (= o21 (select (elts haystack) o20)))
(assert (= o22 (to_rep o21)))
(assert (= o23 (ite (= (to_rep (select (elts needle) j15)) o22) true false)))
(assert (= o24 (ite (not (= o23 true)) true false)))
(assert (= result11 (mk_bool__ref diff16)))
(assert (= diff17 o24))
(assert (not (= diff17 true)))
(assert (= j16 (mk_int__ref j15)))
(assert (= diff18 (mk_bool__ref diff17)))
(assert (= j17 j15))
(assert (= diff19 diff17))
(assert (not (= (partial_match_at needle haystack i3 (int__content j16)) true)))
(check-sat)
(exit)
