...
(declare-const result2 (Array Int (Array Int (Array Int (Array Int et1)))))
(declare-const a3 (Array Int (Array Int (Array Int (Array Int et1)))))
(declare-const a4 (Array Int (Array Int (Array Int (Array Int et1)))))
(declare-const a5 (Array Int (Array Int (Array Int (Array Int et1)))))
(assert (in_range4 i))
(assert (in_range3 j))
(assert (in_range2 k))
(assert (= result a))
...
(assert (= result1 a1))
(assert (= a2 (store a1 i (store (select a1 i) j c))))
(assert (= result2 a2))
(assert (= a3 (store a2 i (store (select a2 i) j (store (select (select a2 i) j) k d)))))
(assert (= a3 a4))
(assert (= a5 a3))
(declare-const n Int)
(assert (<= 90 n))
(assert (<= n 99))
(assert (not (= n i)))
(assert (not (= (bool_eq2 (select a4 n) 1 16 (select a n) 1 16) true)))
(check-sat)
(exit)
