...
(assert (not (= j2 (to_rep1 (last (rt needle))))))
(assert (= j2 result4))
(assert (= j3 (+ j2 1)))
(assert (and (= temp___2991 (+ j3 (- i1 1))) (in_range1 (+ j3 (- i1 1)))))
(assert (and (<= (to_rep1 (first (rt haystack))) temp___2991) (<= temp___2991 (to_rep1 (last (rt haystack))))))
(assert (= o10 temp___2991))
(assert (= o11 (select (elts haystack) o10)))
(assert (= o12 (to_rep o11)))
(assert (= o13 (ite (= (to_rep (select (elts needle) j3)) o12) true false)))
(assert (= o14 (ite (not (= o13 true)) true false)))
(assert (= diff3 result5))
(assert (= diff4 o14))
(assert (not (= diff4 true)))
(assert (not (= (partial_match_at needle haystack i1 j3) true)))
(check-sat)
(exit)
