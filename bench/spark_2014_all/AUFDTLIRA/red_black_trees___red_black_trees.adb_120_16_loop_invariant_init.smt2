...
(assert (<= i1 j))
(assert (<= j 100))
(assert (not (or (= (to_rep (rec__red_black_trees__black_count__depth (us_split_fields11 (select res2 j)))) 100) (<= (to_rep (rec__red_black_trees__black_count__depth (us_split_fields11 (select res2 j)))) n2))))
(check-sat)
(exit)
