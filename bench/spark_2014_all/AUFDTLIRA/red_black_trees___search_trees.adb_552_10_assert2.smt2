...
(assert (= (get (rec__tree_model__path_type__a (us_split_fields1 (select (model__ f1 root) j))) (+ (length (rec__tree_model__path_type__a (us_split_fields1 (select (model__ f1 root) i)))) 1)) 0))
(assert (not (< (to_rep (select values j)) (to_rep (select values i)))))
(check-sat)
(exit)
