...
(assert (forall ((unsorted us_t)) (! (=> (and (dynamic_invariant2 unsorted true true true true) (and (<= (first1 unsorted) (last1 unsorted)) (< (first1 unsorted) 2147483647))) (let ((result (index_of_minimum unsorted))) (and (and (and (<= (first1 unsorted) result) (<= result (last1 unsorted))) (forall ((j Int)) (=> (and (<= (first1 unsorted) j) (<= j (last1 unsorted))) (<= (to_rep (select (to_array unsorted) result)) (to_rep (select (to_array unsorted) j)))))) (dynamic_invariant result true false true true)))) :pattern ((index_of_minimum unsorted)))))
...
(assert (and (= temp___185 (- (to_rep values__last) 1)) (in_range1 (- (to_rep values__last) 1))))
(assert (= result current))
(assert (= current1 (to_rep values__first)))
(assert (<= (to_rep values__first) current1))
(assert (<= current1 temp___185))
...
(assert (= o values))
(assert (= o temp___187))
(assert (= (mk1 current2 (to_rep values__last)) temp___1871))
(assert (= temp___187 o1))
(assert (= (mk (to_rep (first2 temp___1871)) (to_rep (last2 temp___1871))) o2))
...
(assert (= smallest2 o3))
(assert (forall ((j Int)) (=> (and (<= current2 j) (<= j (to_rep values__last))) (<= (to_rep (select values smallest2)) (to_rep (select values j))))))
(assert (not (= smallest2 current2)))
(assert (not (<= (to_rep values__first) smallest2)))
(check-sat)
(exit)
