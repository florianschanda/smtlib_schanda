...
(assert (<= 0 val__))
(assert (not (<= (+ (+ (+ 0 (ite (= (= (to_rep (select a 1)) val__) true) 1 0)) (ite (= (ite (= (not (= (to_rep (select a 1)) val__)) true) (value_found_in_range a val__ 2 10) false) true) 1 0)) (ite (forall ((j Int)) (=> (and (<= 1 j) (<= j 10)) (not (= (to_rep (select a j)) val__)))) 1 0)) 1)))
(check-sat)
(exit)
