...
(assert (forall ((i index_type)) (and (<= 0 (index_typeqtint i)) (<= (index_typeqtint i) 1023))))
...
(assert (exists ((index3 Int)) (and (and (<= 0 index3) (<= index3 1023)) (= (to_rep (select values index3)) 0))))
(assert (= (mk_int__ref result) (mk_int__ref index)))
(assert (= index1 0))
(assert (<= 0 index1))
(assert (<= index1 1023))
(assert (not (= (to_rep (select values index1)) 0)))
(assert (= utility__search_for_zero__result2 utility__search_for_zero__result))
(assert (= utility__search_for_zero__result2 utility__search_for_zero__result1))
(assert (exists ((j1 Int)) (and (and (<= (+ index2 1) j1) (<= j1 1023)) (= (to_rep (select values j1)) 0))))
(assert (and (=> (<= 0 1023) (in_range2 index2)) (and (<= 0 index2) (<= index2 1023))))
(assert (<= (+ index2 1) j))
(assert (<= j 1023))
(assert (not (<= 0 j)))
(check-sat)
