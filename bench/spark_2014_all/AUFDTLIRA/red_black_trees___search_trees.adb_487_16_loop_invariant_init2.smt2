...
(assert (= (get (rec__tree_model__path_type__a (us_split_fields5 (select (model__ f root) k))) (+ (length (rec__tree_model__path_type__a (us_split_fields5 (select (model__ f root) i2)))) 1)) 0))
(assert (not (< (to_rep (select values k)) (to_rep (select values i2)))))
(check-sat)
(exit)
