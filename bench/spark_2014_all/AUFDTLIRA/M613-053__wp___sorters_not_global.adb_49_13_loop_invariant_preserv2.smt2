...
(assert (<= index1 limit))
(assert (and (and (<= starting_at min2) (<= min2 limit)) (forall ((j Int)) (=> (and (<= starting_at j) (<= j (- index2 1))) (<= (to_rep (select values min2)) (to_rep (select values j)))))))
(assert (and (and (=> (<= 1 64) (in_range2 min2)) (=> (<= (+ starting_at 1) limit) (dynamic_property (+ starting_at 1) limit index2))) (and (<= (+ starting_at 1) index2) (<= index2 limit))))
(assert (=> (< (to_rep (select values index2)) (to_rep (select values min2))) (= min2 result2)))
(assert (=> (< (to_rep (select values index2)) (to_rep (select values min2))) (= min3 index2)))
(assert (=> (not (< (to_rep (select values index2)) (to_rep (select values min2)))) (= min3 min2)))
(assert (not (= index2 limit)))
(assert (= result3 index2))
(assert (= index3 (+ index2 1)))
(assert (not (<= starting_at min3)))
(check-sat)
(exit)
