...
(assert (forall ((k1 Int)) (=> (and (<= (to_rep1 (first (rt v))) k1) (<= k1 i2)) (<= (to_rep (select (elts v) k1)) result__3))))
(assert (exists ((k1 Int)) (and (and (<= (to_rep1 (first (rt v))) k1) (<= k1 i2)) (= result__3 (to_rep (select (elts v) k1))))))
...
(assert (<= k i2))
(assert (not (<= (to_rep1 (first (rt v))) k)))
(check-sat)
(exit)
