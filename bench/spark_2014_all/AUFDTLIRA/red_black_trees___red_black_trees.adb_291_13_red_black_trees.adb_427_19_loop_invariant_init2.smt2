...
(assert (<= 1 n5))
(assert (<= n5 100))
(assert (< (to_rep (rec__red_black_trees__black_count__depth (us_split_fields11 (select (nb_blacks c81b) c83b)))) n5))
(assert (not (= (eq (select (nb_blacks c82b) y) (select (nb_blacks c81b) y)) true)))
(check-sat)
(exit)
