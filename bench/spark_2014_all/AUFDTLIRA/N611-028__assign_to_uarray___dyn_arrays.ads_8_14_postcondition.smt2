...
(declare-fun tnat_array_maxP1qtint (tnat_array_maxP1) Int)
(assert (forall ((i tnat_array_maxP1)) (and (<= 1 (tnat_array_maxP1qtint i)) (<= (tnat_array_maxP1qtint i) 100))))
...
(declare-const temp___198 (Array Int natural))
(declare-const temp___1981 t)
...
(assert (= from temp___198))
(assert (= (mk 1 100) temp___1981))
(assert (= o (slide temp___198 (to_rep1 (first temp___1981)) (to_rep1 to____first))))
...
(assert (not (= (to_rep (select to__2 i)) (to_rep (select from (+ (- i (to_rep1 to____first)) 1))))))
(check-sat)
(exit)
