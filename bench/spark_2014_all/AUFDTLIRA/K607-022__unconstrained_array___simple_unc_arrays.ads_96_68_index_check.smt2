...
(assert (<= 1 j))
(assert (<= j (- (to_rep (rec__simple_unc_arrays__table__last a__split_discrs)) 1)))
(assert (and (= temp___1020 (+ j 1)) (in_range1 (+ j 1))))
(assert (not (<= temp___1020 (to_rep1 (last a__split_fields5)))))
(check-sat)
(exit)
