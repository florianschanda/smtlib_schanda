...
(assert (<= 0 i))
(assert (<= i (- size 1)))
(assert (and (= temp___193 (+ (to_rep1 (first (rt b))) i)) (in_range1 (+ (to_rep1 (first (rt b))) i))))
(assert (and (<= (to_rep1 (first (rt b))) temp___193) (<= temp___193 (to_rep1 (last (rt b))))))
(assert (= o temp___193))
(assert (= o1 (select (elts b) o)))
(assert (= o2 (to_rep o1)))
(assert (and (= temp___192 (+ (to_rep1 (first (rt a))) i)) (in_range1 (+ (to_rep1 (first (rt a))) i))))
(assert (not (<= (to_rep1 (first (rt a))) temp___192)))
(check-sat)
(exit)
