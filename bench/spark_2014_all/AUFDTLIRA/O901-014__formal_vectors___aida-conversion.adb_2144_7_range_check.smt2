...
(assert (and (= (length source) 5) (and (= (to_rep (select (elts source) (+ (to_rep1 (first (rt source))) 0))) 45) (forall ((index Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 1) index) (<= index (+ (to_rep1 (first (rt source))) 4))) (= (is_digit (to_rep (select (elts source) index))) true))))))
...
(assert (<= (+ (to_rep1 (first (rt source))) 1) (+ (to_rep1 (first (rt source))) 4)))
(assert (not (<= (+ (to_rep1 (first (rt source))) 4) 2147483647)))
(check-sat)
(exit)
