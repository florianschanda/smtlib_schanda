...
(assert (not (= (length source) 2)))
(assert (not (= (length source) 3)))
(assert (not (= (length source) 4)))
(assert (not (= (length source) 5)))
(assert (not (= (length source) 6)))
(assert (= (length source) 7))
(declare-const index Int)
(assert (<= (to_rep1 (first (rt source))) index))
(assert (<= index (to_rep1 (last (rt source)))))
(assert (not (= (is_digit (to_rep (select (elts source) index))) true)))
(check-sat)
(exit)
