...
(define-fun dynamic_invariant ((temp___expr_217 us_t) (temp___is_init_213 Bool) (temp___skip_constant_214 Bool) (temp___do_toplevel_215 Bool) (temp___do_typ_inv_216 Bool)) Bool (=> (not (= temp___skip_constant_214 true)) (dynamic_property (- 2147483648) 2147483647 (first1 temp___expr_217) (last1 temp___expr_217))))
...
(assert (<= (to_rep (first (rt arr))) i))
(assert (<= i (to_rep (last (rt arr)))))
(assert (not (= i (to_rep (first (rt arr))))))
(assert (not (<= i (to_rep (last (rt arr))))))
(check-sat)
(exit)
