...
(assert (= i1 1))
(assert (<= 1 i1))
(assert (<= i1 (to_rep1 (last (rt (rec__simple_unc_arrays__table__v (us_split_fields1 a)))))))
(declare-const j Int)
(assert (<= 1 j))
(assert (<= j (- i1 1)))
(assert (not (not (= (to_rep2 (select (elts (rec__simple_unc_arrays__table__v (us_split_fields1 a))) j)) v))))
(check-sat)
(exit)
