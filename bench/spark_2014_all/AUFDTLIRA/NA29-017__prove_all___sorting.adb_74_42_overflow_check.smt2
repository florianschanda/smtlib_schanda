...
(assert (forall ((j12 Int)) (=> (and (<= (to_rep a__first) j12) (<= j12 (to_rep a__last))) (and (=> (and (<= a_first j12) (<= j12 (- (+ a_first o1) 1))) (= (to_rep (select o3 j12)) (to_rep (select b6 (- (+ o j12) a_first))))) (=> (=> (<= a_first j12) (not (<= j12 (- (+ a_first o1) 1)))) (= (to_rep (select o3 j12)) (to_rep (select a j12))))))))
(assert (= j9 j7))
(assert (= i9 i11))
(assert (= b8 b10))
(assert (= a1 (mk_map__ref o3)))
(assert (= j10 j8))
(assert (= i10 i8))
(assert (= b9 b7))
(assert (= a2 o3))
(assert (= (= o2 true) (forall ((k Int)) (=> (and (<= (to_rep a__first) k) (<= k (to_rep a__last))) (=> (=> (<= a_first k) (not (<= k (- (+ (+ a_first length1) length2) 1)))) (= (to_rep (select (map__content a1) k)) (to_rep (select a k))))))))
