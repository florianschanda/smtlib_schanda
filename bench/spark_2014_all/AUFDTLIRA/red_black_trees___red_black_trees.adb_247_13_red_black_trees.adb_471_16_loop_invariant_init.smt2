...
(assert (<= 1 i))
(assert (<= i 100))
(assert (= (rec__tree_model__path_type__k (us_split_fields7 (select (model__ (rec__red_black_trees__rbt__struct (us_split_fields9 c94b))) i))) true))
(assert (< (to_rep (rec__red_black_trees__black_count__depth (us_split_fields11 (select (nb_blacks c94b) i)))) n5))
(assert (= (parent (rec__red_black_trees__rbt__struct (us_split_fields9 c95b)) i) c96b))
(assert (not (= (to_rep (rec__red_black_trees__black_count__count (us_split_fields11 (select (nb_blacks c95b) i)))) (+ (to_rep (rec__red_black_trees__black_count__count (us_split_fields11 (select (nb_blacks c94b) i)))) 1))))
(check-sat)
(exit)
