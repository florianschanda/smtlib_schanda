...
(assert (<= 1 n1))
(assert (<= n1 100))
(assert (forall ((i1 Int)) (=> (and (<= 1 i1) (<= i1 100)) (=> (and (and (= (rec__tree_model__path_type__k (us_split_fields7 (select (model__ (rec__red_black_trees__rbt__struct (us_split_fields9 c61b))) i1))) true) (< (to_rep (rec__red_black_trees__black_count__depth (us_split_fields11 (select (nb_blacks c61b) i1)))) n2)) (not (= i1 (root (rec__red_black_trees__rbt__struct (us_split_fields9 c62b)))))) (= (eq (select (nb_blacks c62b) i1) (select (nb_blacks c61b) i1)) true)))))
