...
(assert (= o10 (ite (<= index o9) true false)))
(assert (and (= o8 (+ (to_rep1 (first (rt source))) 6)) (in_range1 (+ (to_rep1 (first (rt source))) 6))))
(assert (= o11 (ite (<= o8 index) true false)))
(assert (= result (ite (= o11 true) o10 false)))
(assert (= (= o12 true) (forall ((index1 Int)) (=> (and (<= (+ (to_rep1 (first (rt source))) 6) index1) (<= index1 (+ (to_rep1 (first (rt source))) 10))) (= (is_digit (to_rep (select (elts source) index1))) true)))))
...
(assert (not (<= temp___2071 (to_rep1 (last (rt source))))))
(check-sat)
(exit)
