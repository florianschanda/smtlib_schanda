...
(assert (forall ((x it4)) (! (in_range1 (to_rep x)) :pattern ((to_rep x)))))
...
(assert (forall ((x et1)) (! (in_range2 (to_rep1 x)) :pattern ((to_rep1 x)))))
...
(assert (forall ((x et2)) (! (in_range3 (to_rep2 x)) :pattern ((to_rep2 x)))))
...
(assert (forall ((x et3)) (! (in_range4 (to_rep3 x)) :pattern ((to_rep3 x)))))
...
(declare-const temp___9537 us_rep3)
...
(assert (<= n 99))
(assert (not (= n i)))
(assert (not (= (bool_eq2 (select (rec__ar__rofaofr2__b2 (us_split_fields__content3 r__split_fields3)) n) (select r__split_fields1 n)) true)))
(check-sat)
(exit)
