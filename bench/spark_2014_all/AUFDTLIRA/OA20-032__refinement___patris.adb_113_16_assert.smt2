...
(assert (<= 0 xx))
(assert (<= xx 3))
(assert (= (get (select possible_i_shapes (to_rep3 cur_piece__split_fields1)) yy xx) true))
(assert (not (= (is_empty cur_board (+ (to_rep2 cur_piece__split_fields3) yy) (+ (to_rep1 cur_piece__split_fields2) xx)) true)))
(check-sat)
(exit)
