...
(assert (=> (= k_1 k_2) (= result (ite (<= (+ k_1 1) i) (ite (<= i (to_rep a_v1__last)) true false) false))))
(assert (=> (not (= k_1 k_2)) (= result false)))
(assert (= result true))
(assert (and (<= (to_rep (first (rt c_v2))) (- i k_1)) (<= (- i k_1) (to_rep (last (rt c_v2))))))
(assert (= o (- i k_1)))
(assert (= o1 (select (elts c_v2) o)))
(assert (= o2 (to_rep o1)))
(assert (not (<= (- i k_1) (to_rep (last (rt c_v1))))))
(check-sat)
(exit)
