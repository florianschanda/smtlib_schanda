...
(assert (= loops_static__loop9__result6 (mk_bool__ref loops_static__loop9__result)))
(assert (= j6 (mk_int__ref j)))
(assert (= bad6 (mk_bool__ref bad)))
(assert (= loops_static__loop9__result3 loops_static__loop9__result1))
(assert (= j3 j1))
(assert (= bad3 bad1))
(assert (forall ((j7 Int)) (=> (and (<= (to_rep1 (first (rt a))) j7) (<= j7 i1)) (= (to_rep (select r2 j7)) 0))))
(assert (= i1 result3))
(assert (= i2 2))
(assert (and (<= (to_rep1 r__first) i2) (<= i2 (to_rep1 r__last))))
(assert (= o2 i2))
(assert (= o3 (store r2 o2 rliteral)))
(assert (= r2 result4))
(assert (= r3 o3))
(assert (not (= i2 (to_rep1 (last (rt a))))))
(assert (= (mk_bool__ref loops_static__loop9__result4) loops_static__loop9__result6))
(assert (= (mk_int__ref j4) j6))
(assert (= (mk_bool__ref bad4) bad6))
(assert (= loops_static__loop9__result5 loops_static__loop9__result3))
(assert (= j5 j3))
(assert (= bad5 bad3))
(declare-const j7 Int)
(assert (<= (to_rep1 (first (rt a))) j7))
(assert (<= j7 i2))
(assert (not (= (to_rep (select r3 j7)) 0)))
(check-sat)
(exit)
