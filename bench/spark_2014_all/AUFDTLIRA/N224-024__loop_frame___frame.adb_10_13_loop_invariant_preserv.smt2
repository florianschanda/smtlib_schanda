...
(assert (forall ((k Int)) (=> (and (<= 1 k) (<= k j2)) (= (to_rep (select a3 k)) (ite (< 10 (to_rep (select a k))) 10 (to_rep (select a k)))))))
...
(assert (=> (< 10 (to_rep (select a3 j3))) (= a4 (store a3 j3 rliteral))))
(assert (=> (not (< 10 (to_rep (select a3 j3)))) (= a4 a3)))
(declare-const k Int)
(assert (<= 1 k))
(assert (<= k j3))
(assert (not (= (to_rep (select a4 k)) (ite (< 10 (to_rep (select a k))) 10 (to_rep (select a k))))))
(check-sat)
(exit)
