...
(assert (<= (to_rep1 (first (rt x))) j1))
(assert (<= j1 (to_rep1 (last (rt x)))))
(assert (and (=> (<= 0 2147483647) (in_range2 s2)) (and (<= (to_rep1 (first (rt x))) j2) (<= j2 (to_rep1 (last (rt x)))))))
(assert (not (in_range1 (+ s2 (to_rep (select (elts x) j2))))))
(check-sat)
(exit)
