...
(assert (<= 1 i1))
(assert (<= i1 10))
(assert (not (or (and (= pos1 0) (forall ((j Int)) (=> (and (<= 1 j) (<= j (- i1 1))) (not (= (to_rep (select t j)) v))))) (and (= (to_rep (select t pos1)) v) (forall ((j Int)) (=> (and (<= 1 j) (<= j (- pos1 1))) (not (= (to_rep (select t j)) v))))))))
(check-sat)
(exit)
