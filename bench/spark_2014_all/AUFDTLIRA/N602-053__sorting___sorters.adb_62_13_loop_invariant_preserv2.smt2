...
(assert (and (and (=> (<= (to_rep (first (rt unsorted))) (to_rep (last (rt unsorted)))) (dynamic_property1 (to_rep (first (rt unsorted))) (to_rep (last (rt unsorted))) index2)) (=> (<= 1 2147483647) (in_range2 min2))) (and (<= (to_rep (first (rt unsorted))) index2) (<= index2 (to_rep (last (rt unsorted)))))))
(assert (and (<= (to_rep (first (rt unsorted))) min2) (<= min2 (to_rep (last (rt unsorted))))))
(assert (= o1 min2))
(assert (= o2 (select (elts unsorted) o1)))
(assert (= o3 (to_rep o2)))
(assert (= result2 (ite (< (to_rep (select (elts unsorted) index2)) o3) true false)))
