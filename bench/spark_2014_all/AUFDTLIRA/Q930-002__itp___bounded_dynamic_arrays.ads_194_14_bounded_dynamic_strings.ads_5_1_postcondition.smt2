...
(assert true)
(assert (forall ((within us_rep)) (forall ((fragment Int)) (! (= (= (contains within fragment) true) (exists ((k Int)) (and (and (<= 1 k) (<= k (length1 within))) (= (to_rep (let ((temp___494 (rec__bounded_dynamic_strings__sequence__content (us_split_fields1 within)))) (select (to_array temp___494) k))) fragment)))) :pattern ((contains within fragment))))))
...
(assert (forall ((this us_rep)) (! (=> (dynamic_invariant1 this true true true true) (dynamic_invariant2 (length1 this) true false true true)) :pattern ((length1 this)))))
...
(assert (not (<= bounded_dynamic_strings__location__3__result16 (to_rep1 (rec__bounded_dynamic_strings__sequence__capacity (us_split_discrs1 within))))))
(check-sat)
(exit)
