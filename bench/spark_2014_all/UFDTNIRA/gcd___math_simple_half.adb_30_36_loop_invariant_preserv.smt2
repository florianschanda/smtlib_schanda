...
(assert (= j1 r4b))
(assert (=> (and (<= r4b j1) (<= j1 (- (ite (< a b) a b) 1))) (= temp___156 j1)))
...
(assert (< 1 c3))
(assert (forall ((x Int)) (=> (and (<= c3 x) (<= x (ite (< a b) a b))) (not (and (= (divides x a) true) (= (divides x b) true))))))
(assert (=> (<= 1 2147483647) (in_range1 c3)))
(assert (and (= o4 (- c3 1)) (in_range1 (- c3 1))))
(assert (= c3 result3))
(assert (= c4 o4))
(assert (< 1 c4))
(assert (not (and (= (mod2 a c4) 0) (= (mod2 b c4) 0))))
(assert (= c4 c5))
(assert (= c6 c4))
(declare-const x Int)
(assert (<= c5 x))
