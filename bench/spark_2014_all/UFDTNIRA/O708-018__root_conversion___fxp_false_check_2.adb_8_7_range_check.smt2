...
(declare-fun to_fixed (my_duration) Int)
(declare-fun of_fixed (Int) my_duration)
...
(assert (forall ((x my_duration)) (! (= (of_fixed (to_fixed x)) x) :pattern ((to_fixed x)))))
(assert (forall ((x my_duration)) (in_range (to_fixed x))))
...
(declare-const md my_duration)
(declare-const o my_duration)
(declare-const result my_duration)
(declare-const md1 my_duration)
(assert (= (to_fixed o) 20000000000))
(assert (= result md))
(assert (= md1 o))
(assert (not (in_range1 (to_fixed md1))))
(check-sat)
(exit)
