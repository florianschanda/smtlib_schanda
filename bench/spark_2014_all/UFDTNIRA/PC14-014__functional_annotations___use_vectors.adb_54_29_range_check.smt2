...
(assert (= result2 (ite (<= first4 j) o2 false)))
(assert (= result2 true))
(assert (and (= us j) (dynamic_property2 first1 last3 j)))
(assert (= o3 j))
(assert (and (= o4 (get (model__ (mk___rep v__split_discrs (mk___split_fields v__split_fields))) o3)) (in_range3 o4)))
(assert (and (= temp___495 (+ j (length (mk___rep v__split_discrs (mk___split_fields v__split_fields))))) (in_range1 (+ j (length (mk___rep v__split_discrs (mk___split_fields v__split_fields)))))))
(assert (not (dynamic_property3 first2 last4 temp___495)))
(check-sat)
(exit)
