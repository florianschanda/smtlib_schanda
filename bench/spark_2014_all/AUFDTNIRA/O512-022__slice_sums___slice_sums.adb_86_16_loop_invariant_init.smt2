...
(assert (<= (to_rep (first (rt x))) lo_index))
(assert (<= lo_index (- current_lo1 1)))
(assert (not (<= (sum__logic x (mk_slice_bounds (mk___split_fields (of_rep lo_index) (of_rep1 (- current_lo1 1))))) 0)))
(check-sat)
(exit)
