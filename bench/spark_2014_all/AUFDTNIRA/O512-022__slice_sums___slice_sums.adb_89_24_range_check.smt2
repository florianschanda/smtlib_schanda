...
(assert (not (< (to_rep (last (rt x))) (to_rep (first (rt x))))))
(assert (and (= o (to_rep (first (rt x)))) (in_range3 (to_rep (first (rt x))))))
(assert (= (indexqtint o1) o))
(assert (= o1 o2))
(assert (= rliteral o3))
(assert (= temp___200 o2))
(assert (= temp___2001 o3))
...
(assert (= current_lo1 (indexqtint (rec__sums__slice_bounds__lo result____split_fields2))))
(assert (= result4 (mk_int__ref current_hi)))
(assert (= current_hi1 (to_rep (first (rt x)))))
(assert (<= (to_rep (first (rt x))) current_hi1))
(assert (<= current_hi1 (to_rep (last (rt x)))))
(assert (and (and (and (<= (to_rep (first (rt x))) current_lo2) (<= current_lo2 current_hi2)) (<= 0 current_sum2)) (<= current_sum2 (* (- current_hi2 current_lo2) 32767))))
(assert (= current_sum2 (sum__logic x (mk_slice_bounds (mk___split_fields (of_rep current_lo2) (of_rep1 (- current_hi2 1)))))))
...
(assert (forall ((lo_index Int)) (=> (and (<= (to_rep (first (rt x))) lo_index) (<= lo_index (- current_hi2 1))) (forall ((hi_index Int)) (=> (and (<= (to_rep (first (rt x))) hi_index) (<= hi_index (- current_hi2 1))) (<= (sum__logic x (mk_slice_bounds (mk___split_fields (of_rep lo_index) (of_rep1 hi_index)))) max_sum2))))))
(assert (forall ((lo_index Int)) (=> (and (<= (to_rep (first (rt x))) lo_index) (<= lo_index (- current_lo2 1))) (<= (sum__logic x (mk_slice_bounds (mk___split_fields (of_rep lo_index) (of_rep1 (- current_lo2 1))))) 0))))
(assert (forall ((hi_index Int)) (=> (and (<= current_lo2 hi_index) (<= hi_index (- current_hi2 1))) (<= 0 (sum__logic x (mk_slice_bounds (mk___split_fields (of_rep current_lo2) (of_rep1 hi_index))))))))
(assert (and (and (and (and (=> (<= 0 2147483647) (in_range2 max_sum2)) (=> (<= (- 2147483648) 2147483647) (in_range1 current_sum2))) (in_range3 current_lo2)) (=> (<= (to_rep (first (rt x))) (to_rep (last (rt x)))) (dynamic_property1 (to_rep (first (rt x))) (to_rep (last (rt x))) current_hi2))) (and (<= (to_rep (first (rt x))) current_hi2) (<= current_hi2 (to_rep (last (rt x)))))))
(assert (<= current_lo2 (- current_hi2 1)))
(assert (not (<= (- current_hi2 1) 65536)))
(check-sat)
(exit)
