...
(assert (forall ((container us_rep)) (! (=> (not (= (is_empty1 container) true)) (let ((result (first_key container))) (and (and (= result (get1 (keys container) 1)) (= (k_smaller_than_range (keys container) 2 (length container) result) true)) (dynamic_invariant6 result true false true true)))) :pattern ((first_key container)))))
...
(assert (forall ((container us_rep2)) (forall ((key Int)) (! (=> (dynamic_invariant6 key true true true true) (let ((result (find container key))) (and (=> (< 0 result) (and (<= result (length2 container)) (= (equivalent_keys key (get1 container result)) true))) (dynamic_invariant result true false true true)))) :pattern ((find container key))))))
...
(define-fun default_initial_assumption2 ((temp___expr_427 us_rep3) (temp___skip_top_level_428 Bool)) Bool (=> (not (= temp___skip_top_level_428 true)) (and (= (is_empty2 temp___expr_427) true) (= (length3 temp___expr_427) 0))))
...
(assert (forall ((left us_rep1) (right us_rep1)) (! (= (= (oeq2 left right) true) (and (forall ((key Int)) (=> (and (in_range2 key) (= (has_key left key) true)) (and (= (has_key right key) true) (= (get right key) (get left key))))) (forall ((key Int)) (=> (and (in_range2 key) (= (has_key right key) true)) (= (has_key left key) true))))) :pattern ((oeq2 left right)))))
...
(assert (forall ((container us_rep3)) (! (= (= (is_empty2 container) true) (forall ((key us_rep4)) (not (= (has_key1 container key) true)))) :pattern ((is_empty2 container)))))
...
(assert (not (< (length (mk___rep m__split_discrs (mk___split_fields m__split_fields))) (to_rep (rec__use_ordered_maps__my_maps__map__capacity m__split_discrs)))))
(check-sat)
(exit)
