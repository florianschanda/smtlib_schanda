...
(assert (forall ((container us_rep)) (! (let ((result (length container))) (and (<= result (to_rep (rec__linked_list__mylists__list__capacity (us_split_discrs1 container)))) (dynamic_invariant result true false true true))) :pattern ((length container)))))
...
(define-fun default_initial_assumption2 ((temp___expr_273 us_rep1) (temp___skip_top_level_274 Bool)) Bool (= (to_rep (rec__linked_list__mylists__cursor__node (us_split_fields3 temp___expr_273))) 0))
...
(assert (not (in_range1 (- (to_rep (rec__linked_list__mylists__list__capacity container__split_discrs)) count))))
(check-sat)
(exit)
