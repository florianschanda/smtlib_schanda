...
(declare-fun first_element (us_rep) us_rep1)
...
(assert (forall ((container us_rep)) (! (=> (not (= (is_empty container) true)) (let ((result (first_element container))) (and (= (bool_eq1 result (get (elements container) 1)) true) (= (e_smaller_than_range (elements container) 2 (length container) result) true)))) :pattern ((first_element container)))))
...
(define-fun e_bigger_than_range ((container1 us_rep2) (fst Int) (lst Int) (item us_rep1)) Bool (ite (forall ((i Int)) (=> (and (<= fst i) (<= i lst)) (= (my_lt (get container1 i) item) true))) true false))
