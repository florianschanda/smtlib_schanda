...
(assert (forall ((n1 Int)) (=> (and (<= first4 n1) (<= n1 (- i2 1))) (= (is_incr (get (model__ (mk___rep v__split_discrs (mk___split_fields v__split_fields))) n1) (get (model__ (mk___rep v__split_discrs v__split_fields2)) n1)) true))))
(assert (forall ((n1 Int)) (=> (and (<= i2 n1) (<= n1 (last2 (model__ (mk___rep v__split_discrs v__split_fields2))))) (= (get (model__ (mk___rep v__split_discrs (mk___split_fields v__split_fields))) n1) (get (model__ (mk___rep v__split_discrs v__split_fields2)) n1)))))
(assert (and (and (dynamic_property 0 last (to_rep (rec__use_vectors__my_vectors__vector__capacity v__split_discrs))) (=> (<= first4 r7b) (dynamic_property6 first4 r7b i2))) (and (<= first4 i2) (<= i2 r7b))))
(assert (and (= o1 (last2 (model__ (mk___rep v__split_discrs v__split_fields2)))) (and (dynamic_property4 first3 last5 o1) (= o1 (last2 (model__ (mk___rep v__split_discrs v__split_fields2)))))))
(assert (= o2 (ite (<= n o1) true false)))
(assert (= result2 (ite (<= i2 n) o2 false)))
(assert (= result2 true))
(assert (and (= us n) (dynamic_property3 first2 last4 n)))
(assert (= o3 n))
(assert (not (<= o3 (last2 (model__ (mk___rep v__split_discrs v__split_fields2))))))
(check-sat)
(exit)
