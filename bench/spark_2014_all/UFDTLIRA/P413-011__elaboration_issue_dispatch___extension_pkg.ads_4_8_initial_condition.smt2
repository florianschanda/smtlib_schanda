...
(assert (= extension_pkg__early_call__assume (op_wrapper (mk___rep (mk___split_fields (hide_ext__ (distinct 0 0) us_null_ext__)) us_tag1) body_elaborated1)))
(assert (= extension_pkg__early_call__assume early_call))
(assert (= result1 body_elaborated1))
(assert (= body_elaborated2 (distinct 1 0)))
(assert (= result2 body_elaborated2))
(assert (not (= body_elaborated2 true)))
(check-sat)
(exit)
