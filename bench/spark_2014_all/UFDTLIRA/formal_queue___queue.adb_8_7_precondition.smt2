...
(define-fun dynamic_invariant2 ((temp___expr_257 Int) (temp___is_init_253 Bool) (temp___skip_constant_254 Bool) (temp___do_toplevel_255 Bool) (temp___do_typ_inv_256 Bool)) Bool (=> (or (= temp___is_init_253 true) (<= (- 2147483648) 2147483647)) (in_range3 temp___expr_257)))
(assert (forall ((container us_rep1)) (forall ((position Int)) (! (=> (and (dynamic_invariant1 position true true true true) (and (<= 1 position) (<= position (last container)))) (dynamic_invariant2 (get container position) true false true true)) :pattern ((get container position))))))
...
(assert (forall ((container us_rep2)) (forall ((key us_rep3)) (! (=> (= (has_key container key) true) (let ((result (get1 container key))) (and (=> (= (distinct 0 0) true) (and (= result (w_get container (witness container key))) (forall ((k us_rep3)) (=> (= (has_key container k) true) (= (= (bool_eq3 k key) true) (= (witness container key) (witness container k))))))) (dynamic_invariant3 result true false true true)))) :pattern ((get1 container key))))))
...
(assert (forall ((container us_rep2)) (forall ((witness1 Int)) (! (=> (and (dynamic_invariant witness1 true true true true) (= (has_witness container witness1) true)) (dynamic_invariant3 (w_get container witness1) true false true true)) :pattern ((w_get container witness1))))))
(declare-const r__split_fields us_main_type)
(declare-const queue__tail__r__assume count_type)
(declare-const queue__tail__r__assume1 us_main_type)
(declare-const result us_main_type)
(declare-const r__split_fields1 us_main_type)
(define-fun queue__tail__r__assume2 () us_rep (mk___rep (mk___split_discrs queue__tail__r__assume) (mk___split_fields queue__tail__r__assume1)))
(assert (< 0 (length q)))
(assert (and (= queue__tail__r__assume2 (copy q 0)) (and (and (= (oeq (model__ queue__tail__r__assume2) (model__ q)) true) (= (oeq1 (positions queue__tail__r__assume2) (positions q)) true)) (= (to_rep queue__tail__r__assume) (to_rep (rec__queue__mylists__list__capacity (us_split_discrs1 q)))))))
...
(assert (not (not (= (is_empty (mk___rep r__split_discrs (mk___split_fields r__split_fields1))) true))))
(check-sat)
(exit)
