...
(declare-const n Int)
(assert (<= first4 n))
(assert (<= n (last2 (model__ (mk___rep v__split_discrs (us_split_fields__content v__split_fields8))))))
(assert (not (= (is_incr (get (model__ (mk___rep v__split_discrs (mk___split_fields v__split_fields))) n) (get (model__ (mk___rep v__split_discrs (us_split_fields__content v__split_fields8))) n)) true)))
(check-sat)
(exit)
