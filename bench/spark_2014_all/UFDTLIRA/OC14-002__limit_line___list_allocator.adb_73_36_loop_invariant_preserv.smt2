...
(assert (forall ((k Int)) (=> (and (<= 1 k) (<= k j2)) (not (= (get s k) r)))))
(assert (and (=> (<= 1 r29b) (dynamic_property 1 r29b j2)) (and (<= 1 j2) (<= j2 r29b))))
(assert (not (= j2 r29b)))
(assert (= j2 result2))
(assert (= j3 (+ j2 1)))
(assert (and (= o1 (get s j3)) (in_range3 o1)))
(assert (= result3 (ite (= o1 r) true false)))
(assert (not (= result3 true)))
(assert (= list_allocator__m__find__result4 list_allocator__m__find__result3))
(declare-const k Int)
(assert (<= 1 k))
(assert (<= k j3))
(assert (not (not (= (get s k) r))))
(check-sat)
(exit)
