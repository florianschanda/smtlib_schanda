...
(assert (forall ((r us_rep)) (! (let ((result (get_i r))) (and (= result (to_rep (rec__mixed__rec__i (us_split_fields1 r)))) (dynamic_invariant result true false true true))) :pattern ((get_i r)))))
...
(assert (and (= o (get_i r)) (and (in_range o) (= o (to_rep (rec__mixed__rec__i (us_split_fields1 r)))))))
(assert (= result n))
(assert (= n1 o))
(assert (not (= n1 0)))
(check-sat)
