...
(assert (= current1 first3))
(assert (<= first3 current1))
(assert (<= current1 r27b))
(assert (forall ((i Int)) (=> (and (<= first3 i) (<= i (- current2 1))) (not (= (get (model__ v) i) e)))))
(assert (and (=> (<= first3 r27b) (dynamic_property5 first3 r27b current2)) (and (<= first3 current2) (<= current2 r27b))))
