...
(assert (forall ((x86__rdx (_ BitVec 64))) (! (=> (dynamic_invariant x86__rdx true true true true) (let ((result (dh x86__rdx))) (and (= result ((_ extract 7 0) (bvurem (bvudiv (bvand x86__rdx #x000000000000FF00) #x0000000000000100) ((_ int2bv 64) 256)))) (dynamic_invariant1 result true false true true)))) :pattern ((dh x86__rdx)))))
...
(assert (= result rdx))
(assert (= rdx1 (bvor (bvand rdx #xFFFFFFFFFFFF00FF) ((_ zero_extend 48) (bvmul ((_ zero_extend 8) val__) #x0100)))))
(assert (= rdx1 rdx2))
(assert (= rdx3 rdx1))
(assert (not (= (dh rdx2) val__)))
(check-sat)
(exit)
