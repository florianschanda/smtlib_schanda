...
(assert (forall ((x86__rdi (_ BitVec 64))) (! (=> (dynamic_invariant x86__rdi true true true true) (let ((result (edi x86__rdi))) (and (= result ((_ extract 31 0) (bvand x86__rdi #x00000000FFFFFFFF))) (dynamic_invariant1 result true false true true)))) :pattern ((edi x86__rdi)))))
...
(assert (= result rdi))
(assert (= rdi1 (bvor (bvand rdi #xFFFFFFFF00000000) ((_ zero_extend 32) val__))))
(assert (= rdi1 rdi2))
(assert (= rdi3 rdi1))
(assert (not (= (edi rdi2) val__)))
(check-sat)
(exit)
