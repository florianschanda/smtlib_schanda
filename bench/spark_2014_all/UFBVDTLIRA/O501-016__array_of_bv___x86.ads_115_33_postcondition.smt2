...
(assert (forall ((x86__rcx (_ BitVec 64))) (! (=> (dynamic_invariant x86__rcx true true true true) (let ((result (ecx x86__rcx))) (and (= result ((_ extract 31 0) (bvand x86__rcx #x00000000FFFFFFFF))) (dynamic_invariant1 result true false true true)))) :pattern ((ecx x86__rcx)))))
...
(assert (= result rcx))
(assert (= rcx1 (bvor (bvand rcx #xFFFFFFFF00000000) ((_ zero_extend 32) val__))))
(assert (= rcx1 rcx2))
(assert (= rcx3 rcx1))
(assert (not (= rcx2 (bvor (bvand rcx #xFFFFFFFF00000000) ((_ zero_extend 32) val__)))))
(check-sat)
(exit)
