...
(assert (forall ((x86__rbx (_ BitVec 64))) (! (=> (dynamic_invariant x86__rbx true true true true) (let ((result (bh x86__rbx))) (and (= result ((_ extract 7 0) (bvurem (bvudiv (bvand x86__rbx #x000000000000FF00) #x0000000000000100) ((_ int2bv 64) 256)))) (dynamic_invariant1 result true false true true)))) :pattern ((bh x86__rbx)))))
...
(assert (= result rbx))
(assert (= rbx1 (bvor (bvand rbx #xFFFFFFFFFFFF00FF) ((_ zero_extend 48) (bvmul ((_ zero_extend 8) val__) #x0100)))))
(assert (= rbx1 rbx2))
(assert (= rbx3 rbx1))
(assert (not (= (bvand rbx2 #xFFFFFFFFFFFF00FF) (bvand rbx #xFFFFFFFFFFFF00FF))))
(check-sat)
(exit)
