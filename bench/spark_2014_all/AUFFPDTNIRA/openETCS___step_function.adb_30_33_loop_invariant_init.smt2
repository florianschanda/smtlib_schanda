...
(assert (<= 1 j))
(assert (<= j i1))
(assert (not (<= (to_rep2 (rec__step_function__delimiter_entry__delimiter (us_split_fields1 (select (rec__step_function__step_function_t__step (us_split_fields3 sfun)) j)))) x)))
(check-sat)
(exit)
