...
(declare-datatypes () ((tTlineSP1__ref (mk_tTlineSP1__ref (tTlineSP1__content integer)))))
(define-fun tTlineSP1__ref___projection ((a tTlineSP1__ref)) integer (tTlineSP1__content a))
(declare-sort t 0)
...
(declare-datatypes () ((tlineS__ref (mk_tlineS__ref (tlineS__content us_t)))))
(define-fun tlineS__ref___projection ((a tlineS__ref)) us_t (tlineS__content a))
...
(define-fun dynamic_property4 ((range_first Int) (range_last Int) (low Int) (high Int)) Bool (and (in_range2 low) (and (in_range2 high) (=> (<= low high) (and (in_range1 low) (in_range1 high))))))
...
(assert (forall ((temp___172 Int) (temp___173 Int) (temp___174 Int)) (let ((temp___171 (elaboration_demo__line__aggregate_def temp___172 temp___173 temp___174))) (=> (dynamic_invariant3 temp___172 true true true true) (and (=> (dynamic_property4 1 2147483647 temp___173 temp___174) (and (= (first3 temp___171) temp___173) (= (last3 temp___171) temp___174))) (forall ((temp___175 Int)) (= (to_rep1 (select (to_array1 temp___171) temp___175)) temp___172)))))))
(define-fun dynamic_invariant6 ((temp___expr_165 us_t) (temp___is_init_161 Bool) (temp___skip_constant_162 Bool) (temp___do_toplevel_163 Bool) (temp___do_typ_inv_164 Bool)) Bool (=> (not (= temp___skip_constant_162 true)) (and (dynamic_property1 1 size (first1 temp___expr_165) (last1 temp___expr_165)) (and (= (first1 temp___expr_165) 1) (= (last1 temp___expr_165) size)))))
...
(assert (= (to_rep (first2 (rt1 (elaboration_demo__line__aggregate_def 32 1 size)))) 1))
(assert (= (to_rep (last2 (rt1 (elaboration_demo__line__aggregate_def 32 1 size)))) size))
(assert (= (mk___t1 temp___177 temp___1771) (elaboration_demo__line__aggregate_def 32 1 size)))
(assert (= (ite (<= (to_rep (first2 temp___1771)) (to_rep (last2 temp___1771))) (+ (- (to_rep (last2 temp___1771)) (to_rep (first2 temp___1771))) 1) 0) (ite (<= 1 size) (+ (- size 1) 1) 0)))
(assert (= temp___177 elaboration_demo__line__assume))
(assert (= (mk (to_rep (first2 temp___1771)) (to_rep (last2 temp___1771))) elaboration_demo__line__assume1))
(assert (= result line))
(assert (= line1 elaboration_demo__line__assume))
(assert (= (to_rep line__first) (to_rep (first elaboration_demo__line__assume1))))
(assert (= (to_rep line__last) (to_rep (last elaboration_demo__line__assume1))))
(assert (and (dynamic_property1 1 size (to_rep line__first) (to_rep line__last)) (and (= (to_rep line__first) 1) (= (to_rep line__last) size))))
(assert (not (or (fp.isZero (fp #b0 #b10000001 #b01000000000000000000000)) (fp.isPositive (fp #b0 #b10000001 #b01000000000000000000000)))))
(check-sat)
(exit)
