(set-info :smt-lib-version 2.6)
(set-logic QF_FP)
(set-info :source |Random FP created by PyMPF|)
(set-info :license |https://www.gnu.org/licenses/gpl-3.0.html|)
(set-info :category random)
(set-info :status unsat)
(declare-const x Float32)
(assert (= x ((_ to_fp 8 24) #xCAC9F43B)))
;; x should be Float32(0xCAC9F43B [Rational(-13235259, 2), -6617629.500000])

(declare-const y Float32)
(assert (= y (fp #b1 #b10011101 #b00010111101000111110010)))
;; y should be Float32(0xCE8BD1F2 [Rational(-1172896000), -1172896000.000000])

(declare-const z Float32)
(assert (= z (fp #b1 #b11111110 #b11111111111111111111111)))
;; z should be Float32(0xFF7FFFFF [Rational(-340282346638528859811704183484516925440), -340282346638528859811704183484516925440.000000])

(declare-const result Float32)
(assert (= result (fp.fma RTZ x y z)))
(assert (not (= result (fp #b1 #b11111110 #b11111111111111111111110))))
(check-sat)
(exit)
