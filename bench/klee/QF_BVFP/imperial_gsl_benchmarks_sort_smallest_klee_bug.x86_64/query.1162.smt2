...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_gsl_benchmarks_sort_smallest_klee_bug.x86_64/query.1162.smt2
|)
...
(assert
 ...
 (let (($x40 (fp.isNaN ?x26)))
 (not $x40))))
(assert
 (let ((?x26 ((_ to_fp 11 53) x_ackermann!4)))
 (let (($x40 (fp.isNaN ?x26)))
 (or (or (fp.isNaN ((_ to_fp 11 53) x_ackermann!0)) $x40) (fp.gt ((_ to_fp 11 53) x_ackermann!0) ?x26)))))
(assert
 (not false))
(check-sat)
(exit)
