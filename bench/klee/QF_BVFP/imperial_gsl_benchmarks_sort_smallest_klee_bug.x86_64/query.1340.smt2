...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_gsl_benchmarks_sort_smallest_klee_bug.x86_64/query.1340.smt2
|)
...
(assert
 ...
 (let (($x41 (fp.isNaN ?x28)))
 (not $x41))))
(assert
 (let ((?x28 ((_ to_fp 11 53) x_ackermann!4)))
 (let (($x41 (fp.isNaN ?x28)))
 (or (or (fp.isNaN ((_ to_fp 11 53) x_ackermann!1)) $x41) (fp.gt ((_ to_fp 11 53) x_ackermann!1) ?x28)))))
(assert
 (not false))
(check-sat)
(exit)
