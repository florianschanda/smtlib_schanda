...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_gsl_benchmarks_sort_smallest_klee_bug.x86_64/query.1739.smt2
|)
...
(assert
 (let ((?x18 ((_ to_fp 11 53) x_ackermann!3)))
 (let (($x19 (fp.isNaN ?x18)))
 (or $x19 (fp.lt ((_ to_fp 11 53) x_ackermann!4) ?x18)))))
(assert
...
(assert
 ...
 (let (($x42 (fp.isNaN ?x33)))
 (not $x42))))
(assert
 (let ((?x33 ((_ to_fp 11 53) x_ackermann!5)))
 (let (($x42 (fp.isNaN ?x33)))
 (or $x42 (fp.gt ((_ to_fp 11 53) x_ackermann!4) ?x33)))))
(assert
 (not false))
(check-sat)
(exit)
