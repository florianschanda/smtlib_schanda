...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_interval_klee_no_bug.x86_64/query.14.smt2
|)
...
(let ((?x29 (fp.add roundTowardNegative (fp.add roundTowardNegative ((_ to_fp 8 24) initial_ackermann!0) ?x21) ?x21)))
(not (and (not (fp.isNaN ?x24)) (fp.leq ?x29 ?x24)))))))
(check-sat)
(exit)
