...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_gsl_benchmarks_polyroots_klee_high_degree_bug.x86_64/query.11.smt2
|)
...
(let ((?x46 (fp.sub roundNearestTiesToEven ?x16 ?x45)))
(let ((?x54 (fp.add roundNearestTiesToEven (fp.abs ?x46) (fp.abs (fp.sub roundNearestTiesToEven ?x22 ?x51)))))
(let ((?x56 (fp.div roundNearestTiesToEven ?x46 (fp.add roundNearestTiesToEven ?x54 ?x22))))
(not (not (fp.lt ?x56 ?x22)))))))))))))))))))))))))
(check-sat)
(exit)
