...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_non_terminating_klee_bug.x86_64/query.22.smt2
|)
...
(let ((?x54 (fp.sub roundNearestTiesToEven ?x52 ?x8)))
(let ((?x56 (fp.sub roundNearestTiesToEven ?x54 ?x8)))
(not (fp.gt (fp.sub roundNearestTiesToEven ?x56 ?x8) ?x20))))))))))))))))))))))))
(check-sat)
(exit)
