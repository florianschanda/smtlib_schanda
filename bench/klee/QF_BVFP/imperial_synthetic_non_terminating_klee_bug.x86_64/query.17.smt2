...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_non_terminating_klee_bug.x86_64/query.17.smt2
|)
...
(let ((?x44 (fp.sub roundNearestTiesToEven ?x42 ?x8)))
(let ((?x46 (fp.sub roundNearestTiesToEven ?x44 ?x8)))
(not (fp.gt (fp.sub roundNearestTiesToEven ?x46 ?x8) ?x20)))))))))))))))))))
(check-sat)
(exit)
