...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_non_terminating_klee_bug.x86_64/query.24.smt2
|)
...
(let ((?x58 (fp.sub roundNearestTiesToEven ?x56 ?x8)))
(let ((?x60 (fp.sub roundNearestTiesToEven ?x58 ?x8)))
(not (fp.gt (fp.sub roundNearestTiesToEven ?x60 ?x8) ?x20))))))))))))))))))))))))))
(check-sat)
(exit)
