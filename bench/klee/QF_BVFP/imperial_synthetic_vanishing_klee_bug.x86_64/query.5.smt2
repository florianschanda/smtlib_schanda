...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_vanishing_klee_bug.x86_64/query.5.smt2
|)
...
(let ((?x16 (fp.add roundNearestTiesToEven ?x9 ?x12)))
(let ((?x19 (fp.add roundNearestTiesToEven ?x16 ?x12)))
(not (not (fp.eq (fp.add roundNearestTiesToEven ?x19 ?x12) ?x19))))))))
(check-sat)
(exit)
