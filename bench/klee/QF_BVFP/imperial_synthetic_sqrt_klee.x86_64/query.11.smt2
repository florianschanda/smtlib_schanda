...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee.x86_64/query.11.smt2
|)
...
(assert
 ...
 (let (($x43 (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x40 ?x8)) ((_ to_fp 8 24) (_ bv869711765 32)))))
 (not $x43))))))))))))
(assert
...
(let ((?x12 ((_ to_fp 8 24) (_ bv0 32))))
(let ((?x37 (ite $x31 ?x12 ?x23)))
(let ((?x39 (fp.mul roundNearestTiesToEven (fp.add roundNearestTiesToEven ((_ to_fp 8 24) ?x35) ?x37) ((_ to_fp 8 24) (_ bv1056964608 32)))))
(let ((?x40 (fp.mul roundNearestTiesToEven ?x39 ?x39)))
(let (($x45 (fp.gt ?x40 ?x8)))
(let (($x52 (fp.eq (ite $x45 ?x37 ?x39) ((_ to_fp 8 24) (ite $x45 fresh_to_ieee_bv_!2 ?x35)))))
(not (not $x52))))))))))))))
(check-sat)
(exit)
