...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee.x86_64/query.08.smt2
|)
...
(let ((?x32 (ite $x31 ?x12 ?x23)))
(let (($x44 (not (fp.gt ?x32 ((_ to_fp 8 24) (ite $x31 fresh_to_ieee_bv_!1 x_ackermann!0))))))
(let (($x42 (and (not (fp.isNaN ?x32)) (not (fp.isNaN ((_ to_fp 8 24) (ite $x31 fresh_to_ieee_bv_!1 x_ackermann!0)))))))
(not (and $x42 $x44)))))))))))
(check-sat)
(exit)
