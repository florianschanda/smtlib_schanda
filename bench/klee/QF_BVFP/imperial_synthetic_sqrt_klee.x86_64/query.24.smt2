...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee.x86_64/query.24.smt2
|)
...
(assert
 ...
 (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x68 ?x8)) ?x28)))))))))))))))))))))))
(assert
 (let ((?x22 ((_ to_fp 8 24) (_ bv1056964608 32))))
...
(let ((?x67 (fp.mul roundNearestTiesToEven ?x66 ?x22)))
(let ((?x68 (fp.mul roundNearestTiesToEven ?x67 ?x67)))
(not (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x8 ?x68)) ?x28))))))))))))))))))))))))
(check-sat)
(exit)
