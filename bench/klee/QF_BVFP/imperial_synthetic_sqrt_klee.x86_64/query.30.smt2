...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee.x86_64/query.30.smt2
|)
...
(let ((?x97 (fp.sub roundNearestTiesToEven (fp.mul roundNearestTiesToEven ?x95 ?x95) ?x8)))
(not (not (fp.lt (fp.abs ?x97) ?x28))))))))))))))))))))))))))))))))))))
(check-sat)
(exit)
