...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee.x86_64/query.29.smt2
|)
...
(assert
 ...
 (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x82 ?x8)) ?x28))))))))))))))))))))))))))))
(assert
 (let ((?x22 ((_ to_fp 8 24) (_ bv1056964608 32))))
...
(let ((?x81 (fp.mul roundNearestTiesToEven (fp.add roundNearestTiesToEven ?x78 (ite $x73 ?x65 ?x67)) ?x22)))
(let ((?x82 (fp.mul roundNearestTiesToEven ?x81 ?x81)))
(not (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x8 ?x82)) ?x28)))))))))))))))))))))))))))))
(check-sat)
(exit)
