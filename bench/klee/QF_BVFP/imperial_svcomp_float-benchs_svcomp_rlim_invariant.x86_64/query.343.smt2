...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_rlim_invariant.x86_64/query.343.smt2
|)
...
(assert
 ...
 (let ((?x64 (fp.add roundNearestTiesToEven ?x17 ?x43)))
 (let ((?x73 ((_ to_fp 8 24) symbolic_0_float_8_ackermann!9)))
 (let ((?x80 (fp.sub roundNearestTiesToEven ?x73 (fp.sub roundNearestTiesToEven ?x64 ?x60))))
...
(let ((?x73 ((_ to_fp 8 24) symbolic_0_float_8_ackermann!9)))
(let ((?x80 (fp.sub roundNearestTiesToEven ?x73 (fp.sub roundNearestTiesToEven ?x64 ?x60))))
(let (($x115 (and (and (not (fp.isNaN ?x80)) (not (fp.isNaN ?x76))) (not (fp.lt ?x80 ?x76)))))
(not $x115))))))))))
(check-sat)
(exit)
