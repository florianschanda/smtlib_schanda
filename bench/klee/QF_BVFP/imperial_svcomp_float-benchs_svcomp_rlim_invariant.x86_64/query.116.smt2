...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_rlim_invariant.x86_64/query.116.smt2
|)
...
(assert
 ...
 (let ((?x56 (fp.sub roundNearestTiesToEven ?x41 ?x37)))
 (let ((?x66 ((_ to_fp 8 24) symbolic_0_float_6_ackermann!1)))
 (let ((?x77 (fp.sub roundNearestTiesToEven ?x66 (fp.add roundNearestTiesToEven ?x56 ?x50))))
...
(let ((?x66 ((_ to_fp 8 24) symbolic_0_float_6_ackermann!1)))
(let ((?x77 (fp.sub roundNearestTiesToEven ?x66 (fp.add roundNearestTiesToEven ?x56 ?x50))))
(let (($x96 (and (and (not (fp.isNaN ?x77)) (not (fp.isNaN ?x73))) (not (fp.lt ?x77 ?x73)))))
(not $x96))))))))))))
(check-sat)
(exit)
