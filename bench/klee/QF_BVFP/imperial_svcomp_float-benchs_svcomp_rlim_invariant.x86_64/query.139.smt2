...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_rlim_invariant.x86_64/query.139.smt2
|)
...
(let ((?x58 ((_ to_fp 8 24) symbolic_0_float_6_ackermann!1)))
(let ((?x65 (fp.sub roundNearestTiesToEven ?x58 (fp.add roundNearestTiesToEven ?x15 ?x41))))
(let (($x98 (and (and (not (fp.isNaN ?x65)) (not (fp.isNaN ?x61))) (not (fp.lt ?x65 ?x61)))))
(not $x98))))))))
(check-sat)
(exit)
