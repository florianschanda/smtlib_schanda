(set-info :smt-lib-version 2.6)
(set-logic QF_BVFP)
(set-info :source |
Generated by: Daniel Liew, Daniel Schemmel, Cristian Cadar, Alastair Donaldson, and Rafael Zähl
Generated on: 2017-4-28
Generator: KLEE
Application: Branch satisfiability check for symbolic execution of C programs
Target solver: Z3 or MathSAT5
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_rlim_invariant.x86_64/query.345.smt2
|)
(set-info :license "https://creativecommons.org/licenses/by/4.0/")
(set-info :category "industrial")
(set-info :status unsat)
(declare-fun symbolic_0_float_8_ackermann!9 () (_ BitVec 32))
(declare-fun symbolic_0_float_9_ackermann!1 () (_ BitVec 32))
(declare-fun symbolic_0_float_7_ackermann!6 () (_ BitVec 32))
(declare-fun symbolic_0_float_5_ackermann!4 () (_ BitVec 32))
(declare-fun symbolic_0_float_2_ackermann!3 () (_ BitVec 32))
(declare-fun symbolic_0_float_3_ackermann!0 () (_ BitVec 32))
(declare-fun symbolic_0_float_1_ackermann!7 () (_ BitVec 32))
(declare-fun symbolic_0_float_4_ackermann!5 () (_ BitVec 32))
(declare-fun symbolic_0_float_6_ackermann!2 () (_ BitVec 32))
(declare-fun symbolic_0_float_ackermann!8 () (_ BitVec 32))
(assert
 (let ((?x19 ((_ to_fp 8 24) (_ bv3271557120 32))))
 (let ((?x17 ((_ to_fp 8 24) symbolic_0_float_8_ackermann!9)))
 (fp.geq ?x17 ?x19))))
(assert
 (let ((?x22 ((_ to_fp 8 24) (_ bv1124073472 32))))
 (let ((?x17 ((_ to_fp 8 24) symbolic_0_float_8_ackermann!9)))
 (fp.leq ?x17 ?x22))))
(assert
 (let ((?x34 ((_ to_fp 8 24) symbolic_0_float_9_ackermann!1)))
 (let ((?x33 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x35 (fp.sub roundNearestTiesToEven ?x33 ?x34)))
 (let ((?x28 ((_ to_fp 8 24) symbolic_0_float_7_ackermann!6)))
 (let ((?x25 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!4)))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (let ((?x27 (fp.add roundNearestTiesToEven ?x24 ?x25)))
 (let ((?x17 ((_ to_fp 8 24) symbolic_0_float_8_ackermann!9)))
 (let ((?x30 (fp.sub roundNearestTiesToEven ?x17 (fp.sub roundNearestTiesToEven ?x27 ?x28))))
 (or (or (fp.isNaN ?x30) (fp.isNaN ?x35)) (fp.gt ?x30 ?x35))))))))))))
(assert
 (let ((?x34 ((_ to_fp 8 24) symbolic_0_float_9_ackermann!1)))
 (let ((?x28 ((_ to_fp 8 24) symbolic_0_float_7_ackermann!6)))
 (let ((?x25 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!4)))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (let ((?x27 (fp.add roundNearestTiesToEven ?x24 ?x25)))
 (let ((?x17 ((_ to_fp 8 24) symbolic_0_float_8_ackermann!9)))
 (let ((?x30 (fp.sub roundNearestTiesToEven ?x17 (fp.sub roundNearestTiesToEven ?x27 ?x28))))
 (or (or (fp.isNaN ?x30) (fp.isNaN ?x34)) (fp.lt ?x30 ?x34))))))))))
(assert
 (let ((?x19 ((_ to_fp 8 24) (_ bv3271557120 32))))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (fp.geq ?x24 ?x19))))
(assert
 (let ((?x22 ((_ to_fp 8 24) (_ bv1124073472 32))))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (fp.leq ?x24 ?x22))))
(assert
 (let ((?x52 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
 (let ((?x33 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x53 (fp.sub roundNearestTiesToEven ?x33 ?x52)))
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!7)))
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (let ((?x50 (fp.sub roundNearestTiesToEven ?x24 (fp.add roundNearestTiesToEven ?x47 ?x48))))
 (or (or (fp.isNaN ?x50) (fp.isNaN ?x53)) (fp.gt ?x50 ?x53))))))))))
(assert
 (let ((?x52 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!7)))
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (let ((?x50 (fp.sub roundNearestTiesToEven ?x24 (fp.add roundNearestTiesToEven ?x47 ?x48))))
 (or (or (fp.isNaN ?x50) (fp.isNaN ?x52)) (fp.lt ?x50 ?x52))))))))
(assert
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x25 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!4)))
 (fp.geq ?x25 ?x47))))
(assert
 (let ((?x64 ((_ to_fp 8 24) (_ bv1098907648 32))))
 (let ((?x25 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!4)))
 (fp.leq ?x25 ?x64))))
(assert
 (let ((?x25 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!4)))
 (let ((?x33 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x66 (fp.sub roundNearestTiesToEven ?x33 ?x25)))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (let ((?x68 ((_ to_fp 8 24) symbolic_0_float_4_ackermann!5)))
 (let ((?x69 (fp.sub roundNearestTiesToEven ?x68 ?x24)))
 (or (fp.isNaN ?x66) (fp.gt ?x69 ?x66)))))))))
(assert
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (let ((?x68 ((_ to_fp 8 24) symbolic_0_float_4_ackermann!5)))
 (let ((?x69 (fp.sub roundNearestTiesToEven ?x68 ?x24)))
 (not (fp.isNaN ?x69))))))
(assert
 (not (fp.isNaN ((_ to_fp 8 24) symbolic_0_float_5_ackermann!4))))
(assert
 (let ((?x25 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!4)))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (let ((?x68 ((_ to_fp 8 24) symbolic_0_float_4_ackermann!5)))
 (let ((?x69 (fp.sub roundNearestTiesToEven ?x68 ?x24)))
 (not (fp.lt ?x69 ?x25)))))))
(assert
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x28 ((_ to_fp 8 24) symbolic_0_float_7_ackermann!6)))
 (fp.geq ?x28 ?x47))))
(assert
 (let ((?x64 ((_ to_fp 8 24) (_ bv1098907648 32))))
 (let ((?x28 ((_ to_fp 8 24) symbolic_0_float_7_ackermann!6)))
 (fp.leq ?x28 ?x64))))
(assert
 (let ((?x25 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!4)))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (let ((?x27 (fp.add roundNearestTiesToEven ?x24 ?x25)))
 (let ((?x80 ((_ to_fp 8 24) symbolic_0_float_6_ackermann!2)))
 (let ((?x81 (fp.sub roundNearestTiesToEven ?x80 ?x27)))
 (not (fp.isNaN ?x81))))))))
(assert
 (let ((?x28 ((_ to_fp 8 24) symbolic_0_float_7_ackermann!6)))
 (let ((?x33 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x84 (fp.sub roundNearestTiesToEven ?x33 ?x28)))
 (not (fp.isNaN ?x84))))))
(assert
 (let ((?x28 ((_ to_fp 8 24) symbolic_0_float_7_ackermann!6)))
 (let ((?x33 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x84 (fp.sub roundNearestTiesToEven ?x33 ?x28)))
 (let ((?x25 ((_ to_fp 8 24) symbolic_0_float_5_ackermann!4)))
 (let ((?x24 ((_ to_fp 8 24) symbolic_0_float_2_ackermann!3)))
 (let ((?x27 (fp.add roundNearestTiesToEven ?x24 ?x25)))
 (let ((?x80 ((_ to_fp 8 24) symbolic_0_float_6_ackermann!2)))
 (let ((?x81 (fp.sub roundNearestTiesToEven ?x80 ?x27)))
 (not (fp.gt ?x81 ?x84)))))))))))
(assert
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x34 ((_ to_fp 8 24) symbolic_0_float_9_ackermann!1)))
 (fp.geq ?x34 ?x47))))
(assert
 (let ((?x64 ((_ to_fp 8 24) (_ bv1098907648 32))))
 (let ((?x34 ((_ to_fp 8 24) symbolic_0_float_9_ackermann!1)))
 (fp.leq ?x34 ?x64))))
(assert
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!7)))
 (fp.geq ?x48 ?x47))))
(assert
 (let ((?x64 ((_ to_fp 8 24) (_ bv1098907648 32))))
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!7)))
 (fp.leq ?x48 ?x64))))
(assert
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!7)))
 (let ((?x33 ((_ to_fp 8 24) (_ bv2147483648 32))))
 (let ((?x93 (fp.sub roundNearestTiesToEven ?x33 ?x48)))
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x95 ((_ to_fp 8 24) symbolic_0_float_ackermann!8)))
 (let ((?x96 (fp.sub roundNearestTiesToEven ?x95 ?x47)))
 (or (fp.isNaN ?x93) (fp.gt ?x96 ?x93)))))))))
(assert
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x95 ((_ to_fp 8 24) symbolic_0_float_ackermann!8)))
 (let ((?x96 (fp.sub roundNearestTiesToEven ?x95 ?x47)))
 (not (fp.isNaN ?x96))))))
(assert
 (not (fp.isNaN ((_ to_fp 8 24) symbolic_0_float_1_ackermann!7))))
(assert
 (let ((?x48 ((_ to_fp 8 24) symbolic_0_float_1_ackermann!7)))
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x95 ((_ to_fp 8 24) symbolic_0_float_ackermann!8)))
 (let ((?x96 (fp.sub roundNearestTiesToEven ?x95 ?x47)))
 (not (fp.lt ?x96 ?x48)))))))
(assert
 (let ((?x47 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x52 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
 (fp.geq ?x52 ?x47))))
(assert
 (let ((?x64 ((_ to_fp 8 24) (_ bv1098907648 32))))
 (let ((?x52 ((_ to_fp 8 24) symbolic_0_float_3_ackermann!0)))
 (fp.leq ?x52 ?x64))))
(assert
 (let ((?x19 ((_ to_fp 8 24) (_ bv3271557120 32))))
 (let ((?x68 ((_ to_fp 8 24) symbolic_0_float_4_ackermann!5)))
 (fp.geq ?x68 ?x19))))
(assert
 (let ((?x22 ((_ to_fp 8 24) (_ bv1124073472 32))))
 (let ((?x68 ((_ to_fp 8 24) symbolic_0_float_4_ackermann!5)))
 (fp.leq ?x68 ?x22))))
(assert
 (let ((?x19 ((_ to_fp 8 24) (_ bv3271557120 32))))
 (let ((?x80 ((_ to_fp 8 24) symbolic_0_float_6_ackermann!2)))
 (fp.geq ?x80 ?x19))))
(assert
 (let ((?x22 ((_ to_fp 8 24) (_ bv1124073472 32))))
 (let ((?x80 ((_ to_fp 8 24) symbolic_0_float_6_ackermann!2)))
 (fp.leq ?x80 ?x22))))
(assert
 (let ((?x19 ((_ to_fp 8 24) (_ bv3271557120 32))))
 (let ((?x95 ((_ to_fp 8 24) symbolic_0_float_ackermann!8)))
 (fp.geq ?x95 ?x19))))
(assert
 (let ((?x22 ((_ to_fp 8 24) (_ bv1124073472 32))))
 (let ((?x95 ((_ to_fp 8 24) symbolic_0_float_ackermann!8)))
 (fp.leq ?x95 ?x22))))
(assert
 (let (($x119 (and (fp.geq ((_ to_fp 8 24) symbolic_0_float_8_ackermann!9) ((_ to_fp 8 24) (_ bv3271622656 32))) (fp.leq ((_ to_fp 8 24) symbolic_0_float_8_ackermann!9) ((_ to_fp 8 24) (_ bv1124139008 32))))))
(not $x119)))
(check-sat)
(exit)
