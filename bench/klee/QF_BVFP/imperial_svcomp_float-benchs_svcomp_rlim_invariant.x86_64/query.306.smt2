...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_rlim_invariant.x86_64/query.306.smt2
|)
...
(assert
 ...
 (let ((?x66 (fp.sub roundNearestTiesToEven ?x51 ?x47)))
 (let ((?x76 ((_ to_fp 8 24) symbolic_0_float_8_ackermann!9)))
 (let ((?x87 (fp.sub roundNearestTiesToEven ?x76 (fp.add roundNearestTiesToEven ?x66 ?x60))))
...
(let ((?x76 ((_ to_fp 8 24) symbolic_0_float_8_ackermann!9)))
(let ((?x87 (fp.sub roundNearestTiesToEven ?x76 (fp.add roundNearestTiesToEven ?x66 ?x60))))
(let (($x110 (and (and (not (fp.isNaN ?x87)) (not (fp.isNaN ?x83))) (not (fp.lt ?x87 ?x83)))))
(not $x110))))))))))))))
(check-sat)
(exit)
