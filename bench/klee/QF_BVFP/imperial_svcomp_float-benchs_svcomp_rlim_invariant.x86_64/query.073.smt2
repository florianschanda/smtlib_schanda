...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_rlim_invariant.x86_64/query.073.smt2
|)
...
(let ((?x51 ((_ to_fp 8 24) symbolic_0_float_4_ackermann!3)))
(let ((?x58 (fp.sub roundNearestTiesToEven ?x51 (fp.sub roundNearestTiesToEven ?x13 ?x37))))
(let (($x75 (and (and (not (fp.isNaN ?x58)) (not (fp.isNaN ?x54))) (not (fp.lt ?x58 ?x54)))))
(not $x75))))))))
(check-sat)
(exit)
