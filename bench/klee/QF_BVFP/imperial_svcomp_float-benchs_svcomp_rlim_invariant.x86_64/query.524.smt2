...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_rlim_invariant.x86_64/query.524.smt2
|)
...
(let ((?x76 ((_ to_fp 8 24) symbolic_0_float_8_ackermann!9)))
(let ((?x83 (fp.sub roundNearestTiesToEven ?x76 (fp.add roundNearestTiesToEven ?x66 ?x60))))
(let (($x118 (and (and (not (fp.isNaN ?x83)) (not (fp.isNaN ?x79))) (not (fp.lt ?x83 ?x79)))))
(not $x118))))))))))
(check-sat)
(exit)
