...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/aachen_real_numerical_recipes_gaussj.x86_64/query.20.smt2
|)
...
(let ((?x53 (fp.abs ?x52)))
(not (and (not (fp.isNaN ?x53)) (not (fp.lt ?x53 ?x34)))))))))))))))))))
(check-sat)
(exit)
