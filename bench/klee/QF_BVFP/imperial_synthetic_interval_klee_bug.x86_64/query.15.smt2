...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_interval_klee_bug.x86_64/query.15.smt2
|)
...
(assert
 (let ((?x29 (fp.add roundNearestTiesToEven ((_ to_fp 11 53) roundNearestTiesToEven ((_ to_fp 8 24) initial_ackermann!0)) ((_ to_fp 11 53) (_ bv4707126720094797824 64)))))
(let ((?x35 (fp.add roundNearestTiesToEven ?x29 ((_ to_fp 11 53) (_ bv4707126720094797824 64)))))
(let ((?x8 ((_ to_fp 8 24) initial_ackermann!0)))
(let ((?x23 (fp.add roundTowardNegative ?x8 ((_ to_fp 8 24) (_ bv1251513984 32)))))
(let ((?x32 ((_ to_fp 11 53) roundNearestTiesToEven (fp.add roundTowardNegative ?x23 ((_ to_fp 8 24) (_ bv1251513984 32))))))
(let (($x41 (and (and (not (fp.isNaN ?x32)) (not (fp.isNaN ?x35))) (not (fp.gt ?x32 ?x35)))))
(not $x41))))))))
(check-sat)
(exit)
