...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_interval_klee_bug.x86_64/query.23.smt2
|)
...
(assert
 ...
 (fp.geq ((_ to_fp 11 53) roundNearestTiesToEven ?x35) (fp.add roundNearestTiesToEven ?x33 ?x28)))))))))))
(assert
 (let ((?x21 ((_ to_fp 8 24) (_ bv1251513984 32))))
(let ((?x8 ((_ to_fp 8 24) initial_ackermann!0)))
(let ((?x23 (fp.add roundTowardNegative ?x8 ?x21)))
(let ((?x31 (fp.add roundTowardNegative ?x23 ?x21)))
(let ((?x35 (fp.add roundTowardNegative ?x31 ?x21)))
(not (not (fp.isNaN (fp.add roundTowardNegative ?x35 ?x21))))))))))
(check-sat)
(exit)
