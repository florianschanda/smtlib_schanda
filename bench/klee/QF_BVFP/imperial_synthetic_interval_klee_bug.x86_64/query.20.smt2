...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_interval_klee_bug.x86_64/query.20.smt2
|)
...
(let ((?x29 (fp.add roundNearestTiesToEven ((_ to_fp 11 53) roundNearestTiesToEven ((_ to_fp 8 24) initial_ackermann!0)) ?x28)))
(let ((?x33 (fp.add roundNearestTiesToEven ?x29 ?x28)))
(let ((?x39 (fp.add roundNearestTiesToEven ?x33 ?x28)))
(let ((?x21 ((_ to_fp 8 24) (_ bv1251513984 32))))
(let ((?x8 ((_ to_fp 8 24) initial_ackermann!0)))
(let ((?x23 (fp.add roundTowardNegative ?x8 ?x21)))
(let ((?x31 (fp.add roundTowardNegative ?x23 ?x21)))
(let ((?x36 ((_ to_fp 11 53) roundNearestTiesToEven (fp.add roundTowardNegative ?x31 ?x21))))
(let (($x45 (and (and (not (fp.isNaN ?x36)) (not (fp.isNaN ?x39))) (not (fp.gt ?x36 ?x39)))))
(not $x45))))))))))))
(check-sat)
(exit)
