...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_interval_klee_bug.x86_64/query.26.smt2
|)
...
(let ((?x31 (fp.add roundTowardNegative ?x23 ?x21)))
(let ((?x35 (fp.add roundTowardNegative ?x31 ?x21)))
(let (($x42 (fp.geq ((_ to_fp 11 53) roundNearestTiesToEven (fp.add roundTowardNegative ?x35 ?x21)) (fp.add roundNearestTiesToEven ?x37 ?x28))))
(not $x42))))))))))))
(check-sat)
(exit)
