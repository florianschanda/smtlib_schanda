...
Corresponding query: An equisatisfiable query (bitvectors replaced with reads from arrays of bitvectors) is available at QF_ABVFP/20170428-Liew-KLEE/imperial_synthetic_interval_klee_bug.x86_64/query.17.smt2
|)
...
(assert
 ...
 (let ((?x8 ((_ to_fp 8 24) initial_ackermann!0)))
 (let ((?x23 (fp.add roundTowardNegative ?x8 ((_ to_fp 8 24) (_ bv1251513984 32)))))
 (let ((?x32 ((_ to_fp 11 53) roundNearestTiesToEven (fp.add roundTowardNegative ?x23 ((_ to_fp 8 24) (_ bv1251513984 32))))))
 (fp.geq ?x32 ?x33))))))))
(assert
 (let ((?x28 ((_ to_fp 11 53) (_ bv4707126720094797824 64))))
(let ((?x29 (fp.add roundNearestTiesToEven ((_ to_fp 11 53) roundNearestTiesToEven ((_ to_fp 8 24) initial_ackermann!0)) ?x28)))
(let ((?x33 (fp.add roundNearestTiesToEven ?x29 ?x28)))
(not (not (fp.isNaN (fp.add roundNearestTiesToEven ?x33 ?x28))))))))
(check-sat)
(exit)
