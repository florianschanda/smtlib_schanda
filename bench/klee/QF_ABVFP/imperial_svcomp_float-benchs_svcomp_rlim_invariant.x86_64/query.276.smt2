...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_rlim_invariant.x86_64/query.276.smt2
|)
...
(let ((?x97 ((_ to_fp 8 24) ?x96)))
(let ((?x112 (fp.sub roundNearestTiesToEven ?x97 (fp.sub roundNearestTiesToEven ?x19 ?x66))))
(let (($x153 (and (and (not (fp.isNaN ?x112)) (not (fp.isNaN ?x108))) (not (fp.lt ?x112 ?x108)))))
(not $x153))))))))))))))))
(check-sat)
(exit)
