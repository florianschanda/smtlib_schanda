...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_rlim_invariant.x86_64/query.059.smt2
|)
...
(let ((?x68 ((_ to_fp 8 24) ?x67)))
(let ((?x84 (fp.sub roundNearestTiesToEven ?x68 ?x19)))
(let (($x120 (and (and (not (fp.isNaN ?x84)) (not (fp.isNaN ?x79))) (not (fp.lt ?x84 ?x79)))))
(not $x120)))))))))))))
(check-sat)
(exit)
