...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_fadd_to_exact_zero_klee_double.x86_64/query.11.smt2
|)
...
(let ((?x30 (concat (select f0 (_ bv7 32)) (concat (select f0 (_ bv6 32)) ?x24))))
(let ((?x31 ((_ to_fp 11 53) ?x30)))
(not (fp.eq (fp.add roundTowardNegative ?x31 ?x50) ((_ to_fp 11 53) (_ bv0 64)))))))))))))
(check-sat)
(exit)
