...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee_bug.x86_64/query.13.smt2
|)
...
(assert
 ...
 (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x49 ?x19)) ?x37)))))))))))
(assert
 (let ((?x22 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x15 (concat (select x0 (_ bv2 32)) (concat (select x0 (_ bv1 32)) (select x0 (_ bv0 32))))))
 (let ((?x18 (concat (select x0 (_ bv3 32)) ?x15)))
...
(let ((?x48 (fp.add roundNearestTiesToEven ((_ to_fp 8 24) (ite $x41 fresh_to_ieee_bv_!0 ?x18)) (ite $x41 ?x22 ?x33))))
(let ((?x49 (fp.mul roundNearestTiesToEven ?x48 ((_ to_fp 8 24) (_ bv1056964608 32)))))
(let ((?x54 (fp.sub roundNearestTiesToEven ?x19 (fp.mul roundNearestTiesToEven ?x49 ?x49))))
(not (not (fp.lt (fp.abs ?x54) ?x37))))))))))))))
(check-sat)
(exit)
