...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee_bug.x86_64/query.24.smt2
|)
...
(assert
 ...
 (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x77 ?x19)) ?x37)))))))))))))))))))))
(assert
 (let ((?x32 ((_ to_fp 8 24) (_ bv1056964608 32))))
 (let ((?x22 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x15 (concat (select x0 (_ bv2 32)) (concat (select x0 (_ bv1 32)) (select x0 (_ bv0 32))))))
...
(let ((?x76 (fp.add roundNearestTiesToEven ((_ to_fp 8 24) (ite $x69 fresh_to_ieee_bv_!2 ?x59)) (ite $x69 ?x61 ?x63))))
(let ((?x77 (fp.mul roundNearestTiesToEven ?x76 ?x32)))
(let ((?x82 (fp.sub roundNearestTiesToEven ?x19 (fp.mul roundNearestTiesToEven ?x77 ?x77))))
(not (not (fp.lt (fp.abs ?x82) ?x37))))))))))))))))))))))))
(check-sat)
(exit)
