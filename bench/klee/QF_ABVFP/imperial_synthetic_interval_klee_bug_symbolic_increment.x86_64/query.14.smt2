...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_interval_klee_bug_symbolic_increment.x86_64/query.14.smt2
|)
...
(let ((?x15 (concat (select initial0 (_ bv2 32)) (concat (select initial0 (_ bv1 32)) (select initial0 (_ bv0 32))))))
(let ((?x19 ((_ to_fp 8 24) (concat (select initial0 (_ bv3 32)) ?x15))))
(let ((?x51 (fp.add roundNearestTiesToEven ((_ to_fp 11 53) roundNearestTiesToEven ?x19) ((_ to_fp 11 53) roundNearestTiesToEven ?x30))))
(let (($x52 (fp.geq ((_ to_fp 11 53) roundNearestTiesToEven (fp.add roundTowardNegative ?x19 ?x30)) ?x51)))
(not $x52))))))))
(check-sat)
(exit)
