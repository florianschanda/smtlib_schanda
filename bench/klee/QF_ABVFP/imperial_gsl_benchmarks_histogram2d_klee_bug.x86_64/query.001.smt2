...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_gsl_benchmarks_histogram2d_klee_bug.x86_64/query.001.smt2
|)
...
(assert
 (let ((?x15 (concat (select x10 (_ bv2 32)) (concat (select x10 (_ bv1 32)) (select x10 (_ bv0 32))))))
(let ((?x21 ((_ to_fp 11 53) roundNearestTiesToEven ((_ to_fp 8 24) (concat (select x10 (_ bv3 32)) ?x15)))))
(not (not (fp.lt ?x21 ((_ to_fp 11 53) (_ bv4600877379321698714 64))))))))
(check-sat)
(exit)
