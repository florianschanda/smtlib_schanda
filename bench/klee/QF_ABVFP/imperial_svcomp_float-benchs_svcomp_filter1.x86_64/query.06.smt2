...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_svcomp_float-benchs_svcomp_filter1.x86_64/query.06.smt2
|)
...
(let ((?x53 (concat (select symbolic_0_double_11 (_ bv7 32)) (concat (select symbolic_0_double_11 (_ bv6 32)) ?x49))))
(let ((?x54 ((_ to_fp 11 53) ?x53)))
(let ((?x61 (fp.sub roundNearestTiesToEven (fp.add roundNearestTiesToEven ((_ to_fp 11 53) (_ bv0 64)) ?x31) ((_ to_fp 11 53) (_ bv0 64)))))
(let ((?x65 (fp.add roundNearestTiesToEven (fp.mul roundNearestTiesToEven ?x61 ((_ to_fp 11 53) (_ bv4607173411600762667 64))) ?x54)))
(let ((?x66 (fp.sub roundNearestTiesToEven ?x65 ?x31)))
(not (and (fp.geq ?x66 ((_ to_fp 11 53) (_ bv13890095209322119168 64))) (fp.leq ?x66 ((_ to_fp 11 53) (_ bv4666723172467343360 64)))))))))))))))))))
(check-sat)
(exit)
