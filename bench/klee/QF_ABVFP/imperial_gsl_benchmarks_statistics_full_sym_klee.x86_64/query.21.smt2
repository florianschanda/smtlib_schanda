...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_gsl_benchmarks_statistics_full_sym_klee.x86_64/query.21.smt2
|)
...
(not (and (not (fp.isNaN ?x169)) (not (fp.lt ?x169 ?x33)))))))))))))))))))))))))))))))
(check-sat)
(exit)
