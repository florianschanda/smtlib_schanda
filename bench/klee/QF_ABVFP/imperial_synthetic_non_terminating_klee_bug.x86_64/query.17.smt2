...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_non_terminating_klee_bug.x86_64/query.17.smt2
|)
...
(let ((?x54 (fp.sub roundNearestTiesToEven ?x52 ?x19)))
(let ((?x56 (fp.sub roundNearestTiesToEven ?x54 ?x19)))
(not (fp.gt (fp.sub roundNearestTiesToEven ?x56 ?x19) ?x30))))))))))))))))))))
(check-sat)
(exit)
