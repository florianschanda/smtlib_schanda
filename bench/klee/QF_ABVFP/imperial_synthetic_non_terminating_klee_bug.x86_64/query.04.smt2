...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_non_terminating_klee_bug.x86_64/query.04.smt2
|)
...
(assert
 ...
 (fp.gt ?x29 ((_ to_fp 8 24) (_ bv0 32)))))))
(assert
 (let ((?x15 (concat (select delta0 (_ bv2 32)) (concat (select delta0 (_ bv1 32)) (select delta0 (_ bv0 32))))))
(let ((?x19 ((_ to_fp 8 24) (concat (select delta0 (_ bv3 32)) ?x15))))
(let ((?x29 (fp.sub roundNearestTiesToEven ((_ to_fp 8 24) (_ bv1120403456 32)) ?x19)))
(not (fp.gt (fp.sub roundNearestTiesToEven ?x29 ?x19) ((_ to_fp 8 24) (_ bv0 32))))))))
(check-sat)
(exit)
