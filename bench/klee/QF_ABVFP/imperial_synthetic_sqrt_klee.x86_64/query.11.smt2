...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_sqrt_klee.x86_64/query.11.smt2
|)
...
(assert
 ...
 (let (($x53 (fp.lt (fp.abs (fp.sub roundNearestTiesToEven ?x50 ?x19)) ((_ to_fp 8 24) (_ bv869711765 32)))))
 (not $x53))))))))))))))
(assert
 (let ((?x22 ((_ to_fp 8 24) (_ bv0 32))))
 (let ((?x15 (concat (select x0 (_ bv2 32)) (concat (select x0 (_ bv1 32)) (select x0 (_ bv0 32))))))
...
(let (($x62 (fp.eq (ite $x55 ?x47 ?x49) ((_ to_fp 8 24) (ite $x55 fresh_to_ieee_bv_!1 ?x45)))))
(not (not $x62))))))))))))))))
(check-sat)
(exit)
