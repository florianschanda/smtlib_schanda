...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_gsl_benchmarks_sort_smallest_klee_bug.x86_64/query.1777.smt2
|)
...
(assert
 ...
 (let (($x173 (not $x172)))
 (not (and $x173 (or (or (fp.isNaN ?x81) $x172) (fp.gt ?x81 ?x31)))))))))))))
(assert
 (let ((?x39 (concat (select x0 (_ bv2 32)) (concat (select x0 (_ bv1 32)) (select x0 (_ bv0 32))))))
 (let ((?x48 (concat (select x0 (_ bv5 32)) (concat (select x0 (_ bv4 32)) (concat (select x0 (_ bv3 32)) ?x39)))))
 (let ((?x55 ((_ to_fp 11 53) (concat (select x0 (_ bv7 32)) (concat (select x0 (_ bv6 32)) ?x48)))))
