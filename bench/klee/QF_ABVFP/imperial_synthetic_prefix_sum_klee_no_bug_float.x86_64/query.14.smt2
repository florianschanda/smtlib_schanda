...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_prefix_sum_klee_no_bug_float.x86_64/query.14.smt2
|)
...
(let ((?x15 (concat (select data_in0 (_ bv2 32)) (concat (select data_in0 (_ bv1 32)) (select data_in0 (_ bv0 32))))))
(let ((?x19 ((_ to_fp 8 24) (concat (select data_in0 (_ bv3 32)) ?x15))))
(let ((?x59 (fp.add roundNearestTiesToEven ?x52 (fp.add roundNearestTiesToEven ?x19 ?x36))))
(not (and (not (fp.isNaN ?x59)) (not (fp.lt ?x59 ?x52))))))))))))
(check-sat)
(exit)
