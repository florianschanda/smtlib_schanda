...
Corresponding query: An equisatisfiable query (arrays replaced with bitvectors) is available at QF_BVFP/20170428-Liew-KLEE/imperial_synthetic_prefix_sum_klee_no_bug_float.x86_64/query.18.smt2
|)
...
(let ((?x64 (concat (select data_in0 (_ bv14 32)) (concat (select data_in0 (_ bv13 32)) (select data_in0 (_ bv12 32))))))
(let ((?x68 ((_ to_fp 8 24) (concat (select data_in0 (_ bv15 32)) ?x64))))
(let ((?x93 (fp.add roundNearestTiesToEven ?x84 (fp.add roundNearestTiesToEven ?x68 ?x91))))
(not (and (not (fp.isNaN ?x93)) (not (fp.lt ?x93 ?x84)))))))))))))))))
(check-sat)
(exit)
