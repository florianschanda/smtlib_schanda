...
;; elsif Applied_X < Big_Position_T (Position_T'First) then
(assert (not (fp.gt applied_x ((_ to_fp 11 53) RNE position_t__last))))
(assert (fp.lt applied_x ((_ to_fp 11 53) RNE position_t__first)))

;;  Ratio := Ratio_T (Big_Position_T (Position_T'First - Origin.X) /
;;             Big_Position_T (Offset.X));
(define-const ratio Float64
  (fp.div RNE ((_ to_fp 11 53) RNE (fp.sub RNE position_t__first origin_x))
              ((_ to_fp 11 53) RNE offset_x)))

;;  divisin+range+overflow check
